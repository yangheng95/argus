...
Postoje dvije vrste agenata u OpenCorvus: primarni agenti i podagenti.
...
OpenCorvus dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenCorvus dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenCorvus dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `opencorvus.json`:
...
- Globalno: `~/.config/opencorvus/agents/`
- Po projektu: `.opencorvus/agents/`
...
Ako temperatura nije navedena, OpenCorvus koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenCorvus konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenCorvus konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenCorvus Zen](/docs/zen), koristili biste `opencorvus/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `opencorvus models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/yangheng95/opencorvus).
:::
