...
describe("build agent goal execution discipline prompt", () => {
  ...
  test("requires observed frontend verification and random preview ports", async () => {
    ...
    expect(normalized).toContain("use `browser_preview_reference_regions` for required bound regions")
    expect(normalized).toContain("produce fresh task-scoped `reference-comparison` evidence")
    ...
    expect(normalized).toContain("cite fresh `reference-comparison` artifacts from `browser_preview_reference_regions`")
    ...
    expect(normalized).toContain(
      "report the exact evidence gap or blocker instead of accepting an unverified screenshot review",
    )
