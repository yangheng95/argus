...
describe("settings primitives — Solid exports", () => {
  ...
  test("Segmented delegates toggle semantics to the shared Kobalte primitive", () => {
    ...
    expect(SEGMENTED_SRC).toContain("function isActivationKey")
    expect(SEGMENTED_SRC).toContain('event.key === "Enter"')
    expect(SEGMENTED_SRC).toContain('event.key === " "')
    expect(SEGMENTED_SRC).toContain('event.key === "Space"')
    expect(SEGMENTED_SRC).toContain('event.key === "Spacebar"')
    expect(SEGMENTED_SRC).toContain("if (isActivationKey(event)) props.onActivate(option.value)")
