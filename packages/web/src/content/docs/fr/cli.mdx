---
title: CLI
description: Options et commandes de la CLI Argus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI Argus démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
argus
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec Argus par programme.
...
Démarrez l’interface utilisateur du terminal Argus.
...
La CLI Argus dispose également des commandes suivantes.

---

### agent

Gérer les agents pour Argus.
...
Attachez un terminal à un serveur backend Argus déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
argus attach [url]
```

Cela permet d'utiliser le TUI avec un backend Argus distant. Par exemple :
...
Argus est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `argus auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/argus/auth.json`.

```bash
argus auth login
```

Lorsque Argus démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans Argus.
...
Exécutez argus en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `argus serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| `--attach`   |       | Connectez-vous à un serveur argus en cours d'exécution (par exemple, http://localhost:4096) |
...
Démarrez un serveur Argus sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
argus serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'Argus sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `argus`).
...
Liste toutes les sessions Argus.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions Argus.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage Argus.

```bash
argus import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage Argus.
...
Démarrez un serveur Argus sans interface graphique avec une interface Web.

```bash
argus web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à Argus via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `argus`).
...
Désinstallez Argus et supprimez tous les fichiers associés.
...
Met à jour argus vers la dernière version ou une version spécifique.
...
La CLI argus prend les flags globaux suivants.
...
Argus peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `argus`) |
