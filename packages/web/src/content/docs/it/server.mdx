---
title: Server
description: Interagisci con il server opencorvus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Il comando `opencorvus serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client opencorvus.
...
| --------------- | --------------------------------------- | ------------------ |
| `--port`        | Porta su cui ascoltare                  | `7878`             |
| `--hostname`    | Hostname su cui ascoltare               | `127.0.0.1`        |
| `--mdns`        | Abilita la scoperta mDNS                | `false`            |
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `opencorvus.local` |
...
Imposta `OPENCORVUS_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `opencorvus`, oppure imposta `OPENCORVUS_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `opencorvus serve` sia per `opencorvus web`.
...
Quando esegui `opencorvus` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server opencorvus per interagire con opencorvus in modo programmatico.
:::

Questa architettura permette a opencorvus di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `opencorvus serve` per avviare un server standalone. Se la TUI di opencorvus e gia in esecuzione, `opencorvus serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenCorvus.
...
Per esempio, `http://localhost:7878/doc`. Usa la spec per generare client o ispezionare i tipi di request/response, oppure visualizzala in uno Swagger explorer.

---

## API

Il server opencorvus espone le seguenti API.
...
| `POST`   | `/permission/:requestID/reply` | Risponde a una richiesta permessi     | body: `{ reply, message? }`, returns `boolean`                                     |
...
| `POST` | `/tui/control/response` | Risponde a una richiesta di controllo (`{ id, body } or { id, error }`) | `boolean`              |
