...
const cliArchiveName = /^opencorvus-(?:linux|darwin|windows)-[A-Za-z0-9-]+\.(?:tar\.gz|zip)$/
...
function stagedName(file: string): string | undefined {
  ...
  if (artifact.startsWith("opencorvus-dist-")) {
    return cliArchiveName.test(basename) ? basename : undefined
  }
