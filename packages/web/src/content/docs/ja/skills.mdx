...
エージェントスキルにより、OpenCorvus はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
OpenCorvus は次の場所を検索します。

- プロジェクト設定: `.opencorvus/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、OpenCorvus は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.opencorvus/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/opencorvus/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.opencorvus/skills/git-release/SKILL.md` を作成します。
...
OpenCorvus では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`opencorvus.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`opencorvus.json` 内):
