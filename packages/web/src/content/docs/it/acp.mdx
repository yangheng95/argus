---
title: Supporto ACP
description: Usa Argus in qualunque editor compatibile con ACP.
---

Argus supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Argus via ACP, configura l'editor in modo che esegua il comando `argus acp`.

Il comando avvia Argus come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Argus' nel selettore agenti di AI Chat.
...
Per usare Argus come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Argus come agente ACP per la chat.
...
Argus funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Argus
