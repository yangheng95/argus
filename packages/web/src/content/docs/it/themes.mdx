...
Con Argus puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Argus usa il tema `argus`.
...
Argus include diversi temi integrati.
...
- Vuole che Argus corrisponda all'aspetto del terminale
...
Argus supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/argus/themes/*.json` o `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.argus/themes/*.json`
4. **Directory di lavoro corrente** - in `./.argus/themes/*.json`
