...
- DOC-08: MirrorTest virtual-agent directories referenced by manifest and generated payload must be staged with the delivery.
...
- Stage MirrorTest virtual-agent directories and re-stage modified records before commit.
...
- Stage the new payload stamp helper, MirrorTest virtual-agent files, isolated test harness files, and related new tests while still excluding the untracked July planning draft.
...
- Repository searches and file reads covered generated build helper imports, MirrorTest virtual-agent payload files, `BrowserPreviewPanel` verification/evidence state, `browser-preview-evidence.test.ts`, `Process.spawn()`, `jsonLines()`, JSON-RPC transport cleanup, and process-tree close boundaries.

Agent feedback:

- Descartes found that `generate-build-artifacts.ts`, `build-overlay-payload-stamp.ts`, and MirrorTest virtual-agent files imported by the generated payload were still untracked, so a clean checkout could fail generate/build/package even when the current workspace passed.
...
- Planck repeated DOC-14: generated build helper modules and MirrorTest virtual-agent payload source files were still untracked before staging. This remains a staging/verification item, not a code-path gap after the trackedness tests were added.
...
- Four read-only agents audited packaging/generated delivery, Browser Preview GUI state, expert-squad isolation, and lifecycle cleanup. They were instructed not to edit files, stage/commit/push, create worktrees, interfere with running OpenCorvus or overlay processes, or spawn subagents.
- Repository searches and file reads covered untracked MirrorTest visual-qa/runner files, generated payload imports, Browser Preview native screenshot tests, process supervisor/group cleanup, Browser MCP node launcher, overlay browser sidecar launcher, and overlay RPC inactivity handling.

Agent feedback:

- Godel found that MirrorTest `visual-qa` and `opentest-runner` files are referenced by the manifest and generated payload but remain untracked, so clean checkout/package delivery would miss payload inputs.
...
- DOC-15: every MirrorTest visual-qa/runner source referenced by manifest or generated payload must be tracked and staged with the record that introduced it.
...
- LIFE-44: the MirrorTest runner tool must use the same process-tree/process-group ownership principle as the runtime process helpers.

Repair plan:

- Stage the MirrorTest visual-qa/runner source files and registration record, regenerate payload, then run full payload generation/trackedness tests.
- Rework MirrorTest runner termination to launch a process group on POSIX, use Windows `taskkill /T /F`, reset no-activity timeout on stdout/stderr bytes, and pin the behavior in package payload tests.
...
- Staged the MirrorTest `opentest-runner`, `visual-qa` prompt/skill, and the visual QA runner registration record that were already referenced by the MirrorTest manifest and generated payload.
...
- Changed MirrorTest runner process ownership to POSIX process groups and Windows `taskkill /T /F`, with no-activity timeout reset on stdout/stderr byte chunks and post-run cleanup of residual POSIX group descendants.
...
- `bun test packages/opencorvus/test/expert-squad/payload-generation.test.ts packages/opencorvus/test/expert-squad/package-manager.test.ts --timeout 60000 --test-name-pattern "payload|opentest|MirrorTest"` passed.
...
- Four read-only agents audited packaging/generated delivery, Browser Preview GUI state, expert-squad isolation, and lifecycle cleanup. They were instructed not to edit files, stage/commit/push, create worktrees, interfere with running OpenCorvus or overlay processes, or spawn subagents.
- Repository searches and file reads covered staged MirrorTest manifest/payload/files, BrowserPreviewPanel refreshKey flow, board refresh updates, local CLI speech-to-text provider, expert-squad resolver/registry/tool projection, and current staged diffs.

Agent feedback:

- Volta found that staged MirrorTest delivery is internally inconsistent: payload and new files include `opentest-runner` and `visual-qa`, but the staged manifest does not register them, so clean checkout would carry unreachable resources.
...
- Kierkegaard found no new high-confidence expert-squad isolation issue beyond the MirrorTest staged delivery inconsistency.
...
- DOC-16: MirrorTest manifest, prompts, route/resolver/skill tests, payload, and package files must be staged as one coherent registration surface.
...
- Stage the MirrorTest registration files and tests that already register `visual-qa` and `opentest/shared/opentest-runner`, regenerate payload, then run registry/resolver/route/skill/payload tests.
...
- Re-generated and staged the MirrorTest payload after staging the matching `visual-qa` virtual agent, `opentest/shared/opentest-runner`, manifest, README, selector, role prompts, resolver, registry, route, skill, payload, and package-manager tests as one coherent registration surface.
...
- `bun test packages/opencorvus/test/expert-squad/registry.test.ts packages/opencorvus/test/expert-squad/prompt-profile-resolver.test.ts packages/opencorvus/test/server/expert-squad-routes.test.ts packages/opencorvus/test/server/skill-routes.test.ts --timeout 60000 --test-name-pattern "opentest|MirrorTest|visual-qa|opentest-runner"` passed for the matched resolver, registry, and skill cases.
...
- Iteration 24 was launched after Iteration 23 repairs passed focused MirrorTest payload/resolver/route/skill tests, Browser Preview real browser tests with screenshot review, local-cli process cleanup tests, package and root typechecks, route/docs checks, docs link health, Rust `cargo check`, whitespace checks, generator validation using local models snapshot, and overlay-server build using local models snapshot.
...
- Lovelace found no new high-confidence expert-squad projection/isolation issue after the MirrorTest registration fixes.
...
- Rawls found that expert-squad package tool projection and bundle cache keys ignore transitive package-local imports. MirrorTest tool behavior can change when `protocol-engine/opentest-contract.json` or `protocol-engine/opentest-protocol-engine.ts` changes, but the current projection hash and bundle cache can remain unchanged.
...
- Add a package-local source-tree digest for projected package tool refs, include it in projection fingerprints and bundle cache keys, and add resolver coverage that mutating MirrorTest protocol-engine sources changes the active projection hash.
...
- Added package source tree hashing for expert-squad package tools, included it in projection package tool digests and Bun bundle cache keys, and extended the resolver hash test to mutate MirrorTest `protocol-engine/opentest-contract.json`.
...
- Einstein found that SDK server startup still has a pre-startup root-exit rejection path that calls `failStartup()` without first cleaning the owned process tree, VS Code E2E visual readiness uses a wall-clock timeout instead of a no-activity timeout reset by event-log progress, and LocalCLI / MirrorTest runner cleanup still has Windows root-exit descendant gaps.
...
- LIFE-50: LocalCLI speech-to-text and the MirrorTest runner must clean Windows descendants after the root process exits; successful root exit cannot make ignored-stdio descendants unreachable or silently skipped.
...
- Extend process cleanup support and tests for LocalCLI speech-to-text and MirrorTest runner Windows-style root-exit descendants, without adding root-only fallback cleanup.
...
- Replaced SDK, LocalCLI, and MirrorTest runner Windows root-exit cleanup with a `ParentProcessId` descendant scan through `Get-CimInstance Win32_Process`, so cleanup still sees descendants after the root process has exited.
- Updated LocalCLI and MirrorTest runner completion paths to invoke cleanup after successful root exit as well as timeout/abort, matching the POSIX process-group cleanup contract.
...
- Regenerated `packages/opencorvus/src/expert-squad/payload.ts` from the updated MirrorTest package runner source.
...
- Full-grep evidence before edits:
  ...
  - MirrorTest runner lifecycle: `opentest-runner`, `context.abort`, `terminationFailure`, `clearInactivityTimer`, and `inactive_timeout_ms` were searched across the MirrorTest package, package-manager tests, payload-generation tests, and lifecycle utility tests.
...
- Boyle found that MirrorTest runner spawns before checking an already-aborted signal and clears its inactivity timer / abort listener only after the termination race resolves; a cleanup failure can leave the long inactivity timer and abort listener live.
...
- LIFE-57: MirrorTest runner must honor an already-aborted signal before spawning and must clear the inactivity timer / abort listener in a `finally` path even when termination fails.
...
- PROJ-22: The current MirrorTest manifest and resolver tests still reference `query_failed_goals` as a standalone built-in scheduler tool even though the canonical scheduler base tool list no longer exposes it. The package projection must use the current `manage_task action=query_failed_goals` surface instead of retaining the retired tool ID.
...
- Harden MirrorTest runner cancellation with an already-aborted pre-spawn check and `try/finally` cleanup around the completion/termination race; update package payload tests and regenerate payload.
...
- Remove `query_failed_goals` from the MirrorTest scheduler manifest and resolver expected scheduler base list, relying on `manage_task action=query_failed_goals` as the current single tool surface.
...
- Changed MirrorTest runner to reject an already-aborted command before spawning and to clear inactivity timer / abort listener in `finally`, even when termination fails. Regenerated expert-squad payload from the updated package source.
...
- Removed the retired standalone `query_failed_goals` built-in scheduler tool from the MirrorTest package projection and resolver expected scheduler base list.
...
- `node test/browser-runner.mjs test/browser/browser-preview-evidence.test.ts` passed.
- `bun test packages/opencorvus/test/script/historical-docs-links.test.ts packages/opencorvus/test/script/document-health.test.ts --timeout 60000` did not pass because `specs/records/2026-07/README.md` links to untracked `2026-07-07-dependency-contract-single-source-repair.md`. This is a docs/index tracking issue outside the SDK/GUI/MirrorTest/catalog code paths and remains unresolved in the current worktree.
...
- Iteration 29 was launched after Iteration 28 SDK/GUI/MirrorTest/catalog repairs passed focused tests, SDK raw pack dry-run, SDK/overlay/opencorvus/root typechecks, route inventory check, Browser Preview and Expert Squad browser tests with screenshot review, and full prompt-profile-resolver validation.
...
- Averroes found that Windows PowerShell cleanup delegates in MirrorTest runner, LocalCLI, and VS Code process-tree cleanup can hang forever before bounded PID waits begin. Averroes also found the outer VS Code E2E idle timer resets only on stdout/stderr, ignoring event-log progress that the suite writes without console output.
- Ptolemy found no new high-confidence expert-squad/catalog/payload/projection issue after checking active/inactive catalog split, active MCP server-ref hash parity, payload release identity checks, runtime built-in `general` boundary, and `query_failed_goals` removal from MirrorTest package projection.
...
- Add bounded timeout handling to Windows PowerShell cleanup helpers in LocalCLI, MirrorTest runner, and VS Code process-tree cleanup, plus static/focused tests for the timeout guard.
...
- Curie found three lifecycle issues: shared Windows process cleanup can miss descendants if the root exits between the initial root-alive check and `taskkill`; LocalCLI/MirrorTest PowerShell cleanup can abort on a stale PID before killing later descendants; VS Code visual screenshot capture uses unbounded synchronous PowerShell and can block the idle cleanup timer.
...
- LIFE-63: LocalCLI and MirrorTest Windows cleanup scripts must tolerate stale target PIDs and continue stopping remaining descendants.
...
- Change LocalCLI/MirrorTest PowerShell cleanup `Stop-Process` calls to `-ErrorAction SilentlyContinue`, matching VS Code process-tree cleanup, then regenerate expert-squad payload.
...
- Changed LocalCLI and MirrorTest Windows cleanup scripts to use `Stop-Process -ErrorAction SilentlyContinue`, so a stale PID cannot abort cleanup of later descendants.
...
- Regenerated `packages/opencorvus/src/expert-squad/payload.ts` after the MirrorTest runner source change.
...
- SDK-LIFE-01: SDK `createOpenCorvusServer()` Windows PowerShell cleanup helper has no timeout. Startup timeout, abort, or close can therefore block forever before the bounded PID wait begins. This is distinct from LocalCLI/MirrorTest/VS Code cleanup helper fixes because it lives in `packages/sdk/js/src/server.ts`.
...
- Add bounded PowerShell cleanup behavior to SDK server `runWindowsPowerShellForProcessIDs`, matching the established LocalCLI/MirrorTest pattern, and change stale target handling to `-ErrorAction SilentlyContinue`.
...
- Schrodinger found no new high-confidence expert-squad catalog/payload/projection bug after reviewing registry, manager, resolver, catalog schema/profile, payload source, MirrorTest package, and projection single-source paths.
