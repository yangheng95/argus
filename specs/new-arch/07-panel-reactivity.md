...
| `session.status`                        | 单一通道。按 `e.sessionID` 定位卡片，`setStore("cards", cardID, { status: e.status, terminalReason: e.terminalReason })`。`status ∈ {streaming, idle, terminal}`；session 未 materialize 时缓冲到 `pendingSessionStatus`，`ensureSessionCard` 在收到首个 `message.updated` 时 drain。所有 session（orchestrator 根 / requirements / frontend-design / architect / workload_analysis / build / visual_qa / integrity / refine / analyze_intent / modify_goal / explore / fact_check / 未来新 phase）走同一条路径，不再有 phase 专属事件。 |
...
architect / frontend-design / workload_analysis / build / visual_qa / integrity /
refine / analyze_intent / modify_goal / explore / fact_check / ...）、未来新加的 phase。**不再有
