      ...
      execute: async (params) => {
        ...
        return {
          ...
          output: [
            ...
            "5. First populate the visual HTML skeleton with source-editable static HTML/CSS/assets copied or adapted from this evidence project and `web-clone-source`, not from memory: `visual-html-skeleton/index.html`, `visual-html-skeleton/styles/tokens.css`, layout/region CSS files, asset references, representative-state markup, task-scoped screenshots/diff artifacts under `visual-html-skeleton/...` when available, and a README/source note that names the skeleton evidence source. This is the required pre-region visual write sequence, and it must happen before selecting a large replacement region. Do not start from an empty app scaffold with isolated handwritten components, compiled app output, raw source DOM dumps, iframe previews, or screenshot wrappers.",
