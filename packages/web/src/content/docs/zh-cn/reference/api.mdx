...
| PATCH | `/global/config` | Update global configuration 错误: `BadRequestError`, `NonCanonicalConfigFileError`. | `global.config.update` |
...
| POST | `/global/providers/discover-models` | Discover global Provider models 请求体: `api`, `apiKey`, `providerID`; 错误: `BadRequestError`. | `global.providers.discover.models` |
...
| DELETE | `/global/providers/{providerID}` | Delete a global provider 错误: `BadRequestError`. | `global.providers.remove` |
| GET | `/global/providers/{providerID}/account-usage` | Get global Provider account usage | `global.providers.account.usage` |
| POST | `/global/providers/{providerID}/auth/execute` | Execute global Provider auth 请求体: `inputs`, `method`; 错误: `BadRequestError`. | `global.providers.auth.execute` |
...
| POST | `/global/providers/{providerID}/oauth/callback` | Complete global Provider subscription authorization 请求体: `code`, `method`; 错误: `BadRequestError`. | `global.providers.oauth.callback` |
| POST | `/global/providers/{providerID}/test` | Test a global Provider connection 请求体: `modelID`; 错误: `BadRequestError`. | `global.providers.test` |
...
| POST | `/global/tasks` | Create a task in an implicit project 请求体: `artifactImports`, `attachments`, `budget`, `channelBinding`, `checks`, `directory`, `expectedPackageDigest`, `metadata`, `model`, `priority`, `productPillar`, `project`, `promptProfile`, `queue`, `request`, `requestID`, `source`, `title`; 错误: `BadRequestError`, `ExternalChildTaskLineageError`, `LogFileNotFoundError`, `NotFoundError`, `TaskCreationIdempotencyConflictError`, `TaskCreatorAuthorityError`, `TaskCreatorSessionError`, `TaskExpectedPackageDigestConflictError`, `TaskPackageRevisionBindingError`. | `task.global.create` |
...
| GET | `/config` | Get configuration 错误: `NonCanonicalConfigFileError`. | `config.get` |
| PATCH | `/config` | Update configuration (JSON Merge Patch) 错误: `BadRequestError`, `NonCanonicalConfigFileError`. | `config.update` |
...
| POST | `/mission/wake` | Wake the Mission agent 请求体: `attachments`, `expertSquadIDs`, `missionID`, `model`, `productPillar`, `text`; 错误: `BadRequestError`. | `mission.wake` |
...
| POST | `/mission/{missionID}/dispatch` | Dispatch a Mission draft 请求体: `model`; 错误: `BadRequestError`, `LogFileNotFoundError`, `NotFoundError`. | `mission.dispatch` |
...
| POST | `/task/{taskID}/replan` | Replan task 错误: `LogFileNotFoundError`, `NotFoundError`, `TaskControlIntentLifecycleConflictError`. | `task.replan` |
| POST | `/task/{taskID}/retry` | Retry task 错误: `LogFileNotFoundError`, `NotFoundError`, `TaskControlIntentLifecycleConflictError`. | `task.retry` |
...
| POST | `/session` | Create session 请求体: `kind`, `metadata`, `parentID`, `permission`, `title`; 错误: `BadRequestError`. | `session.create` |
...
| DELETE | `/session/{sessionID}` | Delete session 错误: `BadRequestError`, `LogFileNotFoundError`, `NotFoundError`, `TaskArtifactDeletionCommittedError`, `TaskBoundSessionDeletionError`, `TaskCancellationIncompleteError`, `UnknownError`. | `session.delete` |
...
| POST | `/session/{sessionID}/abort` | Abort session 错误: `BadRequestError`, `LogFileNotFoundError`, `NotFoundError`, `TaskCancellationIncompleteError`. | `session.abort` |
...
| PATCH | `/session/{sessionID}/config` | Update session configuration overlay 请求体: `agent`, `expert_squads`, `model`, `prompt`, `prompt_profile`, `runtime_templates`, `skill_mounts`; 错误: `BadRequestError`, `LogFileNotFoundError`, `NotFoundError`, `TaskPackageRevisionBindingError`, `TaskPromptProfileImmutableError`. | `session.config.update` |
...
| POST | `/session/{sessionID}/message` | Send message 请求体: `agent`, `format`, `messageID`, `model`, `parts`, `variant`; 错误: `BadRequestError`, `LogFileNotFoundError`, `NotFoundError`. | `session.prompt` |
...
| POST | `/session/{sessionID}/prompt_async` | Send async message 请求体: `agent`, `format`, `messageID`, `model`, `parts`, `variant`; 错误: `BadRequestError`, `LogFileNotFoundError`, `NotFoundError`. | `session.prompt_async` |
...
| Method | 路径 | 说明 | operationId |
|---|---|---|---|
| POST | `/coding/chat/session` | Create right sidebar Chat session 请求体: `model`. | `coding.chat.session.create` |
...
| POST | `/coding/work/session` | Create right sidebar Work session 请求体: `model`. | `coding.work.session.create` |
...
| POST | `/panel/message` | Handle desktop panel message 请求体: `allow_create`, `attachments`, `channel`, `model`, `request_id`, `sessionID`, `source`, `surface`, `taskID`, `text`, `thread`, `user_id`. | `panel.message` |
| POST | `/panel/message/stream` | Handle desktop panel message with streaming 请求体: `allow_create`, `attachments`, `channel`, `model`, `request_id`, `sessionID`, `source`, `surface`, `taskID`, `text`, `thread`, `user_id`. | `panel.message.stream` |

### Expert Squad

| Method | 路径 | 说明 | operationId |
|---|---|---|---|
| GET | `/expert-squad/catalog` | List expert squads and active capability projection 错误: `BadRequestError`, `NotFoundError`, `UnknownError`. | `expertSquad.catalog` |
...
| GET | `/expert-squad/settings` | Inspect project Expert Squad settings 错误: `BadRequestError`, `NotFoundError`, `UnknownError`. | `expertSquad.settings` |
| POST | `/expert-squad/uninstall` | Uninstall an expert squad package 请求体: `id`, `installationScope`, `replacementID`; 错误: `ExpertSquadPackageError`. | `expertSquad.uninstall` |
...
| Method | 路径 | 说明 | operationId |
|---|---|---|---|
| POST | `/computer/return` | Return Computer guest to Agent automation 请求体: `computerID`, `displayID`, `sessionID`; 错误: `BadRequestError`, `UnknownError`. | `computer.return` |
| POST | `/computer/status` | Get Computer guest ownership 请求体: `computerID`, `displayID`, `sessionID`; 错误: `BadRequestError`, `UnknownError`. | `computer.status` |
| POST | `/computer/takeover` | Take over Computer guest 请求体: `computerID`, `displayID`, `sessionID`; 错误: `BadRequestError`, `UnknownError`. | `computer.takeover` |
| POST | `/computer/viewer/open` | Open Computer viewer 请求体: `computerID`, `displayID`, `sessionID`; 错误: `BadRequestError`, `UnknownError`. | `computer.viewer.open` |
...
| DELETE | `/provider/{providerID}` | Delete a project provider 错误: `BadRequestError`. | `provider.remove` |
