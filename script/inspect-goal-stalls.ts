...
const buildSessions = db.query<{ id: string; goal_id: string; time_created: number; time_updated: number }, []>(
...
for (const b of buildSessions) {
  console.log(`${b.id}  goal=${b.goal_id}  start=${new Date(b.time_created).toISOString()}  last=${new Date(b.time_updated).toISOString()}  age=${((b.time_updated - b.time_created) / 1000).toFixed(1)}s`);
}
...
for (const b of buildSessions) {
  ...
  const msgs = db.query<{ id: string; data: string; time_created: number }, []>(
  ...
  for (const m of msgs) {
    ...
    console.log(`${m.id}  role=${d.role}  agent=${d.agent}  finish=${d.finish}  tokens=${d.tokens?.total}  t=${new Date(m.time_created).toISOString()}${errStr ? "  ERR=" + errStr.slice(0, 250) : ""}`);
  }
  console.log(`\n----- last 14 parts -----`);
  const parts = db.query<{ id: string; message_id: string; data: string; time_created: number }, []>(
  ...
  for (const p of parts) {
    ...
    console.log(`${p.id}  msg=${p.message_id}  type=${d.type}  tool=${tool}  status=${status}  title="${title}"  text="${text}"  t=${new Date(p.time_created).toISOString()}`);
  }
}
...
for (const b of buildSessions) {
  ...
  const last = db.query<{ id: string; data: string; time_created: number }, []>(
  ...
  const pendingParts = db.query<{ id: string; data: string; time_created: number }, []>(
  ...
  for (const p of pendingParts) {
    let d: any = {};
    try { d = JSON.parse(p.data); } catch {}
    console.log(`${p.id}  tool=${d.tool}  status=${d.state?.status}  title="${(d.state?.title ?? "").toString().slice(0, 120)}"  t=${new Date(p.time_created).toISOString()}`);
    if (d.state?.input) console.log(`  input: ${JSON.stringify(d.state.input).slice(0, 600)}`);
    if (d.state?.error) console.log(`  error: ${typeof d.state.error === "string" ? d.state.error.slice(0, 600) : JSON.stringify(d.state.error).slice(0, 600)}`);
  }
}

console.log("\n\n===== integrity verdict events for this task =====");
const integrityArts = db.query<{ id: string; payload: string; time_created: number; label: string }, []>(
