...
Puoi aggiungere strumenti esterni a Argus usando il _Model Context Protocol_, o MCP. Argus supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Argus Config](https://argus.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/argus`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Argus gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Argus:
...
Se il server richiede autenticazione, Argus ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `argus mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Argus memorizzera' i token in modo sicuro in `~/.local/share/argus/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Argus insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Argus come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Argus al tuo account Sentry.
