...
本文档基于当前仓库源码、`package.json` workspace 配置、README、`docs/packaging.md` 和 `specs/current/architecture` 索引整理，用于快速理解本代码仓的模块边界。
...
| `specs/` | 架构与历史记录单一来源。`specs/current/architecture` 是当前架构说明，`specs/records/YYYY-MM` 是按月调查/实施记录，`specs/artifacts` 存任务输入或参考制品。 |
| `expert-squads/` | 可安装或内置的专家团包，如 frontend replica、research studio、review debug、mirror prism 等。 |
...
| `AGENTS.md`, `CLAUDE.md` | Agent 协作规则和项目约束。 |
...
| 路径 | 功能 |
| --- | --- |
| `src/provider/*` | LLM provider 配置、鉴权、模型 schema、vendor adapter、DashScope/Hexin discovery、sampling、strict tool schema 等。 |
...
| `expert-squads/mirror/prism` | Mirror Prism 相关专家团。 |
| `expert-squads/tanzeqi/mirror-watch` | Mirror Watch 相关专家团，含网页抓取和 Ainvest design system 技能。 |
| `expert-squads/wujiang/opentest` | OpenTest 相关专家团。 |
...
3. `specs/current/architecture/README.md`：当前架构章节索引。
4. `packages/opencorvus/src/index.ts`：CLI 入口。
5. `packages/opencorvus/src/server/routes/orchestrator.ts` 和 `src/task-api/*`：任务 API 入口。
6. `packages/opencorvus/src/engine/task.ts`, `queue.ts`, `pipeline.ts`：任务生命周期核心。
7. `packages/opencorvus/src/orchestrator/loop.ts` 和各 stage/tool 文件：编排主循环。
8. `packages/opencorvus/src/agent/*`：agent runtime、prompt profile 和工具池。
9. `packages/overlay/src/main.tsx`：桌面 UI 状态和交互入口。
10. `packages/channel-runtime/src/main.ts`：多渠道接入入口。
