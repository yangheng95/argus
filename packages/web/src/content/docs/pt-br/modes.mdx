...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do opencorvus. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no opencorvus permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do opencorvus.
...
O opencorvus vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.opencorvus/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `opencorvus.json`:
...
- Global: `~/.config/opencorvus/modes/`
- Projeto: `.opencorvus/modes/`

```markdown title="~/.config/opencorvus/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o opencorvus usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do opencorvus quanto para a configuração específica do projeto.
...
```json title="opencorvus.json" {4-14}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.opencorvus/modes/` para modos específicos do projeto ou `~/.config/opencorvus/modes/` para modos globais:

```markdown title=".opencorvus/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/opencorvus/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
