---
name: opencorvus-whatsapp-bot-config
description: Configure and troubleshoot WhatsApp bot integration for OpenCorvus. Use when tasks involve WHATSAPP_ACCESS_TOKEN, WHATSAPP_PHONE_NUMBER_ID setup, adapter wiring checks in packages/bot/src/main.ts, updates to packages/bot/.env, or WhatsApp message delivery debugging.
---

# OpenCorvus WhatsApp Bot Config

## Overview

Configure WhatsApp for OpenCorvus using a repeatable checklist and map platform setup to the adapter implementation in packages/bot.
...
- Set required env keys: WHATSAPP_ACCESS_TOKEN, WHATSAPP_PHONE_NUMBER_ID.
- Set optional env keys when needed: WHATSAPP_VERIFY_TOKEN, WHATSAPP_WEBHOOK_HOST, WHATSAPP_WEBHOOK_PORT, WHATSAPP_WEBHOOK_PATH.
...
- Confirm adapter implementation exists at packages/bot/src/adapters/whatsapp.ts.
