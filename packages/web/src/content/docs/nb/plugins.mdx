...
description: Skriv dine egne programtillegg for å utvide opencorvus.
---

Plugins lar deg utvide OpenCorvus ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenCorvus.
...
- `.opencorvus/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/opencorvus/node_modules/`.
...
1. Global konfigurasjon (`~/.config/opencorvus/opencorvus.json`)
2. Prosjektkonfigurasjon (`opencorvus.json`)
3. Global plugin-katalog (`~/.config/opencorvus/plugins/`)
4. Prosjektpluginkatalog (`.opencorvus/plugins/`)
...
OpenCorvus kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenCorvus SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenCorvus.
...
:::note
Hvis du bruker OpenCorvus-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenCorvus fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenCorvus:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenCorvus kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenCorvus sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://opencorvus.ai/docs/sdk) for detaljer.
