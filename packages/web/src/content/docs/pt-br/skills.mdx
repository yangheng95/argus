...
As habilidades do agente permitem que o argus descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O argus pesquisa nesses locais:

- Configuração do projeto: `.argus/skills/<name>/SKILL.md`
- Configuração global: `~/.config/argus/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o argus sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.argus/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.argus/skills/git-release/SKILL.md` assim:
...
O argus lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `argus.json`:
...
**Para agentes embutidos** (em `argus.json`):
