---
title: Servidor
description: Interactuar con el servidor opencorvus a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

El comando `opencorvus serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente opencorvus puede usar.
...
| --------------- | ----------------------------------------------------- | ------------------ |
| `--port`        | Puerto para escuchar                                  | `7878`             |
| `--hostname`    | Nombre de host para escuchar                          | `127.0.0.1`        |
| `--mdns`        | Habilitar el descubrimiento de mDNS                   | `false`            |
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `opencorvus.local` |
...
Configure `OPENCORVUS_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `opencorvus`, o configure `OPENCORVUS_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `opencorvus serve` como a `opencorvus web`.
...
Cuando ejecuta `opencorvus`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor opencorvus para interactuar con opencorvus mediante programación.
:::

Esta arquitectura permite que opencorvus admita múltiples clientes y le permite interactuar con opencorvus mediante programación.

Puede ejecutar `opencorvus serve` para iniciar un servidor independiente. Si tienes el
opencorvus TUI ejecutándose, `opencorvus serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenCorvus [IDE](/docs/ide).
...
Por ejemplo, `http://localhost:7878/doc`. Utilice la especificación para generar clientes o inspeccionar tipos de solicitudes y respuestas. O verlo en un explorador Swagger.

---

## API

El servidor opencorvus expone las siguientes API.
...
| `POST`   | `/permission/:requestID/reply` | Responder a una solicitud de permiso                   | cuerpo: `{ reply, message? }`, devuelve `boolean`                                     |
...
| `POST` | `/tui/control/response` | Responder a una solicitud de control (`{ id, body } or { id, error }`) | `boolean`                      |
