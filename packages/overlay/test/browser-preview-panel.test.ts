...
test("browser preview panel uses mature primitives and HostTransport-backed service", () => {
  ...
  expect(component).toContain("target()?.viewports")
  expect(component).toContain("verifyBrowserPreviewTarget")
  expect(component).toContain("<iframe")
  expect(component).toContain("frameElement.src = frameSrc")
  ...
  expect(component).toContain("target.error")
  ...
  expect(service).toContain('apiJson(`browser-preview/target')
  expect(service).toContain('apiJson("browser-preview/verify"')
