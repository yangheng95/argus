...
Создайте `.argus/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию argus или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем argus [config](/docs/config):
...
- Глобальный: `~/.config/argus/commands/`
- Для каждого проекта: `.argus/commands/`
...
argus включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
