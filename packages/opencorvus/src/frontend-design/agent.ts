...
function buildUserPrompt(
  ...
  sections.push(
    "# Live URL Capture\n\n" +
      ...
      "The final `submit_frontend_template.frontend_project` field should name the visual HTML skeleton root and entrypoints with role=visual_baseline_input only for screenshot-validated visual-skeleton-first work, and `submit_frontend_template.visual_validation_evidence` must carry renderer, rendered_entrypoint, screenshot_artifact, source_reference_artifact, hashes, viewport, review_status, and review_summary; for production/component-system work, report an implementation scaffold as role=implementation_target or report role=blocked with the missing scaffold/screenshot evidence, and submit `implementation_phase_outcomes` as the primary Architect consumption surface. A visual baseline must explicitly say it is source-editable static HTML/CSS, not compiled output, not raw source DOM replay, not the implementation target, not an acceptance app root, and not an independent design source. " +
...
function renderFinalAcceptanceModeInstruction(hostPrepared: boolean): string {
  if (hostPrepared) {
    return [
      ...
      "Set `submit_frontend_template.final_acceptance_mode` to `visual_baseline_allowed` and `frontend_project.role=visual_baseline_input` only when the operator accepts a visual-skeleton first workflow and structured `visual_validation_evidence` proves the skeleton usable.",
