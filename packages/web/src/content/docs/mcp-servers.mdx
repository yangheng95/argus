...
2. **Executor MCP Server** — OpenCorvus starts a stdio MCP subprocess for external coding executors and exposes the OpenCorvus-owned executor toolset through that subprocess.
...
## OpenCorvus Executor MCP Server

External coding executors do not connect to the main `opencorvus serve` HTTP server for the executor MCP surface. OpenCorvus builds a stdio command with `MCPServe.command(cwd)` and injects that command into the executor configuration:

- **Claude Code** receives an stdio MCP server config from the Anthropic Agent SDK integration.
- **Codex** receives an stdio `mcp_servers.opencorvus` config for `codex app-server`.

The stdio server creates an executor-scoped session for the supplied working directory. It exposes the OpenCorvus-owned executor tools `skill`, `memory`, and `task_report`, then proxies tools, prompts, and resources from configured external MCP clients when those tools are allowed for coding executors.

Browser evidence tools are not exposed through the executor MCP server. Frontend evidence is prepared by the upstream frontend-design workflow and must be read from persisted evidence artifacts.
