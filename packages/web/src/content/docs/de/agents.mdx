...
Es gibt zwei Arten von Agenten in OpenCorvus; Primäragenten und Subagenten.
...
OpenCorvus verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `opencorvus.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet OpenCorvus modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale OpenCorvus-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer OpenCorvus-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenCorvus Zen](/docs/zen) verwenden, würden Sie `opencorvus/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `opencorvus models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/yangheng95/opencorvus).
:::
