...
export namespace CodexAppServerClientProcess {
  ...
  export function fromTransport(transport: JsonRpcTransport): CodexAppServerClient {
    return {
      initialize(input) {
        return transport.request("initialize", input as unknown as Record<string, unknown>)
      },
      threadStart(input) {
        return transport.request("thread/start", input).then((result) => result as { thread: { id: string } })
      },
      threadResume(input) {
        return transport.request("thread/resume", input).then((result) => result as { thread: { id: string } })
      },
      turnStart(input) {
        return transport.request("turn/start", input).then((result) => result as { turn: { id: string } })
      },
      turnInterrupt(input) {
        return transport.request("turn/interrupt", input)
      },
      respond(input) {
