...
| `rg -n -F 'selectedRightActivity' packages/overlay/src/main.tsx packages/overlay/test/acceptance-panel-mount.test.ts specs`                               | `main.tsx` and static tests still keep the residual signal. Specs say it should be removed.       | Delete the signal and derive selected/active state from `centerWorkbenchPanels`. |
