...
- **6-f**：`EngineTaskTable` 瘦身（FSM 残余清理）。拆 sub-step：
  ...
  - **6-f-1**：删 `task.status` + `task.phase`（现在是 FSM 缓存）— 39 处读者改为走 describe 层现算；预期需要与 6-g 耦合
  - **6-f-2**：删 `task.active_run_id / task.active_plan_version_id`（FSM 指针，已被 artifact stream 取代）
  - **6-f-3**：删 `task.workflow_state`（workflow 步 FSM）、`task.blocking_reason`（blocker 缓存）、`task.time_status_changed` 依赖项
