...
describe("frontend-design prompt assembly", () => {
  test("core prompt pins visual HTML skeleton restoration workflow", async () => {
    ...
    expect(prompt).toContain("call `update_frontend_visual_region_binding`")
    expect(prompt).toContain('slicing_strategy: "horizontal_component_bands"')
    expect(prompt).toContain("contiguous `source_order`")
  ...
  test("webpage rawproject prompts make frontend_design materialize visual HTML skeleton first", () => {
    ...
    expect(prompt).toContain("For full-page webpage replica references, Frontend Design owns sub-image slicing")
    expect(prompt).toContain("first call `create_visual_region_coordinate_atlas`")
    expect(prompt).toContain("author one top-to-bottom horizontal component band per visible component/region")
    expect(prompt).toContain('slicing_strategy: "horizontal_component_bands"')
    expect(prompt).toContain("contiguous `source_order`")
    expect(prompt).toContain("call `update_frontend_visual_region_binding`")
