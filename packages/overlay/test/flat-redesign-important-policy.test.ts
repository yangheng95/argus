...
const ALLOWED_IMPORTANT_COUNTS = new Map([
  ...
  ["src/styles/surfaces/field.css:-webkit-box-shadow: 0 0 0 1000px var(--surface-inset) inset !important;", 1],
  ["src/styles/surfaces/field.css:-webkit-text-fill-color: var(--text-strong) !important;", 1],
])
...
describe("flat-redesign important policy", () => {
  it("styles keep !important constrained to the explicit browser-behavior whitelist", () => {
    ...
    if (occurrences.length > 5 || violations.length > 0) {
      throw new Error(
        `!important policy violations:\n` +
          `  total: ${occurrences.length}\n` +
          (violations.length ? `  outside whitelist:\n  ${violations.join("\n  ")}\n` : "") +
          "Only base.css hidden/reduced-motion and field.css webkit autofill declarations may use !important.",
      )
    }
