...
describe("orchestrator tool descriptions for integrity stuck loops", () => {
  ...
  test("frontend_research and frontend_design descriptions separate investigation division from UI implementation", () => {
    ...
    expect(tools.frontend_research.description).toContain("Available independently or alongside `frontend_design`")
    expect(tools.frontend_research.description).toContain("partitions prepared webpage evidence")
    expect(tools.frontend_research.description).toContain("does not acquire webpage evidence itself")
    expect(tools.frontend_research.description).toContain("dispatch `frontend_design` first")
