---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Argus zu erweitern.
---

Mit Plugins können Sie Argus erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Argus zu ändern.
...
- `.argus/plugins/` – Plugins auf Projektebene
- `~/.config/argus/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/argus/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/argus/argus.json`)
2. Projektkonfiguration (`argus.json`)
3. Globales Plugin-Verzeichnis (`~/.config/argus/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.argus/plugins/`)
...
Argus führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Argus-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Argus erweitern können.
...
:::note
Wenn Sie die Argus-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Argus `.env`-Dateien liest:
...
Plugins können Argus auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Argus aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Argus zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://argus.ai/docs/sdk).
