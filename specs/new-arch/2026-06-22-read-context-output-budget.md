...
- `specs/bug-hunt-repair-plan-2026-06-17.md` requires `read_context` goal output
  to reuse describe-layer facts. The fix must keep `NEEDS_REDISPATCH`, orphan
  facts, goal run IDs, and child session IDs visible.
- `specs/orchestrator-no-decision-stop-2026-06-18.md` treats `read_context` as an
  observation tool. It must stay read-only and must not become a decision gate.
- `specs/new-arch/2026-06-21-frontend-research-context-digest.md` requires
