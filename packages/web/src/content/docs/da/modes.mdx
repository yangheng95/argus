...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i argus-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i argus giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem argus-konfigurationen.
...
argus leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.argus/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `argus.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger argus modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale argus-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.argus/modes/` for projektspecifikke tilstande eller `~/.config/argus/modes/` for globale tilstande:
