...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. opencorvus yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
opencorvus içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENCORVUS_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENCORVUS_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencorvus provider kullanılırken veya `OPENCORVUS_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

opencorvus başlatırken etkinleştirmek için:
