...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencorvus`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `opencorvus serve` quanto ao `opencorvus web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password opencorvus serve
```
