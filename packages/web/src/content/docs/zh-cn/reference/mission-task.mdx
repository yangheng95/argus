...
所以下载代码仍然下载某个 Task 的 `project-archive`，不是下载 Mission。
...
项目作用域接口要带项目目录，例如 `GET /tasks`、`POST /task`、Task 写操作、browser preview 路由和 `GET /task/{taskID}/project-archive`。单个 Task 记录读取以 `taskID` 为主键，不需要目录，例如 `GET /task/{taskID}/status`、`GET /task/{taskID}/board`、`GET /task/{taskID}/events`。
