...
test("agent file changes render only through the right-panel Files workbench", () => {
  ...
  expect(changesPanel).toContain("active?: () => boolean")
  expect(changesPanel).toContain("const panelActive = () => props.active?.() ?? true")
  expect(changesPanel).toContain("if (!panelActive()) return []")
  expect(changesPanel).toContain("if (!panelActive()) return false")
  ...
  expect(sharedView).toContain('data-ui="file-changes-search-input"')
  ...
  expect(sharedView).toContain('data-ui="file-changes-search-clear"')
  expect(sharedView).toContain("<SegmentedControl<ChangeStatusFilter>")
  expect(sharedView).toContain("onActivate={setStatusFilter}")
  expect(sharedView).toContain('"data-ui": "file-changes-status-filter-option"')
  expect(sharedView).toContain('"data-status": option.value')
  expect(sharedView).toContain("changes-status-option-label")
  expect(sharedView).toContain("changes-status-option-count")
  expect(sharedView).not.toContain('class="changes-filter-clear"')
  expect(sharedView).not.toContain('class="changes-status-chip"')
  expect(sharedView).not.toContain("aria-pressed")
  ...
  expect(sharedView).not.toContain('data-selected={props.selected ? "true" : "false"}')
  expect(sharedView).not.toContain("selected: boolean")
  expect(sharedView).not.toContain("selected={selectedRowKey() === row.key}")
  ...
  expect(sharedView).toContain('event.key !== "Enter" && event.key !== " "')
  expect(sharedView).toContain("event.preventDefault()")
  expect(sharedView).toContain("event.stopPropagation()")
  ...
  expect(sharedView).toContain('Icon name="file-document" class="file-changes-view__heading-icon"')
  ...
  expect(changesCss).toContain(".file-changes-view__heading-icon")
  expect(changesCss).not.toContain(".file-changes-view__heading .oc-icon")
  ...
  expect(changesCss).toContain(".changes-filter-field.search-field")
  expect(changesCss).not.toMatch(/\.changes-filter-field:focus-within\b/)
  expect(changesCss).not.toMatch(/\.changes-filter-input\.field-input\b/)
  expect(changesCss).not.toContain("file-changes-filter-clear")
  expect(changesCss).toContain(".changes-status-strip")
  expect(changesCss).toContain(".changes-status-strip > .oc-tabs")
  expect(changesCss).toContain('.oc-tab[data-ui="file-changes-status-filter-option"]')
  expect(changesCss).toContain(".changes-status-option-label")
  expect(changesCss).toContain(".changes-status-option-count")
  expect(changesCss).not.toMatch(/\.changes-filter-clear\b/)
  expect(changesCss).not.toMatch(/\.changes-status-chip\b/)
  ...
  expect(changesCss).toContain(".change-row[data-highlighted]")
  expect(changesCss).toContain(".change-row[data-selected]")
  expect(changesCss).not.toContain('.change-row[data-selected="true"]')
