...
This page explains how external systems call the OpenCorvus Mission and Task APIs. The exhaustive operation list remains the [HTTP API reference](/docs/reference/api/); this guide covers the common integration path.
...
Single-task record reads are keyed by `taskID` and do not need a project directory. Use `directory` for project-scoped routes such as `GET /tasks`, `POST /task`, task write routes, browser preview routes, and `GET /task/{taskID}/project-archive`.
...
| `kind`        | `workflow` uses the full requirements, design, execution, and acceptance pipeline; `build` starts direct implementation. |
...
curl -X POST http://127.0.0.1:7878/task \
  ...
  -d "{\"request\":\"Fix the login form submit failure and add tests.\",\"kind\":\"workflow\"}"
```
...
| Field             | Meaning                                                                        |
| ----------------- | ------------------------------------------------------------------------------ |
| `status`          | Normalized external state: `running`, `success`, or `failed`.                  |
| `lifecycleStatus` | Raw task lifecycle: `queued`, `active`, `completed`, `failed`, or `cancelled`. |
| `progress`        | Total, completed, failed, running, pending, and percent.                       |
| `workflow.steps`  | Task-level workflow steps.                                                     |
| `goals[].steps`   | Per-goal execution steps and phases.                                           |
...
while (true) {
  ...
  if (snapshot.status !== "running") {
    console.log("done", snapshot.status)
    break
  }
...
| Retry current plan    | `POST /task/{taskID}/retry`     |
...
| `taskCounts` | Total, success, failed, and running counts. |
...
| Abort  | `POST /mission/{missionID}/abort`  | Cancel the active Mission agent loop.                |
...
2. `GET /mission/{missionID}/status` shows aggregate progress across dispatched Tasks.
3. `POST /mission/wake` gives the Mission more direction.
4. Use each concrete Task's `status` and `project-archive` for deliverables.

## Next

- [HTTP API reference](/docs/reference/api/)
- [JavaScript / TypeScript SDK reference](/docs/reference/sdk/)
