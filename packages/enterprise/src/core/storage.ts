...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.OPENCORVUS_STORAGE_BUCKET!
    const region = process.env.OPENCORVUS_STORAGE_REGION || "us-east-1"
  ...
  function r2() {
    const accountId = process.env.OPENCORVUS_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.OPENCORVUS_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.OPENCORVUS_STORAGE_SECRET_ACCESS_KEY!,
    })
  ...
  const adapter = lazy(() => {
    const type = process.env.OPENCORVUS_STORAGE_ADAPTER
