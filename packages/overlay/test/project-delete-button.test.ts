...
describe("project delete button", () => {
  ...
  test("ProjectLedgerGroup owns project action primitives beside the disclosure toggle", () => {
    ...
    expect(PROJECT_LEDGER_GROUP).toContain('<Icon name="plus" size={14} />')
    expect(PROJECT_LEDGER_GROUP).not.toContain('<Icon name="message-add"')
  ...
  test("sidebar CSS reserves a stable project-group action slot", () => {
    ...
    expect(SIDEBAR_CSS).toMatch(/\.project-group-actions\s*\{[^}]*opacity:\s*var\(--ui-opacity-full\);/)
    expect(SIDEBAR_CSS).toMatch(/\.project-group-actions\s*\{[^}]*visibility:\s*visible;/)
    expect(SIDEBAR_CSS).toMatch(/\.project-group-actions\s*\{[^}]*pointer-events:\s*auto;/)
    ...
    expect(SIDEBAR_CSS).not.toContain('.project-group[data-project-actions="true"]:hover .project-group-actions')
    expect(SIDEBAR_CSS).not.toContain('.project-group[data-project-actions="true"]:focus-within .project-group-actions')
    expect(SIDEBAR_CSS).not.toContain('.project-group[data-project-actions="true"]:hover .project-group-count')
    expect(SIDEBAR_CSS).not.toContain('.project-group[data-project-actions="true"]:focus-within .project-group-count')
    expect(SIDEBAR_CSS).not.toContain('.project-group[data-project-actions="true"]:hover .project-group-chevron')
    ...
    expect(SIDEBAR_CSS).toContain('--project-group-action-icon-size: var(--ui-font-body);')
    expect(SIDEBAR_CSS).toContain('.project-group .oc-button[data-ui="project-group-copy"] > svg')
    expect(SIDEBAR_CSS).toContain("width: var(--project-group-action-icon-size);")
