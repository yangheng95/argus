...
describe("overlay window and pane size contract", () => {
  ...
  test("pane and center workbench minimums are token-owned", () => {
    ...
    expect(layoutFrame).toContain('import { layoutTokenNumber, layoutTokenPx } from "./layout-tokens"')
    ...
    expect(layoutFrame).toContain('minimumAspectRatio: layoutTokenNumber("--ui-overlay-min-aspect-ratio")')
    expect(layoutFrame).not.toContain("minimum.width / minimum.height")
    ...
    expect(layoutTokens).toContain("tokenNumber(name: string): number")
    expect(layoutTokens).toContain("tokenPx(name: string): number")
    expect(layoutTokens).toContain("export function layoutTokenNumber(name: string): number")
    ...
    expect(layoutTokens).toContain("return createLayoutTokenResolver().tokenNumber(name)")
    expect(layoutTokens).toContain("calc(var(${name}) * 1px)")
