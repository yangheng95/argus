...
| `rg -n "settings.section.database|settings-db-reset|db_reset" packages/overlay/src packages/overlay/test specs/new-arch` | Database reset is already covered by source tests, i18n keys, and browser reset coverage. | Update the header guard to semantic three-group expectations, not a blind count. |
