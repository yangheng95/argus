...
| `rg -n 'role="status"         | aria-live="polite"                                                                | aria-busy="true"' packages/overlay/src/components packages/overlay/test specs`    | Browser Preview, workflow generating panels, Trace, connection, and provider surfaces already use live status contracts. | Reuse the established `role="status" aria-live="polite" aria-busy="true"` contract. |
