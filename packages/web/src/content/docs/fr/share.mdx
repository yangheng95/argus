---
title: Partager
description: Partagez vos conversations OpenCorvus.
---

La fonctionnalité de partage de OpenCorvus vous permet de créer des liens publics vers vos conversations OpenCorvus, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, OpenCorvus :
...
OpenCorvus prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, OpenCorvus utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
...
Pour appliquer cela au sein de votre équipe pour un projet donné, ajoutez-le au `opencorvus.json` dans votre projet et enregistrez-vous dans Git.
...
[En savoir plus](/docs/enterprise) sur l'utilisation de opencorvus dans votre organisation.
