...
2. **全局任务列表流**（应用级）
   ...
   ```
   代码：`src/services/sse.ts:104` `startTaskListSSE()`。这条流根据服务端盖章的 `notify` 元数据统一负责系统通知，并刷新任务列表徽标投影。
...
| `overlay_settings_load` / `overlay_settings_save` | 设置持久化到 `.opencorvus/overlay.jsonc`                                                              |
...
`~/.opencorvus/overlay.jsonc` 存 UI 偏好（主题、位置、缩放、server URL override）；托盘菜单的 **Restart** 会重启后端并 `location.reload()` 前端。
