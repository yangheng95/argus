---
title: Sunucu
description: argus sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`argus serve` komutu, argus istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `argus.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `argus` değeridir; değiştirmek isterseniz `OPENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `argus serve` hem de `argus web` için geçerlidir.
...
`argus` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
argus ile programatik etkileşim için argus sunucusunu kullanın.
:::

Bu mimari, argus'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`argus serve` ile bağımsız bir sunucu başlatabilirsiniz. argus TUI açıksa, `argus serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum argus [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
argus sunucusu aşağıdaki API'leri sunar.
