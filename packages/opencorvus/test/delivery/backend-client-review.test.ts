...
} from "../../src/delivery/specialists/backend-client"
...
describe("delivery backend API and client contract specialist reviews", () => {
  ...
  test("backend review requires route evidence for selected backend surfaces", async () => {
    const review = await runBackendApiReview({
      ...
      deliveryID: "dlv_backend_missing",
      projectRoot: process.cwd(),
  ...
  test("client contract review flags route/client endpoint drift", async () => {
    ...
    const review = await runClientContractReview({
      ...
      deliveryID: "dlv_client_contract",
      projectRoot: dir,
...
function manifest(
  ...
  const evidence: DeliverySurfaceManifest["evidence"] = []
