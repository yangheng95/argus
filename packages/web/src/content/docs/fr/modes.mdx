...
:::caution
Les modes sont désormais configurés via l'option `agent` dans la configuration argus.
L’option `mode` est désormais obsolète. [En savoir plus](/docs/agents).
:::

Les modes dans argus vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.

Il est livré avec deux modes intégrés : **build** et **plan**. Vous pouvez personnaliser ceux-ci ou configurez les vôtres via la configuration argus.
...
argus est livré avec deux modes intégrés.
...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.argus/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `argus.json` :
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale argus et la configuration spécifique au projet.
...
Créez des fichiers de mode dans `.argus/modes/` pour les modes spécifiques au projet ou `~/.config/argus/modes/` pour les modes globaux :
