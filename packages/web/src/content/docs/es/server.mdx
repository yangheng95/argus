...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `opencorvus`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `opencorvus serve` como a `opencorvus web`.
