...
You'll see: `spec_agent_running` → `goals_agent_running` → `planner_agent_running` → multiple `goal.running` → `evaluator_running` → `delivery_running` → `done`.
