---
title: Slack
description: OpenCorvus 的首选远程 channel——通过 opencorvus slack 子命令直接启动的 Socket Mode 适配器。
---
...
**问题**：同一消息被处理两次
**检查**：去重逻辑在 `packages/channel-runtime/src/adapters/slack.ts`，基于 `event_id`；若看到重复，很可能是同时启动了 `opencorvus slack` 与 `channel-runtime`，二者都在订阅。
