...
export function SessionSwitcherDialog() {
  ...
  const [searchResults, { refetch }] = createResource(
    ...
    async (input) => {
      ...
      const result = await sdk.client.session.list({ search: input, limit: 30 })
      return result.data ?? []
    },
  )
  ...
  const options = createMemo<DialogSelectOption<string>[]>(() => {
    ...
    function buildOption(id: string, category: string): DialogSelectOption<string> | undefined {
      ...
      const isWorking = status?.type === "streaming" || status?.type === "retry"
      ...
      const gutter =
        slot !== undefined || isWorking
          ...
          : undefined
      const titleText = isDeleting ? `Press ${deleteHint()} again to confirm` : x.title
      return {
        title: titleText,
