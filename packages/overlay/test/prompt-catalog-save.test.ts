...
describe("prompt catalog save values", () => {
  ...
  test("PromptCatalog exposes prompt-profile management and import as the editable prompt surface", () => {
    ...
    expect(source).toContain('data-ui="prompt-profile-metadata"')
    ...
    expect(source).toContain('data-ui="prompt-profile-save"')
    expect(source).toContain('data-editable={editableTarget() ? "true" : "false"}')
    expect(source).toContain('data-has-overlay={hasOverlay() ? "true" : "false"}')
