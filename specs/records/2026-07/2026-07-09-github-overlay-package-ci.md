...
| GitHub run evidence | `build.yml` run `28994589511` checked out commit `3ee9bec1c92b86091163d33c68ca8dcc02f77139`, then stalled inside `.github/actions/setup-bun` at `bun install` after `Resolved, downloaded and extracted [297]`; cancellation left orphan process `bun`. Run `28995089473` confirmed `HUSKY=0 bun install --frozen-lockfile --no-progress` still stalled at the same point before any `$ husky` output. |
| Local reproduction boundary | `HUSKY=0 bun install --frozen-lockfile --no-progress` completed locally without file changes, but CI evidence proved Husky was not the whole root cause. `HUSKY=0 bun install --frozen-lockfile --no-progress --ignore-scripts` also completed locally without file changes and directly removes the install-script lifecycle phase where GitHub stalls. |

## Root Cause

The official GitHub package workflow uses the repository-local `.github/actions/setup-bun` composite action. Its install step ran plain `bun install`, which allows install-script lifecycle work after dependency extraction. On the GitHub runner the step repeatedly reached that lifecycle boundary and then remained silent until cancellation, leaving a live Bun process. A follow-up run with `HUSKY=0` still stalled before any Husky output, proving the deeper trigger is dependency or trusted-dependency install scripts rather than the root Git-hook install alone. This is a CI toolchain defect in dependency setup, not an overlay package failure.

## Repair

Make the CI dependency install deterministic and non-interactive:

- use `bun install --frozen-lockfile --no-progress --ignore-scripts` so the workflow consumes the committed lockfile, avoids progress UI noise, and does not enter package install-script lifecycle work before build commands;
