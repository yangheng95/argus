...
`.argus/commands/test.md` oluştur:
...
argus işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

argus [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/argus/commands/`
- Proje başına: `.argus/commands/`
...
argus, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
