...
- `specs/records/2026-06/2026-06-17-hexin-budget-refresh-db-sidecar-cleanup.md` made schema drift explicit: `Database.Client()` throws `DatabaseSchemaResetRequiredError` and the user must run an explicit reset.
- `specs/records/2026-06/2026-06-18-settings-db-reset-safe-route.md` kept `/global/db/reset` behind `Project.findByRegisteredDirectory(...)`, which reads `project` rows from the same SQLite file that reset is supposed to delete.
