...
Opprett `.argus/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom Argus-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Argus [config](/docs/config):
...
Argus inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
