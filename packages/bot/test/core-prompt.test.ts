...
describe("bot core system prompt", () => {
  test("enforces visible execution principles", () => {
    const core = new BotCore() as unknown as { buildSystemPrompt: (platform: string) => string }
...
describe("bot core text formatting", () => {
  test("polishes whitespace for chat readability", () => {
    const core = new BotCore() as unknown as { polish: (text: string) => string }
    const text = "  hello \r\n\r\n\r\nworld  \r\n"
  ...
  test("splits long text without truncation", () => {
    const core = new BotCore() as unknown as { split: (text: string, limit?: number) => string[] }
