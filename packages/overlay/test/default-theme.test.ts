...
describe("overlay default theme", () => {
  test("settings and sanitizers default to VS Code dark", () => {
    expect(DEFAULT_THEME).toBe("vscode-dark");
  ...
  test("pre-render theme bootstrap defaults cold starts to VS Code dark", async () => {
    const html = await fs.readFile(
      path.join(import.meta.dir, "..", "src", "index.html"),
      "utf8",
    );
    expect(html).toContain('!saved ? "vscode-dark"');
    expect(html).toContain('document.body.dataset.theme = "vscode-dark"');
  });
});
