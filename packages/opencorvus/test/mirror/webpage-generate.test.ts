...
describe("webpage-generate dependency guards", () => {
  test("skill declares frontend-design template mirror pipeline only", () => {
    ...
    expect(parsed.content).toContain("baseline-first, region-by-region replacement algorithm")
    expect(parsed.content).toContain("normal frontend_design tool flow")
    expect(parsed.content).toContain("do not create project-internal state machines")
    expect(parsed.content).toContain("sourceDomIterationState.ts")
    expect(parsed.content).toContain("nextSourceDomReplacement")
    expect(parsed.content).toContain("sourceMap")
    expect(parsed.content).toContain("generatedCleanupTargets")
    expect(parsed.content).toContain("verticalSliceSteps")
