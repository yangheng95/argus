---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenCorvus.
---

Les plugins vous permettent d'étendre OpenCorvus en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenCorvus.
...
- `.opencorvus/plugins/` - Plugins au niveau du projet
- `~/.config/opencorvus/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/opencorvus/node_modules/`.
...
1. Configuration globale (`~/.config/opencorvus/opencorvus.json`)
2. Configuration du projet (`opencorvus.json`)
3. Répertoire global des plugins (`~/.config/opencorvus/plugins/`)
4. Répertoire des plugins du projet (`.opencorvus/plugins/`)
...
OpenCorvus exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK opencorvus pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre opencorvus.
...
:::note
Si vous utilisez l'application de bureau OpenCorvus, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez opencorvus de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à opencorvus :
...
L'assistant `tool` crée un outil personnalisé que opencorvus peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour opencorvus aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://opencorvus.ai/docs/sdk) pour plus de détails.
