---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenCorvus zu erweitern.
---

Mit Plugins können Sie OpenCorvus erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenCorvus zu ändern.
...
- `.opencorvus/plugins/` – Plugins auf Projektebene
- `~/.config/opencorvus/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/opencorvus/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/opencorvus/opencorvus.json`)
2. Projektkonfiguration (`opencorvus.json`)
3. Globales Plugin-Verzeichnis (`~/.config/opencorvus/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.opencorvus/plugins/`)
...
OpenCorvus führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenCorvus-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenCorvus erweitern können.
...
:::note
Wenn Sie die OpenCorvus-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenCorvus `.env`-Dateien liest:
...
Plugins können OpenCorvus auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenCorvus aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenCorvus zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://opencorvus.ai/docs/sdk).
