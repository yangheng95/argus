...
describe("BuildAgent external coding system prompt", () => {
  test("injects OpenCorvus MCP executor aliases without reopening mirror tools", () => {
    ...
    expect(composed.system).toContain("`web-clone-source/` package is the only implementation evidence entrypoint")
    expect(composed.system).toContain("Do not search sibling worktrees, primary project directories, absolute external paths, or raw `mirror/`")
    expect(composed.system).toContain("Do not generate a separate source project for webpage clone delivery")
