...
Da biste otklonili probleme s Argus, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\argus\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `argus --log-level DEBUG`.

---

## Pohrana

Argus pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/argus/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\argus`
...
Argus Desktop pokreće lokalni Argus server (`argus-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Argus` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/argus/argus.jsonc` (ili `~/.config/argus/argus.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/argus/argus.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\argus\argus.jsonc`
...
Argus također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/argus/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\argus\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Argus mogao ponovo izgraditi.

1. Potpuno zatvorite Argus Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/argus`
- **Linux**: obrišite `~/.cache/argus` (ili pokrenite `rm -rf ~/.cache/argus`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\argus`

3. Ponovo pokrenite Argus Desktop.

---

### Rješavanje problema sa vezom na serveru

Argus Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `argus.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Argus Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Argus karakteristikama.

---

### Obavještenja se ne prikazuju

Argus Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Argus u postavkama vašeg OS-a, i
...
1. Zatvorite Argus Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Argus Desktop aplikacije):

- `argus.settings.dat` (URL zadanog servera za desktop)
- `argus.global.dat` i `argus.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `argus upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `argus models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\argus`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Argus dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\argus`

2. Ponovo pokrenite Argus da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Argus da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Argus će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
