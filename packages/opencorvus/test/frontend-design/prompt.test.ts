...
describe("frontend-design prompt assembly", () => {
  test("core prompt pins maintainable rawproject refactor algorithm", async () => {
    ...
    expect(prompt).toContain("The defect is delivering a mechanical DOM dump as application source")
    ...
    expect(prompt).toContain("no mechanical DOM dump / `div soup` remains as the primary app source")
    ...
    expect(prompt).toContain("The selected vertical slice means the selected component, its region-owned data/mock/API module when applicable, its scoped style/asset resolver module, and `App.tsx` wiring")
    expect(prompt).toContain("Bootstrap files such as `src/main.tsx`, global reset/theme CSS, root package/config files, and bundler config are valid only in the earlier skeleton-baseline adoption pass")
    expect(prompt).toContain("not a substitute for selected-region extraction")
    ...
    expect(prompt).toContain("PRD-to-skeleton weighting for maintainable webpage replicas")
    expect(prompt).toContain("The implementation contract is PRD-first")
  ...
  test("webpage rawproject prompts make frontend_design materialize the source project itself", () => {
    ...
    expect(prompt).toContain("After baseline adoption, run install/build/render evidence against the target project")
    expect(prompt).toContain("pre-selection browsing only for choosing the next source region")
    expect(prompt).toContain("do not read a second `src/components/source-dom/*` file")
    expect(prompt).toContain("root config files unless they are part of the already completed baseline adoption")
    expect(prompt).not.toContain("Configuration manifests and package-manager commands are late-stage integration work, not the first workload")
    expect(prompt).not.toContain("After the skeleton exists, the first target-project editing pass creates source files only")
    expect(prompt).not.toContain("Before the first target-project write, call `record_frontend_region_selection`")
    expect(prompt).toContain("Then call `record_frontend_region_selection` for the concrete source region being migrated")
    expect(prompt).toContain("write the target component/data/style/App slice next")
    expect(prompt).toContain("the corresponding baseline boundary")
    ...
    expect(prompt).toContain("Do not chase every `AssetPath`, `svgPaths`, `asset_*.path.txt`, CSS token, or `critical.css` variable before the first selected-region write")
    ...
    expect(prompt).not.toContain("The first filesystem-changing operation for a new default target directory must be the source vertical slice itself")
    expect(prompt).not.toContain("The first write inside a new default target directory should be one of those selected-region source files")
    expect(prompt).not.toContain("Do not write `index.html` until `src/App.tsx`, at least one semantic component, at least one data/mock module")
    expect(prompt).toContain("Do not write imports for future components")
    expect(prompt).not.toContain("Do not run install/build/render/dev-server commands until the target project already has `src/App.tsx`")
    ...
    expect(prompt).toContain("Treat `nextSourceDomReplacement` as the first source-region queue item only")
    expect(prompt).toContain("not as permission to skip target baseline adoption")
  ...
  test("skeleton project tool returns bounded source evidence entrypoints", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.output).toContain("Do not start from an empty `web-clone-target/src`")
        expect(result.output).toContain("After the target baseline exists, run install/build/render evidence")
        expect(result.output).toContain("Do not run build/render inside `frontend-design-skeleton`")
        expect(result.output).toContain("Then call `record_frontend_region_selection`")
        expect(result.output).toContain("Target writes after selection must be owned by that selected region")
        ...
        expect(result.output).toContain("Populate the selected component/data/scoped-style/asset resolver and update `App.tsx` by replacing the corresponding baseline boundary")
        ...
        expect(result.output).toContain("do not put records from unselected footer/economy/news/calendar/FAQ/map/chart/card/list regions into the current region's data file")
        ...
        expect(result.output).toContain("If the selected skeleton file is already semantic, port it faithfully first")
