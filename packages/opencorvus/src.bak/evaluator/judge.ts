/**
 * LLM-based evaluation checks: judge, spec check, UI review, code quality/review, dead code review.
 */
...
import { webPage, stripHtml } from "./browser-checker"
import { JudgeResult, SpecCheckResult, ReviewResult, clip } from "./types"
...
export function diffDigest(diffs: Snapshot.FileDiff[], limit: number) {
...
function reviewOutcome(
  ...
  if (!result.ok) {
    return Outcome.failed(name, result.summary, result.evidence, { ...extra, ...result.payload })
  }

  return result.object.verdict === "accepted"
    ? Outcome.passed(name, `${name} accepted the delivery.`, clip(result.object.rationale), { ...extra, ...result.object })
...
export async function codeQualityResult(
  ...
  if (!config?.enabled) return Outcome.empty()
  const mode = config.mode ?? "soft"
  const result = await reviewResult({
    name: "code_quality",
    mode,
...
export async function judgeResult(
  ...
  if (!model) {
    return Outcome.failed("judge", "Judge check unavailable because no model is configured.", "No evaluator judge model available.", { mode, available: false })
  }

  const language = await Provider.getLanguage(model).catch(() => undefined)
  if (!language) {
    return Outcome.failed("judge", "Judge check unavailable because the language model could not be loaded.", "Could not load evaluator judge model.", { mode, available: false })
  }
  ...
  if (!result) {
    return Outcome.failed("judge", "Judge check failed to execute.", "Judge model call failed.", { mode, available: true })
  }

  if (result.object.verdict === "accepted") {
    return Outcome.passed("judge", "Judge check accepted the delivery.", clip(result.object.rationale), result.object)
  }
...
export async function specCheckResult(
  ...
  const model = await judgeModel()
  if (!model) {
    return Outcome.failed("spec_check", "Spec check unavailable because no model is configured.", "No evaluator model available for spec check.", { mode, available: false })
  }

  const language = await Provider.getLanguage(model).catch(() => undefined)
  if (!language) {
    return Outcome.failed("spec_check", "Spec check unavailable because the language model could not be loaded.", "Could not load evaluator model for spec check.", { mode, available: false, specID: activeSpecVersionID })
  }
  ...
  if (!result) {
    return Outcome.failed("spec_check", "Spec check failed to execute.", "Spec check model call failed.", { mode, available: true, specID: activeSpecVersionID })
  }

  const allPassed = result.object.criteria.every((c) => c.status === "passed")
  if (allPassed && result.object.verdict === "accepted") {
    return Outcome.passed("spec_check", "Spec check: all criteria passed.", clip(result.object.rationale), { specID: activeSpecVersionID, ...result.object })
  }
