...
Agentferdigheter lar Argus oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Argus søker etter disse stedene:

- Prosjektkonfigurasjon: `.argus/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/argus/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Argus opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.argus/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.argus/skills/git-release/SKILL.md` slik:
...
Argus viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `argus.json`:
...
**For innebygde agenter** (i `argus.json`):
