...
export const EngineTaskTable = sqliteTable(
  "engine_task",
  {
    ...
    /** Phase-6-f-3: `active_run_id` cache column removed. Derive active
     *  execution via `findActiveRunForTask(taskID)` (latest tip if live);
     *  read `findLatestRunForTask(taskID)` for terminal run history. */
    request_id: text(),
