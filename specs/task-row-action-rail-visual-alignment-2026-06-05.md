...
| `packages/overlay/src/styles/primitives/button.css` icon-action primitive | Generic `data-chrome="icon-action"` is used by composer, file explorer, notifications, mission list, provider panel, and task list.                                        | Do not weaken the primitive globally; other surfaces rely on visible resting icon action chrome.  |
