...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenCorvus posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenCorvus.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENCORVUS_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENCORVUS_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCorvus lub gdy zmienna środowiskowa `OPENCORVUS_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenCorvus:
