...
- **6-f**：`EngineTaskTable` 瘦身（FSM 残余清理）。拆 sub-step：
  ...
  - **6-f-2**：删 `task.status`（FSM 缓存）— 39 处读者改 describe 层现算；耦合 6-g。`task.phase` 审计后不存在：`engine_task` schema 无 `phase` 列（仅 `engine_run.phase` 存在，另一回事）。`task.status` 需要的信号可从 `{time_started, time_completed, error}` 派生出 4 / 5 个状态（queued / active / completed / failed），`cancelled` 需要 disambiguate — 拟通过 `error` 字符串前缀或 `metadata.cancelled_reason` 标记，不再单独列。交付物：describe.ts::describeTaskFromRow 回 derived status 字段；`queue.ts::claimNext` / `runtime.ts::createOperatorRun` / `orchestrator/loop.ts` / `task-api/index.ts` 所有 `task.status === X` gate 改读 `findTask()` 派生；`state.ts::updateTask` 从 `values.status` 迁移到 `values.time_started / time_completed / error` 直接写；DDL + engine.sql 删 status 列。
  ...
  - **6-f-3-bis-b**：删 `task.workflow_state`（workflow 步 FSM，rule 23 违规） — 当前 `orchestrator/agent.ts` 首次唤醒用它判断 "first wake vs continuation" 并选 workflow template；step 位置在 `workflow_state.taskSteps[stepID] = {status:running/completed...}` 追踪。拆除策略：LLM 每次唤醒从 artifact stream + task.kind 现算当前应走 workflow + 当前已完成的 step（`findStepByTool` 遍历 artifacts 看哪些 step 的 tool_call 已产生产物）；`createWorkflowState` 只在内存临时构造给 tool 使用，不再 persist。写入点 4 处（`orchestrator/agent.ts` init + `orchestrator/tools.ts:196/228/279`）全部去 `updateTask({workflow_state})`；读取点 2 处（`orchestrator/agent.ts:122/136/140` + `workbench/board.ts:733`）改现算。`engine.sql.ts` + `ddl.ts` 删列。
