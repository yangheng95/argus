---
title: Server
description: Interager med opencorvus-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Kommandoen `opencorvus serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencorvus-klient kan bruge.
...
| --------------- | -------------------------------------------- | ------------------ |
| `--port`        | Port at lytte på                             | `7878`             |
| `--hostname`    | Værtsnavn at lytte på                        | `127.0.0.1`        |
| `--mdns`        | Aktiver mDNS-opdagelse                       | `false`            |
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `opencorvus.local` |
...
Indstil `OPENCORVUS_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `opencorvus`, eller indstil `OPENCORVUS_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `opencorvus serve` og `opencorvus web`.
...
Når du kører `opencorvus` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencorvus-serveren til at interagere med opencorvus programmatisk.
:::

Denne arkitektur lader opencorvus understøtte flere klienter og lader dig interagere med opencorvus programmatisk.

Du kan køre `opencorvus serve` for at starte en selvstændig server. Hvis du har
opencorvus TUI kørende, vil `opencorvus serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenCorvus [IDE](/docs/ide) plugins.
...
For eksempel `http://localhost:7878/doc`. Brug specifikationen til at generere klienter eller inspicere forespørgsels- og svartyper. Eller se den i en Swagger-utforsker.

---

## API'er

OpenCorvus-serveren viser følgende API'er.
...
| `POST`   | `/permission/:requestID/reply` | Svar på en tilladelsesforespørgsel         | body: `{ reply, message? }`, returnerer `boolean`                                     |
...
| `POST` | `/tui/control/response` | Svar på en kontrolforespørgsel (`{ id, body } or { id, error }`) | `boolean`                  |
