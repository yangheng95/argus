...
| `conversation-agents.ts` | Exports `applyLiveConversationAgentMessageUpdated(sourceKey, event)` for standard `message.updated` payloads. | Reuse this projection for all standard message events, including events synthesized from executor run events. |
...
| `events.ts::routeSSEEvent` `run.progress` / `run.output` branches | `convertExecutorEventToMessages()` creates standard `message.updated` events and writes them to `cardTreeStore`, but does not update `conversationAgentStore`. | After each synthesized `message.updated` is accepted by tree-writer, project it into the same source-keyed rail store. |
...
| `events.ts::replayTaskEventToTree` | Hydration replay converts persisted `run.progress` / `run.output` events into message cards without rail projection. | Keep card replay behavior; do not use replay as a second live source unless the active selected source is known. Authoritative hydrate remains `agentView`. |
...
| `selected-task-recovery.test.ts` | Covers direct selected `message.updated` and DB-backed `task.messages.changed` rail updates. | Add coverage for selected `run.output` conversion producing a message card and rail record in the same live route. |
...
rail store. However, the selected stream has other message-card-producing
edges:

- `run.progress` and `run.output` are converted by
  `convertExecutorEventToMessages()` into standard `message.updated` plus part
  events. Those synthesized messages were written to `cardTreeStore`, but not
  to `conversationAgentStore`.
...
fallback. It is a missing projection edge for the existing standard message
stream after executor events are normalized.
...
1. Factor a small helper in `events.ts` that writes a standard message event to
   ...
   store.
2. Use that helper in the direct selected message branch, the part-first branch,
   and the `run.progress` / `run.output` conversion loops.
...
6. Add focused tests for live selected `run.output`, at least one
   `run.progress` tool-call path, part-first `message.part.updated`,
...
- A selected task `run.output` event that converts into a standard executor
  message creates both the message card and an `executor` rail record
  immediately.
- A selected task `run.progress` event that converts into a standard executor
  message creates both the message card and the rail record immediately.
