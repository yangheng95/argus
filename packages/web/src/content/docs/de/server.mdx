---
title: Server
description: Nutze den opencorvus-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Der Befehl `opencorvus serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein opencorvus-Client nutzen kann.
...
Setze `OPENCORVUS_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `opencorvus`, kann aber mit `OPENCORVUS_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `opencorvus serve` und `opencorvus web`.
...
Wenn du `opencorvus` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opencorvus-Server, um opencorvus programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `opencorvus serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `opencorvus serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenCorvus-[IDE](/docs/ide)-Plugins.
...
Zum Beispiel `http://localhost:7878/doc`.
...
Der opencorvus-Server stellt folgende APIs bereit.
