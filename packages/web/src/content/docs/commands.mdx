...
You can add custom commands through the Argus config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Argus [config](/docs/config):
...
argus includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
