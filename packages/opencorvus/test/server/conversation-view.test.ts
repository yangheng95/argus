...
test("conversationMessageHasDisplay rejects envelope-only and control-only messages", () => {
  ...
  expect(conversationMessageHasDisplay({ ...base, parts: [{ type: "compaction", auto: true }] })).toBe(false)
  expect(conversationMessageHasDisplay({ ...base, parts: [{ type: "snapshot", snapshot: "state" }] })).toBe(false)
  expect(conversationMessageHasDisplay({ ...base, parts: [{ type: "agent", name: "build" }] })).toBe(false)
  expect(conversationMessageHasDisplay({ ...base, parts: [{ type: "retry", attempt: 1 }] })).toBe(false)
  ...
  expect(conversationMessageHasDisplay({ ...base, parts: [{ type: "part-error", message: "corrupt" }] })).toBe(true)
