---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenCorvus-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/opencorvus/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenCorvus-konfigurationen.
...
OpenCorvus Zen er en liste over modeller leveret af OpenCorvus-teamet, som er blevet testet og verificeret til at fungere godt med OpenCorvus. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenCorvus Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opencorvus og gå til [opencorvus.ai/auth](https://opencorvus.ai/auth).
...
Det fungerer som alle andre udbydere i OpenCorvus og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med OpenCorvus:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører opencorvus:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `opencorvus.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenCorvus understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at opencorvus skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem opencorvus-konfigurationen.
...
gemme tokenet i opencorvus auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenCorvus bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenCorvus
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`opencorvus.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `opencorvus.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OpenCorvus:
...
Sådan bruger du Google Vertex AI med OpenCorvus:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører opencorvus.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem opencorvus, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) plugin for automatisk at logge hver OpenCorvus-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenCorvus-samtale opført som en separat session.
...
Du kan konfigurere opencorvus til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere opencorvus til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere opencorvus til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenCorvus. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencorvus) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenCorvus:
...
3. Kopier API-nøglen til brug i OpenCorvus.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenCorvus, skal du hente modelinformationen lokalt:
...
OpenCorvus Zen er en liste over testede og verificerede modeller leveret af OpenCorvus-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenCorvus Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenCorvus API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem opencorvus-konfigurationen.
...
5. Du kan også tilpasse dem gennem opencorvus-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenCorvus:
...
Du kan også tilpasse modeller gennem opencorvus-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med opencorvus. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `opencorvus.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenCorvus forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `opencorvus auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek opencorvus-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i opencorvus-konfigurationen.
