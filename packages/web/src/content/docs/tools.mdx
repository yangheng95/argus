...
Tools allow the LLM to perform actions in your codebase. OpenCorvus comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in OpenCorvus.
...
:::note
This tool is only available when `OPENCORVUS_EXPERIMENTAL_LSP_TOOL=true` (or `OPENCORVUS_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCorvus provider or when the `OPENCORVUS_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching OpenCorvus:
