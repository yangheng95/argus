---
title: LSP-серверы
description: opencorvus интегрируется с вашими серверами LSP.
---

opencorvus интегрируется с вашим протоколом языкового сервера (LSP), чтобы помочь LLM взаимодействовать с вашей кодовой базой. Он использует диагностику для предоставления обратной связи LLM.

---

## Встроенные

opencorvus поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `OPENCORVUS_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда opencorvus открывает файл, он:
...
Вы можете настроить серверы LSP через раздел `lsp` в конфигурации opencorvus.
