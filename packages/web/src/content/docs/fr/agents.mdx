...
Il existe deux types d'agents dans Argus ; agents primaires et sous-agents.
...
Argus est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Argus est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Argus est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `argus.json` :
...
- Global : `~/.config/argus/agents/`
- Par projet : `.argus/agents/`

```markdown title="~/.config/argus/agents/review.md"
---
...
Si aucune température n'est spécifiée, Argus utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Argus et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Argus utilise le format `provider/model-id`. Par exemple, si vous utilisez [Argus Zen](/docs/zen), vous utiliserez `argus/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="argus.json" {3-6,9-12}
{
  "$schema": "https://argus.ai/config.json",
  "tools": {
    "write": true,
...
```json title="argus.json" {3-5,8-10}
{
  "$schema": "https://argus.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `argus models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/yangheng95/argus).
:::
...
```markdown title="~/.config/argus/agents/docs-writer.md"
---
...
```markdown title="~/.config/argus/agents/security-auditor.md"
---
