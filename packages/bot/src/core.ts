...
export class BotCore {
  ...
  private buildSystemPrompt(platform: string): string {
    ...
    return [
      ...
      "4. For coding tasks, use code tools first: read/glob/grep/bash/edit/write/task.",
      "5. For coding tasks, do NOT use screen/input/vision tools unless the user explicitly requests desktop GUI interaction.",
      "",
      "## Non-Coding Tasks",
      "For questions that need real-time data (weather, news, stock prices, etc.), use available tools (web search, bash, etc.).",
      "For setup/integration tasks on external platforms (for example Slack bot registration), first use websearch/webfetch to learn the latest official steps before execution.",
      "Do not rely on stale memory for external platform setup flows when docs may have changed.",
