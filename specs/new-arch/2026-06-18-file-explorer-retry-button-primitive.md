...
| --- | --- | --- |
| `rg -n -F "file-explorer-retry" packages/overlay/src packages/overlay/test specs/new-arch` | Live production owners are `FileExplorerPanel.tsx` and `inspector.css`; `file-explorer-editor.test.ts` currently pins the old class. | Migrate this action directly; no broader explorer row rewrite. |
| `rg -n -F "task-list-error-retry" packages/overlay/src packages/overlay/test specs/new-arch` | Task list retry uses the Button primitive and a stable `data-ui` selector. | Follow the same pattern for file explorer. |
