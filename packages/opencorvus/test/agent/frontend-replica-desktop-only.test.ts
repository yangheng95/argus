...
describe("frontend replica desktop-only generation scope", () => {
  ...
  test("frontend replica profile overlays protect every model decision surface", () => {
    ...
    expect(agents.build).toContain("report the scope defect instead of executing it")
    expect(agents["visual-qa"]).toContain("Do not require tablet/mobile screenshots")
    expect(agents.orchestrator).toContain("Do not route template-derived tablet/mobile text")
