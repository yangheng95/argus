...
- Bash tool calls are routed through the configured shell (`shell` in [config](/docs/config/)). On Windows, the default discovery prefers `pwsh` then `cmd.exe`.
...
- The TUI, overlay, and `opencorvus serve` all run as plain Windows binaries.
...
git clone https://github.com/yangheng95/argus
cd argus/packages/opencorvus
bun install
bun run start
```
...
| Need                                      | Recommendation                                                                      |
| ----------------------------------------- | ----------------------------------------------------------------------------------- |
| Run TUI / Overlay desktop UI              | **Native Windows** — Tauri runs there                                               |
