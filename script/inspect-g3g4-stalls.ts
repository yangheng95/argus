...
const g3msgs = db.query<{ id: string; data: string; time_created: number }, []>(
  `SELECT id, data, time_created FROM message WHERE session_id='${G3_SES}' ORDER BY time_created`,
).all();
let prevT = 0;
for (const m of g3msgs) {
  let d: any = {};
  try { d = JSON.parse(m.data); } catch {}
  const gap = prevT > 0 ? ((m.time_created - prevT) / 1000).toFixed(1) + "s" : "—";
  const errStr = d.error ? `  ERR=${typeof d.error === "string" ? d.error.slice(0, 200) : JSON.stringify(d.error).slice(0, 200)}` : "";
  console.log(`+${gap.padStart(8)}  ${new Date(m.time_created).toISOString()}  ${m.id}  role=${d.role}  finish=${d.finish ?? "—"}  toks=${d.tokens?.total ?? "—"}${errStr}`);
  prevT = m.time_created;
}

console.log("\n\n===== G4 (backend) full message timeline =====");
const g4msgs = db.query<{ id: string; data: string; time_created: number }, []>(
...
const attempts = db.query<{ id: string; goal_run_id: string; label: string; payload: string; time_created: number; time_updated: number }, []>(
...
for (const a of attempts) {
  let p: any = {};
  try { p = JSON.parse(a.payload); } catch {}
  console.log(`${a.id}  glr=${a.goal_run_id}  label=${a.label}  goal=${p.goal_id}  status=${p.status}  block=${p.blocking_reason ?? "-"}  err=${p.error ?? "-"}  t_created=${new Date(a.time_created).toISOString()}  t_updated=${new Date(a.time_updated).toISOString()}`);
}

console.log("\n\n===== run rows =====");
const runs = db.query<{ id: string; run_id: string; label: string; payload: string; time_created: number; time_updated: number }, []>(
...
for (const r of runs) {
  ...
  console.log(`${r.id}  run=${r.run_id}  label=${r.label}  status=${p.status}  phase=${p.phase}  blocking_reason=${p.blocking_reason ?? "-"}  err=${p.error ?? "-"}  t=${new Date(r.time_updated).toISOString()}`);
}

console.log("\n\n===== Last 8 protocol events for each goal =====");
for (const [name, gid] of [["G3", G3], ["G4", G4]] as const) {
  console.log(`\n--- ${name} (${gid}) ---`);
  const evs = db.query<{ type: string; source: string; emitted_at: number; payload: string }, []>(
...
const recent = db.query<{ type: string; source: string; emitted_at: number; payload: string }, []>(
...
const bridgeCounts = db.query<{ session_id: string; n: number; first: number; last: number }, []>(
...
for (const b of bridgeCounts) {
  console.log(`  ses=${b.session_id}  n=${b.n}  first=${new Date(b.first).toISOString()}  last=${new Date(b.last).toISOString()}`);
}

console.log("\n\n===== G3 last 18 parts (with text excerpts) =====");
const g3parts = db.query<{ id: string; message_id: string; data: string; time_created: number }, []>(
...
for (const p of g3parts) {
  ...
  console.log(`  ${new Date(p.time_created).toISOString()}  type=${d.type}  tool=${d.tool ?? "-"}  status=${d.state?.status ?? "-"}  title="${(d.state?.title ?? "").toString().slice(0, 60)}"  text="${text}"`);
}

console.log("\n\n===== G3 most recent assistant message error / fields =====");
const g3last = db.query<{ id: string; data: string; time_created: number }, []>(
...
for (const m of g3last) {
  ...
  console.log(`  finish=${d.finish ?? "—"}  err=${JSON.stringify(d.error ?? null).slice(0, 400)}  tokens.total=${d.tokens?.total ?? "—"}`);
