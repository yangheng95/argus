...
Agent Skills erlauben Argus, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Argus sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Argus vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.argus/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.argus/skills/git-release/SKILL.md` zum Beispiel so:
...
Argus listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `argus.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `argus.json`):
