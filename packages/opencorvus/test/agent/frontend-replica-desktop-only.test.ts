...
describe("frontend replica desktop-only generation scope", () => {
  test("expert squad forbids default tablet/mobile requirements, goals, and build work", async () => {
    ...
    expect(skill).toContain("task-scoped webpage evidence")
    expect(skill).toContain("explicitly declared visual-region manifests")
    ...
    expect(skill).toContain(
      "default to one user-visible implementation component or meaningful source-backed page region per Build goal",
    )
    ...
    expect(skill).toContain(
      "Implementation repair after Visual QA or Integrity must consume the latest blocker evidence",
    )
    expect(skill).toContain("Annotated Visual QA screenshots and DOM diagnostics are repair inputs")
    expect(skill).toContain("current workflow implementation owner owns changed-region rendered proof")
    expect(skill).toContain("Visual QA owns final module source-binding and rendered parity review")
  ...
  test("frontend replica profile overlays protect every model decision surface", async () => {
    ...
    expect(agents.architect).toContain("never register source rows, visual-source rows, style-profile rows")
    expect(agents.architect).toContain("DOM/evidence-table rows")
    ...
    expect(agents.architect).not.toContain("Read Frontend Design's")
    expect(agents.architect).toContain("source artifact")
    expect(agents.build).not.toContain("browser_preview_reference_regions")
    expect(agents.build).toContain("goal-scoped source/reference evidence")
    expect(agents.build).toContain("missing crop evidence is a blocker")
    expect(agents.build).not.toContain("browser_preview_compare_scroll_slices")
    expect(agents.build).toContain("leave module comparison, scroll-slice visual_diff, and layout-geometry diagnostics to Visual QA")
    expect(agents.build).not.toContain("LEFT reference / RIGHT implementation")
    ...
    expect(agents.build).toContain("changed-region proof")
    expect(agents.build).toContain("desktop-width changed-region proof")
    ...
    expect(agents["visual-qa"]).toContain(
      "Own final module source-binding review with browser_preview_reference_regions",
    )
    expect(agents["visual-qa"]).toContain("Browser MCP screenshot/observe tools")
    expect(agents["visual-qa"]).toContain(
      "browser_preview_compare_scroll_slices only for first-viewport and page-slice visual_diff",
    )
    ...
    expect(agents.orchestrator).toContain(
      "Build produces changed-region rendered proof while Visual QA owns scroll-slice evidence",
    )
