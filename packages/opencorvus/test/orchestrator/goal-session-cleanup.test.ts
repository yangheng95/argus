...
describe("orchestrator.goal session cleanup", () => {
  ...
  test("poll archives finished goal child transcripts into the task root session", async () => {
    await using tmp = await tmpdir({ git: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await Session.saveMessage(user)
        await Session.updatePart({
          id: Identifier.ascending("part"),
          sessionID: child.id,
          messageID: userID,
          type: "text",
          text: "Goal child transcript",
        })
        ...
        await Session.saveMessage(assistant)
        await Session.updatePart({
          id: Identifier.ascending("part"),
          sessionID: child.id,
          messageID: assistantID,
          type: "text",
          text: "Goal child answer",
        })
        ...
        const rootMessages = await Session.messages({ sessionID: root.id })
        const copiedUser = rootMessages.find((item) =>
          item.info.role === "user" && item.parts.some((part) => part.type === "text" && part.text === "Goal child transcript"),
        )
        const copiedAssistant = rootMessages.find((item) =>
          item.info.role === "assistant" && item.parts.some((part) => part.type === "text" && part.text === "Goal child answer"),
        )

        expect(copiedUser).toBeTruthy()
        expect(copiedAssistant).toBeTruthy()
        expect(copiedUser?.info.id).not.toBe(userID)
        expect(copiedAssistant?.info.id).not.toBe(assistantID)
        expect(copiedAssistant?.info.role === "assistant" ? copiedAssistant.info.parentID : "").toBe(copiedUser?.info.id)
        expect(Database.use((db) =>
          db.select().from(SessionTable).where(eq(SessionTable.id, child.id)).get(),
        )).toBeUndefined()
      },
