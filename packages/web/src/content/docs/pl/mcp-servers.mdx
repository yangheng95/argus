...
Możesz dodać dodatkowe informacje do argus za pomocą _Model Context Protocol_, w skrócie MCP. argus obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [argus Config](https://argus.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/argus`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
argus automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, argus:
...
Jeśli serwer wymaga uwierzytelnienia, argus poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `argus mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji argus bezpieczne przechowa tokeny w `~/.local/share/argus/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w argus, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie argus, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć argus z kontem Sentry.
