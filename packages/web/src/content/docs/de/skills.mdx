...
Agent Skills erlauben OpenCorvus, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenCorvus sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenCorvus vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.opencorvus/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.opencorvus/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenCorvus listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `opencorvus.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `opencorvus.json`):
