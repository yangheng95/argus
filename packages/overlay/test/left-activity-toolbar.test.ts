...
test("skill panel imports dropped files, directories, and zip archives through the project import route", () => {
  ...
  expect(service).toContain('apiJson("skill/mount"')
  expect(service).toContain('apiJson("skill/unmount"')
  expect(service).toContain('apiJson("skill/import-and-mount"')
  expect(service).toContain("apiJson(path")
