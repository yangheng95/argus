...
const TIPS = [
  ...
  "OpenCorvus auto-handles OAuth for remote MCP servers requiring auth",
  ...
  "OpenCorvus auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "OpenCorvus uses LSP servers for intelligent code analysis",
  ...
  "Create a plugin to prevent OpenCorvus from reading sensitive files",
  ...
  "Use {highlight}opencorvus run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}opencorvus serve{/highlight} for headless API access to OpenCorvus",
  "Use {highlight}opencorvus run --attach{/highlight} to connect to a running server",
  "Run {highlight}opencorvus upgrade{/highlight} to update to the latest version",
  "Run {highlight}opencorvus auth list{/highlight} to see all configured providers",
  "Run {highlight}opencorvus agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencorvus{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}opencorvus github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opencorvus fix this{/highlight} on issues to auto-create PRs",
  ...
  'Set {highlight}"tools": {"exclude": ["bash"]}{/highlight} to hide specific tools from an agent',
  ...
  "Run {highlight}opencorvus debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/yangheng95/opencorvus{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with OpenCorvus Zen for curated, tested models",
