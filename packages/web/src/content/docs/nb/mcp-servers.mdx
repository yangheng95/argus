...
Du kan legge til eksterne verktøy i OpenCorvus ved å bruke _Model Context Protocol_ eller MCP. OpenCorvus støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenCorvus Config](https://opencorvus.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="opencorvus.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/opencorvus`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="opencorvus.jsonc" {15}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenCorvus håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenCorvus:
...
Hvis serveren krever autentisering, vil OpenCorvus be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencorvus mcp auth <server-name>`.
...
```json title="opencorvus.json" {7-11}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenCorvus lagre tokens sikkert i `~/.local/share/opencorvus/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i OpenCorvus, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenCorvus-konfigurasjonen som alle andre verktøy.
...
```json title="opencorvus.json" {11, 14-18}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="opencorvus.json" {4-8}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mcp": {
    "sentry": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenCorvus til Sentry-kontoen din.
...
```json title="opencorvus.json" {4-7}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="opencorvus.json" {7-9}
{
  "$schema": "https://opencorvus.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenCorvus-samtale oppført som en separat økt.
