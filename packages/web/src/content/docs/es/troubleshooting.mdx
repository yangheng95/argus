...
Para depurar problemas con Argus, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\argus\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `argus --log-level DEBUG`.

---

## Almacenamiento

argus almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/argus/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\argus`
...
Argus Desktop ejecuta un servidor Argus local (el sidecar `argus-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Argus` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/argus/argus.jsonc` (o `~/.config/argus/argus.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/argus/argus.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\argus\argus.jsonc`
...
Argus también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/argus/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\argus\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Argus pueda reconstruirlo.

1. Salga de Argus Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/argus`
- **Linux**: eliminar `~/.cache/argus` (o ejecutar `rm -rf ~/.cache/argus`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\argus`

3. Reinicie el escritorio Argus.

---

### Solucionar problemas de conexión del servidor

Argus Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `argus.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio Argus requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Argus.

---

### Notificaciones que no se muestran

Argus Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Argus en la configuración de su sistema operativo, y
...
1. Salga del escritorio Argus.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Argus):

- `argus.settings.dat` (URL del servidor predeterminado de escritorio)
- `argus.global.dat` y `argus.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `argus upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `argus models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\argus`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. argus instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\argus`

2. Reinicie argus para reinstalar los paquetes del proveedor más recientes.

Esto obligará a argus a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
argus detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
