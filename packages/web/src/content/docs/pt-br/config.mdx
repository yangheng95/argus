---
title: Configuração
description: Usando a configuração JSON do opencorvus.
---

Você pode configurar o opencorvus usando um arquivo de configuração JSON.

---

## Formato

O opencorvus suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
Por exemplo, se sua configuração global define `theme: "opencorvus"` e `autoupdate: true`, e sua configuração de projeto define `model: "anthropic/claude-sonnet-4-5"`, a configuração final incluirá as três configurações.
...
1. **Configuração remota** (de `.well-known/opencorvus`) - padrões organizacionais
2. **Configuração global** (`~/.config/opencorvus/opencorvus.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCORVUS_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`opencorvus.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.opencorvus`** - agentes, comandos, plugins
6. **Configuração inline** (`OPENCORVUS_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.opencorvus` e `~/.config/opencorvus` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/opencorvus`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do opencorvus em `~/.config/opencorvus/opencorvus.json`. Use a configuração global para preferências de usuário, como temas, provedores ou atalhos de teclado.
...
Adicione `opencorvus.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.

:::tip
Coloque a configuração específica do projeto na raiz do seu projeto.
:::

Quando o opencorvus é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `OPENCORVUS_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCORVUS_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencorvus`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.opencorvus`, então ele **pode substituir** suas configurações.

---

## Esquema

O arquivo de configuração tem um esquema que está definido em [**`opencorvus.ai/config.json`**](https://opencorvus.ai/config.json).
...
Você pode configurar as configurações do servidor para os comandos `opencorvus serve` e `opencorvus web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor opencorvus.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `opencorvus.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do opencorvus através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o opencorvus tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você pode configurar o tema que deseja usar em sua configuração do opencorvus através da opção `theme`.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencorvus/agents/` ou `.opencorvus/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o opencorvus recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`opencorvus run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencorvus/commands/` ou `.opencorvus/commands/`. [Saiba mais aqui](/docs/commands).
...
O opencorvus fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o opencorvus **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o opencorvus com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.opencorvus/plugins/` ou `~/.config/opencorvus/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o opencorvus para usar apenas provedores específicos, em vez de desabilitá-los um a um.
