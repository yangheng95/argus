...
| `rg -n "field-input-actions | field-input-icon                                                                                                          | field-row                                                                                                                                                                             | field-input-group               | search-field" specs packages/overlay/test packages/overlay/src --glob "_._"` | Before the fix, the target selector family only appeared in `field.css`. Live `.field-input-group` and `.search-field*` owners remained in components/tests. | Keep the live primitive families and guard against exact retired class tokens only. |
