...
describe("build agent goal execution discipline prompt", () => {
  ...
  test("requires observed frontend verification and random preview ports", async () => {
    ...
    expect(normalized).toContain("capture fresh task-scoped screenshots for the changed surface")
    expect(normalized).toContain("inspect the affected region against the source contract yourself")
    expect(normalized).toContain("source/local binding puzzles only as investigation evidence")
    expect(normalized).toContain("Strong parity evidence for a bound region cites fresh `reference-comparison` artifacts")
    expect(normalized).toContain("Treat `browser_preview_bind_local_module` evidence as `source-binding`")
    expect(normalized).toContain("it is a binding puzzle for investigation")
    expect(normalized).toContain("not final Reference vs Implementation proof")
    expect(normalized).toContain("route-health failure")
    expect(normalized).toContain("Viewport width differences remain visible in fresh screenshots")
    ...
    expect(normalized).toContain(
      "report the exact evidence gap or blocker instead of accepting an unbound screenshot review",
    )
