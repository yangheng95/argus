...
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `rg -n "app-notification\[data-dismissed\|data-dismissed=\|dismissedAt" packages/overlay/src packages/overlay/test specs` | The dismissed visual state is only in `notifications.css`; `NotificationCenter.tsx` emits `data-dismissed` from `dismissedAt`; `notify.ts` keeps center-history rows. | Fix the shared CSS state once and keep the store/component contract unchanged.                                         |
| `rg -n -e "NotificationCenter" -e "app-notification" packages/overlay/src packages/overlay/test specs`                    | Toast and panel both use the same component and CSS.                                                                                                                  | Do not add toast/panel special cases.                                                                                  |
