...
此命令启动一个 HTTP 服务器，提供对 OpenCorvus 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencorvus`）。
...
此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenCorvus。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencorvus`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `opencorvus`） |
