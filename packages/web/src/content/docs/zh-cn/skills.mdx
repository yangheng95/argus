...
| `required_tools` | string[] | owning stage 在成功上报前必须完成的 tool 调用 |

## 2. 内置 Skills

随二进制打包（`src/skill/skill.ts` 的 `builtins` 数组），**只有三个**：
...
:::caution
文件缓存后不再重新下载；更新需手动清缓存或 `opencorvus skill remove --url <url>`。
:::
...
Session 初始化时会把命中的 `SKILL.md` 正文注入 agent 的 system prompt。`auto_detect` 命中的 skill 会跨 stage 注入，避免相关经验因为 stage 标记不一致而缺失；`stage` 只表示该 skill 的 `required_tools` 由哪个 stage 拥有，不控制正文可见性。
