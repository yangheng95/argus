...
Usually evaluator's `replan_guidance` isn't providing improvement info. Check:
...
3. `selectorsSatisfied()` correctly treats `skipped` as not-passed (config errors can undo this)

### TypeScript errors undetected

Ensure evaluator tier ≥ `standard` (includes lint), or explicitly add `"lint"` / `"typecheck"` to `qa_rule_selectors`.
...
Per CLAUDE.md principle 7: **fix the tool first, then continue the task**.
