---
title: การกำหนดค่า
description: การใช้การกำหนดค่า OpenCorvus JSON
---

คุณสามารถกำหนดค่า OpenCorvus ได้โดยใช้ไฟล์กำหนดค่า JSON

---

## รูปแบบ

OpenCorvus รองรับทั้งรูปแบบ **JSON** และ **JSONC** (JSON พร้อมความคิดเห็น)
...
ตัวอย่างเช่น หากการกำหนดค่าส่วนกลางของคุณตั้งค่า `theme: "opencorvus"` และ `autoupdate: true` และการกำหนดค่าโปรเจ็กต์ของคุณตั้งค่า `model: "anthropic/claude-sonnet-4-5"` การกำหนดค่าสุดท้ายจะรวมการตั้งค่าทั้งสามรายการไว้ด้วย
...
1. **การกำหนดค่าระยะไกล** (จาก `.well-known/opencorvus`) - ค่าเริ่มต้นขององค์กร
2. **การกำหนดค่าสากล** (`~/.config/opencorvus/opencorvus.json`) - การตั้งค่าของผู้ใช้
3. **การกำหนดค่าแบบกำหนดเอง** (`OPENCORVUS_CONFIG` env var) - การแทนที่แบบกำหนดเอง
4. **การกำหนดค่าโครงการ** (`opencorvus.json` ในโครงการ) - การตั้งค่าเฉพาะโครงการ
5. **`.opencorvus` ไดเรกทอรี** - ตัวแทน คำสั่ง ปลั๊กอิน
6. **การกำหนดค่าแบบอินไลน์** (`OPENCORVUS_CONFIG_CONTENT` env var) - การแทนที่รันไทม์
...
:::note
ไดเรกทอรี `.opencorvus` และ `~/.config/opencorvus` ใช้ **ชื่อพหูพจน์** สำหรับไดเรกทอรีย่อย: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` และ `themes/` ชื่อเอกพจน์ (เช่น `agent/`) ยังรองรับความเข้ากันได้แบบย้อนหลังด้วย
:::
...
องค์กรสามารถจัดเตรียมการกำหนดค่าเริ่มต้นผ่านทางจุดสิ้นสุด `.well-known/opencorvus` สิ่งนี้จะถูกดึงออกมาโดยอัตโนมัติเมื่อคุณตรวจสอบสิทธิ์กับผู้ให้บริการที่รองรับ
...
วางการกำหนดค่า OpenCorvus ส่วนกลางของคุณใน `~/.config/opencorvus/opencorvus.json` ใช้การกำหนดค่าส่วนกลางสำหรับการตั้งค่าทั้งผู้ใช้ เช่น ธีม ผู้ให้บริการ หรือปุ่มลัด
...
เพิ่ม `opencorvus.json` ในรูทโปรเจ็กต์ของคุณ การกำหนดค่าโปรเจ็กต์มีความสำคัญสูงสุดในบรรดาไฟล์กำหนดค่ามาตรฐาน โดยจะแทนที่การกำหนดค่าทั้งส่วนกลางและระยะไกล

:::tip
วางการกำหนดค่าเฉพาะโปรเจ็กต์ไว้ที่รากของโปรเจ็กต์ของคุณ
:::

เมื่อ OpenCorvus เริ่มต้นระบบ จะค้นหาไฟล์กำหนดค่าในไดเร็กทอรีปัจจุบันหรือข้ามไปยังไดเร็กทอรี Git ที่ใกล้ที่สุด
...
ระบุเส้นทางไฟล์กำหนดค่าที่กำหนดเองโดยใช้ตัวแปรสภาพแวดล้อม `OPENCORVUS_CONFIG`
...
ระบุไดเร็กทอรีการกำหนดค่าที่กำหนดเองโดยใช้ `OPENCORVUS_CONFIG_DIR`
ตัวแปรสภาพแวดล้อม ไดเร็กทอรีนี้จะถูกค้นหาตัวแทน, คำสั่ง,
โหมดและปลั๊กอินเหมือนกับไดเร็กทอรี `.opencorvus` มาตรฐานและควร
...
ไดเร็กทอรีที่กำหนดเองถูกโหลดหลังจากไดเร็กทอรี config ส่วนกลางและ `.opencorvus` ดังนั้นจึง **สามารถแทนที่** การตั้งค่าได้

---

## สคีมา

ไฟล์กำหนดค่ามีสคีมาที่กำหนดไว้ใน [**`opencorvus.ai/config.json`**](https://opencorvus.ai/config.json)
...
คุณสามารถกำหนดการตั้งค่าเซิร์ฟเวอร์สำหรับคำสั่ง `opencorvus serve` และ `opencorvus web` ผ่านตัวเลือก `server`
...
- `mdns` - ​​เปิดใช้งานการค้นหาบริการ mDNS ซึ่งช่วยให้อุปกรณ์อื่นๆ บนเครือข่ายค้นพบเซิร์ฟเวอร์ OpenCorvus ของคุณได้
- `mdnsDomain` - ​​ชื่อโดเมนที่กำหนดเองสำหรับบริการ mDNS ค่าเริ่มต้นเป็น `opencorvus.local` มีประโยชน์สำหรับการเรียกใช้หลายอินสแตนซ์บนเครือข่ายเดียวกัน
...
คุณสามารถกำหนดค่าผู้ให้บริการและรุ่นที่คุณต้องการใช้ในการกำหนดค่า OpenCorvus ของคุณได้ผ่านตัวเลือก `provider`, `model` และ `small_model`
...
ตัวเลือก `small_model` จะกำหนดค่าโมเดลแยกต่างหากสำหรับงานที่ไม่ซับซ้อน เช่น การสร้างชื่อเรื่อง ตามค่าเริ่มต้น OpenCorvus จะพยายามใช้โมเดลที่ถูกกว่าหากมีให้บริการจากผู้ให้บริการของคุณ ไม่เช่นนั้นโมเดลจะกลับไปเป็นโมเดลหลักของคุณ
...
คุณสามารถกำหนดค่าธีมที่คุณต้องการใช้ในการกำหนดค่า OpenCorvus ของคุณได้ผ่านตัวเลือก `theme`
...
คุณยังสามารถกำหนดตัวแทนโดยใช้ไฟล์ Markdown ใน `~/.config/opencorvus/agents/` หรือ `.opencorvus/agents/` [เรียนรู้เพิ่มเติมที่นี่](/docs/agents)
...
ตัวแทนเริ่มต้นจะต้องเป็นตัวแทนหลัก (ไม่ใช่ตัวแทนย่อย) ซึ่งอาจเป็นตัวแทนในตัว เช่น `"build"` หรือ `"plan"` หรือ [ตัวแทนที่กำหนดเอง](/docs/agents) ที่คุณกำหนดไว้ หากไม่มีตัวแทนที่ระบุหรือเป็นตัวแทนย่อย OpenCorvus จะถอยกลับไปที่ `"build"` พร้อมคำเตือน

การตั้งค่านี้ใช้กับอินเทอร์เฟซทั้งหมด: TUI, CLI (`opencorvus run`) แอปเดสก์ท็อป และ GitHub Action
...
คุณยังสามารถกำหนดคำสั่งโดยใช้ไฟล์ Markdown ใน `~/.config/opencorvus/commands/` หรือ `.opencorvus/commands/` [เรียนรู้เพิ่มเติมที่นี่](/docs/commands)
...
OpenCorvus จะดาวน์โหลดการอัปเดตใหม่โดยอัตโนมัติเมื่อเริ่มต้นระบบ คุณสามารถปิดการใช้งานนี้ได้โดยใช้ตัวเลือก `autoupdate`
...
ตามค่าเริ่มต้น opencorvus **อนุญาตการดำเนินการทั้งหมด** โดยไม่ต้องมีการอนุมัติอย่างชัดเจน คุณสามารถเปลี่ยนสิ่งนี้ได้โดยใช้ตัวเลือก `permission`
...
[ปลั๊กอิน](/docs/plugins) ขยาย OpenCorvus ด้วยเครื่องมือที่กำหนดเอง hooks และการผสานรวม

วางไฟล์ปลั๊กอินใน `.opencorvus/plugins/` หรือ `~/.config/opencorvus/plugins/` คุณยังสามารถโหลดปลั๊กอินจาก npm ผ่านตัวเลือก `plugin`
...
สิ่งนี้มีประโยชน์เมื่อคุณต้องการจำกัด OpenCorvus ให้ใช้เฉพาะผู้ให้บริการบางราย แทนที่จะปิดการใช้งานทีละราย
