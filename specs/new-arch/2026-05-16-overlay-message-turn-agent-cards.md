...
| Integrity               | `materializeRunningIntegrity` / `materializeIntegrity` use the dedicated review card                                      | Integrity review streams have no durable `messageID`                       | `review.stream.*` / verdict keep `integrity:session:<sid>` as the single source; normal message rows may project usage into it but must not migrate or duplicate the dedicated card |
...
Integrity review streams remain a protocol exception because `review.stream.*` and verdict events do not carry a durable `messageID` and can arrive before message metadata. When the dedicated `integrity:session:<sid>` review card exists, later `message.updated` rows for the same integrity session must project onto that dedicated card instead of migrating it into, or duplicating it as, a message-turn card. Integrity message rows without a dedicated review card still use the normal adjacent message-turn mechanism.

If an integrity completed event arrives before message metadata, keep the pending map keyed by `sessionID`; drain it into the dedicated review card, not into a synthetic or inferred message card.
...
2. Integrity review events remain a protocol exception in P1. Backend `review.stream.started`, `review.stream.progress`, `review.stream.chunk`, and `integrity.review.completed` carry `sessionID` but do not carry a durable `messageID`; `started/progress/chunk` can arrive before any message metadata for that integrity session. Therefore the overlay keeps the single `integrity:session:<sessionID>` card for review protocol events rather than inventing a synthetic message id. If normal `message.updated` rows arrive while that dedicated card exists, they project metadata/usage into the dedicated card and must not create sibling message-turn cards for the same review output.
