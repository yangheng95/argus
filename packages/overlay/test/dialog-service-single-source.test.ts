...
describe("app/session dialog single source", () => {
  ...
  test("task creation no longer carries queue-decision dialog chrome", () => {
    expect(appHost).not.toContain('dialogStore.app.kind === "task-queue-decision"')
    expect(appHost).not.toContain("import { SegmentedControl")
    expect(appHost).not.toContain("<SegmentedControl")
    expect(appHost).not.toContain("chooseTaskDecision")
    expect(appHost).not.toContain("isTaskCardDecision()")
    expect(appHost).not.toContain("recommendedValue")
    expect(appHost).not.toContain("countdownSeconds")
    expect(appDialogService).not.toContain("task-queue-decision")
    expect(appDialogService).not.toContain("recommendedValue")
    expect(appDialogService).not.toContain("countdownSeconds")
    expect(appDialogBrowser).toContain("default queue behavior bypasses the retired task decision dialog")
    expect(appDialogBrowser).toContain('assert.equal((taskBodies[0] as { queue?: unknown }).queue, false)')
    expect(appDialogBrowser).not.toContain("app-dialog-decision__choice")
    expect(appDialogBrowser).not.toContain('await page.keyboard.press("Space")')
  })
})
