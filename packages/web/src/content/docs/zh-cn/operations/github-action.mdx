...
| 参数               | 必填 | 默认                        | 说明                                            |
| ------------------ | ---- | --------------------------- | ----------------------------------------------- |
| `model`            | ✓    | —                           | `provider/model`，如 `alibaba-cn/qwen3.5-plus`  |
...
默认 App/OIDC 安装保持 `GITHUB_TOKEN` 只读。写权限来自 OIDC 换到的 App Token：
...
使用 `use_github_token: true` 时，workflow 直接使用 `GITHUB_TOKEN`，必须显式授予写权限：
...
用户发 `/oc` 时校验其仓库权限必须为 `admin` 或 `write`；失败拒绝执行。方式 B 跳过。
...
`MOCK_TOKEN` 需 `admin` 或 `write` 权限的 PAT。
