---
title: Server
description: Interact with argus server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `argus serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an argus client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `argus`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `argus serve` and `argus web`.
...
When you run `argus` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the argus server to interact with argus programmatically.
:::

This architecture lets argus support multiple clients and allows you to interact with argus programmatically.

You can run `argus serve` to start a standalone server. If you have the
argus TUI running, `argus serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Argus [IDE](/docs/ide) plugins.
...
The argus server exposes the following APIs.
