---
title: Providers
description: Using any LLM provider in Argus.
---

import config from "../../../config.mjs"
export const console = config.console

Argus uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Argus config.
...
You can customize the providers through the `provider` section in your Argus
config.
...
Argus Zen is a list of models provided by the Argus team that have been
tested and verified to work well with Argus. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Argus Zen.
:::

1. Run the `/connect` command in the TUI, select `Argus Zen`, and head to [argus.ai/auth](https://argus.ai/zen).
...
It works like any other provider in Argus and is completely optional to use.

---

## Argus Go

Argus Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Argus team that have been
tested and verified to work well with Argus.

1. Run the `/connect` command in the TUI, select `Argus Go`, and head to [argus.ai/auth](https://argus.ai/zen).
...
To use Amazon Bedrock with Argus:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running argus:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `argus.json`:
...
:::info
Using your Claude Pro/Max subscription in Argus is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for argus to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your argus config.
...
Argus uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Argus
to only use your own GitLab-hosted instance, add the following to your
`argus.json` file. It is also recommended to disable session sharing.
...
More documentation on [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with argus:
...
To use Google Vertex AI with Argus:
...
In the event you see a feature or model from Helicone that isn't configured automatically through argus, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) plugin to automatically log each Argus conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Argus conversation listed as a separate session.
...
You can configure argus to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure argus to use local models through LM Studio.
...
You can configure argus to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Argus. See the [Ollama integration docs](https://docs.ollama.com/integrations/argus) for details.
:::
...
To use Ollama Cloud with Argus:
...
3. Copy the API key for use in Argus.
...
6. **Important**: Before using cloud models in Argus, you must pull the model information locally:
...
Argus Zen is a list of tested and verified models provided by the Argus team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Argus Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Argus Zen**.

   ```txt
   /connect
   ```

3. Enter your Argus API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your argus config.
...
5. You can also customize them through your argus config. Here's an example of specifying a provider
...
You can also customize models through your argus config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with argus. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `argus.json` file in your project directory:
...
The `limit` fields allow Argus to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `argus auth list` to see if the credentials
...
2. For custom providers, check the argus config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your argus config.
