...
Agent skills omogucavaju Argus da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Argus pretrazuje ove lokacije:

- Konfiguracija projekta: `.argus/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/argus/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Argus ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.argus/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.argus/skills/git-release/SKILL.md` ovako:
...
Argus navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `argus.json`:
...
**Za ugradene agente** (u `argus.json`):
