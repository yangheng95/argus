...
- `specs/new-arch/2026-06-09-task-agent-model-context.md`: task operator model resolution is backend-owned through `messageContext()` and `resolveAgentModelRef`; overlay must not guess.
- `specs/new-arch/2026-06-10-task-config-overrides-immediate-effect.md`: runtime task config is live project config plus task-root `configOverlay`; task snapshots are audit metadata only.
