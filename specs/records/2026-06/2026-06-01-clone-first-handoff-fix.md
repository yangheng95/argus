...
- The frontend_design report set `final_acceptance_mode=maintainable_replacement_required` and published a large `baseline_replacement_plan`, even though the immediate benchmark gate was visual clone similarity >= 80%.
...
- In host-prepared webpage-clone frontend_design turns, keep `final_acceptance_mode=visual_baseline_allowed` unless the user explicitly asks to replace generated/source-dom regions before acceptance.
...
- Add a dynamic clone-first handoff section derived from decision_log entries. It tells Requirements, Architect, Build, and Acceptance that maintainability means source-derived baseline adoption, reusable components/libraries for targeted refinements, no screenshot/iframe/freehand rebuild, and explicit reporting of remaining generated regions.
