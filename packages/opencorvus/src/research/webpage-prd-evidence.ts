...
export function renderWebpagePrdEvidencePromptSection(evidence: WebpagePrdEvidence | undefined): string | undefined {
  ...
  return [
    ...
    "The submitted `bundle.full_markdown` for this webpage contract must be complete but bounded enough to submit in one terminal-tool call. Cover every major visible region with evidence-backed work packets, component-kind hypotheses, style/layout questions, interaction/data questions, expected downstream outputs, acceptance checks, and risks; do not pad and do not dump raw artifacts.",
