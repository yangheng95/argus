...
Da biste otklonili probleme s OpenCorvus, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\opencorvus\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `opencorvus --log-level DEBUG`.

---

## Pohrana

OpenCorvus pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opencorvus/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\opencorvus`
...
OpenCorvus Desktop pokreće lokalni OpenCorvus server (`opencorvus-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenCorvus` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/opencorvus/opencorvus.jsonc` (ili `~/.config/opencorvus/opencorvus.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencorvus/opencorvus.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencorvus\opencorvus.jsonc`
...
OpenCorvus također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opencorvus/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencorvus\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenCorvus mogao ponovo izgraditi.

1. Potpuno zatvorite OpenCorvus Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/opencorvus`
- **Linux**: obrišite `~/.cache/opencorvus` (ili pokrenite `rm -rf ~/.cache/opencorvus`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\opencorvus`

3. Ponovo pokrenite OpenCorvus Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenCorvus Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `opencorvus.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENCORVUS_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENCORVUS_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenCorvus Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenCorvus karakteristikama.

---

### Obavještenja se ne prikazuju

OpenCorvus Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenCorvus u postavkama vašeg OS-a, i
...
1. Zatvorite OpenCorvus Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenCorvus Desktop aplikacije):

- `opencorvus.settings.dat` (URL zadanog servera za desktop)
- `opencorvus.global.dat` i `opencorvus.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenCorvus:
...
Da saznate kojim modelima imate pristup, pokrenite `opencorvus models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\opencorvus`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenCorvus dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\opencorvus`

2. Ponovo pokrenite OpenCorvus da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenCorvus da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenCorvus će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
