...
完整的具体定义见仓库中的 `specs/artifacts/portable-expert-squad-template`。Validation 是只读操作并调用真实 Registry；import 是独立、显式的 Manager 操作，不会激活 package。Folder 和 ZIP import 必须声明 `installationScope: "project" | "global"`，两个 scope 共用同一个 Registry 和 manifest identity 规则。只有当项目需要使用它时，才通过唯一的 `prompt_profile.active` 选择已安装 manifest ID。
...
`base` 是默认内置专家团 package，也是 Advanced 的便捷复合型版本。紧凑的 `composite-delivery` workflow 依次调度 Explore `base-researcher`、Delegated Worker `base-planner`、Build `base-developer` 与 Delegated Worker `base-tester`，使每个身份继承与职责一致的 runtime ownership。固定的 `integrity-verified-delivery` 变体在测试后追加 package-owned Integrity `base-integrity-reviewer`；固定的 `visual-verified-delivery` 变体在开发后并行运行 Tester 与 Visual Quality Assurance `base-visual-reviewer`，再由 Integrity Reviewer 汇合两支证据。Orchestrator 精确选择一张 graph，所选 graph 的每个节点都必须执行。四个核心身份依次发布并选择 canonical `base/research-report`、`base/implementation-plan`、`base/development-report` 与 `base/test-report` 交接 Artifact，两个复核身份使用 typed VisualReview 与 IntegrityReview 证据。Base 不创建或消费 RequirementSet、ContractGraph、Goal 或 Delivery Slice 计划事实。
