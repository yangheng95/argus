...
describe("core prompt hygiene", () => {
  ...
  test("webpage replica prompts enforce frontend-owned visual skeleton first workflow", async () => {
    ...
    expect(integrity).toContain("focused visual acceptance virtual")
  ...
  test("orchestrator prompt makes post-build integrity pass the terminal lifecycle path", async () => {
    ...
    expect(normalized).toContain("`completed`, `failed`, and `cancelled` are terminal")
    expect(normalized).toContain("ordinary operator messages, injected messages, queued wakes")
    expect(normalized).toContain("Scheduler activity is legal only while the task is active")
