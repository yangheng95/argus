...
describe("frontend-design prompt assembly", () => {
  ...
  test("webpage rawproject prompts make frontend_design materialize the source project itself", () => {
    ...
    expect(prompt).toContain("After each replacement, render the target project with `webpage_render`")
  ...
  test("skeleton project tool returns bounded source evidence entrypoints", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.output).toContain("leaving future region slots as comments/placeholders without imports")
        ...
        expect(result.output).toContain("use `webpage_render`, compare against `reference.png` with `webpage_evaluate`")
        expect(result.output).toContain("call `webpage_vision_judge` for visible theme/layout/density/control mismatches")
  ...
  test("frontend_design process trace records region selection and source edits", async () => {
    ...
    expect(selectionResult.output).toContain("Before writing files for another region, render the target project for this region")
    expect(selectionResult.output).toContain("repair visible mismatches in this same region")
