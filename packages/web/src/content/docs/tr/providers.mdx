---
title: Sağlayıcılar
description: opencorvus'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

opencorvus, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı opencorvus yapılandırmanızda yapılandırın.
...
`~/.local/share/opencorvus/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları opencorvus'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenCorvus Zen, opencorvus ekibi tarafından sağlanan modellerin bir listesidir.
opencorvus ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenCorvus Zen ile başlamanızı öneririz.
:::
...
opencorvus'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u opencorvus ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   opencorvus'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `opencorvus.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi opencorvus'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   opencorvus'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   OpenCorvus yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in opencorvus auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
opencorvus, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. opencorvus'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`opencorvus.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth) ana sayfasında.

##### Yapılandırma

`opencorvus.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi opencorvus'la kullanmak için:
...
Google Vertex AI'yi opencorvus ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları opencorvus çalıştırılırken ayarlayın.
...
Helicone'un opencorvus aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her opencorvus görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her opencorvus görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
opencorvus'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla opencorvus'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla opencorvus'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini opencorvus için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencorvus)'a bakın.
:::
...
Ollama Cloud'u opencorvus ile kullanmak için:
...
3. opencorvus'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: opencorvus'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenCorvus Zen, opencorvus ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenCorvus Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenCorvus Zen**'i arayın.

   ```txt
   /connect
   ```

3. opencorvus API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   opencorvus yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca opencorvus yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi OpenCorvus ile kullanmak için:
...
Modelleri opencorvus yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı opencorvus ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `opencorvus.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları opencorvus'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `opencorvus auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için opencorvus yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, opencorvus yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
