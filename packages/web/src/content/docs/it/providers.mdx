---
title: Provider
description: Usa qualsiasi provider LLM in Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Argus.
...
Argus Zen e una lista di modelli forniti dal team Argus, testati e
verificati per funzionare bene con Argus. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Argus Zen.
:::

1. Esegui `/connect` nella TUI, seleziona argus e vai su [argus.ai/auth](https://argus.ai/auth).
...
Funziona come qualsiasi altro provider in Argus ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Argus:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui argus:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `argus.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Argus non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché argus funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di argus.
...
salvare il token nello storage di auth di argus.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Argus usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Argus
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `argus.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth).

##### Configurazione

Personalizza tramite `argus.json`:
...
Per usare il tuo abbonamento GitHub Copilot con argus:
...
Per usare Google Vertex AI con Argus:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da argus, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) per loggare automaticamente ogni conversazione di Argus come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Argus elencata come sessione separata.
...
Puoi configurare argus per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare argus per usare modelli locali tramite LM Studio.
...
Puoi configurare argus per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Argus. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/argus) per dettagli.
:::
...
Per usare Ollama Cloud con Argus:
...
3. Copia la chiave API da usare in Argus.
...
6. **Importante**: Prima di usare modelli cloud in Argus, devi scaricare le informazioni del modello localmente:
...
Argus Zen è una lista di modelli testati e verificati forniti dal team Argus. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Argus Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Argus Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Argus.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di argus.
...
5. Puoi anche personalizzarli tramite la configurazione di argus. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di argus. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con argus. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `argus.json` nella directory del progetto:
...
I campi `limit` permettono a Argus di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `argus auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di argus e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config argus.
