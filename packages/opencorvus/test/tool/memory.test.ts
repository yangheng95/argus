  ...
  test("supports global and session-scoped memory lifecycle", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const writeData = JSON.parse(write.output) as { fileId: string; scope: string }
        ...
        const sessionWriteData = JSON.parse(sessionWrite.output) as { fileId: string; scope: string }
        ...
        const searchData = JSON.parse(search.output) as { results: Array<{ fileId: string; scope: string }> }
        ...
        const getData = JSON.parse(get.output) as { text: string; scope: string }
