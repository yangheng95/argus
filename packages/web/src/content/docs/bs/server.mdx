---
title: Server
description: Komunicirajte s argus serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `argus serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji argus klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `argus.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `argus`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `argus serve` i za `argus web`.
...
Kada pokrenete `argus`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite argus server za programsku interakciju sa argus-om.
:::

Ova arhitektura omogucava argus podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `argus serve` da startate standalone server. Ako je argus TUI vec pokrenut, `argus serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Argus [IDE](/docs/ide) pluginovi.
...
argus server izlaže sljedece API-je.
