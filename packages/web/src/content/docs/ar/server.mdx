---
title: الخادم
description: تفاعل مع خادم argus عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `argus serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل argus استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `argus.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `argus`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `argus serve` و `argus web`.
...
عند تشغيل `argus` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم argus للتفاعل مع argus برمجيا.
:::

تتيح هذه البنية لـ argus دعم عدة عملاء وتمكّنك من التفاعل مع argus برمجيا.

يمكنك تشغيل `argus serve` لبدء خادم مستقل. إذا كانت واجهة argus في terminal (TUI)
قيد التشغيل، فسيبدأ `argus serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Argus لـ [IDE](/docs/ide).
...
يعرض خادم argus واجهات API التالية.
