...
Você pode adicionar ferramentas externas ao opencorvus usando o _Model Context Protocol_, ou MCP. O opencorvus suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do opencorvus](https://opencorvus.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/opencorvus`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O opencorvus lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o opencorvus irá:
...
Se o servidor requer autenticação, o opencorvus solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `opencorvus mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o opencorvus armazenará os tokens de forma segura em `~/.local/share/opencorvus/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no opencorvus, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do opencorvus como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o opencorvus à sua conta do Sentry.
