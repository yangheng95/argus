...
U Argus mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Argus koristi nasu `argus` temu.
...
Argus dolazi sa vise ugradenih tema.
...
- Zele da Argus odgovara izgledu njihovog terminala
...
Argus podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/argus/themes/*.json` ili `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.argus/themes/*.json`
4. **Trenutni radni direktorij** - `./.argus/themes/*.json`
