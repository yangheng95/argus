...
OpenCorvus には、OpenCorvus 設定を通じてカスタマイズできるキーバインドのリストがあります。
...
OpenCorvus は、ほとんどのキーバインドに `leader` キーを使用します。これにより、ターミナルでの競合が回避されます。
...
OpenCorvus デスクトップアプリのプロンプト入力は、テキストを編集するための一般的な Readline/Emacs スタイルのショートカットをサポートしています。これらは組み込みであり、現在 `opencorvus.json` を介して構成することはできません。
