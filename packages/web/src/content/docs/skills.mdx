...
OpenCorvus fetches `{url}/index.json` (`{ skills: [{ name, description, files[] }] }`), downloads each file, caches under `~/.cache/opencorvus/skills/`, loads `SKILL.md` (`src/skill/discovery.ts`).
...
Clones into `~/.config/opencorvus/skills-market/<slug>/`, appends to `skills.paths`, writes `.opencorvus-skill-source.json` (`src/skill/manager.ts`).
