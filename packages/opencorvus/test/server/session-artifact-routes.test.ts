...
describe("session artifact read routes", () => {
  ...
  test("todo, task-plan, and scratchpad reads reject sessions outside the active project", async () => {
    ...
    expect(await ownTodo.json()).toEqual([{ content: "project-a-todo-marker", status: "pending", priority: "high" }])
