...
test("CommandPalette uses Kobalte dialog content for the modal shell", () => {
  expect(SOURCE).toContain('import * as KobalteDialog from "@kobalte/core/dialog"')
  expect(SOURCE).toContain("<KobalteDialog.Root")
  expect(SOURCE).toContain("<KobalteDialog.Portal")
  expect(SOURCE).toContain("<KobalteDialog.Content")
