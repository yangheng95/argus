...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("argus-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("argus_lib=debug,argus_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("argus_lib=info,argus_desktop=info,sidecar=info")
        }
    });
