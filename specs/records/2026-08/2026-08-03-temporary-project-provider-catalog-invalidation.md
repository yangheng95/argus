...
- Hard constraints: keep one canonical Provider catalog, fix the root writer/invalidation boundary without fallback or UI workarounds, add positive non-UI contract coverage, do not add or run UI automation tests, preserve unrelated workspace changes, and push the completed change to the legacy remote.
