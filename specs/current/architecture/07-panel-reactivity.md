...
projection and treats `/mailbox/events` as change notification only. Every
change causes a fresh canonical page read; the Server-Sent Events stream is not
a second payload/history source. Project directory changes abort the previous
