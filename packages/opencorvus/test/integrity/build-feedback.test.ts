...
describe("composeIntegrityFeedbackForBuild", () => {
  ...
  test("renders latest blocking and advisory findings with persistent root facts", () => {
    const now = Date.now()
    const stamp = now.toString(16)
      ...
      findings: [
        {
          ...
          description: "Invalid model/temperature/maxTokens values from localStorage reach the API settings.",
    ...
    expect(rootHistory.persistentBlockingRoots[0]?.rootID).toBe("storage-validation")
    expect(composed).toContain("## Persistent Integrity Findings")
    expect(composed).toContain("storage-validation")
    expect(composed).toContain("R1, R2")
    expect(composed).toContain("BF-R2-settings-validation")
  ...
  test("uses supplied spec snapshot lineage instead of active snapshot only", () => {
      ...
      findings: [
        {
          ...
          description: "Old snapshot already found this root.",
      ...
      findings: [
        {
          ...
          description: "Active corrective snapshot still reports the root.",
          evidence: ["src/lineage.ts"],
    ...
    expect(composed).toContain("Integrity has reviewed this task 2 time(s)")
    expect(composed).toContain(`inherited=${oldSpecID}`)
    expect(composed).toContain("lineage-root")
    expect(composed).toContain("R1, R2")
  })
