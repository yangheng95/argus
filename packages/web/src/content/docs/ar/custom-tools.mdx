...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في opencorvus.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في opencorvus مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.opencorvus/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/opencorvus/tools/`.
