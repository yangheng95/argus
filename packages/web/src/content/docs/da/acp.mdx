...
description: Brug Argus i enhver ACP-kompatibel editor.
---

Argus understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge Argus via ACP, konfigurer din editor til at køre kommandoen `argus acp`.

Kommandoen starter Argus som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'Argus'-agent i AI Chat-agentvælgeren.
...
For at bruge Argus som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge Argus som ACP-agent til chat.
...
Argus fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din Argus-konfiguration
