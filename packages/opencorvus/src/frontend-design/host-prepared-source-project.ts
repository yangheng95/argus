...
export function renderHostPreparedFrontendProjectSection(project: HostPreparedFrontendProject): string {
  ...
  const lines = [
    ...
    "Do not output a standalone component checklist or advice-only report. Use `update_frontend_*` result tools to identify the source baseline, visual-region restoration plan, skeleton-to-project quality contract, visual/data contracts, completeness review, and open questions needed for later maintainable project transcription, then finalize with `submit_frontend_template({ final: true })`.",
  ...
  lines.push(
    "After bounded evidence read/review plus visual HTML skeleton restoration you can complete with the exposed tools only when structured `visual_validation_evidence` proves the visual skeleton usable with no blocking debt. Each evidence row must include viewport dimensions/state plus explicit `capture_mode` (`viewport` or `full_page`) matching the materialized screenshot. Populate the contract through `update_frontend_*` tools, then call `submit_frontend_template({ final: true })`. The report must identify the skeleton as `visual_baseline_input`, describe completed visual-region restorations, and name any unfinished visual/source debt as frontend_design incomplete/blocked work. Production/design-system implementation tasks must instead report a real `implementation_target` or `blocked`.",
  )
  lines.push(
