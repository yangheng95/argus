---
title: Plugins
description: Écrivez vos propres plugins pour étendre Argus.
---

Les plugins vous permettent d'étendre Argus en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Argus.
...
- `.argus/plugins/` - Plugins au niveau du projet
- `~/.config/argus/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/argus/node_modules/`.
...
1. Configuration globale (`~/.config/argus/argus.json`)
2. Configuration du projet (`argus.json`)
3. Répertoire global des plugins (`~/.config/argus/plugins/`)
4. Répertoire des plugins du projet (`.argus/plugins/`)
...
Argus exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK argus pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre argus.
...
:::note
Si vous utilisez l'application de bureau Argus, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez argus de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à argus :
...
L'assistant `tool` crée un outil personnalisé que argus peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour argus aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://argus.ai/docs/sdk) pour plus de détails.
