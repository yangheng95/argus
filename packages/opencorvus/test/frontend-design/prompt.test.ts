...
describe("frontend-design prompt assembly", () => {
  ...
  test("webpage rawproject prompts make frontend_design materialize the source project itself", () => {
    ...
    expect(prompt).toContain("Populate the target delivery project directly with file-edit tools")
    expect(prompt).toContain("instead of running project scaffolding commands")
    expect(prompt).toContain("Configuration manifests are late-stage integration files, not the first workload")
    ...
    expect(prompt).toContain("Source coverage comes before manifest/config tuning")
  ...
  test("skeleton project tool returns bounded source evidence entrypoints", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.output).toContain("The next target-project write should be `src/App.tsx`")
