---
title: Tjener
description: Samhandle med OpenCorvus-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Kommandoen `opencorvus serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCorvus-klient kan bruke.
...
| --------------- | --------------------------------------------- | ------------------ |
| `--port`        | Port å lytte på                               | `7878`             |
| `--hostname`    | Vertsnavn å lytte på                          | `127.0.0.1`        |
| `--mdns`        | Aktiver mDNS-oppdagelse                       | `false`            |
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `opencorvus.local` |
...
Sett `OPENCORVUS_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencorvus` som standard, eller sett `OPENCORVUS_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opencorvus serve` og `opencorvus web`.
...
Når du kjører `opencorvus` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenCorvus-serveren til å samhandle med OpenCorvus programmatisk.
:::

Denne arkitekturen lar OpenCorvus støtte flere klienter og lar deg samhandle med OpenCorvus programmatisk.

Du kan kjøre `opencorvus serve` for å starte en frittstående server. Hvis du har
OpenCorvus TUI kjører, vil `opencorvus serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenCorvus [IDE](/docs/ide) plugins.
...
For eksempel `http://localhost:7878/doc`. Bruk spesifikasjonen til å generere klienter eller inspisere forespørsels- og svartyper. Eller se den i en Swagger-utforsker.

---

## APIer

opencorvus-serveren viser følgende APIer.
...
| `POST`   | `/permission/:requestID/reply` | Svar på en tillatelsesforespørsel        | body: `{ reply, message? }`, returnerer `boolean`                                     |
...
| `POST` | `/tui/control/response` | Svar på en kontrollforespørsel (`{ id, body } or { id, error }`) | `boolean`                  |
