---
title: Sunucu
description: opencorvus sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

`opencorvus serve` komutu, opencorvus istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| --------------- | ---------------------------------------------- | ------------------ |
| `--port`        | Dinlenecek bağlantı noktası                    | `7878`             |
| `--hostname`    | Dinlenecek ana bilgisayar adı                  | `127.0.0.1`        |
| `--mdns`        | mDNS keşfini etkinleştir                       | `false`            |
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `opencorvus.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENCORVUS_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencorvus` değeridir; değiştirmek isterseniz `OPENCORVUS_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `opencorvus serve` hem de `opencorvus web` için geçerlidir.
...
`opencorvus` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
opencorvus ile programatik etkileşim için opencorvus sunucusunu kullanın.
:::

Bu mimari, opencorvus'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`opencorvus serve` ile bağımsız bir sunucu başlatabilirsiniz. opencorvus TUI açıksa, `opencorvus serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum opencorvus [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
Örneğin `http://localhost:7878/doc`. İstemci oluşturmak, istek/yanit tiplerini incelemek veya Swagger gezgininde açmak için bu spec'i kullanın.

---

## API'ler

opencorvus sunucusu aşağıdaki API'leri sunar.
...
| `POST`   | `/permission/:requestID/reply` | Bir izin isteğine yanıt ver                 | gövde: `{ reply, message? }`, `boolean` döndürür                                     |
...
| `POST` | `/tui/control/response` | Bir kontrol isteğine yanıt ver (`{ id, body } or { id, error }`)             | `boolean`              |
