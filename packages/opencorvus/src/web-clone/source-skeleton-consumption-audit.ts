...
export async function auditWebCloneSourceSkeletonConsumption(
  ...
  const requiresSourceManifest = path.basename(sourcePackageDir).toLowerCase() === WEB_CLONE_SOURCE_PACKAGE_DIR ||
  ...
  const projectSources = await Promise.all(sourceFiles.map(async (file) => ({
    file,
  ...
  const manualDomMutationDetected = /\bdocument\.create(?:Element|ElementNS|TextNode|DocumentFragment)\s*\(|\.(?:innerHTML|insertAdjacentHTML)\b|\bDOMParser\s*\(/.test(nonBaselineText)
  const htmlReplayDetected = countHtmlStringStats(nonBaselineText).htmlStringBytes > 40_000 || /dangerouslySetInnerHTML\s*=|v-html\s*=|{@html\s+/.test(nonBaselineText)
  const denseInlineAssetDetected = detectDenseInlineAssets(nonBaselineSources, nonBaselineText.match(/data:[^"')\s]+;base64,/g)?.length ?? 0)
  ...
  if (!sourceEvidence.sourceSkeletonExists) findings.push(`Missing ${sourceLabel}/source-skeleton/index.html source skeleton.`)
  ...
  if (!generatedBaselineDetected && repeatedStructureRequiresDataLoops && dataArrayCount === 0) {
    findings.push("Source IR contains tables/lists/cards/repeated groups, but no project-owned data array was detected.")
  }
  if (!generatedBaselineDetected && repeatedStructureRequiresDataLoops && renderLoopCount === 0) {
    findings.push("Source IR contains repeated structures, but no framework render loop (.map/v-for/each) was detected.")
  }
  if (htmlReplayDetected) findings.push("Project-owned source contains large HTML strings or raw HTML injection.")
  if (manualDomMutationDetected) findings.push("Project-owned source uses manual DOM construction/mutation instead of framework components.")
  if (denseInlineAssetDetected) findings.push("Project-owned source contains dense inline assets or base64 data URIs.")
  if (referenceImageReplayDetected) findings.push("Project-owned source renders the reference screenshot as the webpage instead of rebuilding it from framework components.")
  if (hiddenSemanticContentDetected) findings.push("Project-owned source hides semantic skeleton content instead of rendering it visibly.")
  if (mechanicalSkeletonConversionDetected) findings.push("Project-owned source appears to mechanically convert source-skeleton/index.html into a generated framework component.")
...
function readRepeatedStructureCount(contentModel: unknown): number {
  const stats = contentModel && typeof contentModel === "object" ? (contentModel as Record<string, unknown>).stats : undefined
  if (stats && typeof stats === "object") {
    const row = stats as Record<string, unknown>
    return ["totalTables", "totalLists", "totalCards", "totalRepeatedGroups"]
      .reduce((sum, key) => sum + (typeof row[key] === "number" ? row[key] as number : 0), 0)
  }
  return ["tables", "lists", "cards", "repeatedGroups"]
    .reduce((sum, key) => sum + readArrayPath(contentModel, [key]).length, 0)
}
...
function countRenderLoops(text: string): number {
  return (text.match(/\.map\s*\(/g)?.length ?? 0) +
...
function detectDenseInlineAssets(
...
function detectReferenceImageReplay(
  ...
  const namesReferenceImage = normalizedProject.includes(referenceName) ||
  ...
  const cssScreenshotReplay = sources.some((source) =>
    /\.(?:css|scss|sass|less)$/i.test(source.relative) &&
...
function detectHiddenSemanticContent(sources: Array<{ relative: string; text: string }>): boolean {
  ...
  for (const className of hiddenClassNames) {
    const classPattern = new RegExp(`className\\s*=\\s*["'][^"']*\\b${escapeRegExp(className)}\\b[^"']*["']|class\\s*=\\s*["'][^"']*\\b${escapeRegExp(className)}\\b[^"']*["']`)
    if (classPattern.test(sourceText) && /semantic|source|skeleton|coverage|signals?|content/i.test(className)) {
      return true
    }
  }
  return /aria-hidden\s*=\s*["']true["'][\s\S]{0,2000}(?:sourceSignals|sourceTables|source-skeleton|skeleton)/i.test(sourceText)
}

function detectMechanicalSkeletonConversion(sources: Array<{ relative: string; text: string; bytes: number }>): boolean {
  return sources.some((source) => {
    ...
    if (/generate-from-skeleton|source-skeleton\/index\.html|parse5/.test(source.text) && source.bytes > 2_000) return true
...
function isHiddenStyleBody(body: string): boolean {
  ...
  return /\bwidth\s*:\s*1px\b/.test(normalized) && /\bheight\s*:\s*1px\b/.test(normalized) && /\boverflow\s*:\s*hidden\b/.test(normalized)
}
