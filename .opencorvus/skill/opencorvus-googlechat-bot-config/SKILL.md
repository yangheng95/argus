---
name: opencorvus-googlechat-bot-config
description: Configure and troubleshoot Google Chat bot integration for OpenCorvus. Use when tasks involve GOOGLECHAT_SERVICE_ACCOUNT_JSON setup, adapter wiring checks in packages/bot/src/main.ts, updates to packages/bot/.env, or Google Chat message delivery debugging.
---

# OpenCorvus Google Chat Bot Config

## Overview

Configure Google Chat for OpenCorvus using a repeatable checklist and map platform setup to the adapter implementation in packages/bot.
...
- Set required env keys: GOOGLECHAT_SERVICE_ACCOUNT_JSON.
- Set optional env keys when needed: GOOGLECHAT_WEBHOOK_HOST, GOOGLECHAT_WEBHOOK_PORT, GOOGLECHAT_WEBHOOK_PATH.
...
- Confirm adapter implementation exists at packages/bot/src/adapters/googlechat.ts.
