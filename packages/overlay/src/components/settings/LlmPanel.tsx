// ── LlmPanel ──
// Solid.js component that mirrors the LLM provider configuration logic from app.js:
...
//
// The OAuth/prompt flow (providerAuthInputs, authorizeProvider,
// executeProviderAuth, runProviderAuthMethod) is not executed in-panel;
// instead the component triggers the flow via the API and shows status.
// API key input and provider/model selection are all reactive in Solid state.
...
export default function LlmPanel() {
  ...
  let syncSerial = 0;
  // Track last saved [providerID, modelID, apiKey] triple to skip redundant saves
  let savedKey = "";
  ...
  async function load() {
    ...
    try {
      const [catalogData, authData, configData] = await Promise.all([
        apiJson("provider/catalog").catch(() => ({})),
  ...
  async function syncLlmSettings() {
    ...
    try {
      ...
      const configData = await apiJson("config").catch(() => null);
      if (serial !== syncSerial) return;
      if (recordGuard(configData)) setAppConfig(configData);
  ...
  async function handleAuthConnect() {
    ...
    // If there's exactly one method, use it directly
    let chosenMethod: AuthMethodItem | undefined;
    if (methods.length === 1) {
      chosenMethod = methods[0];
    } else {
      // Show a simple select prompt in-page (not native dialog)
      const idx = parseInt(
        prompt(
    ...
    setBusy(true);
    try {
      await apiJson(`provider/${pid}/auth/start`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ method: chosenMethod.index }),
      });
      showNotice(t("llm.auth_connect_title"), "warn", 0);
      // Reload catalog after auth attempt
      const [catalogData, authData, configData] = await Promise.all([
        apiJson("provider/catalog").catch(() => ({})),
        apiJson("provider/auth").catch(() => ({})),
        apiJson("config").catch(() => ({})),
  ...
  return (
    <div class="config-tab-panel active" data-config-panel="llm">
      <Show when={loading()}>
          ...
          <For each={providers()}>
            {(p) => {
              const stateLabel = createMemo(() => {
                // Derive per-provider label using providerState logic
                const connected = catalog().connected || [];
                const config = appConfig();
                const configKey = config?.provider?.[p.id]?.options?.apiKey;
                const key = configKey || p.key;
                const pAuthMethods = parseProviderAuthMethods(
                  providerAuth(),
                  p.id,
                );
      ...
      {/* ── API Key ── */}
      <label class="field">
        <span class="field-label">{t("llm.api_key_label")}</span>
        <div class="field-row llm-api-key-row">
          <input
            class="field-input"
            id="llmApiKey"
          ...
          <button
            type="button"
            class="btn btn-ghost mini icon-btn"
            id="btnLlmApiKeyToggle"
            disabled={busy()}
