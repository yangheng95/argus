---
title: Serwer
description: Interakcja z serwerem argus za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `argus serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient argus.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `argus.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `argus` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `argus serve`, jak i `argus web`.
...
Kiedy uruchomisz `argus`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera argus do programowej interakcji z argus.
:::

Ta architektura umożliwia obsługę wielu klientów przez argus i programową interakcję z kodem otwartym.

Możesz uruchomić `argus serve`, aby uruchomić samodzielny serwer. Jeśli masz
argus TUI działa, `argus serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki argus [IDE](/docs/ide).
...
Serwer argus udostępnia następujące interfejsy API.
