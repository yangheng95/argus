---
title: CLI
description: Opcje i polecenia wiersza poleceń Argus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Argus domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
argus
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Argus programowo.
...
Uruchamia interfejs terminalowy (TUI) Argus.
...
Interfejs CLI Argus zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Argus.
...
Dołącz terminal do już działającego serwera backendu Argus uruchomionego za pomocą `serve` lub `web`.

```bash
argus attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Argus. Na przykład:
...
Argus integruje się z wieloma dostawcami. Możesz użyć `argus auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/argus/auth.json`.

```bash
argus auth login
```

Podczas uruchamiania Argus ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Argus.
...
Uruchom Argus w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `argus serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera Argus (np. http://localhost:4096)     |
...
Uruchom serwer Argus (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
argus serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Argus bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `argus`).
...
Pokaż statystyki wykorzystania tokenów i sesji Argus.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Argus.
...
Uruchom serwer Argus z interfejsem internetowym.

```bash
argus web
```

Uruchamia to serwer HTTP i udostępnia Argus przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `argus`).
...
Odinstaluj Argus i usuń wszystkie powiązane pliki.
...
Aktualizuj Argus do najnowszej lub określonej wersji.
...
Interfejs CLI Argus przyjmuje następujące flagi globalne dla każdego polecenia.
...
Argus można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `argus`)     |
