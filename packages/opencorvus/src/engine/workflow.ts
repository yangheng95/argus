...
const PIPELINE: MiniWorkflow = {
  ...
  steps: [
    ...
    },
    {
      ...
      agentRole: "build",
      label: "Executor",
      hint: "执行器在隔离 worktree 中完成一个 goal。每个 build 收到架构共识输入；build 完成后 architecture_review 的完整反馈会原文返回给 orchestrator，由 orchestrator LLM 自行决定后续动作（manage_task action=modify_goal / dispatch_agent target=build / dispatch_agent target=architect mode=structural_reentry with named invalid artifact evidence / dispatch_agent target=integrity / manage_task action=fail_task）。",
      scope: "goal",
      skippable: false,
