...
| Hard constraints | No fallback, no compatibility branch, no host-side route gate, no new git worktree for this repair, no broad git reset, preserve unrelated changes, code changes require tests, do not restart OpenCorvus / overlay, push to git-cc remote after verification. |
