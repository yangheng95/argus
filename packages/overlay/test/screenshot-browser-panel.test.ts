...
describe("screenshot browser panel", () => {
  ...
  test("component reuses shared image resource and preview paths without local screenshot storage", () => {
    ...
    expect(component).toContain("SCREENSHOT_BROWSER_ITEM_RENDER_CHUNK_SIZE = 12")
    expect(component).toContain("const [renderedItemCount, setRenderedItemCount]")
    expect(component).toContain("const sourceItems = createMemo")
    expect(component).toContain("sourceItems().slice(0, renderedItemCount())")
    expect(component).toContain("data-item-count={sourceItems().length}")
    expect(component).toContain("data-rendered-count={renderedItemCount()}")
    expect(component).not.toContain("renderScreenshotRows")
