...
describe("frontend-design prompt assembly", () => {
  test("core prompt pins maintainable rawproject refactor algorithm", async () => {
    ...
    expect(prompt).toContain("Bootstrap files such as `src/main.tsx`, global reset/theme CSS, root package/config files, and bundler config are integration files")
    expect(prompt).toContain("not valid first target writes")
  ...
  test("webpage rawproject prompts make frontend_design materialize the source project itself", () => {
    ...
    expect(prompt).toContain("Bootstrap entrypoints such as `src/main.tsx`, global reset/theme CSS, root package/config files, and bundler config are integration files")
    expect(prompt).toContain("not the selected-region vertical slice and not valid first target writes")
    expect(prompt).toContain("do not read unrelated skeleton page wrappers, footer/economy/FAQ/chart/list files, whole manifests, root package/config files, broad source data, or exhaustive asset/style catalogs before the selected component/data/style/App slice exists")
    ...
    expect(prompt).toContain("The first write inside a new default target directory should be one of those selected-region source files")
    expect(prompt).toContain("not `src/main.tsx`, global reset/theme CSS, `package.json`, `tsconfig.json`, `index.html`, or bundler config")
  ...
  test("skeleton project tool returns bounded source evidence entrypoints", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.output).toContain("`src/App.tsx` that imports/renders the selected component")
        expect(result.output).toContain("not `src/main.tsx`, global reset/theme CSS")
        ...
        expect(result.output).toContain("before reading unrelated skeleton page wrappers, footer/economy/FAQ/chart/list files, whole manifests, root package/config files, broad source data, or exhaustive asset/style catalogs")
        ...
        expect(result.output).toContain("not `src/main.tsx`, global reset/theme CSS, `bash mkdir`, `index.html`, `package.json`, `tsconfig.json`, or bundler config")
