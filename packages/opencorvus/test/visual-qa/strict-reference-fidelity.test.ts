...
describe("visual-qa final blocker-based acceptance", () => {
  ...
  test("core prompt requires task-scoped screenshot review and supporting comparison for clone work", () => {
    ...
    expect(normalized).toContain("task-scoped preview screenshots and source/local binding puzzles")
    ...
    expect(normalized).toContain("as investigation evidence")
    expect(normalized).toContain("do not treat a binding puzzle or a status flag as final Reference vs Implementation proof")
  ...
  test("runtime delegation requires one-to-one reference-image acceptance evidence", () => {
    ...
    expect(prompt).toContain("do not treat a source-binding puzzle as final Reference vs Implementation proof")
