...
Create `.opencorvus/commands/test.md`:
...
You can add custom commands through the OpenCorvus config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenCorvus [config](/docs/config):
...
- Per-project: `.opencorvus/commands/`
...
opencorvus includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
