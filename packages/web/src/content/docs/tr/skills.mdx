...
Ajan becerileri, opencorvus'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
opencorvus su konumlari tarar:

- Proje konfigurasyonu: `.opencorvus/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
Proje yerel yollarinda opencorvus, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.opencorvus/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.opencorvus/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
opencorvus, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`opencorvus.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`opencorvus.json` icinde):
