...
export namespace SessionPrompt {
    ...
    if (shouldRouteA2A) {
      ...
      if (TaskQueue.queueSize(input.sessionID) >= maxQueue) {
        return createA2AQueueMessage({
          ...
          text: `A2A queue is full (${maxQueue}). Please wait for running tasks to complete and retry.`,
          metadata: { status: "queue_full", maxQueue },
        })
      }

      const promptText = toA2APrompt(input.parts)
      ...
      return createA2AQueueMessage({
        ...
        parentID: message.info.id,
        text: [
          `Task queued for A2A execution.`,
          `task_id: ${task.id}`,
          queuePos > 0 ? `queue_position: ${queuePos}` : "queue_position: 0",
        ].join("\n"),
        metadata: {
          status: "queued",
          taskID: task.id,
  ...
  function toA2APrompt(parts: PromptInput["parts"]): string {
    const text = parts
      .flatMap((part) => {
  ...
  async function createA2AQueueMessage(input: {
