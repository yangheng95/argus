...
| `specs/current/architecture/07-panel-reactivity.md`                             | Says `session.status` is buffered when the session is not materialized and drained by the first message.      | Restore this documented contract.                                                                   |
| `specs/records/2026-06/2026-06-10-agent-rail-identity-empty-card-fix.md`   | Says lifecycle events remain in events but message-less sessions are not display sessions.                    | Apply the same rule to the live tree-writer path.                                                   |
