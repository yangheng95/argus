...
describe("overlay project directory request loop", () => {
  ...
  test("MCP panel refreshes while active so external backend connection changes surface", () => {
    ...
    expect(source).toContain("function mcpCanConnect(status: string)")
    expect(source).toContain('status === "disconnected" || status === "disabled" || status === "failed"')
    expect(source).toContain("function handleConnectMcp(name: string)")
    expect(source).toContain("await connectMcp(name)")
    expect(source).toContain('t("mcp.connect_action")')
    expect(source).toContain("function handleDisconnectMcp(name: string)")
    expect(source).toContain("await disconnectMcp(name)")
    expect(source).toContain('t("mcp.disconnect_action")')
