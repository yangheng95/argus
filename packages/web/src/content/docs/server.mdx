...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencorvus`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `opencorvus serve` and `opencorvus web`.
