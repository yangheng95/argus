...
test("left activity toolbar owns task, assistant, memory, skill, and MCP controls", () => {
  ...
  expect(main).toContain("<SkillsPanel compact />")
  expect(main).toContain("<McpPanel compact />")
  expect(main).toContain("<MemoryPanel taskID={activeTaskID() || undefined} compact />")
