...
function buildUserPrompt(input: {
  ...
  sections.push(
    ...
    "Configuration manifests and package-manager commands are late-stage integration work, not the first workload. After the skeleton exists, the first target-project editing pass creates source files only: `src/App.tsx`, semantic components, data modules, mock/API adapters, CSS modules/sidecars, owned assets, and then `src/main.tsx`. The first write inside a new default target directory should be one of those source files, not `package.json`, `tsconfig.json`, `index.html`, or bundler config. Do not run install/build/render/dev-server commands until the target project already has `src/App.tsx`, at least one semantic component, at least one data/mock module, and at least one project-owned style module. Do not call `write`, `edit`, or `apply_patch` for `package.json`, `tsconfig.json`, or bundler config during that source-coverage pass. Read and edit root config only immediately before a late integration/build command after source coverage exists. " +
