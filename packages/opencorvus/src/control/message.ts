...
async function systemPrompt(input: z.infer<typeof ControlMessageInput>) {
  const lines = [
    ...
    "When creating a panel task, set create_task.queue=true only when the user explicitly wants the new task to queue; omit queue otherwise because omitted queue means the task may start as soon as its directory is idle.",
