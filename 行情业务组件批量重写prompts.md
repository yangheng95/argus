...
> **Status — historical external prompt artifact (2026-06-17):** 本文件保留为一次外部行情组件迁移任务的 prompt 记录，不是 OpenCorvus 当前架构或产品文档。文中的目标路径属于外部项目示例。任何 API 连接失败都必须明确失败并报告；禁止生成 mock 数据来掩盖真实接入失败。
