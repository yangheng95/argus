...
export function renderHostPreparedFrontendProjectSection(project: HostPreparedFrontendProject): string {
  ...
  const lines = [
    ...
    "Webpage/source evidence stays in task runtime paths. Do not instruct downstream agents to move or clean `web-clone-source/`, `frontend-design-skeleton/`, raw `webpage-evidence/`, legacy `mirror/`, `references/`, or `reference.png` into the acceptance root as app-owned deliverables; extract only the observed source structure, data, styles, and assets needed by the target project.",
