...
There are two types of agents in Argus; primary agents and subagents.
...
Argus comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Argus comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Argus comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `argus.json` config file:
...
If no temperature is specified, Argus uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Argus config and the project specific config.
...
The model ID in your Argus config uses the format `provider/model-id`. For example, if you're using [Argus Zen](/docs/zen), you would use `argus/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `argus models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/yangheng95/argus).
:::
