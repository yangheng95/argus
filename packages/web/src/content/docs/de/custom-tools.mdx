...
description: Erstellen Sie Tools, die der LLM in Argus aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Argus wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.argus/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/argus/tools/` platzieren.
