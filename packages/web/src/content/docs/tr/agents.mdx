...
opencorvus'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
opencorvus, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
opencorvus, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

opencorvus iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `opencorvus.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse opencorvus modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global opencorvus yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
opencorvus hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [OpenCorvus Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencorvus/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `opencorvus models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/yangheng95/opencorvus).
:::
