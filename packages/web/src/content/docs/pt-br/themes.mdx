...
Com o opencorvus, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o opencorvus usa nosso próprio tema `opencorvus`.
...
O opencorvus vem com vários temas embutidos.
...
- Querem que o opencorvus corresponda à aparência do seu terminal
...
O opencorvus suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/opencorvus/themes/*.json` ou `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.opencorvus/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.opencorvus/themes/*.json`
