...
2. **Global task-list stream** (app level)
   ...
   ```
   Code: `src/services/sse.ts:104` `startTaskListSSE()`. This stream owns OS notification routing from stamped `notify` metadata and refreshes the task-list badge projection.
...
| `overlay_settings_load` / `overlay_settings_save` | Persists to `.opencorvus/overlay.jsonc`                                                                        |
...
| `overlay_badge_set`                               | Persistent dock/taskbar badge projection (`set_badge_count` on macOS/Linux, alert dot overlay icon on Windows) |
...
`~/.opencorvus/overlay.jsonc` stores UI preferences (theme, position, zoom, server URL override). The tray **Restart** menu restarts the backend process and reloads the frontend.
