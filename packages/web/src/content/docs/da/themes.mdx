...
Med OpenCorvus kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenCorvus vårt eget `opencorvus`-tema.
...
OpenCorvus støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/opencorvus/themes/*.json` eller `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.opencorvus/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.opencorvus/themes/*.json`
