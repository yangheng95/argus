---
name: opencorvus-matrix-bot-config
description: Configure and troubleshoot Matrix bot integration for OpenCorvus. Use when tasks involve MATRIX_HOMESERVER_URL, MATRIX_ACCESS_TOKEN setup, adapter wiring checks in packages/bot/src/main.ts, updates to packages/bot/.env, or Matrix message delivery debugging.
---

# OpenCorvus Matrix Bot Config

## Overview

Configure Matrix for OpenCorvus using a repeatable checklist and map platform setup to the adapter implementation in packages/bot.
...
- Set required env keys: MATRIX_HOMESERVER_URL, MATRIX_ACCESS_TOKEN.
- Set optional env keys when needed: MATRIX_SINCE_TOKEN.
...
- Confirm adapter implementation exists at packages/bot/src/adapters/matrix.ts.
