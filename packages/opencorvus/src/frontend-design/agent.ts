      ...
      execute: async (params) => {
        ...
        return {
          ...
          output: [
            ...
            "Before writing files for another region, build and render this region through one explicit target URL: start one server only if needed, call webpage_render, feed its screenshot into webpage_evaluate, call webpage_vision_judge for visible mismatches, repair this same region, then record this region's replacement result and call record_frontend_region_selection for the next region.",
          ].filter(Boolean).join("\n"),
