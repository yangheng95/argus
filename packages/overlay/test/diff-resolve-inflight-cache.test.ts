...
let vcsCalls = 0;
let scenario: "acceptance-inflight" | "vcs-backfill" | "vcs-inflight" = "acceptance-inflight";
...
mock.module("../src/services/api", () => ({
  ...
  apiJson: async (path: string) => {
    ...
    if (path === "goal-run/gr_vcs_backfill/acceptance" || path === "goal-run/gr_vcs_inflight/acceptance") {
      return { result: { diffs: [] } };
    }
