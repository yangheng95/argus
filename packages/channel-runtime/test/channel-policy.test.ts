...
import { permissionReply, queueLimit } from "../src/channel-policy"
...
describe("channel policy", () => {
  ...
  test("permissionReply defaults to always and accepts known replies", () => {
    ...
    expect(permissionReply({ OPENCORVUS_CHANNEL_PERMISSION_ASK_REPLY: "always" })).toBe("always")
    expect(permissionReply({ OPENCORVUS_CHANNEL_PERMISSION_ASK_REPLY: "once" })).toBe("once")
    expect(permissionReply({ OPENCORVUS_PERMISSION_ASK_REPLY: "reject" })).toBe("reject")
    expect(
      permissionReply({
        OPENCORVUS_PERMISSION_ASK_REPLY: "reject",
        OPENCORVUS_CHANNEL_PERMISSION_ASK_REPLY: "once",
      }),
    ).toBe("once")
    expect(permissionReply({ OPENCORVUS_CHANNEL_PERMISSION_ASK_REPLY: "bad" })).toBe("always")
  })
})
