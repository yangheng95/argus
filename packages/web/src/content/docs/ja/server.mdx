...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `opencorvus` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`opencorvus serve` と `opencorvus web` の両方に当てはまります。
