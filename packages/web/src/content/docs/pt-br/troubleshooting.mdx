...
Para depurar problemas com o opencorvus, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\opencorvus\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `opencorvus --log-level DEBUG`.

---

## Armazenamento

O opencorvus armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/opencorvus/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\opencorvus`
...
O opencorvus Desktop executa um servidor opencorvus local (o sidecar `opencorvus-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `OpenCorvus` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/opencorvus/opencorvus.jsonc` (ou `~/.config/opencorvus/opencorvus.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencorvus/opencorvus.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencorvus\opencorvus.jsonc`
...
O opencorvus também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/opencorvus/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencorvus\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o opencorvus possa reconstruí-lo.

1. Saia completamente do opencorvus Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/opencorvus`
- **Linux**: exclua `~/.cache/opencorvus` (ou execute `rm -rf ~/.cache/opencorvus`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\opencorvus`

3. Reinicie o opencorvus Desktop.

---

### Corrigindo problemas de conexão com o servidor

O opencorvus Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `opencorvus.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `OPENCORVUS_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `OPENCORVUS_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o opencorvus Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do opencorvus.

---

### Notificações não aparecendo

O opencorvus Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o opencorvus nas configurações do seu sistema operacional, e
...
1. Saia do opencorvus Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo opencorvus Desktop):

- `opencorvus.settings.dat` (URL do servidor padrão do desktop)
- `opencorvus.global.dat` e `opencorvus.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o opencorvus:
...
Para descobrir quais modelos você tem acesso, execute `opencorvus models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\opencorvus`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O opencorvus instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\opencorvus`

2. Reinicie o opencorvus para reinstalar os pacotes de provedores mais recentes

Isso forçará o opencorvus a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O opencorvus detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
