...
describe("build agent goal execution discipline prompt", () => {
  ...
  test("requires observed frontend verification and random preview ports", async () => {
    ...
    expect(normalized).toContain("side-by-side and diff image attachments")
    ...
    expect(normalized).toContain("Viewport width differences are normalized by the comparison runner")
    expect(normalized).toContain("do not treat viewport mismatch as the root cause")
