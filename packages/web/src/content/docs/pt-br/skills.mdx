...
As habilidades do agente permitem que o opencorvus descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O opencorvus pesquisa nesses locais:

- Configuração do projeto: `.opencorvus/skills/<name>/SKILL.md`
- Configuração global: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o opencorvus sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.opencorvus/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.opencorvus/skills/git-release/SKILL.md` assim:
...
O opencorvus lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `opencorvus.json`:
...
**Para agentes embutidos** (em `opencorvus.json`):
