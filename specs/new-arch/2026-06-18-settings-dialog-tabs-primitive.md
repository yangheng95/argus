...
| --- | --- | --- |
| `rg -n -F "config-nav-item" packages/overlay/src packages/overlay/test specs/new-arch` | Production owner is `ConfigDialogHost`; CSS and tests preserve `.config-nav-item.active`. | Retire local nav item classes and render sidebar triggers through `Tab`. |
| `rg -n -F "config-tab-panel" packages/overlay/src packages/overlay/test specs/new-arch` | Production owner is the single active panel in `ConfigDialogHost`; CSS/tests expect `.active`. | Render the content through `TabPanel`; keep `config-tab-panel` as the panel surface class but remove `.active` as the state source. |
| `rg -n -F "switchConfigTab" packages/overlay/src packages/overlay/test specs/new-arch` | Used by `ConfigDialogHost`, `focusConfigSection`, and command-palette guard docs. | Keep the service as the state write source; pass it to `Tabs.onValueChange`. |
| `rg -n -F 'from "./ui/Tabs"' packages/overlay/src packages/overlay/test specs/new-arch` | Runtime callers are `FileChangesPanel` and `ExecutorSelector`; `ConfigDialogHost` is missing. | Add `ConfigDialogHost` as a Tabs caller and update primitive guards. |
| `rg -n -F "<Tabs" packages/overlay/src packages/overlay/test specs/new-arch` | Runtime tabbed surfaces are File Changes and Executor only. | Settings dialog becomes the third real tabbed surface. |
