---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Argus.
...
en `~/.local/share/argus/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Argus
configuration.
...
Argus Zen est une liste de modèles fournis par l'équipe Argus qui ont été
testé et vérifié pour fonctionner correctement avec Argus. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Argus Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez argus et dirigez-vous vers [argus.ai/auth](https://argus.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Argus et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec Argus :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de argus :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY argus

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile argus

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX argus
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `argus.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Argus n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que argus fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX argus
   ```
...
Vous pouvez également ajouter des modèles via votre configuration argus.
...
pour stocker le jeton dans le stockage d'authentification argus.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Argus utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Argus
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `argus.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth).

##### Configuration

Personnalisez via `argus.json` :
...
{
  "$schema": "https://argus.ai/config.json",
  "plugin": ["@gitlab/argus-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec argus :
...
Pour utiliser Google Vertex AI avec Argus :
...
Définissez-les lors de l'exécution de argus.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id argus
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via argus, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/argus/argus.jsonc"
{
  "$schema": "https://argus.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) pour enregistrer automatiquement chaque conversation Argus en tant que session dans Helicone.

```bash
npm install -g argus-helicone-session
```
...
```json title="argus.json"
{
  "plugin": ["argus-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Argus répertoriée comme une session distincte.
...
Vous pouvez configurer argus pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="argus.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://argus.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer argus pour utiliser des modèles locaux via LM Studio.

```json title="argus.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://argus.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer argus pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Argus. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/argus) pour plus de détails.
:::

```json title="argus.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://argus.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Argus :
...
3. Copiez la clé API à utiliser dans Argus.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Argus, vous devez extraire les informations du modèle localement :
...
Argus Zen est une liste de modèles testés et vérifiés fournie par l'équipe Argus. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Argus Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Argus Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Argus API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration argus.
...
5. Vous pouvez également les personnaliser via votre configuration argus. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' argus
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group argus
   ```
...
Vous pouvez également personnaliser les modèles via votre configuration argus. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec argus. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in argus.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `argus.json` dans le répertoire de votre projet :

   ```json title="argus.json" ""myprovider"" {5-15}
   {
     "$schema": "https://argus.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="argus.json" {9,11,17-20}
{
  "$schema": "https://argus.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Argus de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `argus auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration argus et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration argus.
