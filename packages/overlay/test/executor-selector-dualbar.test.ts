...
describe("ExecutorSelector dual chip bar", () => {
  ...
  test("Hexin budget row is requested only for the current OpenCorvus Hexin model", () => {
    ...
    expect(SRC).toMatch(/function createHexinBudgetState/)
    expect(SRC).toMatch(/const baseKey = createMemo/)
    expect(SRC).toMatch(/const key = createMemo/)
    ...
    expect(SRC).toMatch(/<Show when=\{hexinBudget\.key\(\)\}>/)
    expect(SRC).toMatch(/meta=\{[\s\S]*?<Show when=\{hexinBudget\.key\(\)\}>/)
    expect(SRC).toMatch(
      /<span class="composer-model-selector-copy">[\s\S]*?<Show when=\{hexinBudget\.key\(\)\}>[\s\S]*?<HexinBudgetInline/,
    )
  ...
  test("Hexin budget CSS belongs to the composer selector surface", () => {
    ...
    expect(CSS).toMatch(/\.composer-model-selector \.executor-budget-inline\s*\{[\s\S]*?border-inline-start:/)
    expect(CSS).toMatch(/\.composer-model-selector-copy\s*\{[\s\S]*?display:\s*flex/)
