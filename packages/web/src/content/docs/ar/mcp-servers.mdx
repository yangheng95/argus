...
يمكنك إضافة أدوات خارجية إلى Argus باستخدام _Model Context Protocol_، أو MCP. يدعم Argus خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Argus](https://argus.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/argus` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Argus تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Argus بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Argus المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `argus mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Argus الرموز بشكل آمن في `~/.local/share/argus/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Argus إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Argus مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Argus بحساب Sentry الخاص بك.
