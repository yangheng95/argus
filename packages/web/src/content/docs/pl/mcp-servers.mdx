...
Możesz dodać dodatkowe informacje do opencorvus za pomocą _Model Context Protocol_, w skrócie MCP. opencorvus obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [opencorvus Config](https://opencorvus.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/opencorvus`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
opencorvus automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, opencorvus:
...
Jeśli serwer wymaga uwierzytelnienia, opencorvus poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `opencorvus mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji opencorvus bezpieczne przechowa tokeny w `~/.local/share/opencorvus/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w opencorvus, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie opencorvus, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć opencorvus z kontem Sentry.
