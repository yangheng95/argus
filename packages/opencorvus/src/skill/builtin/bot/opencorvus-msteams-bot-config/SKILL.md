---
name: opencorvus-msteams-bot-config
description: Configure and troubleshoot Microsoft Teams bot integration for OpenCorvus. Use when tasks involve MSTEAMS_APP_ID, MSTEAMS_APP_SECRET setup, adapter wiring checks in packages/bot/src/main.ts, updates to packages/bot/.env, or Microsoft Teams message delivery debugging.
---

# OpenCorvus Microsoft Teams Bot Config

## Overview

Configure Microsoft Teams for OpenCorvus using a repeatable checklist and map platform setup to the adapter implementation in packages/bot.
...
- Set required env keys: MSTEAMS_APP_ID, MSTEAMS_APP_SECRET.
- Set optional env keys when needed: MSTEAMS_WEBHOOK_HOST, MSTEAMS_WEBHOOK_PORT, MSTEAMS_WEBHOOK_PATH.
...
- Confirm adapter implementation exists at packages/bot/src/adapters/msteams.ts.
