---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Argus.
---

Dodaci vam omogućavaju da proširite Argus spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Argus.
...
- `.argus/plugins/` - Dodaci na nivou projekta
- `~/.config/argus/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/argus/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/argus/argus.json`)
2. Konfiguracija projekta (`argus.json`)
3. Globalni direktorij dodataka (`~/.config/argus/plugins/`)
4. Direktorij dodataka projekta (`.argus/plugins/`)
...
Argus pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Argus.
...
:::note
Ako koristite desktop aplikaciju Argus, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite argus da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u argus:
...
Pomoćnik `tool` kreira prilagođeni alat koji argus može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za argus zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://argus.ai/docs/sdk) za detalje.
