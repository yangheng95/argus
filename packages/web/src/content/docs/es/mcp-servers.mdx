...
Puede agregar herramientas externas a Argus utilizando el _Model Context Protocol_ o MCP. Argus admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Argus Config](https://argus.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/argus`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Argus maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Argus:
...
Si el servidor requiere autenticación, Argus le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `argus mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Argus almacenará los tokens de forma segura en `~/.local/share/argus/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Argus, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Argus como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Argus a su cuenta Sentry.
