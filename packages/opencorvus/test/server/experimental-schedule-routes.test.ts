...
describe("experimental schedule routes", () => {
  ...
  test(
    "cron schedule routes bind project and session ownership to the active project",
    async () => {
      ...
      const ownRow = Database.use((db) =>
        db.select().from(CronJobTable).where(eq(CronJobTable.id, validBody.id)).get(),
      )
      ...
      expect(Database.use((db) => db.select().from(CronJobTable).where(eq(CronJobTable.id, validBody.id)).get())).toBeUndefined()
    },
    30_000,
  )

  test(
    "event schedule routes bind project and session ownership to the active project",
    async () => {
      await using projectA = await tmpdir({ git: true })
      await using projectB = await tmpdir({ git: true })
      const suffix = Math.random().toString(36).slice(2)
      ...
      const validBody = (await validCreate.json()) as { id: string }
      const ownRow = Database.use((db) =>
        db.select().from(EventJobTable).where(eq(EventJobTable.id, validBody.id)).get(),
      )
      expect(ownRow?.project_id).toBe(a.projectID)
      expect(ownRow?.session_id).toBe(a.sessionID)
      ...
      expect(ownDelete.status).toBe(200)
      expect(Database.use((db) => db.select().from(EventJobTable).where(eq(EventJobTable.id, validBody.id)).get())).toBeUndefined()
    },
    30_000,
  )
})
