...
argus używa `permission`, aby uniknąć, czy dana akcja została wykonana automatycznie, zachowana monit lub została zablokowana.
...
Użycie `external_directory`, aby zezwolić na wywołanie narzędzia, które obsługuje obsługę poza katalogiem roboczym, z uruchomieniem argus. Dotyczy każdego narzędzia, które jako dane wejściowe zostało przyjęte (na przykład `read`, `edit`, `list`, `glob`, `grep` i wiele założycieli `bash`).
...
Uprawnienia argus są określane na podstawie nazwy narzędzia i kilku zabezpieczeń:
...
Jeżeli nic nie jest określone, argus zostanie uruchomione od dopuszczalnych rozwiązań diagnostycznych:
...
Gdy argus poprosi o zatwierdzenie, interfejs użytkownika oferujący trzy wyniki:

- `once` — zatwierdź tylko tę prośbę
- `always` — zastosowanie, które następuje po zastosowaniu sugerowanych wzorców (przez następczą bieżącą sesję argus)
- `reject` — odrzucić
