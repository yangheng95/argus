...
test("composer textarea resize bounds scale with UI density", () => {
  expect(composerTextareaResizeBounds(2)).toEqual({ min: 144, max: 520, step: 32 })
})

test("composer textarea keyboard resize clamps to shared bounds", () => {
  const bounds = composerTextareaResizeBounds(1)
  expect(clampComposerTextareaHeight(12, bounds)).toBe(72)
  ...
  expect(nextComposerTextareaKeyboardHeight(100, "Home", bounds)).toBe(72)
