...
function buildVisualQaUserPrompt(input: VisualQaAgent.AnalyzeInput): string {
  ...
  sections.push(
    "# Required Output\n\n" +
      "Call `submit_visual_qa_report` exactly once after registering check items and every report row. The final submit call only sets accepted and summary; do not put findings, blockers, evidence, DOM regions, repairs, coverage, or reference parity arrays in the final payload. A passing report must include registered check_items, fresh screenshot-bearing evidence paths or URLs tied to check_ids, coverage of checked GUI regions/viewports/states/functions tied to check_ids, no failed/inconclusive check_items, no open critical/major finding, no production_blockers, no unresolved_code_module_problems, and evidence that coarse component/function defects named by build evidence or integrity are actually repaired before style polish. If the report covers more than one viewport, it must include a `multi-viewport-alignment` check item with coverage spanning those viewports. If explicit reference parity is in scope, cite `reference-comparison` evidence when available, use per-screen screenshots or scroll-slice comparisons for page-level analysis, register a check item for each required reference region, and name any remaining evidence gap instead of inventing proof or reference-comparison refs. If the surface is not production-ready, set accepted=false and list production_blockers with principle_ids, region, reason, user-visible impact, evidence refs, required correction, and check_ids. For each DOM-localizable visual blocker, add `problem_dom_regions` tied to the blocker ID and check_ids with locator, DOM path, bounded outer_html_excerpt, parent/sibling context, bbox, computed_style, attributes, code_search_terms, and evidence refs. If a production blocker cannot be safely fixed by visual-qa inside the current worktree, report unresolved_code_module_problems tied to blocker IDs and check_ids with a concrete code module reference entity and observed problem; do not submit a new-task request. If you repair files, include changed_files and verification evidence.",
  )
