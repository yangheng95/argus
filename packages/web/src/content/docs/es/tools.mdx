...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCorvus o cuando la variable de entorno `OPENCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCorvus:

```bash
OPENCODE_ENABLE_EXA=1 opencorvus
```
