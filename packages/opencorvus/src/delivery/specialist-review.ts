...
export const DeliveryReviewFindingEvidence = z.object({
  kind: z.enum([
    "file",
    "command",
    "screenshot",
...
export const DeliveryReviewFinding = z.object({
...
export const DeliverySpecialistReview = z.object({
  id: z.string().min(1),
...
export function createDeliverySpecialistReview(input: Omit<DeliverySpecialistReview, "id" | "timeCreated">): DeliverySpecialistReview {
...
export function persistDeliverySpecialistReview(input: {
  review: DeliverySpecialistReview
}) {
...
export function findDeliverySpecialistReviews(input:
  | { deliveryID: string }
  | { taskID: string }
...
export function findDeliverySpecialistReviewsByDelivery(input: {
  deliveryID: string
}): DeliverySpecialistReview[] {
...
export function findDeliverySpecialistReviewsByTask(input: {
  taskID: string
}): DeliverySpecialistReview[] {
  const rows = Database.use((db) =>
    db.select().from(EngineArtifactTable)
