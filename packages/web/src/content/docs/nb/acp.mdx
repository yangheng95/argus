---
title: ACP-støtte
description: Bruk argus i en hvilken som helst ACP-kompatibel editor.
---

Argus støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke Argus via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `argus acp`.

Kommandoen starter Argus som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'Argus'-agenten i AI Chat-agentvelgeren.
...
For å bruke Argus som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke Argus som ACP-agent for chat.
...
Argus fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i Argus-konfigurasjonen
