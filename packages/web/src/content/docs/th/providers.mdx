---
title: ผู้ให้บริการ
description: ใช้ผู้ให้บริการ LLM ใน OpenCorvus
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus ใช้ [AI SDK](https://ai-sdk.dev/) และ [Models.dev](https://models.dev) เพื่อรองรับผู้ให้บริการ **75+ LLM** และรองรับการใช้งานโมเดลท้องถิ่น

หากต้องการเพิ่มผู้ให้บริการ คุณต้อง:

1. เพิ่มคีย์ API สำหรับผู้ให้บริการโดยใช้คำสั่ง `/connect`
2. กำหนดค่าผู้ให้บริการในการกำหนดค่า OpenCorvus ของคุณ
...
ใน `~/.local/share/opencorvus/auth.json`.

---

### การกำหนดค่า

คุณสามารถปรับแต่งผู้ให้บริการได้ผ่านทางส่วน `provider` ใน OpenCorvus ของคุณ
กำหนดค่า
...
OpenCorvus Zen คือรายชื่อโมเดลที่จัดทำโดยทีมงาน OpenCorvus ที่ได้รับ
ทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ OpenCorvus [เรียนรู้เพิ่มเติม](/docs/zen)

:::tip
หากคุณยังใหม่ เราขอแนะนำให้เริ่มต้นด้วย OpenCorvus Zen
:::

1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก opencorvus และไปที่ [opencorvus.ai/auth](https://opencorvus.ai/auth)
...
มันทำงานเหมือนกับผู้ให้บริการรายอื่นใน OpenCorvus และเป็นทางเลือกในการใช้งานโดยสมบูรณ์
...
วิธีใช้ Amazon Bedrock กับ OpenCorvus:
...
2. **กำหนดค่าการตรวจสอบสิทธิ์** โดยใช้วิธีใดวิธีหนึ่งต่อไปนี้:
   ...
   ตั้งค่าหนึ่งในตัวแปรสภาพแวดล้อมเหล่านี้ขณะเรียกใช้ opencorvus:
   ...
   สำหรับการกำหนดค่าเฉพาะโครงการหรือแบบถาวร ให้ใช้ `opencorvus.json`:
...
:::info
การใช้การสมัครสมาชิก Claude Pro/Max ใน OpenCorvus ไม่ได้รับการสนับสนุนอย่างเป็นทางการจาก [Anthropic](https://anthropic.com)
:::
...
2. ไปที่ [Azure AI Foundry](https://ai.azure.com/) และปรับใช้โมเดล
   ...
   :::note
   ชื่อการปรับใช้จะต้องตรงกับชื่อรุ่นเพื่อให้ opencorvus ทำงานได้อย่างถูกต้อง
   :::
...
5. รันคำสั่ง `/models` เพื่อเลือกรุ่น
   ...
   คุณยังสามารถเพิ่มโมเดลผ่านการกำหนดค่า opencorvus ของคุณได้
...
เพื่อจัดเก็บโทเค็นในที่เก็บข้อมูลการรับรองความถูกต้องของ opencorvus
:::

##### GitLab ที่โฮสต์เอง

:::note[บันทึกการปฏิบัติตาม]
OpenCorvus ใช้โมเดลขนาดเล็กสำหรับงาน AI บางอย่าง เช่น การสร้างชื่อเซสชัน
มีการกำหนดค่าให้ใช้ gpt-5-nano ตามค่าเริ่มต้น ซึ่งโฮสต์โดย Zen เพื่อล็อค OpenCorvus
หากต้องการใช้เฉพาะอินสแตนซ์ที่โฮสต์บน GitLab ของคุณเอง ให้เพิ่มสิ่งต่อไปนี้ในของคุณ
`opencorvus.json` ไฟล์. ขอแนะนำให้ปิดใช้งานการแชร์เซสชันด้วย
...
เอกสารประกอบเพิ่มเติมเกี่ยวกับหน้าแรกของ [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth)

##### การกำหนดค่า

ปรับแต่งผ่าน `opencorvus.json`:
...
วิธีใช้การสมัครสมาชิก GitHub Copilot ด้วย opencorvus:
...
วิธีใช้ Google Vertex AI กับ OpenCorvus:
...
2. ตั้งค่าตัวแปรสภาพแวดล้อมที่ต้องการ:
   ...
   ตั้งค่าในขณะที่เรียกใช้ opencorvus
...
ในกรณีที่คุณเห็นคุณสมบัติหรือรุ่นจาก Helicone ที่ไม่ได้กำหนดค่าโดยอัตโนมัติผ่าน opencorvus คุณสามารถกำหนดค่าด้วยตนเองได้ตลอดเวลา
...
คุณลักษณะ [เซสชัน](https://docs.helicone.ai/features/sessions) ของ Helicone ช่วยให้คุณสามารถจัดกลุ่มคำขอ LLM ที่เกี่ยวข้องเข้าด้วยกันได้ ใช้ปลั๊กอิน [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) เพื่อบันทึกการสนทนา OpenCorvus แต่ละรายการเป็นเซสชันใน Helicone โดยอัตโนมัติ
...
ปลั๊กอินจะแทรกส่วนหัว `Helicone-Session-Id` และ `Helicone-Session-Name` ลงในคำขอของคุณ ในหน้าเซสชันของ Helicone คุณจะเห็นการสนทนา OpenCorvus แต่ละรายการแยกเป็นเซสชันแยกกัน
...
คุณสามารถกำหนดค่า opencorvus เพื่อใช้โมเดลท้องถิ่นผ่านยูทิลิตี้ [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server
...
คุณสามารถกำหนดค่า opencorvus เพื่อใช้โมเดลท้องถิ่นผ่าน LM Studio
...
คุณสามารถกำหนดค่า opencorvus เพื่อใช้โมเดลท้องถิ่นผ่าน Ollama

:::tip
Ollama สามารถกำหนดค่าตัวเองสำหรับ OpenCorvus ได้โดยอัตโนมัติ ดู [เอกสารการบูรณาการ Ollama](https://docs.ollama.com/integrations/opencorvus) เพื่อดูรายละเอียด
:::
...
วิธีใช้ Ollama Cloud กับ OpenCorvus:
...
3. คัดลอกคีย์ API เพื่อใช้ใน OpenCorvus
...
6. **สำคัญ**: ก่อนที่จะใช้โมเดลคลาวด์ใน OpenCorvus คุณต้องดึงข้อมูลโมเดลในเครื่อง:
...
OpenCorvus Zen คือรายการโมเดลที่ได้รับการทดสอบและตรวจสอบแล้วโดยทีมงาน OpenCorvus [เรียนรู้เพิ่มเติม](/docs/zen)

1. ลงชื่อเข้าใช้ **<a href={console}>OpenCorvus Zen</a>** และคลิก **สร้าง API คีย์**

2. เรียกใช้คำสั่ง `/connect` และค้นหา **OpenCorvus Zen**

   ```txt
   /connect
   ```

3. ป้อนคีย์ OpenCorvus API ของคุณ
...
4. OpenRouter หลายรุ่นถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น ให้รันคำสั่ง `/models` เพื่อเลือกรุ่นที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า opencorvus ของคุณ
...
5. คุณยังสามารถปรับแต่งได้ผ่านการกำหนดค่า opencorvus ของคุณ นี่คือตัวอย่างการระบุผู้ให้บริการ
...
วิธีใช้ [Scaleway Generative API](https://www.scaleway.com/en/docs/generative-apis/) กับ OpenCorvus:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า opencorvus ของคุณได้อีกด้วย นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางของผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI ด้วย opencorvus ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `opencorvus.json` ในไดเรกทอรีโครงการของคุณ:
...
ช่อง `limit` ช่วยให้ OpenCorvus เข้าใจบริบทที่คุณเหลืออยู่ ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: เรียกใช้ `opencorvus auth list` เพื่อดูว่ามีข้อมูลรับรองหรือไม่
...
2. สำหรับผู้ให้บริการแบบกำหนดเอง ให้ตรวจสอบการกำหนดค่า opencorvus และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า opencorvus ของคุณ
