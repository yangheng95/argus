---
title: SDK
description: Typowany klient JS dla serwera OpenCorvus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Pakiet SDK JS/TS OpenCorvus zapewnia klienta z bezpiecznym typowaniem (type-safe) do interakcji z serwerem.
Użyj go do budowania integracji i programowej kontroli OpenCorvus.
...
Można przekazać obiekt konfiguracyjny, aby dostosować zachowanie. Instancja nadal pobiera `opencorvus.json`, ale możesz zastąpić lub dodać konfigurację bezpośrednio:
...
Jeśli masz już działającą instancję OpenCorvus, możesz utworzyć instancję klienta, aby się z nią połączyć:
...
| Opcja           | Typ        | Opis                                | Domyślne                |
| --------------- | ---------- | ----------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | Adres URL serwera                   | `http://localhost:7878` |
