...
Dzięki argus możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie argus używa naszego własnego motywu `argus`.
...
argus ma kilka wbudowanych motywów.
...
- Want argus to match their terminal's appearance
...
argus obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/argus/themes/*.json` lub `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.argus/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.argus/themes/*.json`
