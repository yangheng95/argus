...
test("syncRun recovers a failed goal run even after evaluation exists and session is cleared", async () => {
  await using tmp = await tmpdir({ git: true })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      Database.transaction((db) => {
        ...
        db.insert(OrchestratorGoalRunTable)
          .values({
            ...
            status: "failed",
            executor: "opencode",
            error: "Implementation is incomplete",
            metadata: {},
            time_completed: now,
            time_created: now,
            time_updated: now,
      ...
      expect(run?.status).toBe("failed")
      expect(run?.error).toBe("Implementation is incomplete")
      expect(task?.status).toBe("failed")
      expect(task?.error).toBe("Implementation is incomplete")
    },
  })
})
