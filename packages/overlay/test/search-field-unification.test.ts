...
const FILE_CHANGES_SOURCE = readFileSync(join(OVERLAY_ROOT, "src/components/FileChangesView.tsx"), "utf8")
...
const CHANGES_CSS = readFileSync(join(OVERLAY_ROOT, "src/styles/surfaces/changes.css"), "utf8")
...
test("task, file, file changes, memory, and provider search inputs use the shared search-field primitive", () => {
  ...
  expect(FILE_CHANGES_SOURCE).toContain('class="changes-filter-icon search-field-icon"')
  expect(FILE_CHANGES_SOURCE).toContain('class="changes-filter-input search-field-input"')
  expect(FILE_CHANGES_SOURCE).toContain('data-ui="file-changes-search-clear"')
  ...
  expect(CHANGES_CSS).not.toMatch(/\.changes-filter-field(?!\.search-field)\b/)
  expect(CHANGES_CSS).not.toMatch(/\.changes-filter-field:focus-within\b/)
  expect(CHANGES_CSS).not.toMatch(/\.changes-filter-input\.field-input\b/)
  expect(CHANGES_CSS).not.toMatch(/file-changes-filter-clear/)
