...
2. `done_definition` commands not executable
...
| Check | Detail |
|---|---|
| Is `experimental.auto_permission` enabled | Default `true`. Required for unattended CI / benchmarks (auto-replies `permission.asked` with `"once"` for active orchestrator runs — `src/engine/auto-permission.ts`). |
| `OPENCORVUS_PERMISSION_TIMEOUT_MS` | Auto-approve timeout, default `120000` ms (`src/permission/next.ts:163`). Only fires when `auto_permission` is on. Minimum honored value is `1000` ms. |
