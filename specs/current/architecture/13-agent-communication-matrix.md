...
- **`integrity` 是 review report tool，不是 lifecycle authority**：对应 Integrity reviewer team（动态 reviewer 计划、replay-aware context、severity discipline、build feedback），输出 pass / non-pass 报告供 Orchestrator 决策；最终完成 / 失败只能由 Orchestrator 的 `manage_task action=complete_task` / `manage_task action=fail_task` 写入。`prosecute` / `prosecutor` 已删除。
- `build -> general/explore`、`general -> explore` 是当前真实存在的 direct 子代理路径；acceptance direct 子代理路径已删除；`general -> general` 自递归被权限拒绝。
- `orchestrator -> EngineService.createTask` 只通过 `manage_task action=propose_task` 间接发生：默认按 `experimental.auto_confirm_proposed_tasks=true` 自动创建"完善上一个 request 的新任务"候选，只有该配置显式为 `false` 时才先询问用户；这不是 `panel` control-plane action，也不是 generic `task` subagent dispatch。
