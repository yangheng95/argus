---
title: Complementos
description: Escriba sus propios complementos para extender OpenCorvus.
---

Los complementos le permiten extender OpenCorvus al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenCorvus.
...
- `.opencorvus/plugins/` - Complementos a nivel de proyecto
- `~/.config/opencorvus/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/opencorvus/node_modules/`.
...
1. Configuración global (`~/.config/opencorvus/opencorvus.json`)
2. Configuración del proyecto (`opencorvus.json`)
3. Directorio global de complementos (`~/.config/opencorvus/plugins/`)
4. Directorio de complementos del proyecto (`.opencorvus/plugins/`)
...
OpenCorvus ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK opencorvus para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar opencorvus.
...
:::note
Si está utilizando la aplicación de escritorio OpenCorvus, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que opencorvus lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a opencorvus:
...
El ayudante `tool` crea una herramienta personalizada a la que opencorvus puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para opencorvus junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://opencorvus.ai/docs/sdk) para obtener más detalles.
