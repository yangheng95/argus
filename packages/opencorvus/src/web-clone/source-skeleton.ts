...
import { inspectWebCloneSourceManifest, readPassedAudit, readPngEvidence } from "./evidence-integrity"

export const WebCloneSourceSkeletonManifestSchema = z.object({
  version: z.literal(1),
  ...
  policy: z.object({
    intendedConsumer: z.literal("LLM build agent"),
...
export type WebCloneSourceSkeletonManifest = z.infer<typeof WebCloneSourceSkeletonManifestSchema>
export type WebCloneSourceSkeletonAudit = z.infer<typeof WebCloneSourceSkeletonAuditSchema>
...
export async function writeWebCloneSourceSkeleton(
  ...
  const manifest = WebCloneSourceSkeletonManifestSchema.parse({
    ...
    policy: {
      intendedConsumer: "LLM build agent",
...
export async function inspectWebCloneSourceSkeletonEvidence(input: {
  ...
  const auditPath = path.join(skeletonDir, "source-skeleton-audit.json")
...
function isHiddenSourceNode(node: WebCloneNode): boolean {
...
function renderAttribute(attr: WebCloneAttribute, assetGraph: WebCloneAssetGraph): [string, string] | undefined {
...
async function renderSkeletonCssBundle(
  ...
  const fullChunks = [
    ...
    "   Keep this as evidence. Build agents should start from critical.css and source-ir/*.json, then inspect this only for missing details. */",
    "",
  ]
  ...
  for (const asset of cssAssets) {
    ...
    const content = await readOptional(assetPath)
    fullChunks.push(`/* CSS asset ${asset.id}: ${asset.path}, ${asset.bytes} bytes */`)
    fullChunks.push(content || `/* missing css asset body: ${asset.path} */`)
    fullChunks.push("")
...
function collectComputedStyleRules(node: WebCloneNode, rules: string[]): void {
...
function renderSkeletonReadme(pageIr: WebClonePageIr, manifest: WebCloneSourceSkeletonManifest): string {
  return [
    "# Source Skeleton Handoff",
    "",
  ...
  walk(root, (node, parent) => {
    ...
    if (looksLikeCard(node, parent)) {
      cards.push({
        ...
        textPreview: text,
        fields: fieldTexts(node).slice(0, 12),
      })
    }
  })
...
function buildStyleTokens(root: WebCloneNode, usedSelectors: CssSelectorReachability) {
...
function buildInteractionHints(root: WebCloneNode) {
  ...
  walk(root, (node) => {
    ...
    const className = attr(node.attrs, "class") ?? ""
    ...
    const kind = inferInteractionKind({ tag, role, className, type, text, ariaExpanded, href })
    if (!kind) return
    hints.push({
      nodeId: node.id,
      kind,
  ...
  const visibleHints = hints.slice(0, 240)
  return {
    version: 1,
...
function withContentModelLimits(model: {
  version: 1
  purpose: "web-clone-content-model"
  tables: unknown[]
  lists: unknown[]
  ...
  return {
    ...model,
    tables: model.tables.slice(0, limit.tables),
    lists: model.lists.slice(0, limit.lists),
    cards: model.cards.slice(0, limit.cards),
    controls: model.controls.slice(0, limit.controls),
    ...
    stats: {
      ...
      omittedControls: Math.max(0, model.controls.length - limit.controls),
...
function buildSelectorNodeIndex(root: WebCloneNode) {
...
function extractTableModel(node: WebCloneNode) {
...
function extractListModel(node: WebCloneNode) {
  const items = (node.children ?? [])
    .filter((child) => child.type === "element" && normalizeTag(child.tag) === "li")
    .map((child) => compactText(visibleText(child), 160))
    .filter(Boolean)
  return { nodeId: node.id, ordered: normalizeTag(node.tag) === "ol", items: items.slice(0, 24) }
}

function controlModel(node: WebCloneNode, tag: string, text: string) {
  return {
    nodeId: node.id,
    tag,
...
function mediaModel(node: WebCloneNode, tag: string, assetGraph: WebCloneAssetGraph) {
...
function collectRepeatedGroups(node: WebCloneNode, repeatedGroups: unknown[]): void {
  const elementChildren = (node.children ?? []).filter((child) => child.type === "element")
  const groups = new Map<string, WebCloneNode[]>()
  ...
  for (const [signature, group] of groups) {
    ...
    repeatedGroups.push({
      ...
      sampleTexts: group.slice(0, 8).map((item) => compactText(visibleText(item), 120)),
...
function structuralSignature(node: WebCloneNode): string {
...
function looksLikeCard(node: WebCloneNode, parent: WebCloneNode | undefined): boolean {
...
function inferInteractionKind(input: {
  ...
  const haystack = `${input.role ?? ""} ${input.className} ${input.text}`.toLowerCase()
  if (input.tag === "a" || input.href) return haystack.includes("tab") ? "tab-link" : "link"
  if (input.tag === "button" || input.role === "button") {
    if (haystack.includes("tab")) return "tab"
    if (input.ariaExpanded !== undefined || haystack.includes("accordion")) return "accordion-toggle"
    if (haystack.includes("dropdown") || haystack.includes("menu")) return "menu-trigger"
    return "button"
  }
  if (input.tag === "input" && (input.type === "search" || haystack.includes("search"))) return "search-input"
...
function flattenElements(root: WebCloneNode): WebCloneNode[] {
  const elements: WebCloneNode[] = []
  walk(root, (node) => {
...
function classTokens(attrs: WebCloneAttribute[] | undefined): string[] {
...
function countRenderedElements(node: WebCloneNode): number {
  let count = 0
...
function countComputedStyleRules(node: WebCloneNode): number {
