...
| **Planner** | 为单个 Goal 产出 PlanSteps | `src/planner/per-goal.ts:45` |
...
| **Evaluator** | 运行确定性检查 + LLM 判决 verdict | `src/evaluator/per-goal.ts` |
...
连接两层的是 MirrorCode executor（`OpencodeExecutor.submit`，`src/executor/opencode.ts:48`）：把 Orchestrator 的 prompt 塞进 TaskQueue，触发 `SessionLoop`。
...
每个 Goal 独立 worktree（`src/orchestrator/goal-pool.ts`），互不干扰。最终合并由 Delivery agent 负责。
