...
:::caution
Modi werden jetzt über die Option `agent` in der Argus-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in Argus können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die Argus-Konfiguration.
...
Argus verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.argus/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `argus.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Argus modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale Argus-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.argus/modes/` für projektspezifische Modi oder `~/.config/argus/modes/` für globale Modi:
