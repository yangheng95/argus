...
export type DeliverySurfaceManifest = {
  id: string
  taskId?: string
  deliveryId?: string
  projectRoot: string
...
export async function detectDeliverySurfaces(input: {
  taskID?: string
  deliveryID?: string
  projectRoot?: string
  changedFiles?: string[]
  ...
  const evidence: DeliverySurfaceEvidence[] = []
  const surfaces = new Set<DeliverySurface>()
...
function addEvidence(
  ...
  surface: DeliverySurface,
  reason: string,
  ...refs: DeliverySurfaceEvidence["refs"]
) {
...
function fileRefs(files: string[]): DeliverySurfaceEvidence["refs"] {
  return files.slice(0, 12).map((file) => ({ kind: "file", ref: file }))
}

function routeRefs(files: string[]): DeliverySurfaceEvidence["refs"] {
