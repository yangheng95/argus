...
| ------------------ | -------- | --------------------------- | --------------------------------------------------------- |
| `model`            | ✓        | —                           | `provider/model`, e.g. `alibaba-cn/qwen3.5-plus`          |
...
Skips OIDC and the permission check.

## `GITHUB_TOKEN` permissions

Default App/OIDC installs keep `GITHUB_TOKEN` read-only. Write access comes from
the App token after OIDC exchange:
...
When `use_github_token: true`, the workflow uses `GITHUB_TOKEN` directly and
must grant write permissions explicitly:
...
When a user posts `/oc`, collaboration permission must be `admin` or `write`; otherwise rejected. Skipped under Option B.

## Workflow examples

### Default (App + DashScope)
...
`MOCK_TOKEN` needs a PAT with `admin` or `write`.
