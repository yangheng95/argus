...
describe("Expert Squad evolution launcher", () => {
  ...
  test("accepts one complete development-only 18-slot launch envelope", () => {
    ...
    const parsed = EvolutionBenchmarkLaunchSchema.parse({
      ...
      inherited_environment_names: ["PATH", "SYSTEMROOT", "HEXIN_API_KEY"],
      credential_source_identity: "environment:HEXIN_API_KEY",
  ...
  test("executes start and resume through the exact Mission-only runtime surface", async () => {
    ...
    try {
      ...
      const benchmarkCatalogPath = path.join(repositoryRoot, "specs/artifacts/expert-squad-evolution-benchmark/manifest.json")
      const developmentManifestPath = path.join(
        repositoryRoot,
        "specs/artifacts/expert-squad-evolution-benchmark/development-manifest.json",
      )
