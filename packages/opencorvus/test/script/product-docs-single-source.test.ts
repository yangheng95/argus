...
describe("product documentation single source", () => {
  ...
  test("Mission and Task API guide reflects SDK directory and message contracts", () => {
    ...
    expect(enGuide).toContain("client.mission.projectArchive")
    expect(enGuide).toContain("GET /mission/{missionID}/project-archive")
    expect(enGuide).toContain("GET /mission/{missionID}/status")
    expect(enGuide).toContain("requires `directory`")
    expect(zhGuide).toContain("client.mission.projectArchive")
    expect(zhGuide).toContain("GET /mission/{missionID}/project-archive")
    expect(zhGuide).toContain("GET /mission/{missionID}/status")
    expect(zhGuide).toContain("必须带 `directory`")
    expect(zhGuide).not.toContain("不是下载 Mission")
  })
})
