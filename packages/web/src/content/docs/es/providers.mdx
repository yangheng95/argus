---
title: Proveedores
description: Usando cualquier proveedor LLM en Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Argus.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Argus
configuración.
...
Argus Zen es una lista de modelos proporcionados por el equipo Argus que han sido
probado y verificado para funcionar bien con Argus. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Argus Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione argus y diríjase a [argus.ai/auth](https://argus.ai/auth).
...
Funciona como cualquier otro proveedor en Argus y su uso es completamente opcional.
...
Para usar Amazon Bedrock con Argus:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta argus:
   ...
   Para una configuración persistente o específica del proyecto, utilice `argus.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Argus no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que argus funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración argus.
...
para almacenar el token en el almacenamiento de autenticación argus.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Argus utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Argus
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `argus.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth).

##### Configuración

Personalizar a través de `argus.json`:
...
Para utilizar su suscripción GitHub Copilot con argus:
...
Para utilizar Google Vertex AI con Argus:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta argus.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de argus, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) para registrar automáticamente cada conversación Argus como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Argus enumerada como una sesión separada.
...
Puede configurar argus para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar argus para usar modelos locales a través de LM Studio.
...
Puedes configurar argus para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Argus. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/argus) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Argus:
...
3. Copie la clave API para usarla en Argus.
...
6. **Importante**: Antes de usar modelos en la nube en Argus, debe obtener la información del modelo localmente:
...
Argus Zen es una lista de modelos probados y verificados proporcionada por el equipo Argus. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Argus Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Argus Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Argus API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración argus.
...
5. También puedes personalizarlos a través de tu configuración argus. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Argus:
...
También puede personalizar modelos a través de su configuración argus. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con argus. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `argus.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Argus comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `argus auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración argus y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración argus.
