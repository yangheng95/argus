---
title: 伺服器
description: 透過 HTTP 與 opencorvus 伺服器互動。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

`opencorvus serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 opencorvus 客戶端使用。
...
| `--mdns-domain` | mDNS 服務的自訂網域名稱 | `opencorvus.local` |
| `--cors`        | 額外允許的瀏覽器來源    | `[]`               |
...
設定 `OPENCORVUS_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencorvus`，也可以設定 `OPENCORVUS_SERVER_USERNAME` 來覆蓋它。這適用於 `opencorvus serve` 和 `opencorvus web`。
...
當你執行 `opencorvus` 時，它會啟動一個 TUI 和一個伺服器。TUI 是與伺服器通訊的客戶端。伺服器暴露一個 OpenAPI 3.1 規範端點。該端點也用於產生 [SDK](/docs/sdk)。

:::tip
使用 opencorvus 伺服器以程式化方式與 opencorvus 互動。
:::

這種架構讓 opencorvus 支援多個客戶端，並允許你以程式化方式與 opencorvus 互動。

你可以執行 `opencorvus serve` 來啟動一個獨立的伺服器。如果你已經在執行 opencorvus TUI，`opencorvus serve` 會啟動一個新的伺服器。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 OpenCorvus [IDE](/docs/ide) 外掛所使用。
...
例如，`http://localhost:7878/doc`。使用該規範可以產生客戶端或檢查請求和回應類型，也可以在 Swagger 瀏覽器中查看。

---

## API

opencorvus 伺服器暴露以下 API。
...
| `POST`   | `/permission/:requestID/reply` | 回應權限請求                   | 請求主體：`{ reply, message? }`，回傳 `boolean`                                     |
...
| `POST` | `/tui/control/response` | 回應控制請求（`{ id, body } or { id, error }`）                     | `boolean`    |
