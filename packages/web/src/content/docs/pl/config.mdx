---
title: Konfiguracja
description: Ustawienia OpenCorvus w pliku JSON.
---

Możesz dostosować OpenCorvus za pomocą pliku konfiguracyjnego JSON.

---

## Format

OpenCorvus obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "opencorvus"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/opencorvus`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/opencorvus/opencorvus.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCORVUS_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`opencorvus.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.opencorvus`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OPENCORVUS_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.opencorvus` i `~/.config/opencorvus` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/opencorvus`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenCorvus w `~/.config/opencorvus/opencorvus.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `opencorvus.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy OpenCorvus się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OPENCORVUS_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCORVUS_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencorvus` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.opencorvus`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`opencorvus.ai/config.json`**](https://opencorvus.ai/config.json).
...
Możesz skonfigurować serwer dla `opencorvus serve` i `opencorvus web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer OpenCorvus.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `opencorvus.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie OpenCorvus wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu OpenCorvus za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/opencorvus/agents/` lub `.opencorvus/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, OpenCorvus powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`opencorvus run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/opencorvus/commands/` lub `.opencorvus/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
OpenCorvus może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy OpenCorvus nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie OpenCorvus **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają OpenCorvus o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.opencorvus/plugins/` lub `~/.config/opencorvus/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie OpenCorvus tylko do określonych dostawców.
