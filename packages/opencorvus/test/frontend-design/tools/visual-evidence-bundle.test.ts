...
test("surfaces corrupt visual evidence JSON instead of treating it as missing", async () => {
  const dir = await tmpdir()
  await Bun.write(path.join(dir.path, "render-result.json"), "{")
  await Bun.write(path.join(dir.path, "eval-result.json"), "{}")
  await Bun.write(path.join(dir.path, "vision-judge.json"), "{}")

  await expect(
    tryMaterializeVisualEvidenceBundle({
      outputDir: dir.path,
      taskID: "tsk_visual_bundle_corrupt_json",
      source: "frontend_design",
    }),
  ).rejects.toThrow("Invalid visual evidence JSON")
  expect(await Bun.file(path.join(dir.path, "visual-evidence-bundle.json")).exists()).toBe(false)
})

test("surfaces invalid visual evidence schema instead of skipping bundle materialization", async () => {
  const dir = await tmpdir()
  await writeJson(path.join(dir.path, "render-result.json"), {
    url: "http://127.0.0.1:4173",
  ...
  await expect(
    tryMaterializeVisualEvidenceBundle({
      outputDir: dir.path,
      taskID: "tsk_visual_bundle_invalid_schema",
      source: "frontend_design",
    }),
  ).rejects.toThrow()
  expect(await Bun.file(path.join(dir.path, "visual-evidence-bundle.json")).exists()).toBe(false)
...
test("surfaces invalid visual evidence PNGs instead of skipping bundle materialization", async () => {
  const dir = await tmpdir()
  const referencePath = path.join(dir.path, "reference.png")
  const renderedPath = path.join(dir.path, "rendered.png")
  ...
  await expect(
    tryMaterializeVisualEvidenceBundle({
      outputDir: dir.path,
      taskID: "tsk_visual_bundle_invalid_png",
      source: "frontend_design",
    }),
  ).rejects.toThrow("Invalid visual evidence PNGs")
  expect(await Bun.file(path.join(dir.path, "visual-evidence-bundle.json")).exists()).toBe(false)
})
