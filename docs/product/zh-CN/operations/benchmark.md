...
| `--executor=mirrorcode\|codex\|claude-code` | 执行器：MirrorCode / Codex / Claude Code |
| `--title=TITLE` | 任务标题 |
...
- 各阶段耗时（spec / goals / plan / execute / evaluate / deliver）
- 每个 Goal 的 check 结果
...
Server 必须带 `DASHSCOPE_API_KEY`，否则 executor 返回空响应，所有任务 <10s 失败。
