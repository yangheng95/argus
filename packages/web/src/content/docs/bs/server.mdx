---
title: Server
description: Komunicirajte s opencorvus serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Komanda `opencorvus serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opencorvus klijent moze koristiti.
...
| --------------- | ----------------------------------- | ------------------ |
| `--port`        | Port na kojem slusa                 | `7878`             |
| `--hostname`    | Hostname na kojem slusa             | `127.0.0.1`        |
| `--mdns`        | Ukljuci mDNS otkrivanje             | `false`            |
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `opencorvus.local` |
...
Postavite `OPENCORVUS_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opencorvus`, ili postavite `OPENCORVUS_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `opencorvus serve` i za `opencorvus web`.
...
Kada pokrenete `opencorvus`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite opencorvus server za programsku interakciju sa opencorvus-om.
:::

Ova arhitektura omogucava opencorvus podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `opencorvus serve` da startate standalone server. Ako je opencorvus TUI vec pokrenut, `opencorvus serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenCorvus [IDE](/docs/ide) pluginovi.
...
Na primjer, `http://localhost:7878/doc`. Koristite specifikaciju da generisete klijente ili pregledate tipove zahtjeva i odgovora. Mozete je otvoriti i u Swagger exploreru.

---

## API-ji

opencorvus server izlaže sljedece API-je.
...
| `POST`   | `/permission/:requestID/reply` | Respond to a permission request       | body: `{ reply, message? }`, returns `boolean`                                     |
...
| `POST` | `/tui/control/response` | Respond to a control request (`{ id, body } or { id, error }`)   | `boolean`              |
