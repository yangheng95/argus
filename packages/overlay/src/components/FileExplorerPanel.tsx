...
async function listDirectory(path: string, scope: FileOperationScope): Promise<FileNode[]> {
...
export function FileExplorerPanel(props: FileExplorerPanelProps = {}) {
  ...
  const loadDirectory = async (path: string, opts?: { force?: boolean; required?: boolean }) => {
    ...
    try {
      const nodes = await listDirectory(normalizedPath, scope)
  ...
  function topLevelActionSelections(items: ExplorerSelection[]): ExplorerSelection[] {
    ...
    return unique.filter(
      (item) => !unique.some((candidate) => candidate.path !== item.path && isPathOrDescendant(item.path, candidate.path)),
    )
  }
  ...
  function updateExplorerDropTarget(event: DragEvent, targetDirInput: string, rowPath?: string): void {
    ...
    if (!status) return
    event.preventDefault()
    if (event.dataTransfer) event.dataTransfer.dropEffect = status === "upload" ? "copy" : status === "move" ? "move" : "none"
  ...
  function openExplorerFile(path: string): void {
    ...
    if (!ownsExplorerOperation(scope)) return
    openFileEditor(path, scope)
  }

  async function runMutation(kind: FileMutationKind, scope: FileOperationScope, fn: () => Promise<void>): Promise<void> {
  ...
  async function applyMoveResults(results: FileMoveResult[], message: string, scope: FileOperationScope): Promise<void> {
    if (!ownsExplorerOperation(scope)) return
  ...
  async function createEntry(type: "file" | "directory", targetDirInput?: string): Promise<void> {
    ...
    await runMutation(type === "file" ? "create-file" : "create-directory", scope, async () => {
      ...
      if (type === "file") openFileEditor(node.path, scope)
  ...
  function openUploadPicker(targetDirInput: string): void {
    ...
    if (!scope) {
      ...
      setUploadMessage("Project directory is required")
      return
    }
  ...
  async function uploadFiles(targetDirInput: string, files: File[], capturedScope?: FileOperationScope | null): Promise<void> {
    const scope = capturedScope ?? currentOperationScope()
    if (!scope) {
      setUploadMessageStatus("error")
      setUploadMessage("Project directory is required")
    ...
    return (
      <ContextMenu.Portal>
        <ContextMenu.Content class="file-explorer-context-menu">
          ...
          <ContextMenu.Separator class="file-explorer-menu-separator" />
          <FileExplorerContextMenuItem
