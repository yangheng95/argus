...
export namespace FrontendDesignAgent {
  ...
  export async function analyze(input: AnalyzeInput): Promise<Result & { sessionID: string }> {
    ...
    const webpageEvidenceTools = await createWebpageEvidenceTools({ taskID: input.taskID, signal: input.signal }, processTrace)
...
function buildUserPrompt(input: {
  ...
  const webpageEvidenceRef = runtimePaths?.webpageEvidenceRelative ?? ".opencorvus/runtime/tasks/<taskID>/frontend-design/webpage-evidence"
  ...
  sections.push(
    ...
    "For webpage replica work, frontend_design's first workflow deliverable is a source-derived static HTML/CSS visual skeleton, not a complete application source tree. Report it as `submit_frontend_template.frontend_project.role=visual_baseline_input`, name its root and entrypoints such as `visual-html-skeleton/index.html`, CSS, assets, screenshots, and diff/evaluation artifacts, and state that it is not the implementation target or acceptance app root. Its source of authority is still `web-clone-source/source-ir/*`, `web-clone-source/source-skeleton/*`, assets, and `web-clone-source/reference.png`; if the skeleton conflicts with those artifacts or visible pixels, the original source evidence wins. Later workflow stages transcribe this HTML skeleton into project source. " +
    `Default visual HTML skeleton root: \`${targetProjectRef}\` only when the target name is still required by existing tooling; prefer reporting the role and entrypoints as \`visual_baseline_input\` and \`visual-html-skeleton/index.html\`. ` +
    ...
    "After the source evidence skeleton exists, the first editing pass is visual skeleton adoption: create/adapt static `index.html`, CSS, asset references, representative-state markup, screenshots/diff artifacts when available, and a README/source note that names the source evidence. This is not freehand scaffolding and not app-source bootstrap. Render that HTML skeleton through a real static harness or browser path, compare it with `reference.png`, and refine visual regions from source evidence until the first workflow can pass HTML-design fidelity review. " +
    ...
    "Default first-workflow stack for webpage replicas: static HTML/CSS/JavaScript only when needed for representative visual states. Put later React/Vue/etc. project transcription constraints into `quality_project_contract`, not into the current skeleton source. " +
    "The final `submit_frontend_template.frontend_project.project_root` should name the visual skeleton root, with entrypoints such as `visual-html-skeleton/index.html`, `visual-html-skeleton/styles.css`, source asset paths, screenshots, and visual diff/evaluation artifacts.",
  )
  ...
  return {
      ...
      execute: async (params) => {
        ...
        return {
          ...
          output: [
            ...
            "Before writing files for another region, render this region through one explicit static URL or file path, feed its screenshot into webpage_evaluate, call webpage_vision_judge for visible mismatches, repair this same region, then record this region's replacement result and call record_frontend_region_selection for the next region.",
    ...
    record_frontend_replacement_result: tool({
      ...
      inputSchema: z.object({
        ...
        filesChanged: z.array(z.string()).default([]).describe("HTML skeleton files/assets changed for this replacement attempt."),
        dataModules: z.array(z.string()).default([]).describe("Source data/content snippets represented or noted for later transcription."),
        styleModules: z.array(z.string()).default([]).describe("CSS/style files extracted or updated."),
        removedGeneratedBoundaries: z.array(z.string()).default([]).describe("Generated source-dom boundaries replaced in the visual skeleton or recorded as later transcription debt."),
        ...
        auditEvidence: z.array(z.string()).default([]).describe("Source traceability or source-quality outputs used for this replacement."),
        remainingSourceDebt: z.array(z.string()).default([]).describe("Named visual/source regions still unfinished after this attempt."),
      ...
      execute: async (params) => {
        ...
        return {
          ...
          output: [
            ...
            params.remainingSourceDebt.length > 0 ? `- Remaining visual/source debt: ${params.remainingSourceDebt.join(", ")}` : undefined,
            params.remainingSourceDebt.length > 0 ? "- Do not describe the HTML skeleton as final while visual/source debt remains; continue source-region restoration or mark the final handoff incomplete/blocked." : undefined,
            "- Completed means source-backed HTML/CSS/assets/content with render/evaluate evidence, not placeholder UI/text/classes, fake spacers, or rendered future-region filler.",
          ].filter(Boolean).join("\n"),
...
async function createWebpageEvidenceTools(input: { taskID?: string; signal?: AbortSignal }, trace = createFrontendProcessTrace()): Promise<ToolSet> {
  const tools = {
