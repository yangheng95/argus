---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w argus.
---

import config from "../../../../config.mjs"
export const console = config.console

argus używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji argus.
...
w `~/.local/share/argus/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim argus
config.
...
Argus Zen to lista modeli dostarczonych przez zespół argus, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z argus. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Argus Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz argus i przejdź do [argus.ai/auth](https://argus.ai/auth).
...
Działa jak każdy inny dostawca w argus i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z argus:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania argus:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `argus.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w argus nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod argus działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji argus.
...
argus używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować argus
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`argus.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth).

##### Konfiguracja

Customize through `argus.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem argus:
...
Aby używać Google Vertex AI z argus:
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą argus, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session), aby automatycznie rejestrować każdą konwersację argus jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja argus będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować argus tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować argus tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować argus tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem argus. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/argus).
:::
...
Aby korzystać z Ollama Cloud z argus:
...
3. Skopiuj klucz API do użycia w argus.
...
6. **Ważne**: Przed użyciem modeli chmurowych w argus musisz pobrać informacje o modelu lokalnie:
...
Argus Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół argus. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Argus Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Argus Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API argus.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji argus.
...
5. Możesz je także dostosować za pomocą konfiguracji argus. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z argus:
...
Możesz także dostosować modele za pomocą konfiguracji argus. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z argus. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `argus.json` w katalogu projektu:
...
Pola `limit` pozwalają argus zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `argus auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację argus i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji argus.
