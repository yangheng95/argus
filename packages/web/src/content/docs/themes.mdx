...
With OpenCorvus you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenCorvus uses our own `opencorvus` theme.
...
OpenCorvus comes with several built-in themes.
...
OpenCorvus supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/opencorvus/themes/*.json` or `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.opencorvus/themes/*.json`
4. **Current working directory** - Defined in `./.opencorvus/themes/*.json`
