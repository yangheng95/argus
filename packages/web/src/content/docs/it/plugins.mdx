---
title: Plugin
description: Scrivi plugin per estendere Argus.
---

I plugin ti permettono di estendere Argus agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di Argus.
...
- `.argus/plugins/` - plugin a livello progetto
- `~/.config/argus/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/argus/node_modules/`.
...
1. Config globale (`~/.config/argus/argus.json`)
2. Config di progetto (`argus.json`)
3. Directory plugin globale (`~/.config/argus/plugins/`)
4. Directory plugin di progetto (`.argus/plugins/`)
...
Argus esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di argus per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere argus.
...
:::note
Se usi l'app desktop di Argus, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a argus di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a argus:
...
L'helper `tool` crea uno strumento personalizzato che argus puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in argus insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://argus.ai/docs/sdk) per i dettagli.
