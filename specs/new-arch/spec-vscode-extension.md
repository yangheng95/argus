...
opencorvus 已具备 VSCode 集成所需的绝大多数后端基础设施（HTTP+SSE server、`@opencorvus-ai/sdk`、ACP stdio server、`Ide.install()` 命令、多 root 支持的 project-scoped `directory` query、SQLite 持久化）。VSCode 扩展的工作 **不是** 实现新 agent，而是给已经存在的 daemon 装一张 IDE 脸。
...
| Project 隔离            | `src/server/server.ts:121`、`src/project/instance.ts`、`packages/transport-protocol/src/index.ts` | 项目作用域路由通过 query `?directory=` 或 header `x-opencorvus-directory` 选择 `Instance.directory`，**单 daemon 多 project 共存**。SDK `createOpenCorvusClient({ directory })` 只给项目作用域请求自动注入 `directory` query；单 task 记录读取从 `taskID` 解析归属，不需要目录。                                                                                                                                                                                        |
...
| 单任务事件 SSE          | `routes/orchestrator.ts:333` `GET /task/:taskID/events?after=N`                          | 游标参数 **`after`**。支持断线 replay：连接时把 `> after` 的历史事件一次性回放，再切实时。`sequence === 0` 是 ephemeral（不计游标）。这是 task 记录读取，不需要 `directory`。                                                                                                                                                                                                                                 |
| 任务 hydrate            | `routes/orchestrator.ts:436` `GET /task/:taskID/conversation`                            | **复合冷启端点**：一次返回 `{lastSequence, board, transcript, timeline, events, eventReplay, view}`。扩展 Inspector 冷启用这条，再用 `lastSequence` 作 SSE `after` 续接。这是 task 记录读取，不需要 `directory`。                                                                                                                                                                                              |
...
| 任务结构数据            | `routes/orchestrator.ts` `/task/:id/{board,transcript,brief,runs,interactions,progress}` | board/transcript 是 plan/messages 视图原料；这些读取路由不需要 `directory`。                                                                                                                                                                                                                                                                                                                                  |
...
- 多 VSCode 窗口：第二个窗口启动时发现已有可用 lock 直接复用（项目列表/创建/写操作通过 `directory` query 区分；单 task 读取通过 `taskID` 归属区分，§3.2）
...
1. **多 root 复用 daemon 的目录切换粒度**：已由 route directory policy 收敛。`GET /tasks`、`POST /task`、task 写操作仍随 SDK `directory` query 绑定当前 workspace；Inspector 的 `GET /task/:id/conversation` 与 `GET /task/:id/events?after=N` 是 task 记录读取，从 `taskID` 解析项目归属，不再携带 `directory`。
