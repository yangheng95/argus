...
| Hard constraints               | Manifest `id` 继续是唯一逻辑选择身份；namespace 继续是来源与安装分区；不从目录名、产品名、README、自定义 display metadata 或 Overlay source kind 猜测；删除旧 `expert_squad_display_prefix` 契约，不保留 fallback/alias/双源；不新增、修改、更新或运行 UI 自动化测试；保留并行工作区改动；提交使用 `dsw-33987` 前缀并推送 `legacy-remote/v0.0.29beta`。                                                                                                                             |
