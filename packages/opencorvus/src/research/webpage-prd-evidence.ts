...
export function renderWebpagePrdEvidencePromptSection(evidence: WebpagePrdEvidence | undefined): string | undefined {
  ...
  return [
    ...
    "Also register `webpage_contract` chunks as the source for the frontend_design Page Skeleton Blueprint. They must partition the rendered page investigation and must cover functional_surfaces, visual_layout, style_requirements, interaction_states, data_content_inventory, fidelity_acceptance, and fidelity_risks as work-packet inputs. Do not substitute a raw artifact/material list for this contract.",
    "Register bundle sections for this webpage contract with complete but bounded single-line points. Under Visual HTML Skeleton Coverage, write the Page Skeleton Blueprint: visible region order/count, major content sections, `functional_surfaces.component_kind_hypothesis`, data/content anchors, interaction states, source evidence ids, and fidelity risks for frontend_design to materialize. Cover every major visible region with evidence-backed work packets, style/layout questions, interaction/data questions, expected downstream outputs, acceptance checks, and risks; do not submit raw markdown or JSON documents as string fields, and split multiline notes into separate array items.",
