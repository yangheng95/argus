...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات argus. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في argus تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات argus.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.argus/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `argus.json` لديك:
...
إذا لم يتم تحديد درجة الحرارة، يستخدم argus القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات argus العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.argus/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/argus/modes/` لأوضاع عالمية:
