...
Možete dodati vanjske alate u OpenCorvus koristeći _Model Context Protocol_, ili MCP. OpenCorvus podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenCorvus Config](https://opencorvus.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/opencorvus`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenCorvus automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenCorvus će:
...
Ako server zahtijeva autentifikaciju, OpenCorvus će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencorvus mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenCorvus sigurno čuva tokene u `~/.local/share/opencorvus/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenCorvus, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenCorvus konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenCorvus sa vašim Sentry nalogom.
