...
test("TaskList task rows stay one-line while preserving detail in tooltips", () => {
  ...
  expect(SIDEBAR_CSS).toMatch(/\.task-row-mini\s*\{[^}]*grid-template-columns:\s*calc\(20px \* var\(--ui-scale\)\) 0 minmax\(0, 1fr\) max-content;/);
  expect(SIDEBAR_CSS).toMatch(/\.task-row-mini\[data-draggable="true"\]\s*\{[^}]*grid-template-columns:\s*calc\(20px \* var\(--ui-scale\)\) calc\(14px \* var\(--ui-scale\)\) minmax\(0, 1fr\) max-content;/);
  ...
  expect(SIDEBAR_CSS).toMatch(/\.task-row-badge\[data-status="completed"\]\s*\{[^}]*background:\s*color-mix\(in srgb, var\(--good\) 10%, transparent\);/);
  expect(SIDEBAR_CSS).toMatch(/\.task-row-badge\[data-status="completed"\]::before\s*\{[^}]*background:\s*color-mix\(in srgb, var\(--good\) 88%, transparent\);/);
  ...
  expect(SIDEBAR_CSS).toMatch(/\.project-group-heading\s*\{[^}]*grid-template-columns:\s*calc\(18px \* var\(--ui-scale\)\) minmax\(0, 1fr\) auto calc\(16px \* var\(--ui-scale\)\);/);
  ...
  expect(SIDEBAR_CSS).toMatch(/\.project-group-body\s*\{[^}]*padding-left:\s*calc\(18px \* var\(--ui-scale\) \+ 5px \* var\(--ui-scale\)\);/);
});
