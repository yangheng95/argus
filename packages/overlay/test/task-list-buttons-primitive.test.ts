...
test("TaskList task rows stay one-line while preserving detail in tooltips", () => {
  ...
  expect(PROJECT_LEDGER_GROUP_SOURCE).toContain('<Icon name="plus" size={14} />')
  expect(PROJECT_LEDGER_GROUP_SOURCE).not.toContain('<Icon name="message-add"')
  ...
  expect(SIDEBAR_CSS).toMatch(/\.oc-button\[data-ui="ledger-row-main"\] strong\s*\{[^}]*color:\s*var\(--text-soft\);/)
