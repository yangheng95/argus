---
title: SDK
description: Client JS type-safe per il server opencorvus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

L'SDK JS/TS di opencorvus fornisce un client type-safe per interagire con il server.
Usalo per creare integrazioni e controllare opencorvus in modo programmatico.
...
| `port`     | `number`      | Porta del server               | `7878`      |
...
Puoi passare un oggetto di configurazione per personalizzare il comportamento. L'istanza legge comunque `opencorvus.json`, ma puoi sovrascrivere o aggiungere configurazione inline:
...
Se hai gia un'istanza di opencorvus in esecuzione, puoi creare un client per collegarti:
...
| Opzione         | Tipo       | Descrizione                         | Predefinito             |
| --------------- | ---------- | ----------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | URL del server                      | `http://localhost:7878` |
