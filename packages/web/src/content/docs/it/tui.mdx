---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenCorvus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenCorvus fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenCorvus avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenCorvus, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenCorvus. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenCorvus. _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di OpenCorvus.
