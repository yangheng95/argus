...
description: Créez des outils que le LLM peut appeler dans argus.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de argus comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.argus/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/argus/tools/`.
...
```ts title=".argus/tools/database.ts" {1}
...
```ts title=".argus/tools/project.ts" {8}
import { tool } from "@argus-ai/plugin"
...
```python title=".argus/tools/add.py"
import sys
...
```ts title=".argus/tools/python-add.ts" {10}
import { tool } from "@argus-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".argus/tools/add.py")
