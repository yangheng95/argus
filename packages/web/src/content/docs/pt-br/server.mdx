---
title: Servidor
description: Interaja com o servidor opencorvus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

O comando `opencorvus serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opencorvus pode usar.
...
| --------------- | ------------------------------------------------- | ------------------ |
| `--port`        | Porta para escutar                                | `7878`             |
| `--hostname`    | Nome do host para escutar                         | `127.0.0.1`        |
| `--mdns`        | Habilitar descoberta mDNS                         | `false`            |
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `opencorvus.local` |
...
Defina `OPENCORVUS_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencorvus`, ou defina `OPENCORVUS_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `opencorvus serve` quanto ao `opencorvus web`.

```bash
OPENCORVUS_SERVER_PASSWORD=your-password opencorvus serve
```
...
Quando você executa `opencorvus`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor opencorvus para interagir com o opencorvus programaticamente.
:::

Essa arquitetura permite que o opencorvus suporte múltiplos clientes e permite que você interaja com o opencorvus programaticamente.

Você pode executar `opencorvus serve` para iniciar um servidor autônomo. Se você tiver o TUI do opencorvus em execução, `opencorvus serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do opencorvus [IDE](/docs/ide).
...
Por exemplo, `http://localhost:7878/doc`. Use a especificação para gerar clientes ou inspecionar tipos de requisição e resposta. Ou visualize em um explorador Swagger.

---

## APIs

O servidor opencorvus expõe as seguintes APIs.
...
| `POST`   | `/permission/:requestID/reply` | Responder a um pedido de permissão                    | corpo: `{ reply, message? }`, retorna `boolean`                                     |
...
| `POST` | `/tui/control/response` | Responder a um pedido de controle (`{ id, body } or { id, error }`) | `boolean`                    |
