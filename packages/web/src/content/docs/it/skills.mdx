...
Le skill degli agenti permettono a OpenCorvus di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenCorvus cerca in queste posizioni:

- Config di progetto: `.opencorvus/skills/<name>/SKILL.md`
- Config globale: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenCorvus risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.opencorvus/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.opencorvus/skills/git-release/SKILL.md` cosi':
...
OpenCorvus elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `opencorvus.json`:
...
**Per agenti integrati** (in `opencorvus.json`):
