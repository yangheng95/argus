---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenCorvus-Konfiguration.
...
in `~/.local/share/opencorvus/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenCorvus anpassen
config.
...
OpenCorvus Zen ist eine Liste von Modellen, die vom OpenCorvus-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenCorvus funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenCorvus Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenCorvus aus und gehen Sie zu [opencorvus.ai/auth](https://opencorvus.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in OpenCorvus und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit OpenCorvus:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenCorvus ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `opencorvus.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenCorvus wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenCorvus ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenCorvus-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `OpenCorvus`, Bereiche: `api`
...
um das Token im OpenCorvus-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenCorvus verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenCorvus
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`opencorvus.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `opencorvus.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenCorvus:
...
So verwenden Sie Google Vertex AI mit OpenCorvus:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenCorvus ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenCorvus konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session)-Plugin, um jede OpenCorvus-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenCorvus-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenCorvus für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können OpenCorvus über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können OpenCorvus über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenCorvus konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencorvus).
:::
...
So verwenden Sie Ollama Cloud mit OpenCorvus:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenCorvus.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenCorvus verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenCorvus Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenCorvus-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenCorvus Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenCorvus API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenCorvus-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenCorvus-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenCorvus:
...
Sie können Modelle auch über Ihre OpenCorvus-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenCorvus verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `opencorvus.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann OpenCorvus erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `opencorvus auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenCorvus-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenCorvus-Konfiguration übereinstimmt.
