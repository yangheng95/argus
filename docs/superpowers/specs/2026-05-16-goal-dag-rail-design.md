...
- **[codex review feedback]** Also add `dependsOn: z.array(z.string())` to the
  `TaskBoardGoalWorkflow` Zod schema (`engine/model.ts:804`). The draft only
  touched `board.ts`; the schema is the contract and must move with it (rule 35 /
  rule 8 — single source). Default `[]` so the field is total.
- Producer chain to keep consistent: `board.ts:77-83` (goal ordering) →
  `board.ts:903-970` (projection) → `board.ts:980` (return).
- Adding a new optional-with-default field is additive; existing `goalWorkflows`
  consumers (Board.tsx, GoalWorkflowGroup.tsx, tree-writer.ts, diff.ts,
  file-change-summary.ts, panel.ts) read other fields and are unaffected — listed
  here for rule 35 completeness, no change required in them.
...
edges overlap and the graph looks accidental. Codex's adopted algorithm (pure
function `layoutDag(nodes)`, no retained state / no timers / no rAF / not a state
machine — satisfies rules 5/6/13):
...
1. **Backend projection**: board-projection test asserts
   `goalWorkflows[].dependsOn` mirrors `EngineGoalTable.depends_on`.
2. **Schema**: extend `test/workbench/board-goal-worktree-schema.test.ts:12-39`
   ...
   (defaulting to `[]`).
3. **`layoutDag` unit**: linear chain → no edge columns; diamond
   (G1→G2, G1→G3, G2→G4, G3→G4) → correct endpoints + distinct columns for the
   two spanning edges; isolated nodes → zero edges.
4. **Render**: running node carries `data-state="running"`; hot-edge class
   applied to the edge into a running node; row click resolves the scroll target.
5. **i18n**: assert `goal_dag.*` keys are referenced and old `progress.*` keys
   are gone from the component.
6. **CSS guards** (mirror `agent-workflow-css-guards`): `.goal-dag` has **no**
   `position: sticky`, **no** `top`, **no** sticky `z-index`, **no**
   `backdrop-filter`; and `.task-progress`, the old bar/fill/fail/pills
   selectors, and `task-progress-pulse` no longer exist. (Rule 36: removed
