---
title: LSP serveri
description: OpenCorvus se integriše sa vašim LSP serverima.
---

OpenCorvus se integriše sa vašim Language Server Protocol (LSP) serverima kako bi pomogao LLM-u u interakciji s kodnom bazom. Koristi dijagnostiku za slanje povratnih informacija modelu.

## Ugrađeni

OpenCorvus dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `OPENCORVUS_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada opencorvus otvori fajl, on:
...
Možete prilagoditi LSP servere kroz `lsp` odjeljak u vašoj opencorvus konfiguraciji.
