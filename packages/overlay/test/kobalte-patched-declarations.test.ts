...
const BUN_MODULES = join(REPO_ROOT, "node_modules/.bun")
...
function kobalteDist(): string {
  const entry = readdirSync(BUN_MODULES).find((name) => name.startsWith("@kobalte+core@0.13.11"))
  if (!entry) throw new Error(`Kobalte package directory is missing under ${BUN_MODULES}`)
  return join(BUN_MODULES, entry, "node_modules/@kobalte/core/dist")
}
