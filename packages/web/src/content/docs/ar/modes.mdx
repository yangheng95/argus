...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات opencorvus. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في opencorvus تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات opencorvus.
...
يأتي opencorvus مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.opencorvus/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `opencorvus.json` لديك:
...
- عالمي: `~/.config/opencorvus/modes/`
- للمشروع: `.opencorvus/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم opencorvus القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات opencorvus العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.opencorvus/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/opencorvus/modes/` لأوضاع عالمية:
