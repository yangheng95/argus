...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) argus, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.argus/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/argus/tools/`.
