  ...
  return (
    <div class="task-overlay" role="dialog" aria-modal="true">
      <div class="task-overlay-header">
        <button type="button" class="task-overlay-back" onClick={props.onClose} title={t("task_overlay.back_title")}>
          ← {t("task_overlay.back")}
        </button>
        <div class="task-overlay-title">
          {t("task_overlay.task_label")} <code>{props.taskID}</code>
        </div>
        <Show when={loading()}>
        ...
        <Show when={error() && !loading()}>
          <span class="task-overlay-error" role="alert">
            ...
            <button type="button" class="task-overlay-error-retry" onClick={() => loadActiveTask(props.taskID)}>
              {t("common.retry")}
            </button>
          </span>
        </Show>
