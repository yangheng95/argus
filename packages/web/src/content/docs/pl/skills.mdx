...
Umiejętności agenta pozwalają argus odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
argus przeszukuje te lokalizacje:

- Project config: `.argus/skills/<name>/SKILL.md`
- Global config: `~/.config/argus/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu argus przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.argus/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.argus/skills/git-release/SKILL.md` w ten sposób:
...
argus wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `argus.json`:
...
**Dla agentów wbudowanych** (w `argus.json`):
