...
The overlay-server compile entrypoint also had to move from `src/overlay-server.ts` to `src/overlay-launcher.ts`. The real server module has static imports, so putting `process.chdir(path.dirname(process.execPath))` inside it is too late for Bun external subpath resolution such as `@parcel/watcher/wrapper` from `B:/~BUN/root/overlay-server.js`. Both CLI and overlay-server artifacts now compile through thin launcher entrypoints that prepare the executable-directory runtime before dynamically importing the real entry module.

Bun compiled external subpath resolution can still use the virtual bunfs referrer after launcher preparation. Production native package loads therefore go through `runtime/package-require.ts`, which creates a `require` rooted at the packaged executable directory's `package.json` when present. `FileWatcher`, GUI screenshot capture, and capability probes must use that resolver instead of static native imports or default `require()`. Build-owned direct screenshot resizing was retired; build visual evidence uses task-scoped backend browser evidence or Browser MCP screenshot/observe instead.
