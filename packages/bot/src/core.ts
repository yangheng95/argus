...
export class BotCore {
  ...
  private buildSystemPrompt(platform: string): string {
    ...
    return [
      ...
      "- **Claude Code**: `bash('start \"Claude Code\" cmd /c \"claude\"')` — opens in a new terminal",
      `- **OpenCorvus TUI**: \`bash('start "OpenCorvus" cmd /c "bun --preload @opentui/solid/preload --conditions=browser ${opencorvusSrc} <project_dir>"')\``,
      ...
      "Before binding to ANY window, you MUST call `screen.list_windows` first to see actual window titles.",
      "Then call `screen.bind_window` with a title substring that matches a real window from the list.",
      "- Window search uses **case-insensitive substring matching** on title and app name.",
      "- If binding fails, call `list_windows` again and use the exact title from the list.",
      ...
      "3. `screen.list_windows` to find the window (look for title containing 'claude' or 'Claude Code')",
      "4. `screen.bind_window` with the title from step 3",
      ...
      "- `input.wait`: ms must be 100-10000 (integer). For longer waits, call wait multiple times.",
