...
export type BrowserPreviewLayoutGeometryRegionSample = {
  regionID: string
...
type LayoutGeometrySidecarResult =
  | {
      ok: true
      samples: Array<Omit<BrowserPreviewLayoutGeometrySample, "regions"> & { regions: BrowserPreviewLayoutGeometryRegionSample[] }>
      diagnostics: string[]
    }
...
async function captureBrowserPreviewLayoutGeometryWithSidecar(
  ...
  const sidecar = await runBrowserNodeSidecar<LayoutGeometrySidecarResult>({
    runtime,
    ...
    payloadEnvName: "OPENCORVUS_BROWSER_PREVIEW_LAYOUT_GEOMETRY_INPUT",
    inactivityTimeoutMs:
      launchTimeoutMs +
  ...
  if (!sidecar.result.ok) {
    throw new Error(
      `Browser preview layout geometry diagnostic failed: ${sidecar.result.message}${
  ...
  return { samples: sidecar.result.samples, diagnostics: sidecar.result.diagnostics }
}
