...
argus ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak argus kendi `argus` temasını kullanır.
...
argus birden fazla yerleşik temayla gelir.
...
- argus'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
argus, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/argus/themes/*.json` veya `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.argus/themes/*.json`
4. **Geçerli çalışma dizini** - `./.argus/themes/*.json`
