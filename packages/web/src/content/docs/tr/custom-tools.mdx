---
title: Özel Araçlar
description: LLM'un opencorvus'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. opencorvus'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.opencorvus/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/opencorvus/tools/` içine yerleştirerek.
