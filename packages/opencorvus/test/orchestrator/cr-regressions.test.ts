...
test("persistEvaluation binds goal-run assessment to the evaluated goal subset", async () => {
  await using tmp = await tmpdir({ git: true })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      persistEvaluation({
        ...
        finalStatus: "failed",
        finalSummary: "auth rejected",
        goals,
        finalizeSpec: false,
      })

      const dir = path.join(tmp.path, ".opencorvus", "evaluations")
      const [file] = (await readdir(dir)).sort()
      const text = await Bun.file(path.join(dir, file!)).text()
