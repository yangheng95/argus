...
describe("sub-agent infrastructure homogeneity manifest", () => {
  ...
  test("general expert-squad selectors load package selector instructions", () => {
    ...
    expect(selectorCatalogSource).toContain('path.join(entry.root, "selector.md")')
    ...
    expect(loadProjectSource).toContain("ExpertSquadRegistry.discover(projectDirectory)")
    expect(loadProjectSource).toContain("candidate.id === profileID")
    expect(loadProjectSource).toContain("lstat(entry.root)")
    expect(loadProjectSource).toContain("ExpertSquadRegistry.loadPackage(entry.root, options)")
    expect(loadProjectSource).not.toContain("path.join(canonicalBase(projectDirectory), profileID)")
    expect(loadProjectSource).not.toContain("ExpertSquadPackageManager.releasePayloadPackages")
  })
