...
describe("tool.memory", () => {
  ...
  test("accepts common action and field aliases used by live models", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const memory = await MemoryTool.init()
        const session = ctx("ses_memory_alias")

        const write = await memory.execute(
          {
            action: "save",
            name: "Runtime notes",
            text: "## Findings\n- use feature/memory-alias while validating recall paths",
            type: "fact",
          },
          session,
        )
        const writeData = JSON.parse(write.output) as { fileId: string; kind: string }
        expect(writeData.kind).toBe("fact")

        const search = await memory.execute(
          {
            action: "find",
            keyword: "feature memory-alias recall paths",
            limit: 5,
          },
          session,
        )
        const searchData = JSON.parse(search.output) as { results: Array<{ fileId: string }> }
        expect(searchData.results.some((item) => item.fileId === writeData.fileId)).toBe(true)

        const get = await memory.execute(
          {
            action: "read",
            memory_id: writeData.fileId,
          },
          session,
        )
        const getData = JSON.parse(get.output) as { text: string }
        expect(getData.text).toContain("memory-alias")

        const del = await memory.execute(
          {
            action: "remove",
            file_id: writeData.fileId,
          },
          session,
        )
        expect(del.title).toContain("Deleted:")
