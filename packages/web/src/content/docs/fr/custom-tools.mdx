...
description: Créez des outils que le LLM peut appeler dans opencorvus.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de opencorvus comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.opencorvus/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/opencorvus/tools/`.
...
```ts title=".opencorvus/tools/database.ts" {1}
...
```ts title=".opencorvus/tools/math.ts"
import { tool } from "@opencorvus-ai/plugin"
...
```ts title=".opencorvus/tools/project.ts" {8}
import { tool } from "@opencorvus-ai/plugin"
...
```python title=".opencorvus/tools/add.py"
import sys
...
```ts title=".opencorvus/tools/python-add.ts" {10}
import { tool } from "@opencorvus-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".opencorvus/tools/add.py")
