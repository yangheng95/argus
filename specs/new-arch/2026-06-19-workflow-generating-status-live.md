...
| `rg -n 'role="status"|aria-live|aria-busy' packages/overlay/src/components packages/overlay/test specs/new-arch` | Loading status/live contracts exist for Browser Preview, Agent Models, Trace, connection and provider surfaces, but not for Architect/FrontendResearch/Requirements generating indicators. | Use the established `role="status" aria-live="polite" aria-busy="true"` contract. |
