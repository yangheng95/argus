---
title: カスタムツール
description: LLM が Argus で呼び出すことができるツールを作成します。
---

カスタムツールは、会話中に LLM が呼び出すことができる作成した関数です。これらは、Argus の[組み込みツール](/docs/tools) (`read`、`write`、`bash` など) と連携して動作します。
...
- ローカルでは、プロジェクトの `.argus/tools/` ディレクトリに配置します。
- または、グローバルに、それらを `~/.config/argus/tools/` に配置します。
