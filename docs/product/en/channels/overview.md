...
1. **Embedded**: `createOpencode()` — launches backend in-process.
2. **External**: `createOpencodeClient({ baseUrl })` — connects to an existing `opencorvus serve`.
...
| Auto ask reply | `OPENCORVUS_CHANNEL_PERMISSION_ASK_REPLY=once\|always\|reject` |
