...
:::caution
Modlar artık opencorvus yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

opencorvus'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya opencorvus yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
opencorvus iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.opencorvus/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`opencorvus.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, opencorvus modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global opencorvus yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.opencorvus/modes/` veya genel modlar için `~/.config/opencorvus/modes/`'de mod dosyaları oluşturun:
