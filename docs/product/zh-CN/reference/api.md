...
> 本文件由 `bun run docs:api` 从 `packages/sdk/openapi.json` 生成。禁止手写编辑——下次 CI 运行会覆盖。

## Authentication

全部端点受 `OPENCORVUS_SERVER_PASSWORD` 保护。设置后客户端必须使用 HTTP Basic Auth（用户名留空，密码 = 环境变量值）。
...
| DELETE | `/session/{sessionID}/message/{messageID}/part/{partID}` | （无说明） | `part.delete` |
| PATCH | `/session/{sessionID}/message/{messageID}/part/{partID}` | （无说明） | `part.update` |
...
| POST | `/mcp/{name}/connect` | （无说明） | `mcp.connect` |
| POST | `/mcp/{name}/disconnect` | （无说明） | `mcp.disconnect` |
