---
title: Сервер
description: Взаимодействуйте с сервером opencorvus через HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Команда `opencorvus serve` запускает автономный HTTP-сервер, который предоставляет конечную точку OpenAPI, которую может использовать клиент с открытым кодом.
...
| --------------- | ----------------------------------- | ------------------ |
| `--port`        | Port to listen on                   | `7878`             |
| `--hostname`    | Hostname to listen on               | `127.0.0.1`        |
| `--mdns`        | Enable mDNS discovery               | `false`            |
| `--mdns-domain` | Custom domain name for mDNS service | `opencorvus.local` |
...
Установите `OPENCORVUS_SERVER_PASSWORD`, чтобы защитить сервер с помощью базовой аутентификации HTTP. Имя пользователя по умолчанию — `opencorvus` или установите `OPENCORVUS_SERVER_USERNAME`, чтобы переопределить его. Это относится как к `opencorvus serve`, так и к `opencorvus web`.
...
Когда вы запускаете `opencorvus`, он запускает TUI и сервер. Где находится TUI
...
:::tip
Используйте сервер opencorvus для программного взаимодействия с открытым кодом.
:::
...
Вы можете запустить `opencorvus serve`, чтобы запустить автономный сервер. Если у вас есть
TUI с открытым кодом запущен, `opencorvus serve` запустит новый сервер.
...
Конечную точку [`/tui`](#tui) можно использовать для управления TUI через сервер. Например, вы можете предварительно заполнить или запустить подсказку. Эта настройка используется плагинами opencorvus [IDE](/docs/ide).
...
For example, `http://localhost:7878/doc`. Use the spec to generate clients or inspect request and response types. Or view it in a Swagger explorer.

---

## API

Сервер opencorvus предоставляет следующие API.
