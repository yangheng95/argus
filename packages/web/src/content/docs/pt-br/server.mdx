---
title: Servidor
description: Interaja com o servidor argus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `argus serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente argus pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `argus.local` |
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `argus`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `argus serve` quanto ao `argus web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password argus serve
```
...
Quando você executa `argus`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor argus para interagir com o argus programaticamente.
:::

Essa arquitetura permite que o argus suporte múltiplos clientes e permite que você interaja com o argus programaticamente.

Você pode executar `argus serve` para iniciar um servidor autônomo. Se você tiver o TUI do argus em execução, `argus serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do argus [IDE](/docs/ide).
...
O servidor argus expõe as seguintes APIs.
