...
| Hard constraints               | 保留所有并行工作区改动；不新增、修改、更新或运行任何 User Interface（UI，用户界面）自动化测试；直接修复布局根因，不增加 gate、fallback、第二选择源或持久化展开状态；桌面端单端修复；提交主题使用 `dsw-33987` 前缀并推送当前主交付分支到 `myhexin`。                                                                                                                                                                                               |
...
7. 二次复核 task-owned diff，fetch 远端，提交并推送 `v0.0.29beta` 到 `myhexin`。
