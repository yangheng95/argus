...
Agent skills omogucavaju OpenCorvus da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenCorvus pretrazuje ove lokacije:

- Konfiguracija projekta: `.opencorvus/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenCorvus ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.opencorvus/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.opencorvus/skills/git-release/SKILL.md` ovako:
...
OpenCorvus navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `opencorvus.json`:
...
**Za ugradene agente** (u `opencorvus.json`):
