---
title: SDK
description: opencorvus 서버를 위한 타입 안전한 JS 클라이언트.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

opencorvus JS/TS SDK는 서버와 상호 작용을 위한 유형 안전한 클라이언트를 제공합니다.
통합 및 제어 opencorvus programmatically를 구축하는 데 사용됩니다.
...
구성 객체를 전달할 수 있습니다. 인스턴스는 여전히 `opencorvus.json`를 선택하지만 구성 인라인을 추가 할 수 있습니다.
...
opencorvus의 실행 인스턴스가 이미 있다면 클라이언트 인스턴스를 만들 수 있습니다.
...
| 옵션            | 유형       | 설명                              | 기본                    |
| --------------- | ---------- | --------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | 서버의 URL                        | `http://localhost:7878` |
