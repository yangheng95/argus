...
const arts = db.query<{ id: string; payload: string; time_created: number }, []>(
...
for (let i = 0; i < arts.length; i++) {
  ...
  try { parsed = JSON.parse(a.payload); } catch {}
...
const orchSes = db.query<{ id: string; time_created: number; time_updated: number }, []>(
...
for (const s of orchSes) {
  console.log(`${s.id}  start=${new Date(s.time_created).toISOString()}  end=${new Date(s.time_updated).toISOString()}  alive=${((s.time_updated - s.time_created) / 1000).toFixed(1)}s`);
}

console.log("\n\n===== first orchestrator-session messages =====");
const m = db.query<{ id: string; session_id: string; data: string; time_created: number }, []>(
  `WITH RECURSIVE st(id) AS (
     SELECT session_id FROM engine_task WHERE id='${TASK}'
     UNION ALL
     SELECT s.id FROM session s JOIN st ON s.parent_id = st.id
...
for (const row of m) {
  ...
  console.log(`${row.id} ses=${row.session_id} role=${d.role} agent=${d.agent} finish=${d.finish} t=${new Date(row.time_created).toISOString()}`);
  if (d.error || d.errorMessage) {
    console.log(`  error: ${typeof d.error === "string" ? d.error.slice(0, 600) : JSON.stringify(d.error).slice(0, 600)}`);
  }
}
