...
describe("frontend replica desktop-only generation scope", () => {
  test("expert squad forbids default tablet/mobile requirements, goals, and build work", async () => {
    ...
    expect(skill).toContain("default to one source component or meaningful source region per goal")
    expect(skill).toContain("Do not mix several source components")
