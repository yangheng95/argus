...
describe("document health audit regressions", () => {
  ...
  test("public GitHub Action examples match the generated workflow contract", () => {
    ...
    expect(Bun.YAML.parse(workflow).jobs.opencorvus.steps).toHaveLength(2)

    const actionReadme = read("github/README.md")
    expect(actionReadme).toContain("comments, issue or PR lifecycle events, scheduled workflows, and manual workflow dispatch events")
