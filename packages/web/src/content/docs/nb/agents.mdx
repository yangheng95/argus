...
Det er to typer agenter i Argus; primære agenter og underagenter.
...
Argus kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Argus kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Argus kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `argus.json`:
...
- Globalt: `~/.config/argus/agents/`
- Per prosjekt: `.argus/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Argus modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Argus-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Argus-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Argus Zen](/docs/zen), vil du bruke `argus/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `argus models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/yangheng95/argus).
:::
