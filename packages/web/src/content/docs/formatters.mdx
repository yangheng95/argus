...
Override or extend in [config](/docs/config/) under `formatter`:
...
Formatters run **after** a successful `write` / `edit` / `apply_patch` tool call but **before** the change is persisted to the snapshot. Failures fall through with a warning — they do not block the agent.
