...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Argus posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Argus.
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Argus lub gdy zmienna środowiskowa `OPENCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Argus:
