...
In Argus esistono due tipi di agenti: agenti primari e subagenti.
...
Argus include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Argus include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Argus include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `argus.json`:
...
- Globale: `~/.config/argus/agents/`
- Per progetto: `.argus/agents/`
...
Se non specifichi una temperature, Argus usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Argus sia per la config specifica del progetto.
...
L'ID modello nella configurazione Argus usa il formato `provider/model-id`. Per esempio, se stai usando [Argus Zen](/docs/zen), useresti `argus/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `argus models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/yangheng95/argus).
:::
