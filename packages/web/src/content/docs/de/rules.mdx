---
title: Regeln
description: Lege eigene Anweisungen fuer opencorvus fest.
---

Du kannst opencorvus ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in opencorvus den Befehl `/init` aus.
...
So kann opencorvus sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht opencorvus sie zu erweitern.
...
opencorvus kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/opencorvus/AGENTS.md` speichern.
Diese gelten in allen opencorvus-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenCorvus dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/opencorvus/AGENTS.md` existiert)
...
Beim Start sucht opencorvus in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/opencorvus/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `opencorvus.json` oder in `~/.config/opencorvus/opencorvus.json` angeben.
...
opencorvus parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit opencorvus.json

Empfohlen ist das `instructions`-Feld in der `opencorvus.json`:
...
Du kannst opencorvus explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `opencorvus.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
