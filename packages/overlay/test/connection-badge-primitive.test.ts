...
test("ConnectionBadge titlebar CSS composes with the shared Button focus ring", () => {
  ...
  expect(css).toMatch(/\.oc-button\[data-ui="connection-badge"\]\.conn-badge\s*\{[\s\S]*?--oc-button-height:\s*auto;/)
  expect(css).toMatch(
    /\.oc-button\[data-ui="connection-badge"\]\.conn-badge\s*\{[\s\S]*?--oc-button-bg:\s*color-mix\(in srgb, var\(--surface-strong\) 84%, transparent\);/,
  )
  expect(css).toMatch(
    /\.conn-badge:hover,\s*\.conn-badge:focus-visible\s*\{[\s\S]*?--oc-button-bg:\s*var\(--surface-hover\);/,
  )
