...
- WuJiang OpenTest lives under `.opencorvus/expert-squads/wujiang/opentest/` with manifest `id = "opentest"`.
...
- Existing OpenTest protocol remains external and project runtime artifacts stay under `.opencorvus/opentest/`, not `.opentest/`.
...
The active profile ID remains the manifest `id`. For example, WuJiang OpenTest is selected by:
...
The namespace is not part of package refs. README `expert_squad_display_prefix` remains the display source. Builtin packages keep `Builtin`; OpenTest uses `WuJiang`.
...
- Kept expert-squad identity manifest-ID based. OpenTest is selected as `opentest`, not `wujiang/opentest`; `software-testing` has no runtime alias.
