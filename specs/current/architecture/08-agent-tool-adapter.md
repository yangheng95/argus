...
| integrity       | include | `INTEGRITY_DECLARED_TOOL_IDS`（preview repair tools + canonical \`skill\`）                                                                               | **走** ToolRegistry；mounted skill visibility uses the same canonical skill surface as other specialists, and专用 integrity team 的内部审查工具由 team runtime 注入 |
