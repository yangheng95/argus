---
title: Tjener
description: Samhandle med Argus-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `argus serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Argus-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `argus.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `argus` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `argus serve` og `argus web`.
...
Når du kjører `argus` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Argus-serveren til å samhandle med Argus programmatisk.
:::

Denne arkitekturen lar Argus støtte flere klienter og lar deg samhandle med Argus programmatisk.

Du kan kjøre `argus serve` for å starte en frittstående server. Hvis du har
Argus TUI kjører, vil `argus serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Argus [IDE](/docs/ide) plugins.
...
argus-serveren viser følgende APIer.
