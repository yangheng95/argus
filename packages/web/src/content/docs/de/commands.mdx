...
Erstellen Sie `.argus/commands/test.md`:

```md title=".argus/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Argus-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Argus [config](/docs/config):

```json title="argus.jsonc" {4-12}
{
...
- Global: `~/.config/argus/commands/`
- Pro Projekt: `.argus/commands/`

```markdown title="~/.config/argus/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".argus/commands/component.md"
---
...
```md title=".argus/commands/create-file.md"
---
...
```md title=".argus/commands/analyze-coverage.md"
---
...
```md title=".argus/commands/review-changes.md"
---
...
```md title=".argus/commands/review-component.md"
---
...
Argus enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
