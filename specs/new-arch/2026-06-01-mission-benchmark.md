...
| Codex benchmark audit, 2026-06-15 | `rg "skip-local-verify|skipLocalVerify|skippedVerify|localVerify|not_run|DEFAULT_MISSION_VERIFY_CMD|evaluateMissionBenchmarkReport" packages/opencorvus/script/benchmark packages/opencorvus/test/benchmark docs specs/new-arch/2026-06-01-mission-benchmark.md` showed `--skip-local-verify` existed only in the mission benchmark script/docs and `not_run` was not rejected by `evaluateMissionBenchmarkReport()`. | Remove `--skip-local-verify`; local verification must run and complete. Missing or skipped local verification rejects the report. |
