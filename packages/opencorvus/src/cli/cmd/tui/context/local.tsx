...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const agent = iife(() => {
      ...
      const agents = createMemo(() => selectableAgents().filter((x) => !x.hidden))
      ...
      return {
        ...
        current() {
          ...
          if (!current) throw new Error("Agent list is not ready")
          return current
        },
        set(name: string) {
    ...
    const model = iife(() => {
      ...
      return {
        ...
        cycle(direction: 1 | -1) {
          ...
          setModelStore("model", selectedAgent.name, { ...val })
        },
        cycleFavorite(direction: 1 | -1) {
          ...
          const selectedAgent = agent.selected()
          if (!selectedAgent) return
          setModelStore("model", selectedAgent.name, { ...next })
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            ...
            const selectedAgent = agent.selected()
            if (!selectedAgent) return
            setModelStore("model", selectedAgent.name, model)
            if (options?.recent) {
    ...
    const result = {
      ...
      mcp,
      get ready() {
        return sync.ready && !!agent.selected()
      },
    }
    return result
  },
