...
function renderPackageJson(packageName: string): string {
  return `${JSON.stringify({
    ...
    type: "module",
    scripts: {
      dev: "node scripts/extract-source-html.mjs && vite --host 127.0.0.1",
      build: "node scripts/extract-source-html.mjs && vite build",
...
function renderReadme(input: {
  ...
  return [
    ...
    "- Do copy/adapt this baseline into the root app before semantic replacement; treating it as reference-only causes a blank freehand rebuild.",
    "- Follow frontend_design `quality_project_contract` to create readable semantic source.",
    "- Do not restart from a blank page.",
    "- Preserve this extracted DOM/CSS baseline until a replacement component passes screenshot comparison.",
    ...
    "- Do not hand-roll complex charts, maps, tables, menus, dialogs, forms, calendars, virtualized lists, or drag/drop behavior when a reusable component or mature library fits.",
    "- Replace content by slot/component boundaries, not by freehand redesign.",
