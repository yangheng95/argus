...
//
// Pin: ONE solo top-level `.task-row-mini:hover` /
// `:focus-within` rule; ONE solo top-level
// `.task-row-mini[data-active="true"]` rule. Theme-scoped
// overrides keep their per-theme palettes (background +
...
describe(".task-row-mini pseudo-state base rules are single source", () => {
  test(":hover base rule appears in exactly one top-level block", () => {
