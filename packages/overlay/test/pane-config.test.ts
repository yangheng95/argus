...
test("default pane resizer exposes separator semantics through the pane service", () => {
  ...
  expect(pane).toContain("function resolveSidebarWidthFromGeometry")
...
test("pane layout writes and handle semantics run in separate frame phases", () => {
  ...
  expect(readPaneGeometryFunction).toContain("const layoutTokens = createLayoutTokenResolver()")
  expect(readPaneGeometryFunction).toContain("config.remainingMinWidth(layoutTokens)")
  expect(readPaneGeometryFunction).toContain('layoutTokens.tokenPx("--ui-rail-min-width")')
  expect(readPaneGeometryFunction).toContain("defaultRailWidth(config, layoutTokens)")
  expect(readPaneGeometryFunction).not.toContain("layoutTokenPx(")
...
test("default pane layout has no retired right pane width source", () => {
  const pane = readSrc("services/pane.ts")
  expect(pane).toContain("const body = document.getElementById(config.bodyId)")
  expect(pane).toContain("function paneResizeBounds")
  expect(pane).toContain("const geometry = readPaneGeometrySnapshot(config)")
  expect(pane).toContain("const max = Math.max(geometry.railMin, paneSidebarMax(geometry))")
  expect(pane).toContain("handleWidth: Math.round(geometry.leftHandle)")
  ...
  expect(pane).not.toContain("paneHandleEnabled")
})
...
test("center workbench panel weights persist through the existing settings source", () => {
  ...
  expect(main).toContain("const range = centerWorkbenchResizeRange(totalWidth, minWidth)")
  expect(main).toContain("clampCenterWorkbenchResizeWidth(metrics.range, rawLeftWidth)")
  expect(main).toContain("const max = Math.round(metrics.range.maxWidth)")
  expect(main).toContain("leftWidth = metrics.range.maxWidth")
  expect(main).not.toContain("metrics.totalWidth - metrics.minWidth")
  expect(main).not.toContain("metrics.totalWidth, metrics.minWidth")
