...
Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenCorvus sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencorvus`).
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenCorvus via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencorvus`).
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencorvus`) |
