...
| --- | --- | --- |
| `rg -n "file-explorer-(new-menu|rename|move|delete|refresh|command-surface|selection)" packages/overlay/src packages/overlay/test specs` | The only product owner is `FileExplorerPanel`; CSS and browser/static tests still assert the visible toolbar. | Remove toolbar markup and CSS. Update tests to assert only context-menu management entries. |
