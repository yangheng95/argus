...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencorvus sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencorvus`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencorvus através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencorvus`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencorvus`) |
