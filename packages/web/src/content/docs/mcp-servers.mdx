...
You can add external tools to Argus using the _Model Context Protocol_, or MCP. Argus supports both local and remote servers.
...
You can define MCP servers in your [Argus Config](https://argus.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/argus` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Argus automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Argus will:
...
If the server requires authentication, Argus will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `argus mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Argus will store the tokens securely in `~/.local/share/argus/mcp-auth.json`.
...
Your MCPs are available as tools in Argus, alongside built-in tools. So you can manage them through the Argus config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Argus to your Sentry account.
