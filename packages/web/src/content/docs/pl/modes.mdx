...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji opencorvus.
...
te lub skonfiguruj własne za pomocą konfiguracji opencorvus.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.opencorvus/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `opencorvus.json`:
...
- Globalnie: `~/.config/opencorvus/modes/`
...
Jeśli nie ma wpływu na temperaturę, opencorvus stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna opencorvus, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.opencorvus/modes/` dla trybów zapisanych dla projektu lub `~/.config/opencorvus/modes/` dla trybów globalnych:
