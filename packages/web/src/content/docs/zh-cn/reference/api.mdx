...
| POST | `/global/db/mysql/import` | Import MySQL transfer snapshot | `global.db.mysql.import` |
...
| GET | `/global/projects/discover` | Discover local OpenCorvus projects | `global.projects.discover` |
...
| POST | `/config/proxy/test` | Test network proxy | `config.proxy.test` |
...
| DELETE | `/project/current` | Delete current project | `project.current.delete` |
| GET | `/project/current` | Get current project | `project.current` |
| PATCH | `/project/current` | Update current project | `project.current.update` |
| GET | `/project/current/cleanup-candidates` | Inspect current project cleanup candidates | `project.current.cleanupCandidates` |
| POST | `/project/current/init-git` | Initialize git in current directory | `project.current.initGit` |
| DELETE | `/project/current/worktrees` | Delete a current project worktree | `project.current.worktrees.delete` |
| GET | `/project/current/worktrees` | List current project worktrees | `project.current.worktrees` |
...
| GET | `/mission/{missionID}/project-archive` | Download Mission project archive | `mission.projectArchive` |
| GET | `/mission/{missionID}/status` | Get Mission status | `mission.status` |
...
| POST | `/task/{taskID}/browser-preview/compare` | Compare browser preview regions against source visual evidence | `browserPreview.compareTaskTargetRegions` |
| GET | `/task/{taskID}/browser-preview/evidence/{evidenceID}` | Read browser preview verification evidence | `browserPreview.readTaskEvidence` |
| GET | `/task/{taskID}/browser-preview/evidence/{evidenceID}/artifact/{artifactName}` | Read browser preview region comparison artifact | `browserPreview.readTaskEvidenceArtifact` |
| GET | `/task/{taskID}/browser-preview/evidence/{evidenceID}/capture.png` | Read browser preview evidence screenshot | `browserPreview.readTaskEvidenceCapture` |
| POST | `/task/{taskID}/browser-preview/live/input` | Send input to interactive browser preview | `browserPreview.liveInput` |
| POST | `/task/{taskID}/browser-preview/live/snapshot` | Capture interactive browser preview snapshot | `browserPreview.liveSnapshot` |
| PUT | `/task/{taskID}/browser-preview/target` | Select task browser preview target | `browserPreview.selectTaskTarget` |
...
| GET | `/task/{taskID}/operator-model-context` | Get task operator model context | `task.operatorModelContext` |
...
| POST | `/task/{taskID}/rewind` | Rewind task timeline; optionally also reset goal worktree files | `task.rewind` |
| POST | `/task/{taskID}/rewind/clear` | Clear the rewind cursor (visibility only; will not unrevert any reset worktree files) | `task.clearRewindCursor` |
...
| GET | `/task/{taskID}/status` | Get task status | `task.status` |
...
| DELETE | `/coding/session/{sessionID}` | Delete right sidebar coding assistant session | `coding.session.delete` |
| GET | `/coding/session/{sessionID}` | Claim right sidebar coding assistant session | `coding.session.get` |
| PATCH | `/coding/session/{sessionID}` | Update right sidebar coding assistant session | `coding.session.update` |
| POST | `/coding/session/{sessionID}/abort` | Abort right sidebar coding assistant session | `coding.session.abort` |
...
| POST | `/skill/import-and-mount` | Import a dropped skill and mount it to an agent | `skill.importAndMount` |
| POST | `/skill/import-file` | Import a dropped skill source | `skill.importFile` |
...
| POST | `/skill/mount` | Mount a skill to an agent | `skill.mount` |
...
| POST | `/skill/unmount` | Unmount a skill from an agent | `skill.unmount` |
...
| Method | 路径 | 说明 | operationId |
|---|---|---|---|
| POST | `/api/v1/notes` | Create QuickNote | `quicknote.create` |
...
| DELETE | `/file/item` | Delete file item | `file.delete` |
| PATCH | `/file/item` | Move file item | `file.move` |
| POST | `/file/item` | Create file item | `file.create` |
| GET | `/file/status` | Get file status | `file.status` |
| POST | `/file/upload` | Upload files | `file.upload` |
