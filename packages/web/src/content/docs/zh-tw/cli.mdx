---
title: CLI
description: Argus CLI 選項和指令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Argus CLI 在不帶任何參數執行時，預設啟動 [TUI](/docs/tui)。

```bash
argus
```

但它也接受本頁面中記錄的指令，使您可以透過程式方式與 Argus 進行互動。
...
將終端機連接到已透過 `serve` 或 `web` 指令啟動的 Argus 後端伺服器。

```bash
argus attach [url]
```

這允許將 TUI 與遠端 Argus 後端搭配使用。例如：
...
Argus 基於 [Models.dev](https://models.dev) 的供應商列表運作，因此您可以使用 `argus auth login` 為任何想要使用的供應商設定 API 金鑰。金鑰儲存在 `~/.local/share/argus/auth.json` 中。

```bash
argus auth login
```

Argus 啟動時會從憑證檔案載入供應商資訊，同時也會載入環境變數或專案中 `.env` 檔案中定義的金鑰。
...
使用 `--refresh` 旗標可以更新快取的模型列表。當供應商新增了模型並且您希望在 Argus 中看到它們時，此功能非常有用。
...
您還可以連接到正在執行的 `argus serve` 實例，以避免每次執行時 MCP 伺服器的冷啟動時間：
...
| `--attach`   |      | 連接到正在執行的 argus 伺服器（例如 http://localhost:4096） |
...
啟動無介面的 Argus 伺服器以提供 API 存取。查看[伺服器文件](/docs/server)了解完整的 HTTP 介面。

```bash
argus serve
```

此指令啟動一個 HTTP 伺服器，提供對 Argus 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `argus`）。
...
顯示 Argus 工作階段的 Token 用量和費用統計資訊。
...
從 JSON 檔案或 Argus 分享連結匯入工作階段資料。

```bash
argus import <file>
```

您可以從本地檔案或 Argus 分享連結匯入。
...
啟動帶有 Web 介面的無介面 Argus 伺服器。

```bash
argus web
```

此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 Argus。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `argus`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `argus`） |
