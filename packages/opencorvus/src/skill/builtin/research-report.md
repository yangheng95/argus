...
5. **Synthesise and write the report.** Produce a single markdown file at the path the goal specifies (typically `report.md` at the worktree root, or whatever `owned_paths` declares). The report should contain whatever sections the brief calls for; a comparison-shaped brief typically wants:
