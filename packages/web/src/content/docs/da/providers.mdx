---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Argus-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/argus/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Argus-konfigurationen.
...
Argus Zen er en liste over modeller leveret af Argus-teamet, som er blevet testet og verificeret til at fungere godt med Argus. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Argus Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg argus og gå til [argus.ai/auth](https://argus.ai/auth).
...
Det fungerer som alle andre udbydere i Argus og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med Argus:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører argus:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `argus.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Argus understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at argus skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem argus-konfigurationen.
...
gemme tokenet i argus auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Argus bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Argus
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`argus.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `argus.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Argus:
...
Sådan bruger du Google Vertex AI med Argus:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem argus, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) plugin for automatisk at logge hver Argus-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Argus-samtale opført som en separat session.
...
Du kan konfigurere argus til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere argus til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere argus til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Argus. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/argus) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Argus:
...
3. Kopier API-nøglen til brug i Argus.
...
6. **Vigtigt**: Før du bruger skymodeller i Argus, skal du hente modelinformationen lokalt:
...
Argus Zen er en liste over testede og verificerede modeller leveret af Argus-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Argus Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Argus Zen**.

   ```txt
   /connect
   ```

3. Indtast Argus API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem argus-konfigurationen.
...
5. Du kan også tilpasse dem gennem argus-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Argus:
...
Du kan også tilpasse modeller gennem argus-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med argus. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `argus.json`-filen i projektkataloget:
...
`limit`-felterne lader Argus forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `argus auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek argus-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i argus-konfigurationen.
