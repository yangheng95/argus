---
title: SDK
description: Client JS de type sécurisé pour le serveur opencorvus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Le SDK opencorvus JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler opencorvus par programme.
...
Créez une instance de opencorvus :

```javascript
import { createOpenCorvus } from "@opencorvus-ai/sdk/v2"

const { client } = await createOpenCorvus()
```
...
| `port`     | `number`      | Port du serveur                                    | `7878`      |
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `opencorvus.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpenCorvus } from "@opencorvus-ai/sdk/v2"

const opencorvus = await createOpenCorvus({
  hostname: "127.0.0.1",
  port: 7878,
  config: {
...
console.log(`Server running at ${opencorvus.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de opencorvus, vous pouvez créer une instance client pour vous y connecter :
...
| Options         | Tapez      | Descriptif                                   | Par défaut              |
| --------------- | ---------- | -------------------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | URL du serveur                               | `http://localhost:7878` |
...
```typescript
import type { Session, Message, Part } from "@opencorvus-ai/sdk/v2"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*opencorvus" },
})
