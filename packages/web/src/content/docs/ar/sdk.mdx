...
description: عميل JavaScript آمن الأنواع لخادم opencorvus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

توفر SDK الخاصة بـ opencorvus لـ JS/TS عميلا آمنا للأنواع للتفاعل مع الخادم.
استخدمها لبناء التكاملات والتحكم في opencorvus برمجيا.
...
يمكنك تمرير كائن إعدادات لتخصيص السلوك. سيستمر المثيل في التقاط `opencorvus.json` لديك، لكن يمكنك تجاوز الإعدادات أو إضافة إعدادات مباشرة:
...
إذا كان لديك مثيل opencorvus يعمل بالفعل، يمكنك إنشاء مثيل عميل للاتصال به:
...
| الخيار          | النوع      | الوصف                             | الافتراضي               |
| --------------- | ---------- | --------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | عنوان URL للخادم                  | `http://localhost:7878` |
