 ...
 *
 * Design: specs/new-arch/2026-04-30-llm-activity-redesign.md
 *
 * Composition:
...
export interface LLMActivityContext {
  sessionID: string
  /** Set only when this activity is build-scoped — used by step-4 to
   *  forward terminal failed/aborted into engine_artifact attempt-{failed,aborted}. */
  goalRunID?: string
  provider: string
  model: string
}
...
export async function withLLMActivity<T>(
  ...
  // External signal proxy — converts external abort into a marked reason so
  // the rest of the runner uses one uniform reason shape.
  const externalProxy = new AbortController()
  const onExternal = () => {
    if (externalProxy.signal.aborted) return
    externalProxy.abort(makeAbortReason("external_abort", "external abort signal fired"))
  }
