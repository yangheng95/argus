...
test("coding assistant activity selects an independent session in the shared message panel", () => {
  ...
  expect(main).toContain('type CenterWorkbenchTab = "explorer" | "diff" | "browser" | "assistant" | "file"')
  expect(main).toContain('{ id: "assistant", icon: "message", labelKey: "coding_assistant.title" }')
  expect(main).toContain('assistant: "coding_assistant.title"')
  expect(main).toContain('assistant: "message"')
  expect(main).toContain('selectCodingAssistantSession()')
  expect(main).toContain('document.getElementById("solidCodingAssistantMount")')
  expect(main).toContain("<WorkspaceCodingCliLaunchers")
  expect(main).toContain('assistant: document.getElementById("centerWorkbenchAssistant")')
  expect(service).toContain('apiJson("coding/sessions?limit=1")')
