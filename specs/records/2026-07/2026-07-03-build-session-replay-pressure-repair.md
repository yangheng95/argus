...
| Hard constraints | No fallback/compatibility path; no host state-machine gate for frontend evidence; no blind patch; no git reset; no new worktree; do not restart or interfere with OpenCorvus/overlay processes; specs live under root `specs/`; code changes require focused tests and final push to `legacy-remote`. |
