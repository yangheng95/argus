---
title: SDK
description: Typesikker JS-klient for OpenCorvus-server.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

OpenCorvus JS/TS SDK gir en typesikker klient for samhandling med serveren.
Bruk den til å bygge integrasjoner og kontrollere OpenCorvus programmatisk.
...
Opprett en forekomst av OpenCorvus:
...
Du kan sende et konfigurasjonsobjekt for å tilpasse virkemåten. Forekomsten henter fortsatt din `opencorvus.json`, men du kan overstyre eller legge til konfigurasjon inline:
...
Hvis du allerede har en kjørende forekomst av OpenCorvus, kan du opprette en klientforekomst for å koble til den:
...
| Alternativ      | Type       | Beskrivelse                       | Standard                |
| --------------- | ---------- | --------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | URL av serveren                   | `http://localhost:7878` |
