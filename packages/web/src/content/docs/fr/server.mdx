---
title: Serveur
description: Interagissez avec le serveur argus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `argus serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client argus peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `argus.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `argus`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `argus serve` et à `argus web`.
...
Lorsque vous exécutez `argus`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur argus pour interagir avec argus par programmation.
:::

Cette architecture permet à argus de prendre en charge plusieurs clients et vous permet d'interagir avec argus par programme.

Vous pouvez exécuter `argus serve` pour démarrer un serveur autonome. Si vous avez le
argus TUI en cours d'exécution, `argus serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Argus [IDE](/docs/ide).
...
Le serveur argus expose les API suivantes.
