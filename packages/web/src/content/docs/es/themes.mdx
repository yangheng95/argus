...
Con OpenCorvus puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenCorvus usa nuestro propio tema `opencorvus`.
...
OpenCorvus viene con varios temas integrados.
...
- Quiere que OpenCorvus coincida con la apariencia de su terminal
...
OpenCorvus admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/opencorvus/themes/*.json` o `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.opencorvus/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.opencorvus/themes/*.json`
