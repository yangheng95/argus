---
title: Proveedores
description: Usando cualquier proveedor LLM en OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración OpenCorvus.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu OpenCorvus
configuración.
...
OpenCorvus Zen es una lista de modelos proporcionados por el equipo OpenCorvus que han sido
probado y verificado para funcionar bien con OpenCorvus. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenCorvus Zen.
:::
...
Funciona como cualquier otro proveedor en OpenCorvus y su uso es completamente opcional.
...
Para usar Amazon Bedrock con OpenCorvus:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta opencorvus:
   ...
   Para una configuración persistente o específica del proyecto, utilice `opencorvus.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en OpenCorvus no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que opencorvus funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración opencorvus.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `OpenCorvus`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación opencorvus.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
OpenCorvus utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear OpenCorvus
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `opencorvus.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth).

##### Configuración

Personalizar a través de `opencorvus.json`:
...
Para utilizar su suscripción GitHub Copilot con opencorvus:
...
Para utilizar Google Vertex AI con OpenCorvus:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta opencorvus.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de opencorvus, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación OpenCorvus enumerada como una sesión separada.
...
Puede configurar opencorvus para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar opencorvus para usar modelos locales a través de LM Studio.
...
Puedes configurar opencorvus para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenCorvus. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencorvus) para obtener más detalles.
:::
...
Para usar Ollama Cloud con OpenCorvus:
...
3. Copie la clave API para usarla en OpenCorvus.
...
6. **Importante**: Antes de usar modelos en la nube en OpenCorvus, debe obtener la información del modelo localmente:
...
OpenCorvus Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenCorvus. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>OpenCorvus Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave OpenCorvus API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración opencorvus.
...
5. También puedes personalizarlos a través de tu configuración opencorvus. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con OpenCorvus:
...
También puede personalizar modelos a través de su configuración opencorvus. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con opencorvus. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `opencorvus.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a OpenCorvus comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `opencorvus auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración opencorvus y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración opencorvus.
