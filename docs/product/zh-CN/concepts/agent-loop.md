...
build / intent-analysis / requirements / architect / design-analyst / integrity / prosecutor / delivery 共用 `agent/sub-agent-protocol.ts` 的结构化输出契约（Zod tool call），结果写入 `engine_artifact`。
