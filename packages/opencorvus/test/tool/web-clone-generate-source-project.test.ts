  ...
  test("splits large generated DOM baselines into source-region components", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const sourceDomDirExists = await Bun.file(regionDir).exists()
        const regionFiles = sourceDomDirExists ? (await fs.readdir(regionDir)).filter((file) => file.endsWith(".tsx")) : []
        ...
        expect(sourceDomPage).not.toContain('from "./source-dom/')
        ...
        expect(regionFiles).toEqual([])
        expect(semanticFiles).toContain("OverviewCards.tsx")
        expect(semanticFiles).toContain("MarketsCards.tsx")
        expect(semanticFiles).toContain("IdeasCards.tsx")
        expect(semanticFiles).toContain("IndicatorsCards.tsx")
        expect(semanticFiles).toContain("NewsCards.tsx")
        expect(semanticFiles).toContain("CalendarCards.tsx")
        expect(semanticFiles).toContain("MapSurface.tsx")
        expect(semanticFiles).toContain("FrequentlyAskedQuestionsFAQ.tsx")
        expect(semanticFiles.length).toBe(8)
        const overviewCards = await Bun.file(path.join(semanticDir, "OverviewCards.tsx")).text()
        expect(overviewCards).toContain("semantic-source-replacement")
        expect(overviewCards).toContain("collection.cards.map")
        expect(overviewCards).toContain("Overview item 1")
        ...
        expect(sourceDomRegions).toContain("sourceDomRegions")
        expect(sourceDomReplacementPlan).toContain("sourceDomReplacementPlan")
        expect(sourceDomIterationState).toContain('"purpose": "source-region-maintainable-progress-metadata"')
        expect(sourceDomIterationState).toContain('"sourceMap"')
        expect(sourceDomIterationState).toContain('"bounds"')
        ...
        expect(sourceProjectManifest.sourceDomRegions.count).toBe(0)
        ...
        expect(sourceProjectManifest.sourceDomRegions.replacementPlanCount).toBe(0)
        expect(sourceProjectManifest.sourceDomRegions.semanticReplacementCount).toBe(8)
        expect(sourceProjectManifest.semanticReplacements.count).toBe(8)
