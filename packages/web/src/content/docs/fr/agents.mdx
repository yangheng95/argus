...
Il existe deux types d'agents dans OpenCorvus ; agents primaires et sous-agents.
...
OpenCorvus est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OpenCorvus est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

OpenCorvus est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `opencorvus.json` :
...
- Global : `~/.config/opencorvus/agents/`
- Par projet : `.opencorvus/agents/`

```markdown title="~/.config/opencorvus/agents/review.md"
---
...
Si aucune température n'est spécifiée, OpenCorvus utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OpenCorvus et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OpenCorvus utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenCorvus Zen](/docs/zen), vous utiliserez `opencorvus/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="opencorvus.json" {3-6,9-12}
{
  "$schema": "https://opencorvus.ai/config.json",
  "tools": {
    "write": true,
...
```json title="opencorvus.json" {3-5,8-10}
{
  "$schema": "https://opencorvus.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="opencorvus.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `opencorvus models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/yangheng95/opencorvus).
:::
...
```markdown title="~/.config/opencorvus/agents/docs-writer.md"
---
...
```markdown title="~/.config/opencorvus/agents/security-auditor.md"
---
