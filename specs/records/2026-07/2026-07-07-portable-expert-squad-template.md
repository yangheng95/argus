...
- Do not make OpenTest or other real external packages automatically gain undeclared runtime roles.
...
- `rg --files .opencorvus/expert-squads packages/opencorvus/src/expert-squad packages/opencorvus/test/expert-squad packages/opencorvus/test/server packages/overlay/test | Sort-Object`
  - Finding: OpenTest already uses `agents/orchestrator` plus role-scoped `virtual-agents/<role>` for projected lifecycle roles.
...
The proposed runtime union is not reasonable for real packages such as OpenTest.
...
Automatically releasing the union into OpenTest would reintroduce the bug repaired in `2026-07-07-dispatch-agent-projected-target-schema.md`: `dispatch_agent` would again expose workflow targets that OpenTest does not support, such as `analyze_intent`, because the package would have been silently widened.
...
- Do not change OpenTest's actual projected agent set.
