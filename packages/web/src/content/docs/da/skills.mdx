...
Agentferdigheter lar OpenCorvus oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
OpenCorvus søker etter disse stedene:

- Prosjektkonfigurasjon: `.opencorvus/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
For prosjektlokale stier går OpenCorvus opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.opencorvus/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.opencorvus/skills/git-release/SKILL.md` slik:
...
OpenCorvus viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `opencorvus.json`:
...
**For innebygde agenter** (i `opencorvus.json`):
