...
test(
  ...
  async () => {
    ...
    try {
      ...
      expect(await page.$('[data-ui="side-activity-button"][data-side="left"][data-activity="tasks"]')).not.toBeNull()
      ...
      expect(((await page.$eval('[data-editor="pycharm"]', (node) => node.textContent)) || "").trim()).toContain(
        "PyCharm",
      )
