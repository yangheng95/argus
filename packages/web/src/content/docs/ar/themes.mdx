...
مع Argus يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Argus سمتنا `argus`.
...
- يريدون أن يطابق Argus مظهر terminal لديهم
...
يدعم Argus نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/argus/themes/*.json` أو `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.argus/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.argus/themes/*.json`
