...
| ---------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ---------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `rg -n "pref-item      | pref-" packages/overlay/src packages/overlay/test specs docs --glob "_._"` | Production hits were CSS-only: `settings.css`, `field.css`, and `inspector.css`. Browser test `pref-*` hits are fixture ids, not classes. | Delete the CSS selector family and add production-source absence guards. |
...
| `rg -n "knowledge-item | market-card                                                                         | channel-doc-card                                                                                                                          | MemoryPanel                                                              | SkillMarketPanel | ChannelsPanel" packages/overlay/src packages/overlay/test specs docs --glob "_._"` | `.knowledge-item`, `.market-card`, and `.channel-doc-card` have live component owners. | Preserve those selectors and only remove `.pref-item*` entries from shared lists. |
