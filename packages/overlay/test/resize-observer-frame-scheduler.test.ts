...
test("ResizeObserver callbacks never run layout-affecting work synchronously", () => {
  ...
  expect(screenshotBrowser).toContain("createAnimationFrameScheduler(commitWidth)")
  expect(screenshotBrowser).toContain("new ResizeObserver((entries) =>")
  expect(screenshotBrowser).toContain("pendingWidth = entry.contentRect.width")
  ...
  expect(screenshotBrowser).not.toContain("element.clientWidth")
...
test("center workbench panel open schedules layout reads after DOM state writes", () => {
  ...
  const separatorRenderStart = main.indexOf("function renderCenterWorkbenchPanelSeparators(): void")
  const separatorRenderEnd = main.indexOf("function renderCenterWorkbenchPanelLayout(): void", separatorRenderStart)
  const layoutStart = main.indexOf("function renderCenterWorkbenchPanelLayout(): void")
  const layoutEnd = main.indexOf("function renderCenterWorkbenchPanelMeasurementsAndReveal(): void", layoutStart)
  const layoutRevealStart = layoutEnd
  ...
  expect(separatorRenderStart).toBeGreaterThan(0)
  expect(separatorRenderEnd).toBeGreaterThan(separatorRenderStart)
  ...
  const separatorRenderFunction = main.slice(separatorRenderStart, separatorRenderEnd)
  const layoutFunction = main.slice(layoutStart, layoutEnd)
  ...
  expect(main).toContain("function renderCenterWorkbenchPanelMeasurementsAndReveal(): void")
  expect(main).toContain("function readCenterWorkbenchPanelGeometrySnapshot(")
  expect(main).toContain("interface CenterWorkbenchPanelGeometrySnapshot")
  expect(main).toContain("rects: Partial<Record<CenterWorkbenchPanel, DOMRect>>")
  ...
  expect(layoutFunction).not.toContain("revealPendingCenterWorkbenchPanel()")
  expect(separatorRenderFunction).toContain("const geometry = readCenterWorkbenchPanelGeometrySnapshot(panels)")
  ...
  expect(settingsWeightEffect).toContain("renderCenterWorkbenchPanelLayoutOnFrame.schedule()")
