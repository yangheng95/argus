...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di argus. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in argus ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di argus.
...
argus include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.argus/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `argus.json`:
...
- Globale: `~/.config/argus/modes/`
- Progetto: `.argus/modes/`
...
Se non viene specificata alcuna temperatura, argus usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di argus sia per quella specifica del progetto.
...
Crea file di modalita in `.argus/modes/` per modalita specifiche del progetto oppure in `~/.config/argus/modes/` per modalita globali:
