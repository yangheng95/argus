...
```bash
bun run build:overlay   # produces .msi / .dmg / .deb
```
...
| Panel | Role | File |
|---|---|---|
| **Board** | Task overview: spec/plan/goals/eval/delivery states | `src/components/Board.tsx` |
...
2. **Global task-list stream** (app level)
   ...
   ```
   Code: `src/services/sse.ts:104` `startTaskListSSE()`.
...
`~/.opencorvus/overlay.json` stores UI preferences (theme, position, zoom, server URL override). The tray **Restart** menu restarts the backend process and reloads the frontend.
