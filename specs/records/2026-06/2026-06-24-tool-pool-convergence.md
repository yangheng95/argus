...
| `specs/current/architecture/08-agent-tool-adapter.md`                    | Current truth is `Agent.Info.tools` plus `ToolRegistry.tools(model, agent)` filtering; the doc admits include/exclude drift and old rows.           |
| `specs/records/2026-06/2026-06-24-agent-abstraction-convergence.md` | Agent abstractions are only `host` and `worker`; role-specific metadata belongs in a backend contract.                                              |
