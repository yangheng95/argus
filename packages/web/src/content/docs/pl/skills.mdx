...
Umiejętności agenta pozwalają opencorvus odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
opencorvus przeszukuje te lokalizacje:

- Project config: `.opencorvus/skills/<name>/SKILL.md`
- Global config: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu opencorvus przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.opencorvus/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.opencorvus/skills/git-release/SKILL.md` w ten sposób:
...
opencorvus wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `opencorvus.json`:
...
**Dla agentów wbudowanych** (w `opencorvus.json`):
