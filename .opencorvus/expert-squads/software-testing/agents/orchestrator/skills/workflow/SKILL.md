...
The OpenTest protocol is external to this skill. Read it by calling `software-testing/shared/opentest-protocol-engine` in `contract` mode. Discover artifacts through the same tool in `inventory` mode. Validate test cases through the same tool in `validate` mode.
