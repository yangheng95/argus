---
title: Custom Tools
description: Create tools the LLM can call in opencorvus.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside opencorvus's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.opencorvus/tools/` directory of your project.
- Or globally, by placing them in `~/.config/opencorvus/tools/`.
...
```ts title=".opencorvus/tools/bash.ts"
import { tool } from "@opencorvus-ai/plugin"
