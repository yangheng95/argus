...
| --- | --- | --- |
| `rg -n "task-dir-recent-trigger|cwd-recent-trigger|DropdownMenu\\.Trigger\\s+as=\\{Button\\}" packages/overlay/src packages/overlay/test specs/new-arch -g "*.*"` | The CWD recent trigger still used raw `class="task-dir-recent-trigger"` while the nearby worktree trigger already used `as={Button}`. | Migrate the recent trigger to the same primitive pattern. |
