...
| Hard constraints | Preserve packaging scripts and workflow artifact upload; do not delete local build outputs; do not add duplicate extension-by-extension ignore rules; commit subjects use `dsw-33987`; push the current main delivery branch to `myhexin`. |
...
4. Run the focused residue and document-health tests, prove `git check-ignore` resolves representative platform output, verify the local files still exist, commit, merge any new `myhexin` changes, and push.
