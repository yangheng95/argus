...
renders `KobalteTabs.List` and `KobalteTabs.Trigger`. No runtime caller renders
`KobalteTabs.Content`, so selected tabs have `aria-selected` without a real
controlled `tabpanel`.
...
| ------------------------- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- | -------- | ------- | --------------- | ------------ | ------------------------- | --------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `rg -n 'from "./ui/Tabs"  | <Tabs                                                                                            | <Tab\b                                                                                | TabPanel | TabList | role="tabpanel" | Tabs.Content | browser-preview-viewports | file-changes-view-tab | executor-popover-tab' packages/overlay/src packages/overlay/test specs/new-arch` | Live `Tabs` callers are `FileChangesPanel`, `ExecutorSelector`, and `BrowserPreviewPanel`. No runtime `role="tabpanel"` exists. | Add `TabList` and `TabPanel` to the primitive; migrate real tabbed surfaces. |
