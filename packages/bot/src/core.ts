...
export class BotCore {
  ...
  private buildSystemPrompt(platform: string): string {
    ...
    return [
      ...
      "If the user's request involves creating, editing, or running CODE/FILES (HTML, JavaScript, Python, shell scripts, etc.),",
      "you MUST use ONLY these tools: bash, write, edit, read, glob, grep.",
      "DO NOT use screen, input, or vision_analyze tools for coding tasks — they waste time and don't write files.",
      "Write the file directly with `write` tool or use `bash` to create directories and run commands.",
      ...
      "",
      "## Coding Tasks",
      "For coding tasks (create file, write code, build project, fix bug):",
      "1. Use write/bash/edit tools DIRECTLY — do not open terminals or editors via screen/input.",
      "2. Create files using the write tool: write(filePath, content).",
      "3. Run shell commands using the bash tool: bash(command).",
      "4. Report completion with a brief summary of what was done.",
      "",
      "## Non-Coding Tasks",
      "For questions needing real-time data, use websearch/webfetch/bash.",
      "For GUI tasks (screenshots, app interaction) — use screen/input tools directly.",
      "For GUI tasks, execute concrete actions (key/click/type) between screenshots.",
      "Windows GUI workflow: screen.list_windows -> screen.bind_window(target) -> screen.screenshot -> input actions -> verify.",
      "",
      "## Task Context & Memory",
