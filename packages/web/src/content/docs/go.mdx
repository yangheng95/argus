...
Argus Go is a low cost **$10/month** subscription that gives you reliable access to popular open coding models.

:::note
Argus Go is currently in beta.
:::

Go works like any other provider in Argus. You subscribe to Argus Go and
...
Argus Go gives you access to these models for **$10/month**.

---

## How it works

Argus Go works like any other provider in Argus.

1. You sign in to **<a href={console}>Argus Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Argus Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Argus Go.
:::
...
Argus Go includes the following limits:
...
Argus Go is a **$10/month** subscription plan. Below are the prices **per 1M tokens**.
...
| ------------ | ------------ | ------------------------------------------------ | --------------------------- |
| GLM-5        | glm-5        | `https://argus.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| Kimi K2.5    | kimi-k2.5    | `https://argus.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiniMax M2.5 | minimax-m2.5 | `https://argus.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |

The [model id](/docs/config/#models) in your Argus config
uses the format `argus-go/<model-id>`. For example, for Kimi K2.5, you would
use `argus-go/kimi-k2.5` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Argus as well.
