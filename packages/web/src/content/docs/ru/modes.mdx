...
:::caution
Режимы теперь настраиваются с помощью опции `agent` в конфигурации opencorvus.
Опция `mode` устарела. [Подробнее ](/docs/agents).
:::

Режимы в opencorvus позволяют настраивать поведение, инструменты и подсказки для различных вариантов использования.

Он имеет два встроенных режима: **сборка** и **планирование**. Вы можете настроить
эти или настройте свои собственные через конфигурацию opencorvus.
...
opencorvus имеет два встроенных режима.
...
- `edit` – невозможно изменить существующие файлы, за исключением файлов, расположенных по адресу `.opencorvus/plans/*.md`, для детализации самого плана.
...
Настройте режимы в файле конфигурации `opencorvus.json`:
...
- Глобальный: `~/.config/opencorvus/modes/`
- Проект: `.opencorvus/modes/`
...
Если температура не указана, opencorvus использует значения по умолчанию для конкретной модели (обычно 0 для большинства моделей, 0,55 для моделей Qwen).
...
как глобальная конфигурация opencorvus, так и конфигурация конкретного проекта.
...
Создайте файлы режимов в `.opencorvus/modes/` для режимов, специфичных для проекта, или в `~/.config/opencorvus/modes/` для глобальных режимов:
