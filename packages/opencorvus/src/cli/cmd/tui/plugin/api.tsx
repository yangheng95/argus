// Copied from OpenCode's TUI plugin API host and adapted to OpenCorvus SDK/state contracts.
import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@opencorvus-ai/plugin/tui"
...
import type { useOpencorvusKeymap } from "../keymap"
...
type EventBus = {
  on: TuiPluginApi["event"]["on"]
}

type Input = {
  tuiConfig: TuiConfig.Resolved
  dialog: ReturnType<typeof useDialog>
  keymap: ReturnType<typeof useOpencorvusKeymap>
  ...
  if (route.data.type === "session") {
    return {
      ...
      params: {
        ...
        prompt: route.data.initialPrompt,
      },
    }
  }
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getOpencorvusModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOpencorvusModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
    ...
    theme: {
      ...
      has(name) {
        return Object.prototype.hasOwnProperty.call(input.theme.all(), name)
      },
      set(name) {
