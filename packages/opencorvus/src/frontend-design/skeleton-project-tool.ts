      ...
      execute: async (params) => {
        ...
        return {
          ...
          output: [
            ...
            "9. After each target-project region replacement, run build/audit/render evidence against the target project: use `webpage_render`, compare against `reference.png` with `webpage_evaluate`, call `webpage_vision_judge` for visible theme/layout/density/control mismatches, repair the same region from source evidence, then record the replacement result.",
          ].join("\n"),
          metadata: result,
        }
      },
