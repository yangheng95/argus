...
- 替代：已删除的 15 号无状态机拆除草案（该草案已并入本文，以本文为准）
...
>   两处 FSM-shaped residue 是已并入本文的 15 号拆除草案尾巴。
...
## 12. 与已删除 15 号无状态机草案的关系

15 号无状态机草案仅砍状态字段保留实体表，是本方案的子集且走了弯路（先砍字段再砍表，第二次返工扩大成本）。**本文取代 15**，阶段 1 的落地动作与 15 轮 1 一致以保证"先止血"立即可做，后续阶段直接跨到表级删除。
