---
title: Configuración
description: Usando la configuración OpenCorvus JSON.
---

Puede configurar OpenCorvus usando un archivo de configuración JSON.

---

## Formato

OpenCorvus admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Por ejemplo, si su configuración global establece `theme: "opencorvus"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/opencorvus`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/opencorvus/opencorvus.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCORVUS_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`opencorvus.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.opencorvus`** - agentes, comandos, complementos
6. **Configuración en línea** (`OPENCORVUS_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.opencorvus` y `~/.config/opencorvus` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/opencorvus`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global OpenCorvus en `~/.config/opencorvus/opencorvus.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `opencorvus.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia OpenCorvus, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `OPENCORVUS_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `OPENCORVUS_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.opencorvus`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.opencorvus`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`opencorvus.ai/config.json`**](https://opencorvus.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `opencorvus serve` y `opencorvus web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor OpenCorvus.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `opencorvus.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración OpenCorvus a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, OpenCorvus intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración OpenCorvus a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencorvus/agents/` o `.opencorvus/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, OpenCorvus recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`opencorvus run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencorvus/commands/` o `.opencorvus/commands/`. [Más información aquí](/docs/commands).
...
OpenCorvus descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, opencorvus **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían OpenCorvus con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.opencorvus/plugins/` o `~/.config/opencorvus/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir OpenCorvus para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
