...
| `rg -n "<select           | <option                   | select\\.field-input              | custom-select" packages/overlay/src packages/overlay/test specs/new-arch -S`                | No overlay component uses native `<select>` or `<option>`. Only legacy CSS remains in `field.css`.                        | Remove the unused native select/custom-select CSS so it cannot become a second styling source. |
