...
export namespace Provider {
  ...
  export type RefreshModelsResult =
    | {
        ok: true
        fetchedAt: number
        providers: Array<{ providerID: "hexin"; count: number; ids: string[] }>
      }
  ...
  export async function refreshModels(config?: Config.Info): Promise<RefreshModelsResult> {
    try {
      ...
      if (!apiKey) return { ok: true, fetchedAt: Date.now(), providers: [] }
      ...
      return {
        ok: true,
        fetchedAt: Date.now(),
        providers: [{ providerID: "hexin", count: ids.length, ids }],
      }
    } catch (error) {
  ...
  async function getSDK(model: Model, opts?: { config?: Config.Info; state?: Promise<ProviderState> }) {
    try {
      ...
      options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
        ...
        const fetchFn =
          customFetch ?? (model.providerID === "hexin" && proxyUrl === undefined ? fetchHexinEndpoint : fetch)
        const opts = init ?? {}
