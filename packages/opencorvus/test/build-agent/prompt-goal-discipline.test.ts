...
describe("build agent goal execution discipline prompt", () => {
  ...
  test("requires observed frontend verification and random preview ports", async () => {
    ...
    expect(normalized).toContain("`reference-comparison` artifacts")
    expect(normalized).toContain("route-health failure")
    expect(normalized).toContain("source/local true-size side-by-side image attachments")
    expect(normalized).toContain("diff attachments only when the true crop dimensions match")
    expect(normalized).toContain("including failed/mismatched regions")
    expect(normalized).toContain("inspect those images and repair the implementation")
    expect(normalized).toContain("Viewport width differences are not normalized away by the comparison runner")
    expect(normalized).toContain("size mismatch is a reference-parity mismatch")
    ...
    expect(normalized).toContain("report the exact evidence gap or blocker instead of accepting an unbound screenshot review")
