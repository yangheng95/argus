...
W OpenCorvus obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OpenCorvus zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OpenCorvus ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

OpenCorvus ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `opencorvus.json`:
...
- Globalnie: `~/.config/opencorvus/agents/`
- W projekcie: `.opencorvus/agents/`
...
Jeśli nie zostanie skonfigurowana, OpenCorvus używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OpenCorvus, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OpenCorvus w `provider/model-id`. Na przykład, [OpenCorvus Zen](/docs/zen), użyj `opencorvus/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `opencorvus models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/yangheng95/opencorvus).
:::
