---
title: Plugins
description: Escreva seus próprios plugins para estender o opencorvus.
---

Plugins permitem que você estenda o opencorvus conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do opencorvus.
...
- `.opencorvus/plugins/` - Plugins em nível de projeto
- `~/.config/opencorvus/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/opencorvus/node_modules/`.
...
1. Configuração global (`~/.config/opencorvus/opencorvus.json`)
2. Configuração do projeto (`opencorvus.json`)
3. Diretório de plugins global (`~/.config/opencorvus/plugins/`)
4. Diretório de plugins do projeto (`.opencorvus/plugins/`)
...
O opencorvus executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do opencorvus para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o opencorvus.
...
:::note
Se você estiver usando o aplicativo desktop opencorvus, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o opencorvus de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao opencorvus:
...
O helper `tool` cria uma ferramenta personalizada que o opencorvus pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o opencorvus junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://opencorvus.ai/docs/sdk) para detalhes.
