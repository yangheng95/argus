...
- **阶段 3-b**：普通 stage agent 迁移：`intent-analysis → design-analyst → requirements → planner → deliver → orchestrator`。每个 agent 独立 PR：
  ...
  **完成状态**：
  ...
  - [ ] **deliver**（pending — 规模大）：`delivery/output-tools.ts` 的 `submit_verdict` 含 ~180 行跨字段 self-correction 校验（accept↔startup/evidence/frontend_check/rejection_details 一致性），不是纯 schema 层。迁移路径：把 `DeliveryVerdict` zod schema 加 `.refine()` 把所有跨字段校验搬进去 → LLM 通过 AI-SDK 的 tool-input-validation 迭代修正。agent.ts 还有 MAX_RETRIES 重试循环（原因：LLM 跳过 submit_verdict），迁移后改由 StructuredOutput 的 `retryCount` 承担。

  **迁移难点（实际代码核查后）**：
    1. `format.retryCount` 当前**未被 SessionLoop 消费**（loop.ts 查过；仅记录在 Message.Format schema），因此 "LLM 跳过 StructuredOutput" 这条路径无内置 retry —— 需要在 agent 层外包重试循环（类似当前 MAX_RETRIES 的 wrapper，但围 SessionPrompt.prompt 重试）。
