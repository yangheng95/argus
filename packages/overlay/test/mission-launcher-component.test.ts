...
test("Mission ledger renders mission-created task projections as task selectors", () => {
  ...
  expect(MISSION_LIST_TSX).toMatch(/<Button[\s\S]*data-ui="mission-task-projection-select"/)
  expect(MISSION_LIST_TSX).toContain('variant="ghost"')
  expect(MISSION_LIST_TSX).toContain('size="md"')
  expect(MISSION_LIST_TSX).toContain('tone="neutral"')
  expect(MISSION_LIST_TSX).not.toMatch(/<button[\s\S]*data-ui="mission-task-projection-select"/)
  expect(MISSION_LIST_TSX).not.toContain('class="task-row-main mission-task-projection-select"')
...
test("Mission ledger is embedded in the left task panel without its retired panel header", () => {
  ...
  expect(MISSION_LIST_TSX).not.toContain('class="mission-row-rename-input"')
  ...
  expect(MISSION_CSS).not.toContain(".mission-row-rename-input")
...
test("Mission task projection selection explicitly rebinds the left toolbar and center panel to Tasks", () => {
  ...
  expect(block).toContain("directory?: string")
