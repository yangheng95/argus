...
_Model Bağlam Protokolü_ veya MCP kullanarak argus'a harici araçlar ekleyebilirsiniz. argus hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [argus Config](https://argus.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/argus` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
argus, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde argus şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa argus, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `argus mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından argus, belirteçleri `~/.local/share/argus/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra argus'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi argus yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve argus'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
