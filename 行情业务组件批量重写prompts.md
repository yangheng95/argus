...
```text
使用 workflow搭配skill 帮我将 C# 的 KeyStatistics 关键数据组件转写成 TS/React 组件。C# 组件目录：Hithink.PrefabLibrary\PrefabLibrary\PrefabLibrary\Business\KeyStatistics。重写后的 TS 组件放在：src\web\src\components\composite\KeyStatisticsMTts。你需要全面调查C#代码和已有TS代码，确保不丢失任何属性，交互和功能细节，例如API和鼠标右键功能。基于现有API（API无法连接则提供mock数据）和复用组件（除非没有可以复用的组件），确保真实场景下接入API完美还原。
```
