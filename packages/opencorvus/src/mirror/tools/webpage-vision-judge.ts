...
Pure transformation, no network besides the LLM call. Deterministic per (model, reference, rendered) triple.`,
  parameters: z.object({
    reference: z
      .string()
      .describe(
        "Path to the reference PNG (defaults to `reference.png` inside the mirror dir).",
      )
      .optional(),
    rendered: z
      .string()
      .describe("Path to the rendered PNG (defaults to `rendered.png` inside the mirror dir).")
      .optional(),
    outputDir: z
      .string()
      .describe(
