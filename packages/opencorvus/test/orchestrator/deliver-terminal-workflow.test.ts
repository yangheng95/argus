...
test("orchestrator workflow retires deliver and uses integrity as the final gate", async () => {
  ...
  expect(normalizedPrompt).toContain("no separate final acceptance object or host-owned acceptance gate")
  ...
  expect(tools).toContain("deliver: disabled")
  expect(tools).toContain("publish_delivery: disabled")
