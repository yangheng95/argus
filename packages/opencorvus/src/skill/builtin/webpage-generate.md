...
- **Generated React source** (`src/App.tsx`, `src/design-tokens.ts`, `src/components/**`): this is the only scaffold path after `webpage_analyze`. Refine these files in place.
...
- Design tokens (colours, fonts, spacing, radii) come from `src/design-tokens.ts` generated by `webpage_analyze` — do not invent hex codes, font sizes, or spacing values.
...
This skill is idempotent on `mirror/` and generated `src/` files. On re-entry:
...
- The generated deliverable source (`src/App.tsx`, `src/design-tokens.ts`, `src/components/**`) plus the dev-server entry.
...
- `src/App.tsx`, `src/design-tokens.ts`, `src/components/**` — generated React source to refine in place
...
- `src/design-tokens.ts` — palette / typography
- `mirror/reference.png` — the visual target

Quote the exact strings, use the exact token values from generated `src/design-tokens.ts`, follow the section ordering from `page-ir.xml`. Do not paraphrase headings, nav labels, or button text.
...
- Wire colours / fonts / spacing through generated `src/design-tokens.ts`. Don't invent values.
