---
title: TUI
description: Using the OpenCorvus terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenCorvus provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenCorvus starts the TUI for the current directory.
...
When using the OpenCorvus TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenCorvus. Allows you to select from available providers and add their API keys.
...
Exit OpenCorvus. _Aliases_: `/quit`, `/q`
...
This is separate from `opencorvus.json`, which configures server/runtime behavior.
...
Use `OPENCORVUS_TUI_CONFIG` to load a custom TUI config path.
