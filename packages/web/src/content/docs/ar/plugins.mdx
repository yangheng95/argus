---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Argus.
---

تتيح لك الإضافات توسيع Argus عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Argus.
...
- `.argus/plugins/` - إضافات على مستوى المشروع
- `~/.config/argus/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/argus/node_modules/`.
...
1. إعدادات عامة (`~/.config/argus/argus.json`)
2. إعدادات المشروع (`argus.json`)
3. دليل الإضافات العام (`~/.config/argus/plugins/`)
4. دليل إضافات المشروع (`.argus/plugins/`)
...
يشغّل Argus الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ argus للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع argus.
...
:::note
إذا كنت تستخدم تطبيق Argus لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى argus:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ argus استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ argus إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://argus.ai/docs/sdk) للتفاصيل.
