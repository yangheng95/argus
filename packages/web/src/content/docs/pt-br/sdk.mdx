---
title: SDK
description: Cliente JS seguro em tipos para o servidor opencorvus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

O SDK JS/TS do opencorvus fornece um cliente seguro em tipos para interagir com o servidor.
Use-o para construir integrações e controlar o opencorvus programaticamente.
...
Crie uma instância do opencorvus:
...
| `port`     | `number`      | Porta do servidor                          | `7878`      |
...
Você pode passar um objeto de configuração para personalizar o comportamento. A instância ainda pega seu `opencorvus.json`, mas você pode substituir ou adicionar configuração inline:
...
Se você já tem uma instância do opencorvus em execução, pode criar uma instância de cliente para se conectar a ela:
...
| Opção           | Tipo       | Descrição                             | Padrão                  |
| --------------- | ---------- | ------------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | URL do servidor                       | `http://localhost:7878` |
