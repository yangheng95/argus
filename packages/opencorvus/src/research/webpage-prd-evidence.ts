...
export function renderWebpagePrdEvidencePromptSection(evidence: WebpagePrdEvidence | undefined): string | undefined {
  ...
  return [
    ...
    "Also submit `webpage_contract`. It must partition the rendered page investigation and must cover functional_surfaces, visual_layout, style_requirements, interaction_states, data_content_inventory, fidelity_acceptance, and fidelity_risks as work-packet inputs. Do not substitute a raw artifact/material list for this contract.",
    "The submitted `bundle.full_markdown_sections` for this webpage contract must be complete but bounded enough to submit in one terminal-tool call. Cover every major visible region with evidence-backed work packets, component-kind hypotheses, style/layout questions, interaction/data questions, expected downstream outputs, acceptance checks, and risks; do not submit raw markdown or JSON documents as string fields, and split multiline notes into separate array items.",
