...
:::caution
Modes are now configured through the `agent` option in the argus config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in argus allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the argus config.
...
argus comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.argus/plans/*.md` to detail the plan itself
...
Configure modes in your `argus.json` config file:
...
both the global argus config and the project specific config.
...
Create mode files in `.argus/modes/` for project-specific modes or `~/.config/argus/modes/` for global modes:
