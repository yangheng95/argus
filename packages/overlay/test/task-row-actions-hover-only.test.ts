...
//
// The task-list rows carry up to four icon buttons (delete, cancel/stop,
// rename, start-now). They used to render at `--ui-opacity-subtle` by default
// and promote to full opacity on hover/focus, which was visually busy and
// easy to mis-click while scanning the list. The contract this test pins is:
//
...
//     opacity to full and re-enables pointer events for all three icon kinds.
//
...
describe("task-row action buttons hover-only contract", () => {
  ...
  test("default state hides all four action button kinds", () => {
  ...
  test("default selector groups delete, cancel, rename, and start-now", () => {
    const stripped = stripCssComments(css);
    const ruleHead = stripped.match(
      /\.task-row-actions \.oc-button\[data-ui="task-row-delete"\],\s*\.task-row-actions \.oc-button\[data-ui="task-row-cancel"\],\s*\.task-row-actions \.oc-button\[data-ui="task-row-rename"\],\s*\.task-row-actions \.oc-button\[data-ui="task-row-start-now"\]\s*\{/,
    );
  ...
  test("reveal selector group covers all four button kinds across hover/focus states", () => {
    const stripped = stripCssComments(css);
    const expected = [
      '.task-row-mini:hover .oc-button[data-ui="task-row-delete"]',
