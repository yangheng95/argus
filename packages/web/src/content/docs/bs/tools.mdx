...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenCorvus dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenCorvus.
...
:::note
Ovaj alat je dostupan samo kada je `OPENCORVUS_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENCORVUS_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCorvus provajdera ili kada je varijabla `OPENCORVUS_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenCorvus:
