---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Argus-konfigurasjonen.
...
i `~/.local/share/argus/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Argus
konfig.
...
Argus Zen er en liste over modeller levert av Argus-teamet som har vært
testet og verifisert for å fungere godt med Argus. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Argus Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg argus og gå til [argus.ai/auth](https://argus.ai/auth).
...
Det fungerer som alle andre leverandører i Argus og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med Argus:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Argus:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `argus.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i argus støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Argus skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Argus-konfigurasjonen.
...
for å lagre token i Argus auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Argus bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Argus
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`argus.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `argus.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Argus:
...
Slik bruker du Google Vertex AI med Argus:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Argus, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) plugin for automatisk å logge hver Argus-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Argus-samtale oppført som en separat økt.
...
Du kan konfigurere Argus for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Argus for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Argus for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Argus. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/argus) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Argus:
...
3. Kopier API-nøkkelen for bruk i Argus.
...
6. **Viktig**: Før du bruker skymodeller i Argus, må du hente modellinformasjonen lokalt:
...
Argus Zen er en liste over testede og verifiserte modeller levert av Argus-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Argus Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Argus Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Argus API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Argus-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Argus-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Argus:
...
Du kan også tilpasse modeller gjennom Argus-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Argus. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `argus.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Argus forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `argus auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Argus-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Argus-konfigurasjonen.
