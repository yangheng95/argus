...
test("ChatBubble inlines child content instead of wrapping child cards in nested bubbles", () => {
  ...
  expect(CHAT_BUBBLE_TSX).toContain('<CardParts parts={child().parts} depth={props.depth + 1} streaming={child().status === "running"} />')
...
test("ChatBubble renders integrity reviewer agent children without requiring a consensus payload", () => {
  ...
  expect(CHAT_BUBBLE_TSX).toContain('<CardParts parts={props.child.parts} depth={props.depth + 1} streaming={props.child.status === "running"} />')
