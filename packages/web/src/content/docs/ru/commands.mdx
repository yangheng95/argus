...
Создайте `.opencorvus/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию opencorvus или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем opencorvus [config](/docs/config):
...
- Глобальный: `~/.config/opencorvus/commands/`
- Для каждого проекта: `.opencorvus/commands/`
...
opencorvus включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
