...
test("selected-task SSE malformed JSON is logged and surfaced", () => {
  ...
  expect(notificationStore.items.some((item) => item.id === "sse:parse-error:selected-task:tsk_parse")).toBe(true)
})
...
test("selected-task SSE dispatch error uploads bounded structured diagnostics", async () => {
  ...
  expect(notificationStore.items.some((item) => item.id === "sse:dispatch-error:tsk_dispatch")).toBe(true)
})
...
test("selected-task SSE dispatch log timeout does not recursively upload the flush failure", async () => {
  ...
  expect(notificationStore.items.some((item) => item.id === "sse:dispatch-error:tsk_dispatch_timeout")).toBe(true)
  expect(notificationStore.items.some((item) => item.id === "system:overlay-log-upload-failed")).toBe(true)
})
...
test("task-list SSE malformed JSON is logged and surfaced", () => {
  ...
  expect(notificationStore.items.some((item) => item.id === "sse:parse-error:task-list:D:/parse")).toBe(true)
})
...
test("task-list SSE dispatch error uploads bounded structured diagnostics", async () => {
  ...
  setBoardStore("tasks", [
    {
      task: {
        get id() {
          throw new Error(`task-list title lookup failed ${"q".repeat(50_000)}`)
        },
      },
    },
  ] as any)
  ...
  expect(notificationStore.items.some((item) => item.id === "task-list-sse:dispatch-error")).toBe(true)
})
...
test("panel message stream malformed JSON rejects the request and surfaces the parse error", async () => {
  ...
  try {
    ...
    expect(notificationStore.items.some((item) => item.id === "task:panel-message-stream-parse-error:req_parse")).toBe(
      true,
    )
  } finally {
