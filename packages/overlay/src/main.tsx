...
function buildTaskDebugBlob(board: any): string {
  ...
  push(
    ...
    `  - When the workflow step list stops at 'architect.completed' with no later step row, the orchestrator has already promoted plan_version + goal_run; downstream progress lives in engine_run / engine_goal_run + protocol_event (session.bridge), not in workflow.step.updated.`,
    ...
    ``,
    `-- Engine runs (one per planner-driven attempt; activeRunID points here)`,
    `SELECT id, status, time_created, time_updated, time_completed`,
    `FROM engine_run WHERE task_id = '${id}' ORDER BY time_created;`,
    ``,
    `-- Goal runs (one per begin-build-attempt; tells you which goal is currently executing)`,
    `SELECT id, run_id, goal_id, status, attempt, time_started, time_completed, time_updated`,
    `FROM engine_goal_run WHERE task_id = '${id}' ORDER BY time_started;`,
    ``,
    `-- Executor sessions (only populated when executor protocol registers a lease;`,
    `--  empty rows do NOT prove no executor is running)`,
    `SELECT id, run_id, goal_run_id, provider, protocol, status, lease_owner, lease_until, time_started, time_completed`,
    ...
    `SELECT id, goal_run_id, delivery_id, kind, label, time_created FROM engine_artifact`,
