...
const log = Log.create({ service: "delivery-lkg-isolated-eval" })
...
/**
 * Evaluate delivery LKG inside a detached temporary git worktree.
 *
...
export async function evaluateLKGInIsolatedWorktree(input: {
  ...
  try {
    ...
    if (created) {
      ...
      log.info("removed isolated delivery LKG worktree", {
