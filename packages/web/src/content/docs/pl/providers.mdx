---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w opencorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

opencorvus używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji opencorvus.
...
w `~/.local/share/opencorvus/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim opencorvus
config.
...
OpenCorvus Zen to lista modeli dostarczonych przez zespół opencorvus, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencorvus. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenCorvus Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz opencorvus i przejdź do [opencorvus.ai/auth](https://opencorvus.ai/auth).
...
Działa jak każdy inny dostawca w opencorvus i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z opencorvus:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania opencorvus:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `opencorvus.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w opencorvus nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod opencorvus działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji opencorvus.
...
opencorvus używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować opencorvus
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`opencorvus.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth).

##### Konfiguracja

Customize through `opencorvus.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem opencorvus:
...
Aby używać Google Vertex AI z opencorvus:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running opencorvus.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą opencorvus, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session), aby automatycznie rejestrować każdą konwersację opencorvus jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja opencorvus będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować opencorvus tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować opencorvus tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować opencorvus tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem opencorvus. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencorvus).
:::
...
Aby korzystać z Ollama Cloud z opencorvus:
...
3. Skopiuj klucz API do użycia w opencorvus.
...
6. **Ważne**: Przed użyciem modeli chmurowych w opencorvus musisz pobrać informacje o modelu lokalnie:
...
OpenCorvus Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencorvus. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenCorvus Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API opencorvus.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji opencorvus.
...
5. Możesz je także dostosować za pomocą konfiguracji opencorvus. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z opencorvus:
...
Możesz także dostosować modele za pomocą konfiguracji opencorvus. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z opencorvus. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `opencorvus.json` w katalogu projektu:
...
Pola `limit` pozwalają opencorvus zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `opencorvus auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację opencorvus i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji opencorvus.
