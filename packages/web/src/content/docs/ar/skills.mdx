...
تتيح مهارات الوكيل لـ OpenCorvus اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث OpenCorvus في هذه المواقع:

- إعدادات المشروع: `.opencorvus/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج OpenCorvus صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.opencorvus/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/opencorvus/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.opencorvus/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض OpenCorvus المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `opencorvus.json`:
...
**للوكلاء المدمجين** (في `opencorvus.json`):
