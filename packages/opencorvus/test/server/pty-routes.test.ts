...
describe("server.pty-routes", () => {
  ...
  test("Node PTY bridge kill escalates by child PID after protocol kill", async () => {
    ...
    expect(source).toContain("await waitForBridgeSpawn(child, runtime.nodeExecutable)")
    expect(source).toContain('send({ type: "ready" })')
    expect(source).toContain('send({ type: "startup_error"')
    expect(source).toContain("const bridgeReady = new Promise<void>")
    expect(source).toContain("await bridgeReady")
    expect(source.indexOf("await bridgeReady")).toBeLessThan(source.indexOf("return {\n    pid: child.pid ?? 0"))
    expect(source).toContain('bridgeMessage(child, { type: "kill" })')
    expect(source).toContain("if (await waitForBridgeExit(child, 1_000)) return")
    expect(source).toContain('ProcessSupervisor.terminateProcessTree(child.pid, `PTY bridge process tree ${child.pid}`)')
    expect(source).not.toContain('nodeSpawn("taskkill.exe"')
    expect(source).toContain('child.kill("SIGKILL")')
    expect(source).toContain("await closeSession(session, \"PTY session removed\")")
    expect(source).toContain("await session.process?.kill()")
    expect(source.indexOf("await session.process?.kill()")).toBeLessThan(source.indexOf("session.process = null"))
