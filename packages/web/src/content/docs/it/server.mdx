---
title: Server
description: Interagisci con il server argus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `argus serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client argus.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `argus.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `argus`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `argus serve` sia per `argus web`.
...
Quando esegui `argus` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server argus per interagire con argus in modo programmatico.
:::

Questa architettura permette a argus di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `argus serve` per avviare un server standalone. Se la TUI di argus e gia in esecuzione, `argus serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Argus.
...
Il server argus espone le seguenti API.
