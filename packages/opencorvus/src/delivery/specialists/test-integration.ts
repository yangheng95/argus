...
export async function runTestIntegrationReview(input: {
  ...
  if (testFiles.length === 0) {
    findings.push({
      proposedSeverity: "blocking",
      category: "test_quality",
      claim: "The delivery requires test integration evidence, but no test files were found.",
      evidence: [{
        kind: "file",
  ...
  if (requiredIDs.length > 0 && testFileTexts.length > 0 && !mentionsAnyRequirement(testFileTexts, requiredIDs)) {
    findings.push({
      ...
      category: "test_quality",
      claim: "Tests do not reference any requirement ID covered by the delivery goals.",
