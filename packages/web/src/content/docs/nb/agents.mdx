...
Det er to typer agenter i OpenCorvus; primære agenter og underagenter.
...
OpenCorvus kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenCorvus kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenCorvus kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `opencorvus.json`:
...
- Globalt: `~/.config/opencorvus/agents/`
- Per prosjekt: `.opencorvus/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenCorvus modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenCorvus-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenCorvus-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCorvus Zen](/docs/zen), vil du bruke `opencorvus/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `opencorvus models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/yangheng95/opencorvus).
:::
