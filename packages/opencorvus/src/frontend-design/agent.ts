...
function buildUserPrompt(input: {
  ...
  sections.push(
    ...
    "Populate the target delivery project directly with file-edit tools for source files (`src/App.tsx`, semantic components, data modules, CSS modules/sidecars, owned assets, and then `src/main.tsx`) instead of running project scaffolding commands that expect an empty directory. " +
    "Configuration manifests and package-manager commands are late-stage integration work, not the first workload. After the skeleton exists, the first target-project editing pass creates source files only: `src/App.tsx`, semantic components, data modules, mock/API adapters, CSS modules/sidecars, owned assets, and then `src/main.tsx`. Before the first target-project write, call `record_frontend_region_selection` for the concrete source region being migrated; target writes without a recorded source region are off-track. After region selection, change pace from source browsing to target extraction: if the selected region file has already been read, the next file-changing action should write the selected vertical slice into the target project. The selected vertical slice means the selected component, its region-owned data/mock/API module when applicable, its scoped style/asset resolver module, and `App.tsx` wiring that imports and renders that selected component. Bootstrap entrypoints such as `src/main.tsx`, global reset/theme CSS, root package/config files, and bundler config are integration files, not the selected-region vertical slice and not valid first target writes. Further reads after selection must be limited to direct imports or explicitly named data/style/asset sidecars for that selected region; do not read unrelated skeleton page wrappers, footer/economy/FAQ/chart/list files, whole manifests, root package/config files, broad source data, or exhaustive asset/style catalogs before the selected component/data/style/App slice exists. Do not chase every `AssetPath`, `svgPaths`, `asset_*.path.txt`, CSS token, or `critical.css` variable before the first target write; preserve observed asset IDs/token names through a target-owned resolver/data module and let same-region render/audit feedback drive exact path/token repair. Selection starts a source-migration transaction, not another open-ended exploration pass. One region selection authorizes edits only for that named source region's data, component, style, assets, and App wiring. Do not call `record_frontend_region_selection` for another region until the current region has a factual `record_frontend_replacement_result`. A data module, mock fixture, or API adapter is not a neutral shared setup file during region extraction: it belongs to the currently selected region, must contain only data proven by that region's row/source evidence, and must not include footer/economy/news/calendar/FAQ/map/chart/card/list records from regions that have not been selected and recorded yet. App wiring is also region-scoped: `src/App.tsx` may import and mount only the current region's component plus components whose regions already have completed replacement results; future region slots may be non-rendering comments/source debt only, never visible placeholder panels, fake spacers, `min-height` filler, placeholder text, or imports for components that are not actually implemented. Before writing files for a different source region such as footer, economy tables, news, calendar, FAQ, map, chart, or card/list groups, first record the current region result and then call `record_frontend_region_selection` for that next source region. Cross-region combined data indexes may be created only after each included region has its own completed replacement result. The first filesystem-changing operation for a new default target directory must be the source vertical slice itself, not a directory-only shell command followed by root config. The file write tools create parent directories for new files, so do not start with `bash mkdir web-clone-target` or scaffold/setup commands. The first write inside a new default target directory should be one of those selected-region source files, not `src/main.tsx`, global reset/theme CSS, `package.json`, `tsconfig.json`, `index.html`, or bundler config. Do not write `index.html` until `src/App.tsx`, at least one semantic component, at least one data/mock module, and one project-owned style module already exist. Do not write imports for future components; create each imported component/data/style module in the same source-coverage pass. Before styling the selected region, inspect `reference.png`/reference-pixel evidence and current-region style sidecars; visible pixels override generic site-theme assumptions. Do not run install/build/render/dev-server commands until the target project already has `src/App.tsx`, at least one semantic component, at least one data/mock module, and at least one project-owned style module. Do not call `write`, `edit`, or `apply_patch` for `package.json`, `tsconfig.json`, or bundler config during that source-coverage pass. Read and edit root config only immediately before a late integration/build command after source coverage exists. " +
