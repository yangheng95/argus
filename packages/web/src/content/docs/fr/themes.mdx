...
Avec OpenCorvus, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenCorvus utilise notre propre thème `opencorvus`.
...
OpenCorvus est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenCorvus corresponde à l'apparence de leur terminal
...
OpenCorvus prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/opencorvus/themes/*.json` ou `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.opencorvus/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.opencorvus/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/opencorvus/themes
vim ~/.config/opencorvus/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .opencorvus/themes
vim .opencorvus/themes/my-theme.json
```
...
{
  "$schema": "https://opencorvus.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
