...
abstract class BaseAgent<TInbox extends z.ZodType, TOutbox extends z.ZodType> {
  // ── 身份 ────────────────────────────────────────────────
  ...
  protected abstract execute(input: z.infer<TInbox>, ctx: AgentContext): Promise<z.infer<TOutbox>>
   ...
   */
  async receive(message: MailboxMessage<z.infer<TInbox>>, senderAgentId: string): Promise<z.infer<TOutbox>>

  /**
   * 向目标 agent 发送消息。目标必须在本 agent 的 sendWhitelist 内。
   * payload schema 验证通过后放入目标 agent 的 inbox。
   */
  protected async send(targetAgentId: string, payload: z.infer<TOutbox>): Promise<void>
...
> **2026-06-17 correction:** `AcceptanceReview` and `ProsecutorAgent` are retired historical
> roles, not current `PipelineAgent` subclasses. Current acceptance code is review/artifact
> infrastructure under `packages/opencorvus/src/acceptance/**`; adversarial review responsibility is
...
```typescript
abstract class PipelineAgent<TIn extends z.ZodType, TOut extends z.ZodType> extends BaseAgent<TIn, TOut> {
...
```typescript
abstract class SessionAgent<TIn extends z.ZodType, TOut extends z.ZodType> extends BaseAgent<TIn, TOut> {
  protected async execute(input: z.infer<TIn>, ctx: AgentContext): Promise<z.infer<TOut>> {
    const system = this.buildSystemPrompt(ctx)
...
| `reviewIntegrity()` （`integrity/team-agent.ts`，函数式入口；本文 §三的 `IntegrityAgent` 类是未来形态） | `PipelineAgent` | `IntegrityInputSchema`      | `IntegrityResultSchema`      | 已在 `.txt`，已接线 orchestrator `integrity` tool；旧固定维度实现已退役                         |
