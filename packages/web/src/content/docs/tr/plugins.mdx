---
title: Eklentiler
description: opencorvus'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek opencorvus'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya opencorvus'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.opencorvus/plugins/` - Proje düzeyinde eklentiler
- `~/.config/opencorvus/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/opencorvus/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/opencorvus/opencorvus.json`)
2. Project config (`opencorvus.json`)
3. Global eklenti dizini (`~/.config/opencorvus/plugins/`)
4. Proje eklenti dizini (`.opencorvus/plugins/`)
...
opencorvus bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için opencorvus'lu bir SDK istemcisi.
...
opencorvus'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
opencorvus masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
opencorvus'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca opencorvus'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, opencorvus'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://opencorvus.ai/docs/sdk)'e bakın.
