...
export const OPENCORVUS_BASE_MODE = "base"

export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENCORVUS_MODE_KEY = "opencorvus.mode"

export const OpencorvusKeymapProvider = KeymapProvider
export const useOpencorvusKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OpencorvusModeStack = ReturnType<typeof createOpencorvusModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OpencorvusModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOpencorvusModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENCORVUS_MODE_KEY, OPENCORVUS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENCORVUS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENCORVUS_MODE_KEY, stack.at(-1)?.mode ?? OPENCORVUS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENCORVUS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENCORVUS_MODE_KEY, undefined)
...
export function useOpencorvusModeStack() {
  return getOpencorvusModeStack(useOpencorvusKeymap())
}

export function getOpencorvusModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Opencorvus mode stack is not registered for this keymap")
  return value
}
...
export function registerOpencorvusKeymap(
  keymap: OpenTuiKeymap,
  renderer: CliRenderer,
  config: Pick<TuiConfig.Resolved, "keybinds" | "leader_timeout">,
) {
  const modeStack = createOpencorvusModeStack(keymap)
