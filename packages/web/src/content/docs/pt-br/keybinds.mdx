...
O argus tem uma lista de atalhos de teclado que você pode personalizar através da configuração do argus.
...
O argus usa uma tecla `leader` para a maioria dos atalhos de teclado. Isso evita conflitos no seu terminal.
...
O prompt de entrada do aplicativo desktop argus suporta atalhos comuns de estilo Readline/Emacs para edição de texto. Estes são embutidos e atualmente não são configuráveis via `argus.json`.
