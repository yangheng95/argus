...
Dzięki opencorvus możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie opencorvus używa naszego własnego motywu `opencorvus`.
...
opencorvus ma kilka wbudowanych motywów.
...
- Want opencorvus to match their terminal's appearance
...
opencorvus obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/opencorvus/themes/*.json` lub `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.opencorvus/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.opencorvus/themes/*.json`
