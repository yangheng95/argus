...
Навыки агента позволяют opencorvus обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
opencorvus выполняет поиск в следующих местах:

- Конфигурация проекта: `.opencorvus/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/opencorvus/skills/<name>/SKILL.md`.
...
Для локальных путей проекта opencorvus переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.opencorvus/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.opencorvus/skills/git-release/SKILL.md` следующим образом:
...
opencorvus перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `opencorvus.json`:
...
**Для встроенных агентов** (в формате `opencorvus.json`):
