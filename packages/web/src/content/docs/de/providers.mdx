---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Argus-Konfiguration.
...
in `~/.local/share/argus/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem Argus anpassen
config.
...
Argus Zen ist eine Liste von Modellen, die vom Argus-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Argus funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Argus Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in Argus und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit Argus:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Argus ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `argus.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Argus wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Argus ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Argus-Konfiguration hinzufügen.
...
um das Token im Argus-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Argus verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Argus
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`argus.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `argus.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Argus:
...
So verwenden Sie Google Vertex AI mit Argus:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Argus ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Argus konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session)-Plugin, um jede Argus-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Argus-Konversation als separate Sitzung aufgeführt.
...
Sie können Argus für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Argus über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Argus über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Argus konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/argus).
:::
...
So verwenden Sie Ollama Cloud mit Argus:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Argus.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Argus verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Argus Zen ist eine Liste getesteter und verifizierter Modelle, die vom Argus-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Argus Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Argus Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Argus API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Argus-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Argus-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Argus:
...
Sie können Modelle auch über Ihre Argus-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Argus verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `argus.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Argus erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `argus auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Argus-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Argus-Konfiguration übereinstimmt.
