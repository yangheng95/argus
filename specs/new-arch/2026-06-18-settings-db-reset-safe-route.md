...
- `specs/bug-hunt-repair-plan-2026-06-17.md` BH-024 defines the HTTP reset authority boundary: `/global/db/reset` must reject unknown absolute `projectDir` values before `Instance.disposeAll()` or `Database.reset(...)`.
- `specs/new-arch/2026-06-17-hexin-budget-refresh-db-sidecar-cleanup.md` keeps explicit `Database.reset()` as the destructive path after schema drift stops auto-recreating SQLite. It does not revoke the BH-024 registered-directory boundary.
