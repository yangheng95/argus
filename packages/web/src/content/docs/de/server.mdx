---
title: Server
description: Nutze den argus-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `argus serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein argus-Client nutzen kann.
...
Der Benutzername ist standardmaessig `argus`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `argus serve` und `argus web`.
...
Wenn du `argus` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den argus-Server, um argus programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `argus serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `argus serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Argus-[IDE](/docs/ide)-Plugins.
...
Der argus-Server stellt folgende APIs bereit.
