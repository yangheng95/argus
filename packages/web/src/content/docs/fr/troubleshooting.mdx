...
Pour déboguer les problèmes avec OpenCorvus, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\opencorvus\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `opencorvus --log-level DEBUG`.

---

## Stockage

opencorvus stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/opencorvus/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\opencorvus`
...
OpenCorvus Desktop exécute un serveur OpenCorvus local (le side-car `opencorvus-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `OpenCorvus` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencorvus/opencorvus.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencorvus\opencorvus.jsonc`
...
OpenCorvus peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/opencorvus/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencorvus\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que OpenCorvus puisse le reconstruire.

1. Quittez complètement OpenCorvus Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/opencorvus`
- **Linux** : supprimez `~/.cache/opencorvus` (ou exécutez `rm -rf ~/.cache/opencorvus`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\opencorvus`

3. Redémarrez le bureau OpenCorvus.

---

### Résoudre les problèmes de connexion au serveur

OpenCorvus Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `opencorvus.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `OPENCORVUS_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `OPENCORVUS_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, OpenCorvus Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de OpenCorvus.

---

### Les notifications ne s'affichent pas

OpenCorvus Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour OpenCorvus dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau OpenCorvus.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application OpenCorvus Desktop) :

- `opencorvus.settings.dat` (serveur de bureau par défaut URL)
- `opencorvus.global.dat` et `opencorvus.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec OpenCorvus :
...
3. Assurez-vous d'avoir la dernière version avec `opencorvus upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `opencorvus models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\opencorvus`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. opencorvus installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\opencorvus`

2. Redémarrez opencorvus pour réinstaller les derniers packages du fournisseur

Cela forcera opencorvus à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
opencorvus détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
