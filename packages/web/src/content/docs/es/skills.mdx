...
Las habilidades del agente permiten a Argus descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Argus busca estas ubicaciones:

- Configuración del proyecto: `.argus/skills/<name>/SKILL.md`
- Configuración global: `~/.config/argus/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Argus sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.argus/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.argus/skills/git-release/SKILL.md` así:
...
Argus enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `argus.json`:
...
**Para agentes integrados** (en `argus.json`):
