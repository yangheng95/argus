---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

Argus koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Argus konfiguraciji.
...
u `~/.local/share/argus/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Argus
config.
...
Argus Zen je lista modela koje je obezbedio Argus tim koji su bili
testirano i potvrđeno da dobro radi sa Argus. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Argus Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite argus i idite na [argus.ai/auth](https://argus.ai/auth).
...
Radi kao i svaki drugi provajder u Argus i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s Argus:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete argus:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `argus.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Argus.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi argus ispravno radio.
   :::
...
Također možete dodati modele kroz svoju argus konfiguraciju.
...
da pohrani token u argus auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Argus koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Argus
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`argus.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth).

##### Konfiguracija

Prilagodite putem `argus.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s argus:
...
Za korištenje Google Vertex AI s Argus:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće argus.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem argusa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) da automatski prijavite svaki Argus razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Argus razgovor naveden kao zasebna sesija.
...
Možete konfigurirati argus za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati argus za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati argus za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Argus. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/argus) za detalje.
:::
...
Da biste koristili Ollama Cloud s Argus:
...
3. Kopirajte API ključ za korištenje u Argus.
...
6. **Važno**: Prije upotrebe modela oblaka u Argus, morate lokalno povući informacije o modelu:
...
Argus Zen je lista testiranih i verifikovanih modela koju je obezbedio Argus tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Argus Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Argus Zen**.

```txt
   /connect
```

3. Unesite svoj Argus API ključ.
...
Također možete dodati dodatne modele putem vaše argus konfiguracije.
...
5. Također ih možete prilagoditi putem vaše argus konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju argus konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s argus-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `argus.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Argus da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `argus auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Argus konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj argus konfiguraciji.
