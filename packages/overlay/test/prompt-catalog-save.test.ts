...
describe("prompt catalog save values", () => {
  ...
  test("PromptCatalog exposes prompt-profile management and import as the editable prompt surface", () => {
    ...
    expect(source).toContain('class="field-input prompt-profile-textarea"')
    expect(source).toContain("aria-labelledby={labelID}")
    expect(source).toContain('class="prompt-profile-list-item"')
    ...
    expect(source).not.toContain('class="field-input prompt-profile-textarea"\n                                  aria-label=')
  })
})
