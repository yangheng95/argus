...
أنشئ الملف `.argus/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Argus أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Argus:
...
- على مستوى النظام: `~/.config/argus/commands/`
- لكل مشروع: `.argus/commands/`
...
يتضمن argus عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
