...
opencorvus ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak opencorvus kendi `opencorvus` temasını kullanır.
...
opencorvus birden fazla yerleşik temayla gelir.
...
- opencorvus'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
opencorvus, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/opencorvus/themes/*.json` veya `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.opencorvus/themes/*.json`
4. **Geçerli çalışma dizini** - `./.opencorvus/themes/*.json`
