...
Render the Node Browser MCP operation process in the preview panel, but only after the backend persists it as task-scoped browser preview operation evidence.
...
| Preview evidence runner | `packages/opencorvus/src/browser-preview/evidence-runner.ts` writes Playwright capture manifests. | Keep capture evidence separate; operation evidence can reference screenshot files but must not replace capture verification. |
...
| Browser MCP tool tracing | `packages/opencorvus/src/mcp/browser/tools.ts` wraps `server.registerTool` with `traced`. | Extend tracing to emit a structured operation callback; keep result content behavior unchanged. |
...
| Browser MCP host wrapping | `packages/opencorvus/src/session/loop.ts` wraps `browser_*` MCP tools and materializes images. | Attach task preview context to Browser MCP calls and persist operation evidence after tool completion. Do not revive automatic URL target extraction from MCP output. |
...
| Agent prompts | `orchestrator-core.txt`, `visual-qa-core.txt`, `integrity-team-core.txt` now require explicit `browser_preview`. | Clarify that Browser MCP operations shown in preview are persisted operation evidence, not automatic target discovery. |
...
- `findReadableBrowserPreviewOperationFrame({ taskID, operationID })` validates screenshot path readability and hash before serving the image.
...
The context is created from the persisted browser preview target selected by `browser_preview`. It is passed through the existing session-loop MCP wrapper when the tool name starts with `browser_`.
...
Extend `traced` in `packages/opencorvus/src/mcp/browser/tools.ts` so each Browser MCP operation can emit:

- start event: tool name, summarized args, MCP session ID, start time
- completion event: status, elapsed time, current URL/title/viewport when available
- optional screenshot event: PNG metadata and persisted file reference when the tool output contains image data
- diagnostics snapshot: counts from `getDiagnostics(sessionId)`

The Browser MCP server should expose a small hook interface rather than import `engine_artifact` directly. The OpenCorvus session-loop host installs the hook when it launches or calls the built-in Browser MCP. This keeps the MCP package usable as a tool server while letting OpenCorvus persist task evidence.
...
- `GET /task/:taskID/browser-preview/operations/:operationID/frame.png`
  - returns a persisted PNG frame for a screenshot-bearing operation.

No route accepts a URL. No route accepts an MCP monitor URL or MCP port.

### Events

Emit `TaskUpdated` after operation completion and after screenshot-bearing operation persistence. The overlay already refreshes browser preview state from task updates; if that is too coarse, add a backend route field `latestOperationID` to the target response. Do not add an overlay-local event bus as a second source.
...
Orchestrator, visual-qa, and integrity keep using explicit `browser_preview` to create/select the task preview target. Browser MCP calls may then render as operation evidence if the session has task preview context.
...
- cite persisted browser preview operation IDs or capture evidence IDs in findings
...
- Add a Browser MCP operation hook interface.
- Install the hook from the session-loop MCP wrapper for task-associated Browser MCP calls.
- Persist operation start/completion records for `browser_*` calls without reviving automatic target extraction.
- Persist screenshot metadata for `browser_screenshot` and `browser_observe` image-bearing results.
...
- Update orchestrator, visual-qa, and integrity prompts to cite operation evidence when using Browser MCP.
- Add prompt/tool tests proving `browser_preview` remains the explicit target owner and Browser MCP operation evidence is not target discovery.
...
- Route: frame route rejects missing files, hash mismatch, wrong task, and wrong artifact kind.
...
- Prompt: orchestrator, visual-qa, and integrity prompts mention persisted operation evidence and keep explicit `browser_preview` target ownership.
...
- The first implementation should prefer a small hook interface over importing OpenCorvus engine modules from `mcp/browser/tools.ts`; that preserves Browser MCP as a reusable tool server.
...
- Running operations can be persisted as rows or emitted only after completion. Persisting a running row makes the preview feel live, but it must be updated by the same backend artifact owner; the overlay must still read via task routes.
