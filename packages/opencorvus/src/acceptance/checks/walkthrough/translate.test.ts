...
describe("scenario walkthrough translation", () => {
  test("accepts validated streaming tool-call steps", async () => {
    const steps = await translateScenarioToStepsWithDependencies({ spec: scenarioSpec() }, deps([{
      type: "tool-call",
      toolName: "submit_walkthrough_steps",
      args: { steps: [{ action: "goto", path: "/" }, { action: "assertSelector", selector: "#app" }] },
    }]))
    expect(steps.at(-1)).toEqual({ action: "assertSelector", selector: "#app" })
  })

  test("requires final path or selector assertion", async () => {
    await expect(translateScenarioToStepsWithDependencies({ spec: scenarioSpec() }, deps([{
      type: "tool-call",
      toolName: "submit_walkthrough_steps",
      args: { steps: [{ action: "goto", path: "/" }, { action: "assertText", text: "Ready" }] },
  ...
  test("includes worked examples and selector absence guidance in the streaming prompt", async () => {
    ...
    await translateScenarioToStepsWithDependencies({ spec: scenarioSpec() }, deps([{
      type: "tool-call",
      toolName: "submit_walkthrough_steps",
      args: { steps: [{ action: "goto", path: "/" }, { action: "assertSelector", selector: "#app" }] },
    }], prompts))
...
function deps(parts: unknown[], prompts?: string[]): TranslateScenarioDependencies {
  return {
    resolveModel: async () => ({ providerID: "test", id: "mock", api: { id: "mock" } }) as Awaited<ReturnType<TranslateScenarioDependencies["resolveModel"]>>,
