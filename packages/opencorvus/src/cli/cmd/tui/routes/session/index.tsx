...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@opencorvus-ai/sdk"
...
import type { SearchCodeTool } from "@/tool/grep"
...
export function Session() {
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}opencorvus -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      run: () => {
        ...
        void copyFormattedMessage(
          lastAssistantMessage,
          sync.data.part[lastAssistantMessage.id] ?? [],
          {
  ...
  const sessionCommands = createMemo(() =>
    sessionCommandList().map(({ value, slash, ...command }) => ({
      namespace: "palette",
      name: value,
...
function CopyMessageButton(props: { message: UserMessage | AssistantMessage; parts: Part[] }) {
  const ctx = use()
  const { theme } = useTheme()
  const toast = useToast()
        ...
        <Match when={props.last || final() || props.message.error?.name === "MessageAbortedError"}>
          <box paddingLeft={3} marginTop={1} flexDirection="row" gap={1} alignItems="center">
            <text>
              ...
              <span style={{ fg: theme.text }}>{Locale.titlecase(props.message.agent || "assistant")}</span>
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.OPENCORVUS_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "search_code"}>
          <SearchCode {...toolprops} />
        </Match>
        ...
        <Match when={props.part.tool === "external_code_search"}>
          <ExternalCodeSearch {...toolprops} />
        </Match>
...
function SearchCode(props: ToolProps<typeof SearchCodeTool>) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="Searching content..." complete={props.input.pattern} part={props.part}>
      Search Code "{props.input.pattern}"{" "}
      <Show when={props.input.path}>in {pathFormatter.format(props.input.path)} </Show>
...
function List(props: ToolProps<typeof ListTool>) {
  ...
  const dir = createMemo(() => {
    if (props.input.path) {
      return pathFormatter.format(props.input.path)
    }
    return ""
  })
  return (
...
function ExternalCodeSearch(props: ToolProps<any>) {
  const input = props.input as any
  const metadata = props.metadata as any
  return (
    <InlineTool icon="◇" pending="Searching code..." complete={input.query} part={props.part}>
      External Code Search "{input.query}" <Show when={metadata.results}>({metadata.results} results)</Show>
    </InlineTool>
  )
}
