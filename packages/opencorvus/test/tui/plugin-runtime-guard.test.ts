...
describe("OpenCode-derived TUI plugin substrate", () => {
  ...
  test("wires the TUI app through plugin runtime and host slots instead of hard-coded sidebars", async () => {
    ...
    const terminalUtil = await readFile(path.join(tuiRoot, "util/terminal.ts"), "utf8")
    ...
    expect(terminalUtil).toContain("Copied from OpenCode")
    expect(terminalUtil).toContain("export namespace Terminal")
    expect(terminalUtil).toContain("export async function colors()")
    ...
    expect(sessionRoute).toContain("const INLINE_TOOL_ICON_WIDTH = 2")
    expect(sessionRoute).toContain("export function InlineToolRow")
    expect(sessionRoute).toContain("setErrorExpanded((value) => !value)")
    expect(sessionRoute).toContain("props.failed && props.errorExpanded")
