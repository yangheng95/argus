---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenCorvus konfiguraciji.
...
u `~/.local/share/opencorvus/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenCorvus
config.
...
OpenCorvus Zen je lista modela koje je obezbedio OpenCorvus tim koji su bili
testirano i potvrđeno da dobro radi sa OpenCorvus. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenCorvus Zen.
:::
...
Radi kao i svaki drugi provajder u OpenCorvus i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s OpenCorvus:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete opencorvus:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `opencorvus.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenCorvus.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi opencorvus ispravno radio.
   :::
...
Također možete dodati modele kroz svoju opencorvus konfiguraciju.
...
da pohrani token u opencorvus auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenCorvus koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenCorvus
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`opencorvus.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth).

##### Konfiguracija

Prilagodite putem `opencorvus.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s opencorvus:
...
Za korištenje Google Vertex AI s OpenCorvus:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće opencorvus.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem opencorvusa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) da automatski prijavite svaki OpenCorvus razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenCorvus razgovor naveden kao zasebna sesija.
...
Možete konfigurirati opencorvus za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati opencorvus za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati opencorvus za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenCorvus. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencorvus) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenCorvus:
...
3. Kopirajte API ključ za korištenje u OpenCorvus.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenCorvus, morate lokalno povući informacije o modelu:
...
OpenCorvus Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenCorvus tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenCorvus Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenCorvus Zen**.

```txt
   /connect
```

3. Unesite svoj OpenCorvus API ključ.
...
Također možete dodati dodatne modele putem vaše opencorvus konfiguracije.
...
5. Također ih možete prilagoditi putem vaše opencorvus konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju opencorvus konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s opencorvus-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `opencorvus.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenCorvus da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `opencorvus auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenCorvus konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj opencorvus konfiguraciji.
