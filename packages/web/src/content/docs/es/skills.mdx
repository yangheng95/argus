...
Las habilidades del agente permiten a OpenCorvus descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenCorvus busca estas ubicaciones:

- Configuración del proyecto: `.opencorvus/skills/<name>/SKILL.md`
- Configuración global: `~/.config/opencorvus/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenCorvus sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.opencorvus/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/opencorvus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.opencorvus/skills/git-release/SKILL.md` así:
...
OpenCorvus enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `opencorvus.json`:
...
**Para agentes integrados** (en `opencorvus.json`):
