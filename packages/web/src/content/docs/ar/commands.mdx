...
أنشئ الملف `.opencorvus/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenCorvus أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenCorvus:
...
- على مستوى النظام: `~/.config/opencorvus/commands/`
- لكل مشروع: `.opencorvus/commands/`
...
يتضمن opencorvus عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
