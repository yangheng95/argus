// Copied from OpenCode's notification TUI plugin and adapted to OpenCorvus session status names.
import type { Event } from "@opencorvus-ai/sdk"
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@opencorvus-ai/plugin/tui"
...
const tui: TuiPlugin = async (api) => {
  ...
  api.event.on("session.status", (event) => {
    ...
    if (event.properties.status.type === "streaming" || event.properties.status.type === "retry") {
      active.add(sessionID)
      errored.delete(sessionID)
      return
    }

    if (event.properties.status.type !== "idle" && event.properties.status.type !== "terminal") return
