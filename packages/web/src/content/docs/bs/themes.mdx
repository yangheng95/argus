...
U OpenCorvus mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenCorvus koristi nasu `opencorvus` temu.
...
OpenCorvus dolazi sa vise ugradenih tema.
...
- Zele da OpenCorvus odgovara izgledu njihovog terminala
...
OpenCorvus podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/opencorvus/themes/*.json` ili `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.opencorvus/themes/*.json`
4. **Trenutni radni direktorij** - `./.opencorvus/themes/*.json`
