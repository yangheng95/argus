...
Avec Argus, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Argus utilise notre propre thème `argus`.
...
Argus est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Argus corresponde à l'apparence de leur terminal
...
Argus prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/argus/themes/*.json` ou `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.argus/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.argus/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/argus/themes
vim ~/.config/argus/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .argus/themes
vim .argus/themes/my-theme.json
```
...
{
  "$schema": "https://argus.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
