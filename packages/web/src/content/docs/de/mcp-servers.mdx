...
Sie können externe Tools zu Argus hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Argus unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Argus Config](https://argus.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/argus`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Argus übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Argus Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Argus beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `argus mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Argus die Token sicher in `~/.local/share/argus/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Argus verfügbar. Sie können sie also wie jedes andere Tool über die Argus-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Argus mit Ihrem Sentry-Konto zu verbinden.
