---
title: Serwer
description: Interakcja z serwerem opencorvus za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Komenda `opencorvus serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opencorvus.
...
| Flaga           | Opis                                         | Domyślne           |
| --------------- | -------------------------------------------- | ------------------ |
| `--port`        | Port nasłuchiwania                           | `7878`             |
| `--hostname`    | Nazwa hosta do nasłuchiwania                 | `127.0.0.1`        |
| `--mdns`        | Włącz wykrywanie mDNS                        | `false`            |
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `opencorvus.local` |
...
Ustaw `OPENCORVUS_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `opencorvus` lub ustaw `OPENCORVUS_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `opencorvus serve`, jak i `opencorvus web`.
...
Kiedy uruchomisz `opencorvus`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera opencorvus do programowej interakcji z opencorvus.
:::

Ta architektura umożliwia obsługę wielu klientów przez opencorvus i programową interakcję z kodem otwartym.

Możesz uruchomić `opencorvus serve`, aby uruchomić samodzielny serwer. Jeśli masz
opencorvus TUI działa, `opencorvus serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki opencorvus [IDE](/docs/ide).
...
Na przykład `http://localhost:7878/doc`. Użyj specyfikacji, aby wygenerować klientów lub sprawdzić typy żądań i odpowiedzi. Możesz też wyświetlić go w eksploratorze Swagger.

---

## API

Serwer opencorvus udostępnia następujące interfejsy API.
...
| `POST`   | `/permission/:requestID/reply` | Odpowiedz na prośbę o pozwolenie            | treść: `{ reply, message? }`, zwraca `boolean`                                     |
