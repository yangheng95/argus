---
title: Serveur
description: Interagissez avec le serveur opencorvus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

La commande `opencorvus serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opencorvus peut utiliser.
...
| --------------- | -------------------------------------------------- | ------------------ |
| `--port`        | Port à écouter sur                                 | `7878`             |
| `--hostname`    | Nom d'hôte sur lequel écouter                      | `127.0.0.1`        |
| `--mdns`        | Activer la découverte mDNS                         | `false`            |
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `opencorvus.local` |
...
Définissez `OPENCORVUS_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `opencorvus`, ou définissez `OPENCORVUS_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `opencorvus serve` et à `opencorvus web`.
...
Lorsque vous exécutez `opencorvus`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur opencorvus pour interagir avec opencorvus par programmation.
:::

Cette architecture permet à opencorvus de prendre en charge plusieurs clients et vous permet d'interagir avec opencorvus par programme.

Vous pouvez exécuter `opencorvus serve` pour démarrer un serveur autonome. Si vous avez le
opencorvus TUI en cours d'exécution, `opencorvus serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenCorvus [IDE](/docs/ide).
...
Par exemple, `http://localhost:7878/doc`. Utilisez la spécification pour générer des clients ou inspecter les types de requêtes et de réponses. Ou visualisez-le dans un explorateur Swagger.

---

## APIs

Le serveur opencorvus expose les API suivantes.
...
| `POST`   | `/permission/:requestID/reply` | Répondre à une demande d'autorisation                 | corps : `{ reply, message? }`, renvoie `boolean`                                     |
...
| `POST`  | `/tui/control/response` | Répondre à une demande de contrôle (`{ id, body } or { id, error }`)    | `boolean`                    |
