...
تتيح مهارات الوكيل لـ Argus اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Argus في هذه المواقع:

- إعدادات المشروع: `.argus/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/argus/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Argus صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.argus/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/argus/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.argus/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Argus المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `argus.json`:
...
**للوكلاء المدمجين** (في `argus.json`):
