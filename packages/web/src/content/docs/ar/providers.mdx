---
title: المزوّدون
description: استخدام أي موفّر LLM في Argus.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Argus ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Argus.
...
في `~/.local/share/argus/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Argus.
...
Argus Zen هي قائمة نماذج يوفّرها فريق Argus وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Argus. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Argus Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر argus، ثم انتقل إلى [argus.ai/auth](https://argus.ai/auth).
...
يعمل مثل أي مزوّد آخر في Argus واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع Argus:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل argus:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `argus.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Argus غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل argus بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات argus.
...
تخزين الرمز في مساحة تخزين مصادقة argus.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Argus نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Argus
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`argus.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع argus:
...
لاستخدام Google Vertex AI مع Argus:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر argus، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) لتسجيل كل محادثة Argus تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Argus مدرجة كجلسة مستقلة.
...
يمكنك ضبط argus لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط argus لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط argus لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Argus. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/argus) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Argus:
...
3. انسخ مفتاح API لاستخدامه في Argus.
...
6. **مهم**: قبل استخدام نماذج السحابة في Argus، يجب سحب معلومات النموذج محليا:
...
Argus Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Argus. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Argus Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Argus Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Argus.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات argus.
...
5. يمكنك أيضا تخصيصها عبر إعدادات argus. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات argus. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع argus. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `argus.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Argus بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `argus auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات argus وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات argus.
