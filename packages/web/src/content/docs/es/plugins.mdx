---
title: Complementos
description: Escriba sus propios complementos para extender Argus.
---

Los complementos le permiten extender Argus al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Argus.
...
- `.argus/plugins/` - Complementos a nivel de proyecto
- `~/.config/argus/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/argus/node_modules/`.
...
1. Configuración global (`~/.config/argus/argus.json`)
2. Configuración del proyecto (`argus.json`)
3. Directorio global de complementos (`~/.config/argus/plugins/`)
4. Directorio de complementos del proyecto (`.argus/plugins/`)
...
Argus ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK argus para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar argus.
...
:::note
Si está utilizando la aplicación de escritorio Argus, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que argus lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a argus:
...
El ayudante `tool` crea una herramienta personalizada a la que argus puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para argus junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://argus.ai/docs/sdk) para obtener más detalles.
