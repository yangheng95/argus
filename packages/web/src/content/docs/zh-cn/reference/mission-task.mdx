...
所有项目作用域接口都要带项目目录。推荐在 SDK client 上统一传：
...
```text
http://127.0.0.1:7878/task/task_.../status?directory=D:\repo\my-project
```

SDK 会把 `directory` 作为 query 参数加到项目作用域请求上；`x-opencorvus-directory` 是裸 HTTP 调用时可用的等价方式。
