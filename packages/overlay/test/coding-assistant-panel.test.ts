...
test("coding assistant activity selects an independent session in the shared message panel", () => {
  ...
  expect(main).toContain("selectCodingAssistantSession({ signal: controller.signal })")
  ...
  expect(service).toContain('apiJson("coding/sessions?limit=1", { signal })')
  expect(service).toContain('apiJson("coding/session", { method: "POST", signal })')
