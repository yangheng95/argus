...
test("browser preview panel uses mature primitives and task evidence-backed service", () => {
  ...
  expect(component).toContain("const liveFrame = createMemo")
  expect(component).toContain("if (!url) return undefined")
  expect(component).toContain("Browser preview live frame style requires a selected backend viewport.")
  expect(component).toContain("function liveFrameStyle(viewport: { width: number; height: number }): JSX.CSSProperties")
  expect(component).toContain('"--browser-preview-live-aspect-ratio": `${viewport.width} / ${viewport.height}`')
  ...
  expect(component).toContain("const invalidateLiveImageRectAndScheduleMeasure = () =>")
  expect(component).toContain("new ResizeObserver(invalidateLiveImageRectAndScheduleMeasure)")
  expect(component).toContain("pendingLiveInputs.length > 0 && pendingLiveInputScopeKey === key && !liveInputRequestInFlight")
  expect(component).toContain("scrollElement: () => HTMLElement | null")
  expect(component).toContain("const scrollElement = props.scrollElement()")
  expect(component).not.toContain('element.closest<HTMLElement>(".center-workbench-body")')
  expect(main).toContain('scrollElement={() => document.querySelector<HTMLElement>(".center-workbench-body")}')
  expect(component).toContain('addEventListener("scroll", invalidateLiveImageRectAndScheduleMeasure, { passive: true })')
  expect(component).toContain('removeEventListener("scroll", invalidateLiveImageRectAndScheduleMeasure)')
  expect(component).toContain("liveFrameRequestSequence += 1")
  expect(component).not.toContain("isCurrentLiveScope")
  expect(component).toContain("style={liveFrameStyle(frame().viewport)}")
  ...
  expect(component).toContain("onError={() => handleLiveImageDecodeError(frame().url)}")
  ...
  expect(stageSource.indexOf("when={renderedEvidence()}")).toBeLessThan(stageSource.indexOf("when={liveFrame()}"))
  expect(stageSource.indexOf('data-ui="browser-preview-live-error"')).toBeLessThan(
  ...
  expect(component).toContain("onError={() => handleLiveImageDecodeError(frame().url)}")
  expect(service).not.toContain("browserPreviewLiveFrameDecoder")
  expect(service).not.toContain("decodeBrowserPreviewLiveFrame")
  expect(service).not.toContain("image.decode()")
  expect(service).not.toContain("createImageBitmap")
  expect(service).not.toContain("new Image()")
  ...
  expect(workspaceCss).toContain("flex-shrink: 0;")
  expect(workspaceCss).toContain("min-width: var(--ui-workbench-panel-min-width);")
  expect(workspaceCss).not.toContain('.center-workbench-view[data-open="true"][data-selected="true"]')
  expect(workspaceCss).not.toContain("flex: 0 0 min(calc(var(--ui-overlay-shell-height) * 0.6)")
  ...
  const liveBlock = css.match(/\.browser-preview-live\s*{[^}]*}/)?.[0] ?? ""
  const liveFrameBlock = css.match(/\.browser-preview-live-frame\s*{[^}]*}/)?.[0] ?? ""
  const liveImageBlock = css.match(/\.browser-preview-live-frame img\s*{[^}]*}/)?.[0] ?? ""
  const evidenceBlock = css.match(/\.browser-preview-evidence\s*{[^}]*}/)?.[0] ?? ""
  expect(liveBlock).toMatch(/flex:\s*1 1 0;/)
  expect(liveBlock).toMatch(/width:\s*100%;/)
  expect(liveBlock).toMatch(/max-width:\s*100%;/)
  expect(evidenceBlock).toMatch(/flex:\s*1 1 0;/)
  expect(evidenceBlock).toMatch(/max-width:\s*100%;/)
  expect(liveFrameBlock).toMatch(/width:\s*100%;/)
  expect(liveFrameBlock).toMatch(/max-width:\s*100%;/)
  expect(liveFrameBlock).toMatch(/aspect-ratio:\s*var\(--browser-preview-live-aspect-ratio\);/)
  expect(liveFrameBlock).toMatch(/contain:\s*layout;/)
  expect(liveFrameBlock).not.toMatch(/transform\s*:/)
  expect(liveImageBlock).toMatch(/width:\s*100%;/)
  expect(liveImageBlock).toMatch(/height:\s*auto;/)
  expect(liveImageBlock).not.toMatch(/transform\s*:/)
