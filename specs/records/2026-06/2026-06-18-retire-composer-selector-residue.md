...
| `rg -n -F "chat-compose-meta-left a" packages/overlay/src packages/overlay/test specs` | Only `composer.css` and architecture tests referenced the nested anchor selector; `ChatComposer.tsx` renders prompt-profile Select plus `ExecutorSelector`, not an anchor inside `.chat-compose-meta-left`. |
