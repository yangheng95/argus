...
function buildUserPrompt(
  ...
  sections.push(
    "# Visual HTML Skeleton Contract\n\n" +
      ...
      "`update_frontend_project.project_root` should name the visual skeleton root, with entrypoints such as `visual-html-skeleton/index.html`, `visual-html-skeleton/styles/tokens.css`, region CSS files, source asset paths, screenshots, and visual diff/evaluation artifacts.",
  )
  ...
  sections.push(
    "# Live URL Capture\n\n" +
      ...
      "`update_frontend_project` should name the visual HTML skeleton root and entrypoints with role=visual_baseline_input only for screenshot-validated visual-skeleton-first work, and `update_frontend_visual_evidence` must carry renderer, rendered_entrypoint, screenshot_artifact, source_reference_artifact, hashes, viewport, review_status, and review_summary; for production/component-system work, report an implementation scaffold as role=implementation_target or report role=blocked with the missing scaffold/screenshot evidence, and submit `implementation_phase_outcomes` through `update_frontend_phase` as the primary Architect consumption surface. A visual baseline must explicitly say it is source-editable static HTML/CSS, not compiled output, not raw source DOM replay, not the implementation target, not an acceptance app root, and not an independent design source. " +
