...
2. Creates `@opencorvus-ai/sdk/v2` client against that internal server
...
Transport: **newline-delimited JSON over stdio** — no external TCP port (`src/cli/cmd/acp.ts:28-67`).
...
Zed spawns `opencorvus acp` on demand and communicates via stdin/stdout (`src/acp/README.md:67-78`).
...
`authenticate` ⚠️ **not implemented** — throws `"Authentication not implemented"` (`src/acp/agent.ts:564-566`).

## Permission handling

ACP forwards internal `permission.asked` events to the client via `connection.requestPermission()`: `once` / `always` / `reject`.

For `edit` permissions, after client approval, ACP calls `connection.writeTextFile()` to push the patched content to the editor (`src/acp/agent.ts:184-261`).
