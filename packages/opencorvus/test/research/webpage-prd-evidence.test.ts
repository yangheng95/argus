...
describe("research webpage PRD evidence", () => {
  test("prepares rendered webpage evidence as the research prompt surface", async () => {
    ...
    expect(prompt).toContain("missing metadata, source text, or linked-source confirmation becomes an investigation-packet risk")
    ...
    expect(prompt).toContain("`bundle.full_markdown_sections`")
    expect(prompt).toContain("bounded enough to submit in one terminal-tool call")
