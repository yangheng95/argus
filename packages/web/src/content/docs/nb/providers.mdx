---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenCorvus-konfigurasjonen.
...
i `~/.local/share/opencorvus/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenCorvus
konfig.
...
OpenCorvus Zen er en liste over modeller levert av OpenCorvus-teamet som har vært
testet og verifisert for å fungere godt med OpenCorvus. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenCorvus Zen.
:::
...
Det fungerer som alle andre leverandører i OpenCorvus og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med OpenCorvus:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenCorvus:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `opencorvus.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i opencorvus støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenCorvus skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenCorvus-konfigurasjonen.
...
for å lagre token i OpenCorvus auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenCorvus bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenCorvus
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`opencorvus.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `opencorvus.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenCorvus:
...
Slik bruker du Google Vertex AI med OpenCorvus:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenCorvus.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenCorvus, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) plugin for automatisk å logge hver OpenCorvus-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenCorvus-samtale oppført som en separat økt.
...
Du kan konfigurere OpenCorvus for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenCorvus for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenCorvus for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenCorvus. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencorvus) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenCorvus:
...
3. Kopier API-nøkkelen for bruk i OpenCorvus.
...
6. **Viktig**: Før du bruker skymodeller i OpenCorvus, må du hente modellinformasjonen lokalt:
...
OpenCorvus Zen er en liste over testede og verifiserte modeller levert av OpenCorvus-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenCorvus Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenCorvus API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenCorvus-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenCorvus-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenCorvus:
...
Du kan også tilpasse modeller gjennom OpenCorvus-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenCorvus. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `opencorvus.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenCorvus forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `opencorvus auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenCorvus-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenCorvus-konfigurasjonen.
