...
const PROVIDERS_PANEL_SOURCE = readFileSync(join(OVERLAY_ROOT, "src/components/settings/ProvidersPanel.tsx"), "utf8")
...
test("task, file, memory, and provider search inputs use the shared search-field primitive", () => {
  ...
  expect(PROVIDERS_PANEL_SOURCE).toContain('class="provider-search-field search-field"')
  expect(PROVIDERS_PANEL_SOURCE).toContain('class="provider-search-icon search-field-icon"')
  expect(PROVIDERS_PANEL_SOURCE).toContain('class="provider-search-input search-field-input"')
  expect(PROVIDERS_PANEL_SOURCE).toContain('aria-label={t("provider.search.label")}')
  ...
  expect(SETTINGS_CSS).not.toMatch(/\.provider-search-icon\b/)
  expect(SETTINGS_CSS).not.toMatch(/\.provider-search-field\s+\.provider-search-input\b/)
  expect(SETTINGS_CSS).not.toMatch(/\.provider-search-field\s+\.oc-button\[data-ui="provider-search-clear"\]/)
