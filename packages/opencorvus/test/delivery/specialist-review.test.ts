...
describe("delivery specialist review contract", () => {
  test("creates scoped evidence-only specialist reviews without a final verdict field", () => {
    const review = createDeliverySpecialistReview({
  ...
  test("persists specialist reviews as evidence artifacts instead of verdict artifacts", () => {
    ...
    const deliveryId = `dlv_specialist_${stamp}`
    ...
    persistDeliverySpecialistReview({ review })

    const reviews = findDeliverySpecialistReviews({ deliveryID: deliveryId })
