...
Per diagnosticare problemi con OpenCorvus, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\opencorvus\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `opencorvus --log-level DEBUG`.

---

## Archiviazione

opencorvus salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/opencorvus/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\opencorvus`
...
OpenCorvus Desktop esegue in background un server locale di OpenCorvus (il sidecar `opencorvus-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `OpenCorvus` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencorvus\opencorvus.jsonc`
...
OpenCorvus puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/opencorvus/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencorvus\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che OpenCorvus possa ricostruirla.

1. Quit OpenCorvus Desktop completely.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/opencorvus`
- **Linux**: elimina `~/.cache/opencorvus` (oppure esegui `rm -rf ~/.cache/opencorvus`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\opencorvus`
...
OpenCorvus Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `opencorvus.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OPENCORVUS_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OPENCORVUS_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, OpenCorvus Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di OpenCorvus.

---

### Notifiche non visualizzate

OpenCorvus Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per OpenCorvus nelle impostazioni del sistema operativo, e
...
2. Trova ed elimina questi file (si trovano nella directory dati dell'app OpenCorvus Desktop):

- `opencorvus.settings.dat` (desktop default server URL)
- `opencorvus.global.dat` e `opencorvus.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con OpenCorvus:
...
Per capire a quali modelli hai accesso, esegui `opencorvus models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\opencorvus`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. opencorvus installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\opencorvus`

2. Riavvia opencorvus per reinstallare i pacchetti provider piu recenti

Questo forzera opencorvus a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
opencorvus rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
