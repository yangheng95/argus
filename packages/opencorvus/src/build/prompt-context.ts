...
function renderVisualQaRepairOverlay(visualQaFeedback: string): string {
  return [
    ...
    "Use `problem_dom_regions` as the first source-code repair map: locator, Document Object Model (DOM) path, HTML excerpt, computed styles, attributes, bbox, annotated_evidence_refs, and code-search terms. Inspect every annotated screenshot, visual feedback comparison artifact, and Visual QA diagnostic artifact in the Build Evidence Pack before editing. Layout-geometry manifests describe concrete page rails, edge offsets, width behavior, and alignment group spreads; use them for page-edge or shared-rail drift. List annotated screenshot urls in `consumed_visual_qa_annotation_refs`, visual feedback comparison urls in `consumed_visual_feedback_comparison_refs`, and diagnostic urls in `consumed_visual_qa_diagnostic_refs`, then after repair prove the blocker is gone with fresh screenshot or comparison evidence; DOM text alone is not visual proof.",
    "",
