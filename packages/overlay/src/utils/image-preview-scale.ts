...
export function clampImagePreviewScale(value: number): number {
  return Math.min(
    ...
    Math.max(IMAGE_PREVIEW_MIN_SCALE, Number.isFinite(value) ? value : 1),
  )
}
...
export function calculateImagePreviewFitScale(imageSize: ImagePreviewSize, viewportSize: ImagePreviewSize): number {
  ...
  return clampImagePreviewScale(Math.min(1, viewportSize.width / imageSize.width, viewportSize.height / imageSize.height))
}
