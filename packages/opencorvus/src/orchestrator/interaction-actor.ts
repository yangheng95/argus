...
export namespace OrchestratorInteractionActor {
  const log = Log.create({ service: "orchestrator.interaction-actor" })
  ...
  function ensure(key: string) {
    ...
    void serve(key, actor).catch((cause) => {
      log.error("interaction actor failed", { key, cause })
      delete actors()[key]
      actor.inbox.close()
    })
    return actor
  ...
  export async function submit<T>(key: string, exec: () => Promise<T>): Promise<T> {
    ...
    if (!actor.inbox.send({ exec, reply })) throw new Error(`Interaction actor closed: ${key}`)
    const result = await reply.recv()
    if (!result) throw new Error(`Interaction actor closed: ${key}`)
