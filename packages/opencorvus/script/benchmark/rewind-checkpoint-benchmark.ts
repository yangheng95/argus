...
const home = await fs.mkdtemp(path.join(os.tmpdir(), "opencorvus-rewind-bench-home-"))
...
async function makeRepo(label: string) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `opencorvus-rewind-bench-${label}-`))
