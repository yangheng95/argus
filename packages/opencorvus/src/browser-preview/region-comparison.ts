...
export async function compareBrowserPreviewRegions(
  ...
  const capture = await runBrowserPreviewRegionComparisonCapture({
    projectRoot: input.projectRoot,
    taskID: input.taskID,
    ...
    if (error instanceof BrowserPreviewEvidenceTargetNotFoundError) {
      throw new BrowserPreviewRegionComparisonTargetNotFoundError(error.targetID)
    }
    throw error
  })
  ...
  const manifestPath = path.join(capture.outDir, "manifest.json")
