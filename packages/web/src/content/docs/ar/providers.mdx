---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenCorvus ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenCorvus.
...
في `~/.local/share/opencorvus/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenCorvus.
...
OpenCorvus Zen هي قائمة نماذج يوفّرها فريق OpenCorvus وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenCorvus. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenCorvus Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencorvus، ثم انتقل إلى [opencorvus.ai/auth](https://opencorvus.ai/auth).
...
يعمل مثل أي مزوّد آخر في OpenCorvus واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع OpenCorvus:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل opencorvus:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `opencorvus.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenCorvus غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل opencorvus بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات opencorvus.
...
تخزين الرمز في مساحة تخزين مصادقة opencorvus.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenCorvus نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenCorvus
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`opencorvus.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth).

##### التهيئة

خصّص عبر `opencorvus.json`:
...
لاستخدام اشتراك GitHub Copilot مع opencorvus:
...
لاستخدام Google Vertex AI مع OpenCorvus:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر opencorvus، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) لتسجيل كل محادثة OpenCorvus تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenCorvus مدرجة كجلسة مستقلة.
...
يمكنك ضبط opencorvus لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط opencorvus لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط opencorvus لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenCorvus. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencorvus) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenCorvus:
...
3. انسخ مفتاح API لاستخدامه في OpenCorvus.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenCorvus، يجب سحب معلومات النموذج محليا:
...
OpenCorvus Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenCorvus. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenCorvus Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenCorvus.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات opencorvus.
...
5. يمكنك أيضا تخصيصها عبر إعدادات opencorvus. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع OpenCorvus:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات opencorvus. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع opencorvus. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `opencorvus.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenCorvus بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `opencorvus auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات opencorvus وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات opencorvus.
