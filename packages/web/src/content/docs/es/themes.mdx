...
Con Argus puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Argus usa nuestro propio tema `argus`.
...
Argus viene con varios temas integrados.
...
- Quiere que Argus coincida con la apariencia de su terminal
...
Argus admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/argus/themes/*.json` o `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.argus/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.argus/themes/*.json`
