# Landing CTA Contrast and legacy remote Source

## Recall

| Item | Recorded context |
| --- | --- |
| User requests | The outlined “查看当前架构” button text is too pale to read. No visible `GitHub` wording should remain on the promotional landing page; its repository entry may point to legacy remote instead. |
| Acceptance target | The outlined architecture action has stable dark text on the light call-to-action surface in its normal state and a legible green hover state. English and Simplified Chinese promotional pages contain no visible or accessible `GitHub` label. Header and footer source links navigate to `https://github.com/yangheng95/opencorvus.git`. |
...
| Call site | Decision |
| --- | --- |
| `packages/web/config.mjs` | Add an accurately named promotional `repository` property pointing to the user-approved legacy remote project page. Preserve the separate documentation-wide `github` property consumed by Starlight social and edit links. |
...
| `web-landing-page.test.ts` | Assert the legacy remote target, localized source labels, absence of visible/accessibility GitHub wording, renamed automation copy, and strong CTA color ownership. |
...
- Commit with a `dsw-33987` subject and push the current delivery branch to `legacy-remote`.
...
- English and Chinese promotional pages expose zero visible leaf-text occurrences of `GitHub`. Their header and footer source links use `Source / 源码`, resolve to `https://github.com/yangheng95/opencorvus.git`, and expose localized accessible names.
...
- Goal-bound screenshots are saved at `.scratch/landing-cta-legacy-remote/{chinese-cta-contrast,chinese-legacy-remote-source}.png`.
