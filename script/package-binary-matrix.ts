...
import { packageLinuxBinary, type LinuxBinaryArtifact, type PackageLinuxBinaryOptions } from "./package-linux-binary"
...
export async function packageBinaryMatrix(
  ...
  for (const row of BINARY_PACKAGE_MATRIX) {
    ...
    if (row.id === "linux-x64") {
      const artifacts = await packageLinuxBinary(repoRoot, packageOpts)
...
async function main() {
  ...
  for (const result of results) {
    ...
    for (const artifact of result.artifacts) {
      console.log(`    ${path.relative(repoRoot, artifact.output)} (${artifactSizeMiB(artifact.output)} MiB)`)
    }
  }
}
