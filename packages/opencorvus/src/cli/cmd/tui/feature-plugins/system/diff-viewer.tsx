...
const normalizeDiffs = (diffs: readonly (VcsFileDiff | TuiSessionDiffItem)[]): DiffFile[] =>
  diffs.flatMap((item) =>
    item.file
      ? [
          {
...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const [diff] = createResource(diffInput, async (input) => {
    ...
    const result = await props.api.client.vcs.diff(
      { directory: input.directory, mode: input.mode, context: WORKING_TREE_DIFF_CONTEXT_LINES },
      { throwOnError: true },
    )
    return normalizeDiffs(result.data ?? [])
  })
