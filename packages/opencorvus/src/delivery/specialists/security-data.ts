...
function securityFiles(manifest: DeliverySurfaceManifest) {
  return [...new Set(
    manifest.evidence
...
function securityDependencies(manifest: DeliverySurfaceManifest) {
...
function evidenceRefs(manifest: DeliverySurfaceManifest, files: string[]) {
  const refs = [
    ...manifest.evidence
      .filter((item) => item.surface === "security_data")
