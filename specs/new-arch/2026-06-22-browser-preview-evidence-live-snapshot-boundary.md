...
or when a verification capture is the current preview owner.
...
| `BrowserPreviewPanel.tsx` `liveScope` | It currently becomes ready as soon as panel, task, directory, target, and viewport are ready. | Return `undefined` while `latestEvidenceScope()` exists or `currentVerificationRequest()` exists. |
...
3. Make `liveScope` return `undefined` while persisted evidence is expected for
   the selected viewport or a verification request owns the preview.
...
- Verification capture ownership prevents live snapshot requests while capture is current.
...
- `liveScope` returns no live owner while persisted evidence or the current
  verification request owns the selected target/viewport.
...
- Rechecked `BrowserPreviewPanel.tsx`; live ownership is disabled before
  ...
  `currentVerificationRequest()` exists.
