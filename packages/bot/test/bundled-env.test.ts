...
const temp: string[] = []
const vars = [
  "OPENCORVUS_BOT_BUNDLED_ENV_FILE",
  "OPENCORVUS_BOT_BUNDLED_STATE_FILE",
  "OPENCORVUS_BOT_BUNDLED_TTL_HOURS",
  "BUNDLE_TOKEN",
  "BUNDLE_REGION",
  "BUNDLE_ONLY",
]
...
async function fixture() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "bot-bundle-"))
  temp.push(dir)
  const file = path.join(dir, "bundle.env")
  const state = path.join(dir, "state", "bundle.json")
  process.env.OPENCORVUS_BOT_BUNDLED_ENV_FILE = file
  process.env.OPENCORVUS_BOT_BUNDLED_STATE_FILE = state
  return { file, state }
}
