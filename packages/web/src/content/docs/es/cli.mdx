---
title: CLI
description: Opciones y comandos de la CLI de Argus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Argus de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
argus
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Argus mediante programación.
...
Inicie la interfaz de usuario del terminal Argus.
...
La CLI de Argus también tiene los siguientes comandos.

---

### agent

Administrar agentes para Argus.
...
Conecte una terminal a un servidor backend Argus que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
argus attach [url]
```

Esto permite usar TUI con un backend remoto Argus. Por ejemplo:
...
Argus funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `argus auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/argus/auth.json`.

```bash
argus auth login
```

Cuando se inicia Argus, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Argus.
...
Ejecute argus en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `argus serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| `--attach`   |       | Adjuntar a un servidor argus en ejecución (por ejemplo, http://localhost:4096)   |
...
Inicie un servidor Argus sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
argus serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad argus sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `argus`).
...
Enumere todas las sesiones Argus.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Argus.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Argus.

```bash
argus import <file>
```

Puede importar desde un archivo local o una URL compartida Argus.
...
Inicie un servidor Argus sin cabeza con una interfaz web.

```bash
argus web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Argus a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `argus`).
...
Desinstale Argus y elimine todos los archivos relacionados.
...
Actualiza argus a la última versión o a una versión específica.
...
La CLI de Argus toma las siguientes banderas globales.
...
Argus se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `argus`) |
