...
describe("visual-qa final blocker-based acceptance", () => {
  ...
  test("core prompt requires task-scoped region bind and comparison for clone work", () => {
    ...
    expect(normalized).toContain("Inspect the returned true-size side-by-side attachments yourself")
    expect(normalized).toContain("inspect diff attachments when true crop dimensions match")
