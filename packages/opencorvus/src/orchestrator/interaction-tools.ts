...
export const ORCHESTRATOR_QUESTION_DESCRIPTION =
  ...
  "The verification-budget interaction asks one concise question, recommends required_only for the complete mandatory acceptance floor, and offers extra_assurance only when the active package can execute a concrete additional confidence increment with a stated relative time/token cost. Rejection or automatic deadline expiry means required_only; never repeat an answered preference or ask after domain dispatch exists. " +
