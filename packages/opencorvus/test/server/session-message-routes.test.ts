...
describe("session message routes", () => {
  ...
  test(
    "message and part delete routes require complete row ownership before publishing removal events",
    async () => {
      await using one = await tmpdir({ git: true })
      await using two = await tmpdir({ git: true })
      const app = Server.App()
          ...
          fn: async () => {
            ...
            unsubscribers.push(
              ...
              Bus.subscribe(Message.Event.PartRemoved, (event) => {
                events.push(
                  `part:${event.properties.sessionID}:${event.properties.messageID}:${event.properties.partID}`,
                )
              }),
            )
          },
