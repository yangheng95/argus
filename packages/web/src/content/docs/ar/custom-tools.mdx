...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في argus.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في argus مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.argus/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/argus/tools/`.
