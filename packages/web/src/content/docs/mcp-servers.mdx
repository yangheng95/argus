...
You can add external tools to OpenCorvus using the _Model Context Protocol_, or MCP. OpenCorvus supports both local and remote servers.
...
You can define MCP servers in your [OpenCorvus Config](https://opencorvus.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/opencorvus` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenCorvus automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenCorvus will:
...
If the server requires authentication, OpenCorvus will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencorvus mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenCorvus will store the tokens securely in `~/.local/share/opencorvus/mcp-auth.json`.
...
Your MCPs are available as tools in OpenCorvus, alongside built-in tools. So you can manage them through the OpenCorvus config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenCorvus to your Sentry account.
