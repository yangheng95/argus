...
Le skill degli agenti permettono a Argus di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Argus cerca in queste posizioni:

- Config di progetto: `.argus/skills/<name>/SKILL.md`
- Config globale: `~/.config/argus/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Argus risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.argus/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.argus/skills/git-release/SKILL.md` cosi':
...
Argus elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `argus.json`:
...
**Per agenti integrati** (in `argus.json`):
