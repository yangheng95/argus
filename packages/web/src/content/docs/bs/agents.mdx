...
Postoje dvije vrste agenata u Argus: primarni agenti i podagenti.
...
Argus dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Argus dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Argus dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `argus.json`:
...
- Globalno: `~/.config/argus/agents/`
- Po projektu: `.argus/agents/`
...
Ako temperatura nije navedena, Argus koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Argus konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Argus konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Argus Zen](/docs/zen), koristili biste `argus/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `argus models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/yangheng95/argus).
:::
