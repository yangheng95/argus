...
Las herramientas permiten que LLM realice acciones en su código base. OpenCorvus viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenCorvus.
...
:::note
Esta herramienta solo está disponible cuando `OPENCORVUS_EXPERIMENTAL_LSP_TOOL=true` (o `OPENCORVUS_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCorvus o cuando la variable de entorno `OPENCORVUS_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCorvus:

```bash
OPENCORVUS_ENABLE_EXA=1 opencorvus
```
