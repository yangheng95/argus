---
title: SDK
description: OpenCorvus 服务器的类型安全 JS / TS 客户端。
---
...
对应 REST API（见 [server 参考](/docs/zh-cn/server/)）：
...
只有一份 SDK 入口：`@opencorvus-ai/sdk`。历史 `createOpencode*` 名称仍是 alias，新代码使用 `createOpenCorvus*`。

## 底层实现

SDK 内部 = `@hey-api/openapi-ts` 生成的 fetch client + 生成的 TypeScript 类型 + SSE helper。若需自定义 HTTP 层（代理、拦截器），可以在 client config 里传入自定义 `fetch`，也可以直接用 REST API，见 [server](/docs/zh-cn/server/)。
