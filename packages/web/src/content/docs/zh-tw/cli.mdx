...
此指令啟動一個 HTTP 伺服器，提供對 OpenCorvus 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencorvus`）。
...
此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 OpenCorvus。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencorvus`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `opencorvus`） |
