---
title: Sağlayıcılar
description: argus'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

argus, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı argus yapılandırmanızda yapılandırın.
...
`~/.local/share/argus/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları argus'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Argus Zen, argus ekibi tarafından sağlanan modellerin bir listesidir.
argus ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Argus Zen ile başlamanızı öneririz.
:::
...
argus'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u argus ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   argus'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `argus.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi argus'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   argus'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in argus auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
argus, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. argus'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`argus.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [argus-gitlab-auth](https://www.npmjs.com/package/@gitlab/argus-gitlab-auth) ana sayfasında.

##### Yapılandırma

`argus.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi argus'la kullanmak için:
...
Google Vertex AI'yi argus ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları argus çalıştırılırken ayarlayın.
...
Helicone'un argus aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her argus görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [argus-helicone-session](https://github.com/H2Shami/argus-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her argus görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
argus'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla argus'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla argus'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini argus için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/argus)'a bakın.
:::
...
Ollama Cloud'u argus ile kullanmak için:
...
3. argus'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: argus'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Argus Zen, argus ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Argus Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Argus Zen**'i arayın.

   ```txt
   /connect
   ```

3. argus API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   argus yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca argus yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri argus yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı argus ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `argus.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları argus'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `argus auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için argus yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, argus yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
