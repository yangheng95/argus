...
13. **DB reset + 一次 bench dry-run** 验真：`cd packages/opencorvus && OPENCORVUS_DISABLE_DEFAULT_PLUGINS=1 ... bun run script/benchmark/overlay-web-benchmark.ts --executor=mirrorcode`，确认 overlay 正常显示新维度名 + REQ-id chip。
14. **commit + push**。
