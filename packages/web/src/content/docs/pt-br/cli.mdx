---
title: CLI
description: Opções e comandos da CLI do argus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do argus, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
argus
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o argus programaticamente.
...
Inicie a interface de usuário do terminal do argus.
...
A CLI do argus também possui os seguintes comandos.

---

### agent

Gerencie agentes para o argus.
...
Anexe um terminal a um servidor backend do argus já em execução, iniciado via comandos `serve` ou `web`.

```bash
argus attach [url]
```

Isso permite usar o TUI com um backend argus remoto. Por exemplo:
...
O argus é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `argus auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/argus/auth.json`.

```bash
argus auth login
```

Quando o argus é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no argus.
...
Execute o argus em modo não interativo passando um prompt diretamente.
...
```bash "argus run"
argus run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `argus serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
argus run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor argus em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor argus sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
argus serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do argus sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `argus`).
...
Liste todas as sessões do argus.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do argus.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do argus.

```bash
argus import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do argus.
...
Inicie um servidor argus sem cabeça com uma interface web.

```bash
argus web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o argus através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `argus`).
...
Desinstale o argus e remova todos os arquivos relacionados.
...
Atualiza o argus para a versão mais recente ou uma versão específica.
...
A CLI do argus aceita as seguintes flags globais.
...
O argus pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `argus`) |
