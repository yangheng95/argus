 ...
 *
 * Usage:
 *   bun script/benchmark/mirror-baidu-clone.ts \
 *     [--output-dir=<path>] [--max-iterations=5] [--target-score=85] \
 *     [--reference-url=https://www.baidu.com/] [--viewport=1440x900]
 */
...
const REFERENCE_URL = flag("--reference-url", "https://www.baidu.com/") ?? "https://www.baidu.com/"
...
const OUTPUT_DIR =
  flag("--output-dir") ??
  mkdtempSync(path.join(os.tmpdir(), `mirror-baidu-${Date.now()}-`))
...
process.on("uncaughtException", async (err) => {
  console.error(`[baidu-clone] uncaughtException: ${err.message}\n${err.stack ?? ""}`)
})
...
async function main() {
  ...
  console.log(`[baidu-clone] output dir: ${OUTPUT_DIR}`)
  console.log(`[baidu-clone] model:      ${modelStr}`)
  console.log(`[baidu-clone] reference:  ${REFERENCE_URL}`)
  console.log(`[baidu-clone] viewport:   ${VIEWPORT.width}x${VIEWPORT.height}`)
  console.log(`[baidu-clone] target:     ≥ ${TARGET_SCORE}%`)

  await ensureBenchmarkModel(import.meta.dir, modelStr)

  // ── 1. Extract the reference page ───────────────────────────────────────
  console.log(`[baidu-clone] step 1/4: extracting ${REFERENCE_URL}`)
  ...
  console.log(`[baidu-clone] step 2/4: analysing page (tokens + patterns + scaffold)`)
  const scaffold: ProjectScaffold = analyzePage(page)

  // ── 3. Compile page + write pre-generated files ─────────────────────────
  console.log(`[baidu-clone] step 3/4: compiling XML IR + pre-generated files`)
  ...
  // ── 4. Build-agent loop ────────────────────────────────────────────────
  console.log(`[baidu-clone] step 4/4: build-agent loop (max ${MAX_ITERATIONS} iterations)`)
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const session = await Session.createNext({
        kind: "build",
        title: `Baidu clone benchmark — ${new Date().toISOString()}`,
        directory: OUTPUT_DIR,
      })
      ...
      for (let iter = 1; iter <= MAX_ITERATIONS; iter++) {
        ...
        console.log(`\n[baidu-clone] iteration ${iter}/${MAX_ITERATIONS}`)
        ...
        if (record.score >= TARGET_SCORE) {
          ...
          console.log(`\n[baidu-clone] ✅ accepted at iteration ${iter} (score ${record.score} ≥ ${TARGET_SCORE})`)
          break
        }
  ...
  console.log(
    `\n[baidu-clone] done: accepted=${report.accepted} finalScore=${report.finalScore} iterations=${report.iterations.length} duration=${(report.totalMs / 1000).toFixed(1)}s`,
  )
  console.log(`[baidu-clone] report: ${REPORT_FILE}`)
...
main().catch(async (err) => {
  console.error(`[baidu-clone] fatal: ${err instanceof Error ? err.message : String(err)}`)
