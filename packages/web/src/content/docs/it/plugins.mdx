---
title: Plugin
description: Scrivi plugin per estendere OpenCorvus.
---

I plugin ti permettono di estendere OpenCorvus agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di OpenCorvus.
...
- `.opencorvus/plugins/` - plugin a livello progetto
- `~/.config/opencorvus/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/opencorvus/node_modules/`.
...
1. Config globale (`~/.config/opencorvus/opencorvus.json`)
2. Config di progetto (`opencorvus.json`)
3. Directory plugin globale (`~/.config/opencorvus/plugins/`)
4. Directory plugin di progetto (`.opencorvus/plugins/`)
...
OpenCorvus esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di opencorvus per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere opencorvus.
...
:::note
Se usi l'app desktop di OpenCorvus, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a opencorvus di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a opencorvus:
...
L'helper `tool` crea uno strumento personalizzato che opencorvus puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in opencorvus insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://opencorvus.ai/docs/sdk) per i dettagli.
