...
describe("frontend-design prompt assembly", () => {
  ...
  test("webpage rawproject prompts make frontend_design materialize visual HTML skeleton first", () => {
    ...
    expect(prompt).toContain("frontend_design's first workflow deliverable is a source-derived static HTML/CSS visual skeleton")
    ...
    expect(prompt).toContain("Default visual HTML skeleton root: `web-clone-target` only when the target name is still required by existing tooling")
    ...
    expect(prompt).toContain("create/adapt static `index.html`, CSS, asset references")
    ...
    expect(prompt).toContain("compare it against `reference.png` with `webpage_render`, `webpage_evaluate`, and `webpage_vision_judge` evidence when available")
    ...
    expect(prompt).toContain("the skeleton is not the implementation target, not an acceptance app root, and not an independent design source")
  ...
  test("visual HTML skeleton prompts keep static baseline below source evidence", () => {
    ...
    expect(prompt).toContain("# Visual HTML Skeleton Contract")
    expect(prompt).toContain("source-derived static HTML/CSS visual skeleton")
    expect(prompt).toContain("visual-html-skeleton/index.html")
  ...
  test("skeleton project tool returns bounded source evidence entrypoints", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.output).toContain("First populate the visual HTML skeleton with static HTML/CSS/assets")
