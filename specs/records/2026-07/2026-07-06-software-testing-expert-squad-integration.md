...
- The integration does not claim that MirrorTest's internal sub-agent topology is required for correctness; OpenCorvus v1 coverage is validated by protocol/tool/artifact behavior through existing roles.
...
- Do not overfit the integration to MirrorTest's sub-agent count. Keep the runtime surface small unless a concrete missing capability requires expanding `AgentRoleContract`.
...
| `C:\Users\chuan\myhexin-local\iwc-aigc-test-agent\CLAUDE.md` | MirrorTest model: `TEST.md` + `script.ts`; stable `ctx` abstraction; Plan designs tests, Execute runs/debugs; stale script vs real bug distinction. |
