...
| Git baseline | `ff03be1bce17b2a647f4cbb7bda57fed03d70512` on `v0.0.18beta`, equal to `myhexin/v0.0.18beta` before implementation. The worktree contains unrelated concurrent edits which must remain unstaged and untouched. |
...
6. Stage only task-owned files, commit with the required prefix, fetch/reconcile the git-cc branch without disturbing parallel changes, push to `myhexin`, and verify remote equality.
