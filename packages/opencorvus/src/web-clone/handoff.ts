...
export function buildWebCloneHandoff(pageIr: WebClonePageIr, assetGraph: WebCloneAssetGraph): WebCloneHandoff {
...
function findNestedSegmentRoots(nodes: WebCloneNode[]): WebCloneNode[] {
  const candidates: WebCloneNode[] = []
...
function collectNestedSegmentRoots(node: WebCloneNode, depth: number, candidates: WebCloneNode[]): void {
...
function isMajorLayoutRegion(node: WebCloneNode): boolean {
...
function dedupeNodes(nodes: WebCloneNode[]): WebCloneNode[] {
...
function summarizeNode(node: WebCloneNode, parentId: string | undefined): WebCloneSegmentNodeSummary {
  return {
    id: node.id,
    parentId,
...
function summarizeAssets(assetIds: string[], assetGraph: WebCloneAssetGraph): WebCloneSegmentAssetSummary[] {
...
function childElementCount(node: WebCloneNode): number {
  let count = 0
  walk(node, (child) => {
