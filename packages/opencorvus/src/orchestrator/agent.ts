...
const ORCHESTRATOR_INSTRUCTIONS = [
  ...
  "",
  "**(2) Pipeline workflow** — `analyze_intent → (design_analysis) → requirements → architect → fidelity → per-goal[build] → deliver → prosecute → publish_delivery` (loop):",
  "  Decompose into goals first; each goal is implemented by an isolated build invocation in its own worktree;",
  "  deliver verifies the integrated result and rejection re-dispatches affected goals; prosecute files",
  "  adversarial counterexamples between deliver iterations.",
  ...
  "  use `modify_goal` (contract-level point fix), `build({ goalID })` (single-goal rework), or re-run `architect`+`fidelity` (structural rewrite) and call `build({ goalID })` on the new set.",
  ...
  "- **analyze_intent** — front-of-pipeline disambiguator. Triangulates the user's terse request against the on-task work record (decision log, prior delivery rejections, refine notes) and a read-only tour of the repo, and emits an IntentAnalysisResult (intent class, complexity, slots, missing info, blocker / nice clarifications, summary). Pipeline only. Call BEFORE requirements on a fresh task. Re-run on re-entry whenever scope might have shifted (operator_message that changes direction, refine, restart_from_stage). If it returns blocker clarifications, call `question` with them BEFORE spending budget on requirements.",
  ...
  "- **architect** — authoritative goal decomposer: produces goals, metric specs, challenge seeds, traceability, and cross-goal contracts. Always call after requirements; re-run on delivery rejection to refine (add / modify / split / remove) the goal set. Architect does NOT run fidelity — call `fidelity` separately after every architect run.",
  "- **fidelity** — verify the persisted goal set actually covers the ORIGINAL user request. Reads the active spec snapshot, compares goals against requirements + decisions + visual contract, and on `needs_correction` re-upserts the corrected set against the same snapshot. Call AFTER every `architect` run, BEFORE the first `build`. Re-run when delivery feedback hints the goal decomposition has drifted from user intent (different signal from `architect` re-run, which is for structural issues).",
  ...
  "- **prosecute** — orchestrator-driven adversarial pass. Files concrete counterexamples for failure modes the deliver agent missed and (capped at 1/iter, 3/task) proposes diagnostic challenge metrics. Call AFTER every `deliver`, BEFORE `publish_delivery` or the next `build` retry. Side effects feed the next deliver iteration's trajectory query.",
  ...
  "3. **Pipeline**:",
  "   - analyze_intent (always, FIRST). If it returns blocker clarifications, call `question` with them BEFORE the next step. Re-run on every re-entry where the user's direction may have shifted.",
  ...
  "   - requirements (always)",
  "   - architect (always)",
  "   - fidelity (always, immediately after architect). If verdict is `needs_correction`, the goal set has been re-upserted in place — proceed to build on the corrected set without re-running architect.",
  "   - For every goal emitted by architect / corrected by fidelity: call `build({ request, reason, goalID })` — batch multiple goals in ONE step via parallel tool_calls when they have independent owned_paths.",
  "   - After all builds return, call `deliver`. Then call `prosecute`. If deliver rejected, inspect rejection_details before deciding the next move; either re-call `build({ goalID })` with refined guidance for the affected goals, or call `modify_goal` to patch the contract, or re-run `architect`+`fidelity` if structure / coverage is wrong.",
  ...
  "",
  "Direct workflow:",
  "  - Call `build` again with the rejection feedback as part of the request. Then call `deliver` again, then `prosecute`.",
  ...
  "  3. Missing functionality → re-run **architect** with the rejection feedback so it refines the goal set (add / modify / split / remove), then call **fidelity** to confirm the new set covers user intent before re-dispatching builds.",
  ...
  "  6. Two consecutive rejections on the SAME goal-set under the same contract → the goal decomposition itself is suspect → re-run **architect** to fully regenerate the goal list while keeping requirements, then **fidelity** on the new set. Do not keep retrying the same goals a third time.",
