...
ove ili konfigurirajte svoje putem opencorvus config.
...
opencorvus dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.opencorvus/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `opencorvus.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/opencorvus/modes/`
- Projekat: `.opencorvus/modes/`
...
Ako temperatura nije navedena, opencorvus koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.opencorvus/modes/` za specifične načine rada ili `~/.config/opencorvus/modes/` za globalne načine:
