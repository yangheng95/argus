---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenCorvus.
---

Dodaci vam omogućavaju da proširite OpenCorvus spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenCorvus.
...
- `.opencorvus/plugins/` - Dodaci na nivou projekta
- `~/.config/opencorvus/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/opencorvus/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/opencorvus/opencorvus.json`)
2. Konfiguracija projekta (`opencorvus.json`)
3. Globalni direktorij dodataka (`~/.config/opencorvus/plugins/`)
4. Direktorij dodataka projekta (`.opencorvus/plugins/`)
...
OpenCorvus pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: OpenCorvus SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenCorvus.
...
:::note
Ako koristite desktop aplikaciju OpenCorvus, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite opencorvus da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u opencorvus:
...
Pomoćnik `tool` kreira prilagođeni alat koji opencorvus može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za opencorvus zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://opencorvus.ai/docs/sdk) za detalje.
