---
title: الخادم
description: تفاعل مع خادم opencorvus عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

يشغّل الأمر `opencorvus serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل opencorvus استخدامها.
...
| --------------- | ------------------------------------- | ------------------ |
| `--port`        | المنفذ الذي يستمع عليه                | `7878`             |
| `--hostname`    | اسم المضيف الذي يستمع عليه            | `127.0.0.1`        |
| `--mdns`        | تفعيل اكتشاف mDNS                     | `false`            |
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `opencorvus.local` |
...
عيّن `OPENCORVUS_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `opencorvus`، أو عيّن `OPENCORVUS_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `opencorvus serve` و `opencorvus web`.
...
عند تشغيل `opencorvus` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم opencorvus للتفاعل مع opencorvus برمجيا.
:::

تتيح هذه البنية لـ opencorvus دعم عدة عملاء وتمكّنك من التفاعل مع opencorvus برمجيا.

يمكنك تشغيل `opencorvus serve` لبدء خادم مستقل. إذا كانت واجهة opencorvus في terminal (TUI)
قيد التشغيل، فسيبدأ `opencorvus serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenCorvus لـ [IDE](/docs/ide).
...
على سبيل المثال: `http://localhost:7878/doc`. استخدم المواصفة لتوليد عملاء أو لفحص أنواع الطلبات والاستجابات. أو اعرضها في مستكشف Swagger.

---

## واجهات API

يعرض خادم opencorvus واجهات API التالية.
...
| `POST`   | `/permission/:requestID/reply` | الرد على طلب إذن                      | المتن: `{ reply, message? }`، يعيد `boolean`                                     |
...
| `POST`  | `/tui/control/response` | الاستجابة لطلب تحكم (`{ id, body } or { id, error }`)           | `boolean`     |
