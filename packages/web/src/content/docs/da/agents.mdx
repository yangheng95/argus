...
Der er to typer agenter i OpenCorvus; primære agenter og subagenter.
...
OpenCorvus leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
OpenCorvus leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

OpenCorvus leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `opencorvus.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger OpenCorvus modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale OpenCorvus-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din OpenCorvus-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenCorvus Zen](/docs/zen), vil du bruge `opencorvus/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `opencorvus models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/yangheng95/opencorvus).
:::
