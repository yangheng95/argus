...
Run `29003866170` then proved the host `ripgrep` install fixes native Linux package construction and reached the musl Docker path. That path installed APK packages after starting the container with `--user "$(id -u):$(id -g)"`, so Alpine failed with `Unable to lock database: Permission denied`. The ownership constraint is still required because host-side steps write into `packages/opencorvus/dist` after the container exits; the correct package contract is root for APK installation and `su-exec` to run Bun commands as the host runner UID/GID.
...
- run the musl Docker container as root only for `apk add`, then use Alpine `su-exec` with `HOST_UID` / `HOST_GID` for the Bun install and package build commands so generated artifacts remain host-runner-owned;
