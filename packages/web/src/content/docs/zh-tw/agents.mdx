...
Argus 內建了兩個主代理：**Build** 和 **Plan**。我們將在下面介紹它們。
...
Argus 內建了兩個子代理：**General** 和 **Explore**。我們將在下面介紹它們。

---

## 內建代理

Argus 內建了兩個主代理和兩個子代理。
...
如果未指定溫度，Argus 將使用模型特定的預設值；大多數模型通常為 0，Qwen 模型為 0.55。
...
此路徑相對於設定檔所在位置。因此它同時適用於全域 Argus 設定和專案級設定。
...
Argus 設定中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [Argus Zen](/docs/zen)，則可以使用 `argus/gpt-5.1-codex` 來表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理嗎？[提交 PR](https://github.com/yangheng95/argus)。
:::
