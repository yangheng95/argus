---
title: Server
description: Interact with opencorvus server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

The `opencorvus serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencorvus client can use.
...
Set `OPENCORVUS_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencorvus`, or set `OPENCORVUS_SERVER_USERNAME` to override it. This applies to both `opencorvus serve` and `opencorvus web`.
...
When you run `opencorvus` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the opencorvus server to interact with opencorvus programmatically.
:::

This architecture lets opencorvus support multiple clients and allows you to interact with opencorvus programmatically.

You can run `opencorvus serve` to start a standalone server. If you have the
opencorvus TUI running, `opencorvus serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenCorvus [IDE](/docs/ide) plugins.
...
The opencorvus server exposes the following APIs.
