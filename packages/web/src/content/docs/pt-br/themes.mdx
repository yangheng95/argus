...
Com o argus, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o argus usa nosso próprio tema `argus`.
...
O argus vem com vários temas embutidos.
...
- Querem que o argus corresponda à aparência do seu terminal
...
O argus suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/argus/themes/*.json` ou `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.argus/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.argus/themes/*.json`
