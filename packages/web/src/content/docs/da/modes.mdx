...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i opencorvus-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i opencorvus giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem opencorvus-konfigurationen.
...
opencorvus leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.opencorvus/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `opencorvus.json`-konfigurationsfil:
...
- Globalt: `~/.config/opencorvus/modes/`
...
Hvis der ikke er angivet nogen temperatur, bruger opencorvus modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale opencorvus-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.opencorvus/modes/` for projektspecifikke tilstande eller `~/.config/opencorvus/modes/` for globale tilstande:
