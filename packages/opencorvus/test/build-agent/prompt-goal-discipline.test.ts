...
describe("build agent goal execution discipline prompt", () => {
  ...
  test("requires observed frontend verification and random preview ports", async () => {
    ...
    expect(normalized).not.toContain("browser_preview_compare_scroll_slices")
    expect(normalized).toContain("Build does not own screen-by-screen scroll-slice comparison")
    expect(normalized).toContain("leave slice-by-slice `visual_diff` review to Visual QA")
    ...
    expect(normalized).toContain("Inspect the returned comparison artifacts yourself")
    expect(normalized).not.toContain("browser_preview_layout_geometry")
    expect(normalized).toContain("route-health failure")
    expect(normalized).toContain("Viewport width differences remain visible in bound-region comparisons and ordinary screenshots")
