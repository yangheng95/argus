...
- live task `tsk_dd200fc58001606QiIAbsibN5r` 中 build session `ses_22dedf211ffdRvpavHkEV022Qv` 同形态空转：启动 73 秒后触发 compaction，session 已累计 `summary_additions=249969 / summary_deletions=58226 / summary_files=36`，但 `merge_back` 与 `StructuredOutput` 均未被调用，`time_completed=NULL`
...
`opencorvus.db` 表 `session` / `message` / `protocol_event`，task `tsk_dd200fc58001606QiIAbsibN5r`：
...
- 模型在 worktree 中已经写出 25 万行级别的代码（`summary_additions`），但**从未发起 `merge_back`/`StructuredOutput` 调用**，外层 `beginBuildAttempt` 已重启 3 次。
...
| P6 | build agent surface 同样命中 P0–P3 | live task `tsk_dd200fc58001606QiIAbsibN5r` build session 73 秒 compaction、never-finalize、25 万行 worktree 未 merge_back | 修复必须在 build surface 上同时验收，不能仅修 integrity |
...
- 抽出单一 helper，例如 `estimateToolSchemaPayload(tools, model)`。
- 对每个 tool 使用 AI SDK 的 `asSchema(tool.inputSchema).jsonSchema` 取得 JSON Schema。
- 再经过 `ProviderTransform.schema(model, schema)`，保持估算口径与 provider transform 一致。
...
### 阶段 C: context-cold session 禁止 predictive compaction

**目标**: 没有可压缩历史时不允许 compaction 空转。

**文件**: `packages/opencorvus/src/session/loop.ts`

**改动**:

- 当 `totalTokensEst > limit` 且 `assistantMsgCount === 0` 时，不创建 compaction task。
...
- `assistantMsgCount=0`、`totalTokensEst > limit` 时，断言不调用 `SessionCompaction.create()`。
...
- 如果没有完成状态的 `StructuredOutput` tool part，stamp `StructuredOutputError`。
...
- `isComplete` 可以返回 `failureKind`，但只能用于决定是否停止 retry，不用于创建多套执行策略。
...
| 3 | 阶段 C: context-cold fail-fast | step 1 超限不再 compact |
...
- 不迁移数据库。
- 不为某个 provider 写兼容分支。
- delivery 的工具 schema 重构仍不在本方案范围；如出现同形态再单独立项。
- build surface 的修复**不再视为 out-of-scope**：阶段 A–F 的所有改动都在 session 层共享路径上，build/integrity 均会受益；本方案验收必须同时覆盖两个 surface（见第 6 节）。仅当阶段 G 触发条件命中时，build agent 自身的工具集（34 bypass tools + merge_back + StructuredOutput）才需要单独的 schema 重构方案。
