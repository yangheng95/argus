---
title: Server
description: Interager med argus-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `argus serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en argus-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `argus.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `argus`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `argus serve` og `argus web`.
...
Når du kører `argus` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug argus-serveren til at interagere med argus programmatisk.
:::

Denne arkitektur lader argus understøtte flere klienter og lader dig interagere med argus programmatisk.

Du kan køre `argus serve` for at starte en selvstændig server. Hvis du har
argus TUI kørende, vil `argus serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Argus [IDE](/docs/ide) plugins.
...
Argus-serveren viser følgende API'er.
