...
test("TaskStatusHeader uses selected-task SSE activity for active elapsed time", () => {
  ...
  expect(src).toContain("startedTime")
  expect(src).toContain("startedAt: startedTime()")
  expect(src).toContain("missing start time")
  ...
  expect(src).not.toContain("task.time.created for terminal")
