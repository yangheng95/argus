---
title: Servidor
description: Interactuar con el servidor argus a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `argus serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente argus puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `argus.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `argus`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `argus serve` como a `argus web`.
...
Cuando ejecuta `argus`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor argus para interactuar con argus mediante programación.
:::

Esta arquitectura permite que argus admita múltiples clientes y le permite interactuar con argus mediante programación.

Puede ejecutar `argus serve` para iniciar un servidor independiente. Si tienes el
argus TUI ejecutándose, `argus serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Argus [IDE](/docs/ide).
...
El servidor argus expone las siguientes API.
