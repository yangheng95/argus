...
describe("Review & Debug to MirrorTest collaboration", () => {
  ...
  test("preserves the product-source and test-owned repair boundary in package prompts", async () => {
    const [reviewDebug, openTest] = await Promise.all([
      readFile(path.join(repositoryRoot, "expert-squads/builtin/review-debug/agents/orchestrator/system.md"), "utf8"),
      readFile(path.join(repositoryRoot, "expert-squads/wujiang/opentest/agents/orchestrator/system.md"), "utf8"),
    ])

    expect(reviewDebug).toContain("product-source repair")
    expect(reviewDebug).toContain("later audit is a separate fixed-profile MirrorTest Task")
    expect(openTest).toContain("may repair only test-owned cases")
    expect(openTest).toContain("Never dispatch an MirrorTest worker to diagnose product root cause")
  })
})
