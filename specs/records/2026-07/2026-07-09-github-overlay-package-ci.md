...
- use `bun install --frozen-lockfile --no-progress --ignore-scripts --backend=copyfile --network-concurrency=8` so the workflow consumes the committed lockfile, avoids progress UI noise, does not enter package install-script lifecycle work before build commands, avoids the GitHub runner hardlink backend path, and keeps Bun's cold-cache download/extract concurrency below the level that reproduced tarball extraction failures;
...
- empty-cache local repro with `--network-concurrency=8`: installed 2692 packages and exited successfully in 193.77 seconds;
