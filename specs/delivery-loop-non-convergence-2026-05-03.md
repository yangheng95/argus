...
- `runtime:web:.` failed during isolated render launch because delivery capture tried to own project startup/install instead of consuming a live preview URL. That made a project-environment failure look like a page-completeness failure.
...
2. `delivery/checks/visual.ts` mixed rendering with project launch/install responsibility.
...
3. Remove project launch/install from delivery rendering. Capture must be URL-only; the agent or preview resolver owns startup.
...
- Do not add a package-manager cache to delivery capture. Runtime capture is cross-platform because it only consumes the live URL and does not infer package managers.
