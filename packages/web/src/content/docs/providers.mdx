---
title: Providers
description: Using any LLM provider in OpenCorvus.
---

import config from "../../../config.mjs"
export const console = config.console

OpenCorvus uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenCorvus config.
...
You can customize the providers through the `provider` section in your OpenCorvus
config.
...
OpenCorvus Zen is a list of models provided by the OpenCorvus team that have been
tested and verified to work well with OpenCorvus. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenCorvus Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenCorvus Zen`, and head to [opencorvus.ai/auth](https://opencorvus.ai/zen).
...
It works like any other provider in OpenCorvus and is completely optional to use.

---

## OpenCorvus Go

OpenCorvus Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenCorvus team that have been
tested and verified to work well with OpenCorvus.

1. Run the `/connect` command in the TUI, select `OpenCorvus Go`, and head to [opencorvus.ai/auth](https://opencorvus.ai/zen).
...
To use Amazon Bedrock with OpenCorvus:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running opencorvus:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `opencorvus.json`:
...
:::info
Using your Claude Pro/Max subscription in OpenCorvus is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for opencorvus to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your opencorvus config.
...
OpenCorvus uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenCorvus
to only use your own GitLab-hosted instance, add the following to your
`opencorvus.json` file. It is also recommended to disable session sharing.
...
More documentation on [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with opencorvus:
...
To use Google Vertex AI with OpenCorvus:
...
In the event you see a feature or model from Helicone that isn't configured automatically through opencorvus, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) plugin to automatically log each OpenCorvus conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenCorvus conversation listed as a separate session.
...
You can configure opencorvus to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure opencorvus to use local models through LM Studio.
...
You can configure opencorvus to use local models through Ollama.

:::tip
Ollama can automatically configure itself for OpenCorvus. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencorvus) for details.
:::
...
To use Ollama Cloud with OpenCorvus:
...
3. Copy the API key for use in OpenCorvus.
...
6. **Important**: Before using cloud models in OpenCorvus, you must pull the model information locally:
...
OpenCorvus Zen is a list of tested and verified models provided by the OpenCorvus team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenCorvus Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenCorvus API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your opencorvus config.
...
5. You can also customize them through your opencorvus config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with OpenCorvus:
...
You can also customize models through your opencorvus config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with opencorvus. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `opencorvus.json` file in your project directory:
...
The `limit` fields allow OpenCorvus to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `opencorvus auth list` to see if the credentials
...
2. For custom providers, check the opencorvus config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your opencorvus config.
