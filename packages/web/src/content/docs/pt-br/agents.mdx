...
Existem dois tipos de agentes no argus; agentes primários e subagentes.
...
argus vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
argus vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

argus vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `argus.json`:
...
Se nenhuma temperatura for especificada, o argus usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do argus quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do argus usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Argus Zen](/docs/zen), você usaria `argus/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `argus models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/yangheng95/argus).
:::
