...
表已经原生支持 `goal_run_id` 与 `delivery_id` 两个可空外键 —— "验证证据" (Verification
...
1. **机械校验的真值延迟**。executor 声明完成 → `goal_run.status = "completed"` → 合并
   → delivery-time 才跑 acceptance_specs。per-goal 评估虽然已经在 `goal-pool.ts:699`
   ...
   化证据供下游（retry prompt、rework 无进展检测、delivery 短路）复用。

2. **delivery-time 重复跑**。`orchestrator/tools.ts:2179` 又对合并后的 worktree 调了
   一遍 `evaluateGoal`。同样的 shell 命令、同样的 spec，跑两次：一次在 goal 自己的
   worktree 里（落到 task.metadata），一次在合并 worktree 里（落到 delivery 级）。两
...
5. **on_goal / on_delivery trigger 语义没被文档提升**。`acceptance/types.ts:125-130`
   ...
   essential → `on_goal`，其它 → `on_delivery`。`per-goal.ts:242-253` 对 on_delivery
   的 heuristic 显式延后（`"deferred to delivery — not executed at goal stage"`）。
...
- `scope="goal_run"` — 在某个 goal_run 的 worktree 里跑 **该 goal 的 on_goal scorer**。
  ...
  之前。
- `scope="delivery"` — 在合并 worktree 里跑 **全部 goal 的 on_delivery scorer +
  delivery-agent 判决**。写入点：现有 `persistDelivery()` (`persist.ts:470`) 的延伸；
  `orchestrator/tools.ts` delivery 分支不再重复 eval，而是读 goal_run 级 evidence +
  补跑 on_delivery scorer。

一个 goal 在它的生命周期里会产出 N 条 `scope=goal_run` 的 evidence（每次 retry 一条）
和若干条 `scope=delivery` 的 evidence（每一轮 delivery rework 一条，按 `run_id` 串起来）。
...
export const EngineEvaluationTable = sqliteTable("engine_evaluation", {
  ...
  delivery_id: text().references(...),        // 当前保留

  // ── 新增 ──
  scope: text().$type<"goal_run" | "delivery">().notNull(),
...
约束：`scope="goal_run" ⇒ goal_run_id NOT NULL`；`scope="delivery" ⇒ delivery_id NOT NULL`。
...
### on_goal / on_delivery 契约提升

`scope="goal_run"` 的 evidence **只**承载 `trigger="on_goal"` 的 check 结果。
`on_delivery` scorer 在 per-goal 阶段仍然被显式列入 checks，但 `status="skipped"`，
`evidence="deferred to delivery — not executed at goal stage"` — 保留现有行为，只是
现在这个信息被显式写进 evidence，下游（retry prompt、UI）一眼能看到"这条留到 delivery
再验"。

`scope="delivery"` 的 evidence 合并：每个 goal 的 `on_delivery` scorer × 合并 worktree
结果 + delivery-agent 的判决（记为 `family="delivery_verdict"` 的 check 条目，
...
**delivery scope**（`orchestrator/tools.ts`，deliver 分支）：

1. 收集每个 goal 最新的 `scope="goal_run"` evidence。任何 strict failure → 短路路径
   合成 rejected（现有短路逻辑保留，只是判断依据从一次性计算的 `strictFailedChecks`
   变成读 evidence）。
2. 否则跑 delivery-agent 并运行 on_delivery scorer。结果合并成一条 `scope="delivery"`
   evidence，写到 `persistDelivery`/`updateEvaluationFromDeliveryVerdict` 的同一行上
   （两段各自 setter，不引入并行表）。
3. rework 分支读当前这条 delivery evidence 与前一条（同一 task 内 `scope="delivery"`
...
- **不破坏 1:1 delivery↔evaluation 不变量**（`persist.ts:465-467` 明文说明）。delivery
  scope evidence 仍走 `persistDelivery → updateEvaluationFromDeliveryVerdict` 这条成熟
...
3. 新模块 `src/verification/`（建议文件：`signature.ts` — computeSignature 纯函数；
   `persist.ts` — `persistEvaluation`, `findLatestEvidence`, `findLatestDeliveryEvidence`;
...
### Phase D — delivery scope 合并

8. `persistDelivery` + `updateEvaluationFromDeliveryVerdict` 两侧各加上 scope 字段
   填 `"delivery"`、signature 字段填本次 delivery evidence 的签名。
9. `orchestrator/tools.ts` delivery 分支：
   - 短路判断改读 `findLatestEvidenceForGoal(scope="goal_run")` 的 strict check
     数，而非 in-loop 调 evaluateGoal。
   - 不再重复跑 goal 的 on_goal scorer；只跑 on_delivery scorer。
...
- `engine_evaluation.delivery_id NOT NULL ⇔ scope="delivery"`
- 一个 delivery row 最多一条 evaluation row（1:1 不变量，`persist.ts:465` 明文）
...
- bench 级：同样的 usage-replica-vague 场景，delivery rework 发生时：
...
- 代码行数：净减 ~200 行（delete prefetchDeliveryContext + 重复 evaluateGoal + dead
