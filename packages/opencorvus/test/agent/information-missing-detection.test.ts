...
describe("messageHasInformationMissing", () => {
  test("text part with the open tag → true", () => {
    expect(messageHasInformationMissing({
      info: { role: "assistant" },
      parts: [
  ...
  test("no text part contains the tag → false", () => {
    expect(messageHasInformationMissing({
      info: { role: "assistant" },
      parts: [
        { type: "text", text: "Normal output without the tag." },
      ],
    })).toBe(false)
  })
  ...
  test("non-text part type with the tag in some other field → false", () => {
    ...
    expect(messageHasInformationMissing({
      info: { role: "assistant" },
      parts: [
        { type: "tool_use", text: "<INFORMATION MISSING><item>x</item></INFORMATION MISSING>" } as any,
      ],
    })).toBe(false)
  })
  ...
  test("case-sensitive: lowercase 'information missing' does NOT trigger", () => {
    ...
    expect(messageHasInformationMissing({
      info: { role: "assistant" },
      parts: [
        { type: "text", text: "I am missing information about the contract." },
      ],
    })).toBe(false)
  })
...
describe("extractInformationMissingBlock", () => {
  ...
  test("returns null when no part carries the tag", () => {
    expect(extractInformationMissingBlock({
      info: { role: "assistant" },
      parts: [
