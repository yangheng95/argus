---
title: Eklentiler
description: argus'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek argus'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya argus'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.argus/plugins/` - Proje düzeyinde eklentiler
- `~/.config/argus/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/argus/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/argus/argus.json`)
2. Project config (`argus.json`)
3. Global eklenti dizini (`~/.config/argus/plugins/`)
4. Proje eklenti dizini (`.argus/plugins/`)
...
argus bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için argus'lu bir SDK istemcisi.
...
argus'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
argus masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
argus'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca argus'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, argus'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://argus.ai/docs/sdk)'e bakın.
