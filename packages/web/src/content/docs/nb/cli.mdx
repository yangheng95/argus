---
title: CLI
description: Argus CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Argus CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
argus
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Argus programmatisk.
...
Start Argus-terminalbrukergrensesnittet.
...
Koble en terminal til en allerede kjørende Argus-backend-server startet via `serve` eller `web` kommandoer.

```bash
argus attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Argus-backend. For eksempel:
...
Argus drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `argus auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/argus/auth.json`.

```bash
argus auth login
```

Når Argus starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Argus.
...
Kjør Argus i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `argus serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende Argus-server (f.eks. http://localhost:4096)     |
...
Start en headless Argus-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
argus serve
```

Dette starter en HTTP-server som gir API tilgang til Argus-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `argus`).
...
Vis tokenbruk og kostnadsstatistikk for Argus-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Argus share-URL.
...
Start en headless Argus-server med et webgrensesnitt.

```bash
argus web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Argus via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `argus`).
...
Avinstaller Argus og fjern alle relaterte filer.
...
Oppdaterer Argus til siste versjon eller en spesifikk versjon.
...
Argus CLI bruker følgende globale flagg.
...
Argus kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `argus`) |
