...
test("index.html declares right-panel Files/Explorer/Inspector tab bodies", async () => {
...
test("`delivery:focus-changes` event contract — DeliveryPanel dispatches, ChangesPanel listens", async () => {
  ...
  const filesPanel = await readSrc("src/components/RightFilesPanel.tsx")
  const main = await readSrc("src/main.tsx")
  ...
  // Listener side: the right-panel Files workbench switches to changed files; FileChangesView owns row selection state.
  expect(filesPanel).toContain('"delivery:focus-changes"')
  expect(filesPanel).toContain('setActiveView("changes")')
  expect(filesPanel).not.toContain("showWorkbenchPane")
  expect(main).toContain('"delivery:focus-changes"')
