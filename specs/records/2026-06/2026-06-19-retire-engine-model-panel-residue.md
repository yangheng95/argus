...
| `rg -n "engine-model-panel" packages/overlay/test specs -g "*.ts" -g "*.md"` | No active test or plan protects the selector.                                                                                                                               | Add a retired-selector guard.                                               |
| `rg -n "agent-model-select                                                            | agent-model-assignment                                                                                                                                                      | provider-panel                                                              | executor-model-listbox" packages/overlay/src packages/overlay/test specs -g "_._"` | Live model surfaces use `.agent-model-select`, `.agent-model-assignment`, `.provider-panel`, and `.executor-model-listbox`. | Keep current owners unchanged. |
