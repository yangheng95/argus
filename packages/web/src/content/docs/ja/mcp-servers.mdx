...
_Model Context Protocol_ (MCP) を使用して、OpenCorvus に外部ツールを追加できます。 OpenCorvus は、ローカルサーバーとリモートサーバーの両方をサポートします。
...
MCP サーバーは、`mcp` の下の [OpenCorvus 設定](https://opencorvus.ai/docs/config/) で定義できます。各 MCP を一意の名前で追加します。 LLM にプロンプ​​トを表示するときに、その MCP を名前で参照できます。
...
組織は、`.well-known/opencorvus` エンドポイント経由でデフォルトの MCP サーバーを提供できます。これらのサーバーはデフォルトで無効になっている場合があり、ユーザーは必要なサーバーにオプトインできます。
...
OpenCorvus は、リモート MCP サーバーの OAuth 認証を自動的に処理します。サーバーが認証を必要とする場合、OpenCorvus は次のことを行います。
...
サーバーが認証を必要とする場合、OpenCorvus を初めて使用しようとすると、認証を求めるプロンプトが表示されます。そうでない場合は、`opencorvus mcp auth <server-name>` を使用して [flow](#authenticating) を手動でトリガーできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、OpenCorvus はトークンを `~/.local/share/opencorvus/mcp-auth.json` に安全に保存します。
...
MCP は、組み込みツールと並んで、OpenCorvus のツールとして利用できます。したがって、他のツールと同様に、OpenCorvus 設定を通じてそれらを管理できます。
...
これにより、ブラウザウィンドウが開き、OAuth フローが完了し、OpenCorvus が Sentry アカウントに接続されます。
...
MCP は、組み込みツールと並んで、OpenCorvus のツールとして利用できます。したがって、他のツールと同様に、OpenCorvus config を通じてそれらを管理できます。
...
これにより、ブラウザ ウィンドウが開き、OAuth フローが完了し、OpenCorvus が Sentry アカウントに接続されます。
