...
| Route registration/docs     | `packages/opencorvus/src/server/routes/app.ts` mounts `MissionRoutes` at `/mission`; `packages/web/src/content/docs/{reference/api,zh-cn/reference/api}.mdx`, `packages/sdk/openapi.json`, and generated SDK files currently list only `/mission/wake`.       | Adding `GET /mission` changes the API surface and generated docs/SDK snapshots.            |
