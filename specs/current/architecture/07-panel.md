...
scale owner. Page headings use `--ui-font-heading`; standalone section and
group headings use the Section primitive's `.oc-section-heading` role backed by
`--ui-font-title`; ordinary copy and controls use `--ui-font-body` or
`--ui-font-control`; metadata uses the smaller metadata tiers. The title tier
must remain strictly larger than the body tier.

Surface styles may own section-heading layout, color, and context-specific
weight, but must not locally resize `.oc-section-heading`. Item identities such
as task titles, filenames, menu options, code labels, status pills, and
timestamps are not section headings merely because their selector contains
`title`, `head`, or `label`.
...
classification use the Environment's shared 13px title tier; facts, counts,
Goal rows, Worktrees, Tools, Sources, and operation feedback retain the same
13px menu-content tier. Every classification and named region uses one regular,
muted, 32px title-row recipe, and every category indicator occupies the same
far-edge 16px column with a quiet resting opacity. Environment facts,
...
Mission Skill，通过 `/global/composer-references` 投影，不需要也不得为查询目录而创建
...
后者保留完整 catalog。没有结构化 reference 的普通 Code / Work 请求仍进入各自 conversation；
...
selection writes only `metadata.mission.visibleExpertSquadIDs`: an empty list
exposes the complete canonical catalog, while a non-empty list limits the
Mission-only `panel.expert_squad_catalog` recommendation surface to those exact
IDs. That Mission projection is derived from the same validated catalog but
contains only Squad identity/display fields, selector summary/guidance, and
declared workflow summaries; Settings-only capability graphs, Agent inventories,
package paths, hashes, README bodies, and selector instruction bodies never enter
the Mission tool result. Mission assigns each outcome-complete Task to one fixed
...
and `read_task_artifact` for exact canonical body reads under same-Mission
terminal-child authority. `resume_task` is the separate mutation for an
...
Charts carry inline rows and use a deny-by-default Vega loader. Maps carry
inline GeoJSON and never accept model-selected styles, tiles, scripts, or
network data. The Overlay's single basemap configuration supplies the
...
board's invocation DAG and task-scoped artifact timestamps, and the blob lists
DAG status counts plus nonterminal sessions. Runtime activity therefore remains
