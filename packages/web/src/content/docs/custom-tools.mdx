---
title: Custom Tools
description: Create tools the LLM can call in argus.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside argus's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.argus/tools/` directory of your project.
- Or globally, by placing them in `~/.config/argus/tools/`.
