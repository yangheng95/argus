...
Créez `.opencorvus/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration OpenCorvus ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre OpenCorvus [config](/docs/config) :
...
- Par projet : `.opencorvus/commands/`
...
opencorvus comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
