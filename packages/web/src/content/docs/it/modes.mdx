...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di opencorvus. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in opencorvus ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di opencorvus.
...
opencorvus include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.opencorvus/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `opencorvus.json`:
...
- Globale: `~/.config/opencorvus/modes/`
- Progetto: `.opencorvus/modes/`
...
Se non viene specificata alcuna temperatura, opencorvus usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di opencorvus sia per quella specifica del progetto.
...
Crea file di modalita in `.opencorvus/modes/` per modalita specifiche del progetto oppure in `~/.config/opencorvus/modes/` per modalita globali:
