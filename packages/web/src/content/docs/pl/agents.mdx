...
W Argus obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Argus zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Argus ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Argus ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `argus.json`:
...
- Globalnie: `~/.config/argus/agents/`
- W projekcie: `.argus/agents/`
...
Jeśli nie zostanie skonfigurowana, Argus używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Argus, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Argus w `provider/model-id`. Na przykład, [Argus Zen](/docs/zen), użyj `argus/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `argus models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/yangheng95/argus).
:::
