...
Du kan legge til eksterne verktøy i Argus ved å bruke _Model Context Protocol_ eller MCP. Argus støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Argus Config](https://argus.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="argus.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/argus`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Argus håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Argus:
...
Hvis serveren krever autentisering, vil Argus be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `argus mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Argus lagre tokens sikkert i `~/.local/share/argus/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Argus, sammen med innebygde verktøy. Så du kan administrere dem gjennom Argus-konfigurasjonen som alle andre verktøy.
...
```json title="argus.json" {11, 14-18}
{
  "$schema": "https://argus.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Argus til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Argus-samtale oppført som en separat økt.
