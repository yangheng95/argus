...
import type { BindingCommandMap, BindingConfig, BindingDefaults, BindingLookup } from "@opentui/keymap/extras"
...
export type BindingValueSchema = z.output<typeof BindingValueSchema>
...
// Copied from OpenCode's TUI keybind definitions and adapted to OpenCorvus command names only where the local TUI already diverges.
...
export const KeybindOverrides = z
  .object(
    Object.fromEntries(Object.keys(Definitions).map((name) => [name, BindingValueSchema.optional()])) as Record<
      KeybindName,
      z.ZodOptional<typeof BindingValueSchema>
    >,
  )
  .strict()
...
export type BindingLookupView = BindingLookup<Renderable, KeyEvent>
...
export const CommandMap = {
  ...
  username_toggle: "session.toggle.username",
  status_view: "opencorvus.status",
...
export function parse(keybinds: KeybindOverrides): Keybinds {
  ...
  return Object.fromEntries(
    Object.entries(Definitions).map(([name, item]) => [
      name,
      BindingValueSchema.parse(keybinds[name as KeybindName] ?? item.default),
    ]),
  ) as Keybinds
}
...
export function createLookup(keybinds: Keybinds): BindingLookupView {
  return createBindingLookup<Renderable, KeyEvent>(toBindingConfig(keybinds), {
