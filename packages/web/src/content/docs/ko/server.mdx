---
title: 서버
description: HTTP를 통해 OpenCorvus 서버와 상호 작용합니다.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

`opencorvus serve` 명령은 opencorvus 클라이언트가 사용할 수 있는 OpenAPI 엔드포인트를 노출하는 headless HTTP 서버를 실행합니다.
...
| `--mdns-domain` | mDNS 서비스의 사용자 지정 도메인 이름 | `opencorvus.local` |
...
`OPENCORVUS_SERVER_PASSWORD`를 설정하여 서버를 HTTP Basic auth로 보호합니다. `opencorvus` 또는 `OPENCORVUS_SERVER_USERNAME`를 오버라이드로 설정하는 사용자의 기본값. 이것은 `opencorvus serve`와 `opencorvus web` 둘 다에 적용합니다.
...
`opencorvus`를 실행하면 TUI와 서버를 시작합니다. TUI는 어디에 있습니까?
서버와 대화하는 클라이언트. 서버는 OpenAPI 3.1 spec을 노출
끝점. 이 엔드포인트는 [SDK](/docs/sdk)을 생성하는 데도 사용됩니다.

:::tip
opencorvus 서버를 사용하여 opencorvus programmatically와 상호 작용합니다.
:::

이 아키텍처는 opencorvus 지원 여러 클라이언트를 허용하고 opencorvus programmatically와 상호 작용 할 수 있습니다.

독립 서버를 시작하려면 `opencorvus serve`를 실행할 수 있습니다. 당신이 있는 경우에
opencorvus TUI 실행, `opencorvus serve` 새로운 서버를 시작합니다.
...
[`/tui`](#tui) 엔드포인트는 서버를 통해 TUI를 구동하는 데 사용될 수 있습니다. 예를 들어 미리 작성하거나 프롬프트를 실행할 수 있습니다. 이 설정은 opencorvus [IDE](/docs/ide) 플러그인에 의해 사용됩니다.
...
예를 들어, `http://localhost:7878/doc`. 클라이언트를 생성하거나 요청 및 응답 유형을 검사하는 spec를 사용하십시오. 또는 Swagger 탐험가에서 볼 수 있습니다.

---

## API

opencorvus 서버는 다음과 같은 API를 노출합니다.
...
| `POST`   | `/permission/:requestID/reply` | 허가 요청 대응                     | 본체: `{ reply, message? }`, `boolean`                                        |
...
| `POST` | `/tui/control/response` | 통제 요청(`{ id, body } or { id, error }`) 대응                | `boolean`      |
