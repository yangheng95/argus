---
title: Regeln
description: Lege eigene Anweisungen fuer argus fest.
---

Du kannst argus ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in argus den Befehl `/init` aus.
...
So kann argus sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht argus sie zu erweitern.
...
argus kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/argus/AGENTS.md` speichern.
Diese gelten in allen argus-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Argus dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/argus/AGENTS.md` existiert)
...
Beim Start sucht argus in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/argus/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `argus.json` oder in `~/.config/argus/argus.json` angeben.
...
argus parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit argus.json

Empfohlen ist das `instructions`-Feld in der `argus.json`:
...
Du kannst argus explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `argus.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
