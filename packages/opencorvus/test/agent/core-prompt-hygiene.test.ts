...
describe("core prompt hygiene", () => {
  ...
  test("visual-qa core prompt stays focused on GUI fidelity and functional testing", async () => {
    ...
    const normalized = text.replace(/\s+/g, " ")
    expect(normalized).toContain("post-integrity visual GUI fidelity and functional testing agent")
    expect(normalized).toContain("GUI means Graphical User Interface")
    expect(normalized).toContain("Test the real running product")
    expect(normalized).toContain("latest integrity review as the primary repair brief")
  ...
  test("orchestrator prompt forbids workflow bypass; direct build is the narrow exception", async () => {
    ...
    expect(normalized).toContain("run `integrity` first")
    expect(normalized).toContain("call `visual_qa` after integrity")
    ...
    expect(normalized).toContain("rerun `integrity` for final acceptance")
    expect(normalized).not.toContain("call `visual_qa` before `integrity`")
