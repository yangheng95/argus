...
| Hard constraints | Use one localized content source; do not add fallback copy or a parallel rendering path. Update the focused landing-page contract. Render the real desktop page, inspect a task-scoped screenshot, perform a second diff review, commit with a `dsw-33987` subject, and push to `myhexin`. Do not restart or interfere with a running OpenCorvus or Overlay process. |
| Existing records read | `2026-07-27-codebuddy-inspired-product-demo-landing.md`, `2026-07-28-real-client-promo-recording.md`, `2026-07-28-landing-cta-contrast-and-gitcc-source.md`, `packages/web/src/content/landing.ts`, `packages/web/src/components/Lander.astro`, and `packages/opencorvus/test/script/web-landing-page.test.ts`. |
...
- Commit and push the current delivery branch to `myhexin`.
