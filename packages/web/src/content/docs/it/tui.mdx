---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Argus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Argus fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Argus avvia la TUI per la directory corrente.
...
Quando usi la TUI di Argus, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Argus. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Puoi personalizzare il comportamento della TUI tramite il file di config di Argus.
