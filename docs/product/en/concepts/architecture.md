...
Bridge: the MirrorCode executor (`OpencodeExecutor.submit`, `src/executor/opencode.ts:48`) injects the orchestrator's prompt into `TaskQueueService`, which fires the session loop.
