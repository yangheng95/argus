...
> 这样 bubble body 把 child cards 的 _内容_ inline 进自己的流，不引入额外卡片/气泡 chrome；`<InteractionCard/>` 保持其 self-contained 的 form chrome。Helper 函数 `collectCardText / collectLatestActivityText / collectActivityCounts` 递归 childIDs 仍然语义正确（copy 含 interaction 文本；折叠预览拉子卡最新活动）。
...
- `message` / `agent` 是 transcript 消息卡片，默认展开，但必须接入 `cardExpanded`，允许用户手动折叠和恢复。
- `ChatBubble` 不复用 `CardHeader` 的视觉 DOM，但必须复用同一折叠契约：head 是语义按钮，单击双向 toggle，Enter/Space 双向 toggle，`aria-expanded` 反映当前状态。
