...
const embeddedOverlayUi = buildFlavor === "overlay-server" ? await prepareEmbeddedOverlayUiPlugin(repoRoot) : undefined
...
for (const item of targets) {
  ...
  await runTimedStage(`${name} executable compile`, async () => {
    await Bun.build({
      conditions: ["browser"],
  ...
  await runTimedStage(`${name} runtime payload`, async () => {
