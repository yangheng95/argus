---
title: LSP Server
description: OpenCorvus lässt sich in Ihren LSP-Server integrieren.
---

OpenCorvus lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

OpenCorvus verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `OPENCORVUS_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn OpenCorvus eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer OpenCorvus-Konfiguration anpassen.
...
```json title="opencorvus.json" {5-7}
{
  "$schema": "https://opencorvus.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="opencorvus.json" {5-9}
{
  "$schema": "https://opencorvus.ai/config.json",
  "lsp": {
    "typescript": {
