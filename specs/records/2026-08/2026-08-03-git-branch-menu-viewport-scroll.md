...
| Git baseline               | Branch `work-v0.0.29beta-yr-0803` tracks `myhexin/work-v0.0.29beta-yr-0803`. The task-start checkpoint `d1f095cc3e` captured only the branch-menu rules in `conversation.css`. The unrelated unstaged `card.css` and Conversation error-indicator record changes remain excluded from this delivery.                                                            |
...
6. Stop only the exact task-owned processes, review every final diff, exclude unrelated
   work, commit with the `dsw-33987` prefix, and push the current branch to `myhexin`.
