...
Ajan becerileri, argus'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
argus su konumlari tarar:

- Proje konfigurasyonu: `.argus/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/argus/skills/<name>/SKILL.md`
...
Proje yerel yollarinda argus, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.argus/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/argus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.argus/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
argus, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`argus.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`argus.json` icinde):
