...
Med Argus kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Argus vårt eget `argus`-tema.
...
Argus kommer med flere innebygde temaer.
...
- Vil at Argus skal matche terminalens utseende
...
Argus støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/argus/themes/*.json` eller `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.argus/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.argus/themes/*.json`
