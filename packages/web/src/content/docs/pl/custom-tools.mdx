...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) opencorvus, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.opencorvus/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/opencorvus/tools/`.
