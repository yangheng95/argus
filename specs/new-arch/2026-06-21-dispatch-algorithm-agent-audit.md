...
| AGENT-005 | high | pending next fix | `fact_check` and `integrity` are terminal-tool agents but still lack same-session finalizer-miss recovery at the orchestrator wrapper boundary. | Confucius and Hubble both traced terminal tools `report_fact_check_result` and `submit_integrity_consensus` to wrappers that do not expose `continuation_artifact_id` and do not pass `continuation` into the underlying agents. The exact semantics differ from AGENT-001: `fact_check` currently has a synthetic `tool_error` persistence contract, and `integrity` has ownership/session semantics. | Do not mix with the `deep_research` / `workload_analysis` / `visual_qa` patch. Next repair must separately define per-tool error contract, add tests, then wire continuation only where it preserves existing visible error semantics. |
