...
| Hard constraints | 只改视觉（样式/断言），不改信息架构、功能、文案、交互流；禁止 fallback/双源/gate；不重启用户正在运行的 overlay（rule 39）；Playwright 用 Node 启动（AGENTS.md）；不创建 worktree；保留其他并发进程的脏改动，只按路径 stage 自己的文件；commit subject 以 `dsw-33987` 开头并 push git-cc（remote `myhexin`）。 |
