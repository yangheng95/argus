...
function buildTaskDebugBlob(board: any): string {
  ...
  push(
    ``,
    `Notes:`,
    `  - engine_goal stores contracts only; runtime status (pending/running/etc.) lives in the board view layer derived from plan_version + iteration progress.`,
    `  - A goal with plan_version_id = NULL has not been picked up by a planner pass yet — architect produced the contract but no plan_version was promoted.`,
    `  - Workflow steps emit type='workflow.step.updated'. If the last step is 'completed' but no further step row appears, look at architect.integrity events in the protocol_event tail (architect post-checks block step transition until they finalize).`,
    ``,
    ...
    ``,
    `-- Executor sessions (per-goal worker invocations + lease state)`,
