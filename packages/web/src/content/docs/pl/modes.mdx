...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji argus.
...
te lub skonfiguruj własne za pomocą konfiguracji argus.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.argus/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `argus.json`:
...
Jeśli nie ma wpływu na temperaturę, argus stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna argus, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.argus/modes/` dla trybów zapisanych dla projektu lub `~/.config/argus/modes/` dla trybów globalnych:
