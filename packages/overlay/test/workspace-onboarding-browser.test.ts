...
test(
  "browser overlay opens a project by submitting an explicit server path",
  async () => {
    ...
    try {
      ...
      expect(state.hostIsBrowser).toBe(true)
      expect(state.directory).toBe(projectPath)
      expect(state.savedDirectory).toBe(projectPath)
      expect(state.persistedDirectory).toBe(projectPath)
      expect(state.hasNativeOpenButton).toBe(false)
      expect(requestedDirectories).toContain(projectPath)
      await page.close()
    } finally {
