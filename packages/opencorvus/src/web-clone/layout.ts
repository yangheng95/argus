...
export interface WebCloneExtractedLayoutElement {
  selector?: string
  tag: string
  role?: string
  bounds?: WebCloneBounds
  ...
  classes?: string[]
  children?: WebCloneExtractedLayoutElement[]
}

export interface WebCloneExtractedLayoutPage {
  tree?: WebCloneExtractedLayoutElement[]
}
...
interface ExtractedLayoutCandidate {
  element: WebCloneExtractedLayoutElement
  order: number
}

export function mergeExtractedLayoutIntoPageIr(
  pageIr: WebClonePageIr,
  extractedPage: WebCloneExtractedLayoutPage,
): WebClonePageIr {
  const next = JSON.parse(JSON.stringify(pageIr)) as WebClonePageIr
  ...
  return WebClonePageIrSchema.parse({
    ...next,
    stats: {
      ...next.stats,
...
function layoutFor(element: WebCloneExtractedLayoutElement, confidence: number): WebCloneLayout {
  return {
...
function flattenCloneElements(root: WebCloneNode): CloneElementCandidate[] {
...
function flattenExtractedElements(tree: WebCloneExtractedLayoutElement[]): ExtractedLayoutCandidate[] {
  const elements: ExtractedLayoutCandidate[] = []
  const visit = (element: WebCloneExtractedLayoutElement) => {
...
function collectText(node: WebCloneNode): string {
  const values: string[] = []
  walk(node, (child) => {
