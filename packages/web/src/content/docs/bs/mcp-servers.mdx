...
Možete dodati vanjske alate u Argus koristeći _Model Context Protocol_, ili MCP. Argus podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Argus Config](https://argus.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/argus`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Argus automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Argus će:
...
Ako server zahtijeva autentifikaciju, Argus će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `argus mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Argus sigurno čuva tokene u `~/.local/share/argus/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Argus, zajedno s ugrađenim alatima. Možete njima upravljati kroz Argus konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Argus sa vašim Sentry nalogom.
