...
- **6-d**：`EngineGoalRunTable` 删除 — 这是最深的依赖（98 处引用 × 18 个文件）。必须先把读者搬到 artifact-based 推导再删表。拆 sub-step：
  ...
  - **6-d-1**：`EngineArtifactKind` 加 `"goal_run_attempt"`；`persist.ts` 4 个 writer（`createGoalRun` / `supersedeGoalRun` / `updateGoalRun` / `updateGoalRunWhere`）改为写 artifact；`goal-status.ts::syncGoalStatus` + `deriveGoalStatus` 改读 artifact 版 goal_run chain；`goal-run-watchdog.ts` query 改读 artifact（last_progress_at 放 payload）
  - **6-d-2**：`store.ts` 7 个 goal_run reader（`listGoalRunsForTask` / `listGoalRunsByGoal` / `findGoalRun` / `findActiveGoalRunsForCoordinator` / `findQueuedGoalRunsForCoordinator` / `findAllGoalRunsForCoordinator` / `findLiveGoalRunTipsForProject`）改读 artifact + `artifactRowToGoalRunRow` 映射 + `latestPerGoalRun` collapse；`GoalRunRow` 改手写类型
  - **6-d-3**：`workbench/board.ts` + `verification/persist.ts::findLatestGoalRunEvidence` 的 innerJoin 改为 artifact→artifact join via `goal_run_id` column；`EngineGoalRunTable` 定义删除 + DDL `engine_goal_run` CREATE 删除 + `engine/index.ts`、`storage/schema.ts`、`metrics/metrics.sql.ts`、`protocol/protocol.sql.ts` FK 解耦；test fixtures migrate
  - **风险点**：`goal-run-watchdog` 是 wall-clock 栈底防线（session-llm gate + executor-events gate 之下），迁 artifact 时必须保证 last_progress_at 每 chunk 更新；`superseded_reason` 语义是 LLM prompt 依据，payload 里必须保留一字不差
