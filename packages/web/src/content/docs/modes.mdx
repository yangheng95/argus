...
:::caution
Modes are now configured through the `agent` option in the opencorvus config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in opencorvus allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the opencorvus config.
...
opencorvus comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.opencorvus/plans/*.md` to detail the plan itself
...
Configure modes in your `opencorvus.json` config file:
...
both the global opencorvus config and the project specific config.
...
Create mode files in `.opencorvus/modes/` for project-specific modes or `~/.config/opencorvus/modes/` for global modes:
