---
title: 設定
description: OpenCorvus JSON 設定を使用します。
---

JSON 設定ファイルを使用して OpenCorvus を構成できます。

---

## 形式

OpenCorvus は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
たとえば、グローバル設定で `theme: "opencorvus"` と `autoupdate: true` が設定され、プロジェクト設定で `model: "anthropic/claude-sonnet-4-5"` が設定されている場合、最終的な設定には 3 つの設定がすべて含まれます。
...
1. **リモート設定** (`.well-known/opencorvus` から) - 組織のデフォルト
2. **グローバル設定** (`~/.config/opencorvus/opencorvus.json`) - ユーザー設定
3. **カスタム設定** (`OPENCORVUS_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `opencorvus.json`) - プロジェクト固有の設定
5. **`.opencorvus` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`OPENCORVUS_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド

つまり、プロジェクト設定はグローバルのデフォルトをオーバーライドでき、グローバル設定はリモート組織のデフォルトをオーバーライドできます。

:::note
`.opencorvus` および `~/.config/opencorvus` ディレクトリでは、サブディレクトリに **複数名** が使用されています: `agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/`、および `themes/`。下位互換性のために、単数形の名前 (`agent/` など) もサポートされています。
:::
...
組織は、`.well-known/opencorvus` エンドポイント経由でデフォルト設定を提供できます。これは、それをサポートするプロバイダーで認証するときに自動的に取得されます。
...
グローバル OpenCorvus 設定を `~/.config/opencorvus/opencorvus.json` に配置します。テーマ、プロバイダー、キーバインドなどのユーザー全体の設定にはグローバル設定を使用します。
...
プロジェクトのルートに `opencorvus.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。

:::tip
プロジェクト固有の設定をプロジェクトのルートに配置します。
:::

OpenCorvus が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
`OPENCORVUS_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`OPENCORVUS_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
環境変数。このディレクトリでは、エージェント、コマンド、
モードとプラグインは標準の `.opencorvus` ディレクトリと同様であり、次のようにする必要があります。
同じ構造に従います。
...
カスタムディレクトリはグローバル config ディレクトリと `.opencorvus` ディレクトリの後にロードされるため、それらの設定を**オーバーライド**できます。

---

## スキーマ

設定ファイルには、[**`opencorvus.ai/config.json`**](https://opencorvus.ai/config.json) を使用します。
...
`opencorvus serve` オプションを使用して、`opencorvus web` および `server` コマンドのサーバー設定を構成できます。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが OpenCorvus サーバーを検出できるようになります。
- `mdnsDomain` - mDNS サービスのカスタムドメイン名。デフォルトは `opencorvus.local` です。同じネットワーク上で複数のインスタンスを実行する場合に便利です。
...
`provider`、`model`、および `small_model` オプションを使用して、OpenCorvus 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトル生成などの軽量タスク用に別のモデルを構成します。デフォルトでは、OpenCorvus は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`theme` オプションを使用して、OpenCorvus 設定で使用するテーマを構成できます。
...
`~/.config/opencorvus/agents/` または `.opencorvus/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、OpenCorvus は警告とともに `"build"` にフォールバックします。

この設定は、TUI、CLI (`opencorvus run`)、デスクトップアプリ、および GitHub Action のすべてのインターフェイスに適用されます。
...
`~/.config/opencorvus/commands/` または `.opencorvus/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
OpenCorvus は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
デフォルトでは、opencorvus は明示的な承認を必要とせずに **すべての操作を許可**します。これは、`permission` オプションを使用して変更できます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して OpenCorvus を拡張します。

プラグインファイルを `.opencorvus/plugins/` または `~/.config/opencorvus/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
...
これは、OpenCorvus を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
