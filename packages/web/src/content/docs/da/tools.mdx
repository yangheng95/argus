...
Her er alle de innebygde verktøyene som er tilgængelige i OpenCorvus.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENCORVUS_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENCORVUS_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCorvus-leverandøren eller når miljøvariabelen `OPENCORVUS_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
