...
| Work Ledger bug follow-up | 2026-07-08 user screenshot/debug reports Work Ledger row actions are visually misaligned with real buttons, project groups do not collapse reliably, and newly created Missions are hard to find after creation. Source review found Work Ledger row actions reuse the old `.task-row-actions` absolute-position panel while nesting it inside `.work-row-right`, so the visual action slot, status chip, timestamp, and hit area can disagree. `main.tsx` already passes `missionSharedRefreshToken()` to `WorkLedger` and bumps it after `openMissionSession()`, but this contract needs tests so future Mission/Chat creation cannot bypass the unified ledger refresh. |
| Notification directory follow-up | 2026-07-08 user notification screenshot reports `task ... has inconsistent project directories` after clicking Open. Source review found `NotificationCenter.activateTaskNotification()` passes the notification item's frozen `taskDirectory` into `selectTask()`, but `selectTask()` only reloads project-scope tasks when it actually switches directories. If the notification directory already equals `settingsStore.directory`, stale task rows from an older project-scope list can still participate in `taskOwningDirectory()` and falsely conflict with the explicit notification directory. The fix must refresh the target directory's task list before ownership validation; it must not swallow real same-task directory conflicts that remain after reload. |
| Task action restoration follow-up | 2026-07-08 user reports previous task controls such as stop, delete, and rename disappeared after the unified Work Ledger change. Source review found old `TaskList` task rows exposed Start now, Cancel, Download project archive, Rename, and Delete, while `WorkLedger` only exposed Stop/Delete on top-level rows and no actions on Mission child task rows. The unified ledger must restore task actions through the existing task services (`cancelTask`, `deleteTask`, `renameTask`, `downloadTaskProjectArchive`, `startQueuedTaskNow`) and apply them to both standalone task rows and Mission-mounted child task rows, without reintroducing the retired separate Task panel. |
| Work Ledger action overlap follow-up | 2026-07-08 user screenshot reports the row kind icon and functional button area do not align and visually overlap after task controls were restored. Source review found Work Ledger rows reuse the old task-row grid while the restored task action rail can grow to five buttons; inline metadata, status mark, and timestamp were still participating in the same right-side flex layout, so the action rail competed with row text. The fix must give Work Ledger action rails an explicit width per action count, keep the action rail in normal flex flow for Work Ledger rows, and collapse inline metadata/status/timestamp while actions are visible through hover or keyboard-open state. |
| Work Ledger stop icon follow-up | 2026-07-08 user screenshot reports the Work Ledger stop button still displays incorrectly. Source review found the button used `<Icon name="stop">`, and `Icon.tsx` maps `stop` to lucide `Square`; inside a 20px row action this renders as a hollow square that visually reads as an unchecked checkbox. Source review also found Work Ledger action widths covered 1/3/4/5 actions but not the common Mission row case of 2 actions (`stop` + `delete`), so the action rail could use an undefined inherited width. The Work Ledger action rail must render stop/cancel as a filled stop block, promote it to warn color when the row action rail is visible, and define the 2-action rail width without changing the underlying stop/cancel action handlers or the composer stop button. |
| Work Ledger duplicate-stop visual follow-up | 2026-07-08 user asks "确定需要两个停止按钮？" after seeing the Mission row action pair. Source review found the second icon was not a second stop action; it was `RowDeleteAction` defaulting to `<Icon name="close">`, so the delete action looked like an X/close/cancel control next to the stop block. Work Ledger delete actions must use the trash/delete icon so stop and delete have distinct visual semantics. |
| Work Ledger hierarchy stop follow-up | 2026-07-08 user reports the row actions are "乱七八糟的". Source review found parent Mission rows and mounted child Task rows can both expose stop/cancel while the child task row is visible. The unified Work Ledger must show one visible stop control per Mission hierarchy: if a Mission has a visible queued/active child task row, the child task owns the stop/cancel action and the parent Mission row suppresses its abort icon. |
