...
:::caution
Режимы теперь настраиваются с помощью опции `agent` в конфигурации argus.
Опция `mode` устарела. [Подробнее ](/docs/agents).
:::

Режимы в argus позволяют настраивать поведение, инструменты и подсказки для различных вариантов использования.

Он имеет два встроенных режима: **сборка** и **планирование**. Вы можете настроить
эти или настройте свои собственные через конфигурацию argus.
...
argus имеет два встроенных режима.
...
- `edit` – невозможно изменить существующие файлы, за исключением файлов, расположенных по адресу `.argus/plans/*.md`, для детализации самого плана.
...
Настройте режимы в файле конфигурации `argus.json`:
...
Если температура не указана, argus использует значения по умолчанию для конкретной модели (обычно 0 для большинства моделей, 0,55 для моделей Qwen).
...
как глобальная конфигурация argus, так и конфигурация конкретного проекта.
...
Создайте файлы режимов в `.argus/modes/` для режимов, специфичных для проекта, или в `~/.config/argus/modes/` для глобальных режимов:
