...
With Argus you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Argus uses our own `argus` theme.
...
Argus comes with several built-in themes.
...
Argus supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/argus/themes/*.json` or `$XDG_CONFIG_HOME/argus/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.argus/themes/*.json`
4. **Current working directory** - Defined in `./.argus/themes/*.json`
