...
description: Создавайте инструменты, которые LLM может вызывать в argus.
---

Пользовательские инструменты — это создаваемые вами функции, которые LLM может вызывать во время разговоров. Они работают вместе со [встроенными инструментами ](/docs/tools) argus, такими как `read`, `write` и `bash`.
...
- Локально, поместив их в каталог `.argus/tools/` вашего проекта.
- Или глобально, поместив их в `~/.config/argus/tools/`.
