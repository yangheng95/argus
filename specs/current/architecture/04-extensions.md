...
Codex skill。运行时内置 package 只保留默认便捷开发包 `base`、完整通用团队 `advanced` 与研究交付团队 `research-studio`；其他 squad 的分发形态是
...
Composer 选择 expert squad 时会把 manifest `id` 作为 `promptProfile` 启动 Mission；Mission wake route 校验该 ID，
把它写入 Mission session 的 `prompt_profile.active`。Mission 是用户可见的 Task 管理者，负责把工作规划并创建为
Mission-owned child Task；Overlay 不直接创建 expert-squad Task。Mission 创建 child Task 时省略可选 `promptProfile`
会继承调用 session 的有效 active profile，再由 `PromptProfileResolver` 投影 Task 运行时能力；只有显式 child override
才传该字段。
...
- 专家团必须 self-contained：模型可见资料只存在于一个显式投影 Skill 的目录闭包，工具专用的不可变模板和数据只存在于顶层 `assets/**`，并由 package tool 静态 import 进入内容寻址 bundle。运行时禁止解析 project/global 安装路径、回读源仓库、复制平行 authority、环境变量或路径 fallback。普通 Task 输入与显式外部服务仍由调用契约声明，不伪装为 package asset；
...
- `getLoadedBuiltInPackages()` 是内置 package 的唯一加载面，只返回默认 `base`、完整 `advanced` 与 `research-studio`；不存在
...
- Advanced 的 `planned-delivery`、`evidence-investigation`、`greenfield-interface-delivery` 和 `reference-interface-delivery` 是四张不可变 manifest v1 scheduler contract 图；greenfield 图自带完整需求、架构、原创界面设计、工作量复核、真实渲染复核与界面完整性谱系，不需要参考 URL，也不得与 `planned-delivery` 重叠选择；只有 reference 图要求 `interface-investigator` 与一个 operator 已提供的 source URL。Orchestrator 可见地选择精确图后必须完成全部声明 node/依赖，但这些图不是 host 执行引擎、状态机、active/default workflow 字段或持久化进度；
...
- catalog 的 `squads[]` 只读取 manifest、README 和 selector 声明，使用覆盖这些声明内容但不覆盖绝对路径或未激活运行资源的 `declaration_hash`。未激活 package 的 prompt、skill、tool、MCP 和 `lib` 不会被解析或编译；只有精确 active package 才进行完整资源校验和 capability expansion；
...
Mission 对每个独立领域阶段先读取 `panel.expert_squad_catalog` 的 canonical recommendation，并把
该次返回的精确 ID 集合固定为整个 Mission 的 held-Squad authority。Composer 中一个或多个可见
`@squad("<id>")` 引用只授权这些 ID；没有 Squad 引用时，包括只选择
`@mission("<name>")`，launch path 把当时全部已安装 Squad 的精确 ID 快照持久化。后续新安装不会
扩展该 Mission；panel catalog 只按持久化快照读取。Mission Skill 只约束编排，不增加、生成、安装或
