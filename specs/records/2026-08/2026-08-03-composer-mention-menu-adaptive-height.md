...
| Hard constraints        | Keep the native textarea, visible mention query, grouped Kobalte Listbox, and current catalog as the single interaction/data path. Do not add a Portal, fallback placement, second menu, fixed home-only height, route gate, or state machine. Do not add, modify, update, or run User Interface (UI) automated tests. Preserve concurrent work, do not restart the running OpenCorvus/Overlay, commit only task-owned changes with the `dsw-33987` prefix, and push to `myhexin`.                                                                                                 |
...
5. Re-read the final diff and screenshot, commit only task-owned paths, fetch the
   current git-cc branch, integrate if required, push to `myhexin`, and verify
