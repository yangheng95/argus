---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenCorvus.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCorvus utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenCorvus.
...
en `~/.local/share/opencorvus/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenCorvus
configuration.
...
OpenCorvus Zen est une liste de modèles fournis par l'équipe OpenCorvus qui ont été
testé et vérifié pour fonctionner correctement avec OpenCorvus. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenCorvus Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencorvus et dirigez-vous vers [opencorvus.ai/auth](https://opencorvus.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenCorvus et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec OpenCorvus :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de opencorvus :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY opencorvus

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile opencorvus

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX opencorvus
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `opencorvus.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenCorvus n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que opencorvus fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX opencorvus
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX opencorvus
   ```
...
Vous pouvez également ajouter des modèles via votre configuration opencorvus.
...
pour stocker le jeton dans le stockage d'authentification opencorvus.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
OpenCorvus utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenCorvus
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `opencorvus.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [opencorvus-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencorvus-gitlab-auth).

##### Configuration

Personnalisez via `opencorvus.json` :
...
{
  "$schema": "https://opencorvus.ai/config.json",
  "plugin": ["@gitlab/opencorvus-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec opencorvus :
...
Pour utiliser Google Vertex AI avec OpenCorvus :
...
Définissez-les lors de l'exécution de opencorvus.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id opencorvus
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via opencorvus, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/opencorvus/opencorvus.jsonc"
{
  "$schema": "https://opencorvus.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencorvus-helicone-session](https://github.com/H2Shami/opencorvus-helicone-session) pour enregistrer automatiquement chaque conversation OpenCorvus en tant que session dans Helicone.

```bash
npm install -g opencorvus-helicone-session
```
...
```json title="opencorvus.json"
{
  "plugin": ["opencorvus-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenCorvus répertoriée comme une session distincte.
...
Vous pouvez configurer opencorvus pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="opencorvus.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencorvus.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer opencorvus pour utiliser des modèles locaux via LM Studio.

```json title="opencorvus.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencorvus.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer opencorvus pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenCorvus. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencorvus) pour plus de détails.
:::

```json title="opencorvus.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencorvus.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec OpenCorvus :
...
3. Copiez la clé API à utiliser dans OpenCorvus.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenCorvus, vous devez extraire les informations du modèle localement :
...
OpenCorvus Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenCorvus. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenCorvus Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenCorvus Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenCorvus API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration opencorvus.
...
5. Vous pouvez également les personnaliser via votre configuration opencorvus. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' opencorvus
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group opencorvus
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec OpenCorvus :
...
Vous pouvez également personnaliser les modèles via votre configuration opencorvus. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec opencorvus. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in opencorvus.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `opencorvus.json` dans le répertoire de votre projet :

   ```json title="opencorvus.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencorvus.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="opencorvus.json" {9,11,17-20}
{
  "$schema": "https://opencorvus.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à OpenCorvus de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `opencorvus auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration opencorvus et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration opencorvus.
