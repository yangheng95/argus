---
title: SDK
description: Client JS de type sécurisé pour le serveur argus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK argus JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler argus par programme.
...
Créez une instance de argus :

```javascript
import { createOpenCorvus } from "@argus-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `argus.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpenCorvus } from "@argus-ai/sdk"

const argus = await createOpenCorvus({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${argus.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de argus, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpenCorvusClient } from "@argus-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@argus-ai/sdk"
```
