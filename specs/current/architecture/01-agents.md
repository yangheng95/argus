...
└─────────────────┬───────────────────────────┘
                  ...
                  ▼
        ┌─────────────────────────┐
        │  dispatch_agent         │  orchestrator/tools.ts (target=build)
        │  goal 创建/复用 worktree  │  direct 使用当前 workDir
        │  (旧 goal pool / pipeline executor
        │   已删除，执行职责在 dispatch_agent target=build + BuildAgent)
        └─────────────────┬───────┘
                          │
                          ▼
        ┌─────────────────────────┐
...
| Agent                 | 代码                                                                                                                                                                                 | 职责                                                                                                                                                                                                         | 何时被调用                                                                                          |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| Intent Analysis       | `intent-analysis/agent.ts`                                                                                                                                                           | 解读用户的简短/模糊请求，输出 `IntentAnalysisResult`（intent class / complexity band / missing-info / clarifications）                                                                                       | 由 `dispatch_agent target=analyze_intent` 调起；可选 stage（已接线，旧 13 号文档"not wired yet"已过期） |
| Requirements          | `requirements/agent.ts`                                                                                                                                                              | Zod tool 输出 REQ-N + foundational decisions；不产出 goals                                                                                                                                                   | 当前 scheduler workflow 通过 `dispatch_agent target=requirements` 调起，或 Orchestrator 判断需要 requirements evidence |
...
| Integrity Reviewer    | `integrity/team-agent.ts`                                                                                                                                                            | 对抗性 integrity review team：根据真实任务面动态选择 reviewer，复核 requirement scope、runtime evidence、实现质量与验收风险，输出 pass / non-pass 报告；旧固定维度 review 与 prosecutor 职责已并入此 team                               | 由 `dispatch_agent target=integrity` 调起（旧 `fidelity` kind 已并入此 agent）                      |
| Build                 | `build/agent.ts`（`BuildAgent.run`；同包 `index.ts` / `report.ts` / `types.ts`） + `goal/runner.ts`（`cleanupGoalWorkspace` 清理）+ `agent/sub-agent-protocol.ts`（共享 subagent 协议） | Orchestrator 通过 `worktreeUsage` 选择 managed worktree 或当前项目 caller-owned `workDir`；默认 goal-scoped 为 managed worktree、task-level implementation 为 current project；调用 `ExecutorRegistry.requireCoding` 并实际写代码；通过 task-scoped backend browser evidence 或 Browser MCP screenshot/observe 获取运行时截图证据；`goal/runner.ts` 不创建 worktree、不调用 executor | Orchestrator 通过 `dispatch_agent target=build` 调起                                             |
...
**Acceptance review 已删除**：任务完成 / 失败归属 Orchestrator 的 `manage_task action=complete_task` / `manage_task action=fail_task` 决策；`dispatch_agent target=integrity` 只输出 review report，运行时截图证据归属 Build；旧 acceptance tool/service surface 不再存在。

**Orchestrator 当前显式 tool surface**（2026-07-07，真源是 `src/agent/tool-pool-contract.ts` 的 orchestrator private/global 列表和 `src/orchestrator/tools.ts` 实现；`deliver` / `publish_acceptance` / `steer_subagent` 已删除）：
...
orchestrator 通过 `manage_task action=propose_task` 提供"完善上一个 request 的新任务"候选；默认按 `experimental.auto_confirm_proposed_tasks=true` 直接调用 `EngineService.createTask`，只有该配置显式为 `false` 时才先等待用户确认。这不是恢复 `panel`，也不是恢复 generic `task` subagent 工具。
