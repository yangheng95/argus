...
Aby debugować problemy z opencorvus, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\opencorvus\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `opencorvus --log-level DEBUG`.

---

## Przechowywanie danych

opencorvus przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/opencorvus/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\opencorvus`
...
opencorvus Desktop uruchamia w tle lokalny serwer opencorvus (sidecar `opencorvus-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenCorvus` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opencorvus/opencorvus.jsonc` (lub `~/.config/opencorvus/opencorvus.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencorvus/opencorvus.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencorvus\opencorvus.jsonc`
...
opencorvus może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/opencorvus/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencorvus\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby opencorvus mógł ją odbudować.

1. Całkowicie zamknij opencorvus Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/opencorvus`
- **Linux**: usuń `~/.cache/opencorvus` (lub uruchom `rm -rf ~/.cache/opencorvus`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\opencorvus`

3. Uruchom ponownie opencorvus Desktop.

---

### Napraw problemy z połączeniem z serwerem

opencorvus Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `opencorvus.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OPENCORVUS_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OPENCORVUS_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows opencorvus Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami opencorvus.

---

### Brak powiadomień

opencorvus Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla opencorvus w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie opencorvus Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji opencorvus Desktop):

- `opencorvus.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `opencorvus.global.dat` i `opencorvus.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z opencorvus:
...
3. Upewnij się, że masz najnowszą wersję z `opencorvus upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `opencorvus models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\opencorvus`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. opencorvus dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\opencorvus`

2. Uruchom ponownie kod opencorvus, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to opencorvus do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
opencorvus wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
