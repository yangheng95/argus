---
title: Plugins
description: Skriv dine egne plugins for at udvide Argus.
---

Plugins giver dig mulighed for at udvide Argus ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Arguss standardadfærd.
...
- `.argus/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/argus/node_modules/`.
...
1. Global konfiguration (`~/.config/argus/argus.json`)
2. Projektkonfiguration (`argus.json`)
3. Global plugin-mappe (`~/.config/argus/plugins/`)
4. Projekt plugin bibliotek (`.argus/plugins/`)
...
Argus kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En argus SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide argus.
...
:::note
Hvis du bruger Argus desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå argus i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til argus:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som argus kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for argus sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://argus.ai/docs/sdk) for detaljer.
