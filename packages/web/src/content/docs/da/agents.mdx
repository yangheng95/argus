...
Der er to typer agenter i Argus; primære agenter og subagenter.
...
Argus leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Argus leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

Argus leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `argus.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger Argus modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Argus-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Argus-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Argus Zen](/docs/zen), vil du bruge `argus/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `argus models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/yangheng95/argus).
:::
