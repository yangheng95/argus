...
console.log("--- task ---")
const row = db.query("SELECT id,title,executor,request,time_created,time_updated FROM engine_task WHERE id=?").get(t) as Record<string, unknown> | undefined
console.log(row)
...
for (const r of db.query(
  "SELECT type, source, emitted_at FROM protocol_event WHERE task_id=? ORDER BY emitted_at DESC LIMIT 40",
).all(t)) {
  console.log(r)
}
