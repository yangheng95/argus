...
describe("session runtime contract wakes", () => {
  ...
  test("internal orchestrator wake runs one turn without appending a user message or replaying stale user system", async () => {
  ...
  test("internal orchestrator wake resumes an existing standby loop without a new user message", async () => {
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
