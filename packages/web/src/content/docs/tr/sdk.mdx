---
title: SDK
description: opencorvus sunucusu için type-safe JS istemcisi.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

opencorvus JS/TS SDK, sunucu ile etkileşmek için type-safe bir istemci sağlar.
opencorvus'u programatik olarak kontrol etmek ve entegrasyonlar oluşturmak için kullanabilirsiniz.
...
Bir opencorvus örneği oluşturun:
...
| `port`     | `number`      | Sunucu bağlantı noktası                       | `7878`      |
...
Davranışı özelleştirmek için bir yapılandırma nesnesi iletebilirsiniz. Örnek yine de `opencorvus.json` dosyanızı alır, ancak yapılandırmayı satır içi olarak geçersiz kılabilir veya ekleyebilirsiniz:
...
Halihazırda çalışan bir opencorvus örneğiniz varsa, ona bağlanmak için bir istemci örneği oluşturabilirsiniz:
...
| Seçenek         | Tip        | Açıklama                          | Varsayılan              |
| --------------- | ---------- | --------------------------------- | ----------------------- |
| `baseUrl`       | `string`   | Sunucunun URL'si                  | `http://localhost:7878` |
