...
argus'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
argus, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
argus, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

argus iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `argus.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse argus modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global argus yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
argus hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [Argus Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `argus/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `argus models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/yangheng95/argus).
:::
