...
function renderScopeBoundedMaturityEvidenceSection(input: ReviewPromptInput): string {
  ...
  if (input.requirements?.length) {
    for (const req of input.requirements) {
      ...
      if (req.nonGoals?.length) {
        lines.push(`  non_goals: ${req.nonGoals.map((item) => sanitizePromptLine(item, "generic")).join("; ")}`)
      }
      if (req.userRequestQuotes?.length) {
        lines.push(
          `  user_request_quotes: ${req.userRequestQuotes.map((item) => `"${sanitizePromptLine(item, "user_request_quote")}"`).join(", ")}`,
        )
      }
    }
  } else {
