...
ove ili konfigurirajte svoje putem argus config.
...
argus dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.argus/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `argus.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/argus/modes/`
- Projekat: `.argus/modes/`
...
Ako temperatura nije navedena, argus koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.argus/modes/` za specifične načine rada ili `~/.config/argus/modes/` za globalne načine:
