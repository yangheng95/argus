...
مع OpenCorvus يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenCorvus سمتنا `opencorvus`.
...
يأتي OpenCorvus مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenCorvus مظهر terminal لديهم
...
يدعم OpenCorvus نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/opencorvus/themes/*.json` أو `$XDG_CONFIG_HOME/opencorvus/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.opencorvus/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.opencorvus/themes/*.json`
