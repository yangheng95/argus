...
In OpenCorvus esistono due tipi di agenti: agenti primari e subagenti.
...
OpenCorvus include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenCorvus include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenCorvus include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `opencorvus.json`:
...
- Globale: `~/.config/opencorvus/agents/`
- Per progetto: `.opencorvus/agents/`
...
Se non specifichi una temperature, OpenCorvus usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenCorvus sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenCorvus usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCorvus Zen](/docs/zen), useresti `opencorvus/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `opencorvus models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/yangheng95/opencorvus).
:::
