...
`local source-binding helper` and `region comparison tool` are
...
- `2026-06-16-local-module-source-binding.md` introduces
  `local source-binding helper` as the source/local binding step before
...
```powershell
rg -n "initialized\.execute|inputSchema:\s*initialized\.parameters|Tool\.Info|BrowserPreviewToolParameters|local source-binding helper|region comparison tool|ToolRegistry\.tools|tools:\s*\{\s*include" packages/opencorvus/src packages/opencorvus/test -g "*.ts"
...
rg -n "browser_preview|BrowserPreviewTool|LocalSourceBindingHelper|RegionComparisonTool|INTEGRITY_PREVIEW_TOOL|VISUAL_QA_STATIC_TOOL|FRONTEND_DESIGN_STATIC_TOOL" packages/opencorvus/src packages/opencorvus/test -g "*.ts"
```

| Surface                                | Finding                                                                                                                                    | Decision                                                                                                                                                                                           |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/tool/registry.ts`                 | Registers `browser_preview`, `local source-binding helper`, and `region comparison tool`.                                   | Keep registry as the public tool definition surface.                                                                                                                                               |
