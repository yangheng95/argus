...
For at fejlfinde problemer med OpenCorvus, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\opencorvus\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `opencorvus --log-level DEBUG`.

---

## Lagring

opencorvus gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencorvus/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\opencorvus`
...
OpenCorvus Desktop kører en lokal OpenCorvus-server (`opencorvus-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenCorvus`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opencorvus/opencorvus.jsonc` (eller `~/.config/opencorvus/opencorvus.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opencorvus/opencorvus.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencorvus\opencorvus.jsonc`
...
OpenCorvus kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencorvus/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencorvus\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenCorvus kan genopbygge den.

1. Afslut OpenCorvus Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/opencorvus`
- **Linux**: slet `~/.cache/opencorvus` (eller kør `rm -rf ~/.cache/opencorvus`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\opencorvus`

3. Start OpenCorvus Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenCorvus Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `opencorvus.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OPENCORVUS_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENCORVUS_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver OpenCorvus Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenCorvus.

---

### Meddelelser vises ikke

OpenCorvus Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenCorvus i dine OS-indstillinger, og
...
1. Afslut OpenCorvus Desktop.
2. Find og slet disse filer (de findes i OpenCorvus Desktop-appens datamappe):

- `opencorvus.settings.dat` (skrivebordsstandardserver URL)
- `opencorvus.global.dat` og `opencorvus.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenCorvus:
...
For at finde ud af hvilke modeller du har adgang til, kør `opencorvus models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\opencorvus`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. opencorvus installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\opencorvus`

2. Start opencorvus på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge opencorvus til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
opencorvus vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
