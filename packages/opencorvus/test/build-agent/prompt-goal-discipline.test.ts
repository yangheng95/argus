...
describe("build agent goal execution discipline prompt", () => {
  ...
  test("requires observed frontend verification and random preview ports", async () => {
    ...
    expect(normalized).toContain("Build owns ordinary task-scoped rendered proof")
    expect(normalized).toContain("Build does not own module source-binding comparison")
    expect(normalized).not.toContain("browser_preview_compare_scroll_slices")
    expect(normalized).not.toContain("browser_preview_reference_regions")
    expect(normalized).toContain("Build does not own module source-binding comparison, slice-by-slice visual diff review, or layout-geometry diagnostics")
    expect(normalized).toContain("specialist Visual QA responsibilities")
    expect(normalized).toContain("body/main/app-root locator")
    expect(normalized).toContain("whole-page glance")
    expect(normalized).toContain("Browser MCP screenshot/observe/navigation tools")
    expect(normalized).toContain("missing content")
    expect(normalized).toContain("wrong component styling")
    expect(normalized).toContain("Visual QA diagnostic artifacts")
    ...
    expect(normalized).toContain("any exact evidence gap that prevents truthful parity")
