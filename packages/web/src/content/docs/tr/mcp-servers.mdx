...
_Model Bağlam Protokolü_ veya MCP kullanarak opencorvus'a harici araçlar ekleyebilirsiniz. opencorvus hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [opencorvus Config](https://opencorvus.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/opencorvus` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
opencorvus, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde opencorvus şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa opencorvus, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `opencorvus mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından opencorvus, belirteçleri `~/.local/share/opencorvus/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra opencorvus'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi opencorvus yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve opencorvus'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
